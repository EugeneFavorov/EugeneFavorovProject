...
DEFINE VARIABLE mCorrChk      AS   CHARACTER    INIT "off-corracct"
                                                NO-UNDO.
DEFINE VARIABLE mTxtMsg1      AS   CHARACTER    NO-UNDO.
DEFINE VARIABLE mTxtMsg2      AS   CHARACTER    NO-UNDO.
...
DO TRANSACTION ON ERROR  UNDO _chst_tran, LEAVE _chst_tran
   ...
   tt:
      ...
      mTxtMsg1 = GetXAttrValueEx("op",STRING(op.op),"СообщКонтр","").
      IF {assigned mTxtMsg1 } THEN
      DO ON ERROR UNDO, LEAVE ON ENDKEY 
                  UNDO, LEAVE WITH FRAME edt1:
         DISPLAY mTxtMsg1 LABEL "Сообщение"
                          FORMAT "x(300)"
                          VIEW-AS EDITOR SIZE 60 BY 3
                          HELP "Текст сообщения об ошибке"
                 mTxtMsg2 LABEL "Комментарий"
                          FORMAT "x(300)"
                          VIEW-AS EDITOR SIZE 60 BY 3
                          HELP "Текст комментария, вносимого при подтверждении документа" 
         WITH FRAME edt1 CENTERED ROW 10 OVERLAY SIDE-LABELS 1 COL.
         UPDATE mTxtMsg2 WITH FRAME edt1.

         IF {assigned mTxtMsg2 } THEN
