...
/* +++ pp-itax.p was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:12am +++ */
      ...
      DO:
         ...
         THEN DO:
            ...
            ttBlockAcct.msg = "Внимание! Импорт блокировки за прошедшую дату " + STRING(vBegPeriod) + " невозможен.".
             RUN Fill-AlertSysMes IN h_tmess("","",1,STRING(ttBlockAcct.msg)).
...
DO ON ERROR UNDO MAIN, RETRY MAIN:
   ...
   {update.i &THIS_FRAME = "f1" &EXFILE = "pp-itax.p.st1" {&*}}.
...
IF vSFlag THEN DO:
   OUTPUT STREAM sReq CLOSE.
END.
OS-DELETE VALUE ( {&RELATIVE_2_ABSOLUTE}( vReqFileName ) ).
...
USR-SEL:
   REPEAT:
      ...
      IF {&KEY_FUNCTION}({&LAST_KEY}) EQ "END-ERROR" THEN DO:
...
ON F1 OF vText IN FRAME f1 DO:
   DO:
      ...
      END.
      IF {&KEY_FUNCTION}({&LAST_KEY}) NE "END-ERROR"
...
END.
   ...
   {update.i &THIS_FRAME = "f1" &EXFILE = "pp-itax.p.st2" {&*}}.
