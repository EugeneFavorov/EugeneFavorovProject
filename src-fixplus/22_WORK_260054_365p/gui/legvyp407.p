...
/* +++ legvyp407.p was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:11am +++ */
       ...
       FOR EACH ttRetVal WHERE NO-LOCK:
          ...
          DO :
             ...
             IF vCust-Id NE bacct.cust-id
                THEN
             DO:
                RUN Fill-AlertSysMes IN h_tmess("","",1,"Выбраны счета разных клиентов.").
...
PROCEDURE GetDate:
   ...
   {wait_for.i &THIS_FRAME = "VYP-FRAME" &EXFILE = "legvyp407.p.wf1" {&*}} .
