...
/* +++ crd2doc.p was humbly modified by (c)blodd converter v.1.09 on 9/4/2015 3:43pm +++ */
...
{update.i &THIS_FRAME = "enter-cond" &EXFILE = "crd2doc.p.st1" &EDT = "YES" {&*}}      {&EDITING_WORD}
       {&READKEY_WORD}.
       IF {&LAST_KEY} = 301 THEN DO:
          RUN calend.p.
          IF ({&LAST_KEY} = 13 OR {&LAST_KEY} = 10) AND pick-value <> ? THEN {&FRAME_VALUE} = pick-value.
       END.
       ELSE
          APPLY {&LAST_KEY}.
END.
