...
/* +++ pp-op.p was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 11:41am +++ */
...
PROCEDURE CheckAcctOE PRIVATE:
   ...
   DO:
       ...
       IF AVAIL(oacct) THEN 
       DO:
          RUN calc-crd2(oacct.acct,bAcct.acct,bAcct.currency, OUTPUT vSum1).
          RELEASE oacct.
       END.
       ...
       IF AVAIL(oacct) THEN 
       DO:
          RUN calc-crd2(oacct.acct,bAcct.acct,bAcct.currency, OUTPUT vSum2).
          RELEASE oacct.
       END.
   ...
   DO:
      ...
      DO:
         IF iSide = "db" THEN
         DO:
            vNoOrdBlk = BlockAcctNoOrderPay(bAcct.acct + "," + bAcct.number,
                                            DATETIME(TODAY, MTIME)).
            IF AVAIL BlockObject THEN DO:
               IF    (LOOKUP(mAcctStatType,"Блок,БлокДб") NE 0 
                   OR LOOKUP(mAcctStatType,"Блок,БлокДб,БлокКр,БлокСумм") EQ 0)
                  AND ( NOT CAN-DO(BlockObject.txt[1],vOrderPay) 
                   OR Chk_AcctRec_For_CBLACCT(vAcctRec) 
                   OR {assigned vNoOrdBlk}) THEN DO:
   ...
   CASE iException:
      ...
      DO:
         ...
         IF NOT {assigned vTerrProc} THEN
            vTerrProc = "lg7001".
         RUN SetSysConf IN h_base ("OpTerrChck","YES").
         RUN VALUE(vTerrProc + '.p') (INPUT vOP, OUTPUT vOk).
         RUN SetSysConf IN h_base ("OpTerrChck","").
         IF vOk THEN 
         DO:
...
/* $LINTDATE='17/07/2015 08:08:23.410+04:00' */
/* $LINTUSER='paus' */
/* $LINTMODE='1' */
/* $LINTFILE='pp-op.p' */
/*prosignDuJwsTofGFKFg+ljfsHvUA*/
/* --- pp-op.p was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 11:41am --- */
