...
{def-wf.i new} 
{history.def}
{intrface.get refer}    /* Инструмент для работы с indicate */
...
IF mAvtStrSmnStatus NE "<НЕТ>" THEN 
RUN PresetAnswers IN h_tmess (mAvtStrSmnStatus).

FIND FIRST code WHERE
           code.class EQ "Статус" AND
           code.code EQ vstatus NO-LOCK NO-ERROR.
{status.set &1=tt-status &2=code}
FIND FIRST tt-status WHERE
           tt-status.class EQ "Статус" AND
           tt-status.code EQ vstatus NO-LOCK NO-ERROR.

RUN rid-rest.p (OUTPUT TABLE op-tmprecid).
{empty tmprecid}
{empty cl-tmprecid}
/* Отключено ТТ: 0167901
...
DO TRANSACTION ON ERROR  UNDO _chst_tran, LEAVE _chst_tran
   ...
   OPEN QUERY qOp FOR EACH op-tmprecid,
        FIRST bOp WHERE RECID(bOp) EQ op-tmprecid.id
           EXCLUSIVE-LOCK.
   ...
   tt:
      ...
      IF ERROR-STATUS:ERROR THEN DO:
         FIND FIRST op WHERE
              RECID(op) EQ op-tmprecid.id NO-LOCK NO-ERROR.
      ...
      IF GetStatusSessions(INT64(GetXAttrValueEx("op",STRING(op.op),"Dpr-Id",?))) EQ "ЗАКРЫТА" THEN
      DO:
      ...
      mDprIdList = "".
      IF (FGetSetting("СтатНиз", "", "ФПС") GT vStatus) THEN
      DO:
...
smf:
         DO ON ERROR UNDO, LEAVE smf ON ENDKEY UNDO , LEAVE smf WITH FRAME edt:
            summa  = IF op-entry.curr EQ "" THEN op-entry.amt-rub ELSE op-entry.amt-cur.
            summa1 = summa.
            DISPLAY "Проводить всю сумму?"
                     summa1
               WITH FRAME edt CENTERED ROW 10 OVERLAY no-label.
            UPDATE  summa1
                     VALIDATE (summa1 NE 0 AND
                               (IF summa GT 0 THEN summa1 <= summa AND summa1 > 0
      ...
      END.
      ELSE DO: /* расщепляем */
         FIND cop WHERE recid(cop) EQ recid(op) EXCLUSIVE-LOCK no-wait NO-ERROR.
         ...
         ELSE DO:
            ...
            cop-en.amt-rub = CurToBase("УЧЕТНЫЙ",
                                       op-entry.currency,
                                       (IF op-entry.value-date NE ? THEN op-entry.value-date
         ...
         IF code.misc[1]       EQ ""   OR
            TRIM(code.misc[1]) EQ "да" THEN DO:
            {op-entry.upd
...
Отключено по ТТ:0167901 */
         ASSIGN
            vLinkOpStat = GetRefVal("СинхСтат", 
                                    in-op-date, 
                                    SUBST("&1,&2", op-kind.op-kind, vStatus))
            vLinkOpStat = vStatus   WHEN vStatus BEGINS 'А'
         .

         IF {assigned vLinkOpStat} THEN
         DO:
            /* Проверим cуществование статуса в кл-ре "Статус" */
            FIND FIRST code WHERE code.class  EQ "Статус"
                              AND code.parent EQ "Статус"
                              AND code.code   EQ vLinkOpStat
                            NO-LOCK NO-ERROR.
            IF NOT AVAIL(code) THEN
            DO:
               vStr = SUBST("Статус '&1' не существует в системе.", vLinkOpStat).
               RUN putlog(vStr).      

            END. /* IF NOT AVAIL(code) THEN */
            ELSE
            DO:
               /* По стаусу основгого документа нашли связанный статус комиссионного документа */
               ASSIGN
                  vTmpStatus = vStatus       /* Запоминаем статус основного док-та             */
                  vStatus    = vLinkOpStat
               .
               FOR EACH cop WHERE cop.op-transaction = vOpTran 
                            NO-LOCK:

                  IF IsOsnLinkDoc(cop.op) THEN 
                     NEXT.
              
                  IF cop.op-status <> vStatus THEN 
                  DO:
                     /* Проверим права доступа */
                     RUN CheckOpRight IN h_base(RECID(cop), ?, "ChgSts") NO-ERROR.
                     IF ERROR-STATUS:ERROR THEN 
                     DO:
                        FIND FIRST op WHERE RECID(op) EQ RECID(cop) 
                                      NO-LOCK NO-ERROR.
                        RUN putlog(RETURN-VALUE).      
                        UNDO tt, RETRY tt.
                     END.
                     ...
                     FIND FIRST code WHERE code.class  EQ "Статус"
                                       AND code.parent EQ "Статус"
                                       AND code.code   EQ cop.op-status
                                       AND CAN-DO(code.val, vStatus)
                                     NO-LOCK NO-ERROR.
                     IF NOT AVAIL(code) THEN
                     DO:
                        FIND FIRST op WHERE RECID(op) EQ RECID(cop) 
                                      NO-LOCK NO-ERROR.
                        vStr = SUBST("Переход со статуса '&1' в '&2' невозможен согласно модели переходов.",
                                     cop.op-status,
                                     vStatus).
                        RUN putlog(vStr).      
                     ...
                     DO:
                        /* Смена статуса */
                        FIND FIRST op WHERE op.op = cop.op 
                        ...
                        DO:
                           ...
                           END. /* IF LOCKED(op) THEN */
                           ELSE 
                              RUN putlog("Запись удалена другим пользователем.").                                             
                       
                           UNDO tt, RETRY tt.
                       
                        END. /* {chst(op.i ... ELSE */
                     END. /* IF NOT AVAIL(code) THEN ... ELSE */
                  END. /* IF cop.op-status <> vStatus THEN */
               END. /* FOR EACH cop WHERE cop.op-transaction = vOpTran */
            END. /* IF NOT AVAIL(code) THEN ... ELSE */ 
            /* Возвращаем значение основного статуса */
            vStatus = vTmpStatus. 
         END. /* IF {assigned vLinkOpStat} THEN */
      END.
      ...
      FOR EACH obj-transaction WHERE
               obj-transaction.op-transaction EQ vOpTran 
           AND obj-transaction.FILE-NAME      EQ "op" 
           AND obj-transaction.surrogate      NE STRING(vOpNum) USE-INDEX op-transaction
      NO-LOCK:
         FIND FIRST op WHERE 
