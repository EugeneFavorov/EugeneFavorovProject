...
------------------------------------------------------------------------------*/
    ...
    IF tt-BlockAcct.block-type:SCREEN-VALUE IN FRAME fMain = "БлокСумм" AND
       DECIMAL(tt-BlockAcct.val[3]:SCREEN-VALUE IN FRAME fMain) GE 0 THEN DO:
          RUN Fill-SysMes IN h_tmess ("", "", "-1",
                                    "Для блокировки типа \"БлокСумм\" " +
                                    "сумма блокировки должна быть отрицательной!").
        RETURN ERROR.
    END.
...
/* $LINTDATE='08/10/2014 08:44:51.420+04:00' */
/* $LINTFILE='f-blkac.p' */
/*prosignd+cF2ak6Hx4KDTE9AVUvgw*/