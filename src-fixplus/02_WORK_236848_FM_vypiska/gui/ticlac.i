...
/* +++ ticlac.i was humbly modified by (c)blodd converter v.1.09 on 7/27/2015 8:09am +++ */
...
{getdate.i
   ...
   &AddLookUp="
               IF {&LAST_KEY} EQ KEYCODE('F1') 
                  AND {&FRAME_FIELD} EQ 'mZDate' THEN
               DO:
                  RUN calend.p.

                  IF pick-value NE ? AND ({&LAST_KEY} EQ 13 OR {&LAST_KEY} EQ 10) THEN
                  DO:
               ...
               ELSE
               IF {&LAST_KEY} EQ KEYCODE('F1') 
                  AND {&FRAME_FIELD} EQ 'beg-date' THEN
               DO:
                  RUN calend.p.
...
{getdate.i
   ...
   &AddLookUp="
               IF {&LAST_KEY} EQ KEYCODE('F1') 
                  AND {&FRAME_FIELD} EQ 'mCodeNalog' THEN
               DO:
                  RUN pclass.p('НИ', 'НИ', 'Налоговая инспекция',6).

                  IF pick-value NE ? AND ({&LAST_KEY} = 10 OR {&LAST_KEY} = 13) THEN
                  DO:
...
{getdate.i
   ...
   &return=" HIDE FRAME dateframe2 no-pause."
   &AddLookUp="IF {&LAST_KEY} EQ KEYCODE('F1') 
                  AND {&FRAME_FIELD} EQ 'beg-date' THEN
               DO:
                  RUN calend.p.
                  IF pick-value NE ? AND ({&LAST_KEY} EQ 13 OR {&LAST_KEY} EQ 10) THEN
...
DEFINE VARIABLE vFileName AS CHAR.
   ...
   IF vInX EQ 0 THEN DO:
       RUN Fill-AlertSysMes IN h_tmess("","",1,"Некорректное имя файла").
