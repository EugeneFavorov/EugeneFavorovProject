...
/* +++ sum-int1.p was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:14am +++ */
...
DO TRANSACTION
   ...
   ON ENDKEY UNDO, LEAVE:
    ...
    do:
       ...
       ELSE 
           RUN Fill-AlertSysMes IN h_tmess("","",1,program-name(1) + CHR(32) + "~n" + CHR(32) + 'Ошибка при монопольном доступе к договору ' + CHR(32) + STRING(intcode)).
    ...
    DO:
       ...
       DO:
          ...
          DO:
             ...
             ELSE
                RUN Fill-AlertSysMes IN h_tmess("","",1,program-name(1) + CHR(32) + "~n" + CHR(32) + 'Ошибка при монопольном доступе к договору ' + CHR(32) + ENTRY(1,loan.cont-code," ")).
    ...
    DO:
       ...
       DO:
         ...
         IF mErrSaveHist THEN
             RUN Fill-AlertSysMes IN h_tmess("","",1,program-name(1) + CHR(32) + "~n" + CHR(32) + 'Ошибка сохранения истории' + CHR(32) + STRING(intcode)).
