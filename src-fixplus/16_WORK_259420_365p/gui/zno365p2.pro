...
/* +++ zno365p2.pro was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:07am +++ */
...
PROCEDURE FillZNOInfoPart365p.
    ...
    IF vRequestKind = {&REQ-KIND-TICLAM} THEN
        ASSIGN
            ...
            vCounterAttr = "НомВыпис"
        .
    ELSE
        ASSIGN
            vCounterName = "Справки365"
            vCounterAttr = "НомСправ"
        .
...
PROCEDURE FillZNOAcctPart365p.                       
    ...
    vDataPereh = DATE(FGetSetting("Настройка_365П", "ДатаПерехода", "")) NO-ERROR.
    ...
    REPEAT:
        ...
        CASE vRequestKind:
            ...
            WHEN {&REQ-KIND-TICLAM} THEN DO:
               ...
               IF FGetSetting("Настройка_365П", "ПровАрхив","") = "Да" THEN DO:
...
PROCEDURE FillZNOOpPart365p.
    ...
    BY op.op-date:
        ...
        vCash = AVAILABLE acct-corr AND
                (CAN-DO(FGetSetting("НазнСчКас", "", "Касса"),
        ...
        vInternal = NOT vCash            AND
                    ...
                    (NOT AVAILABLE acct-corr
                     ...
                     NOT CAN-DO(FGetSetting("НазнСчМБР", "", ""),
                                acct-corr.contract)).
        IF vInternal                  AND
        ...
        THEN
            ASSIGN
                vINN   = FGetSettingMF("ИНН", "", "",acct.filial-id,NO)
                vKPP   = fst(FGetSettingMF("БанкКПП", "", "",acct.filial-id,NO))
                vPName = TRIM(FGetSettingMF("Банк", "", "",acct.filial-id,NO))
            .
        ELSE
