...
/* +++ core365p.pro was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:07am +++ */
...
PROCEDURE ValidateFilial365p.
    ...
    IF iMFO = FGetSetting("БанкМФО", "", "") THEN DO:
...
PROCEDURE FilterCustByKPP365p.
    ...
    IF vInfoType = {&INFO-TYPE-ZNO} THEN DO TRANSACTION:
        IF NOT {assigned iKPP} OR
           ...
           FGetSetting("Настройка_365П", "ИгнорКППНП", "Нет") = "Да"
        THEN
            RETURN.
...
PROCEDURE SetMFMode365p.
   ...
   mMFMode365p = NOT shMode OR
                 (vInfoType    = {&INFO-TYPE-ZNO} AND
                  vRequestType = {&REQ-TYPE-ALL}  AND
                  FGetSetting("Настройка_365П", "ВсеФилиалы", "Нет") = "Да").
END PROCEDURE.
...
PROCEDURE FindAcctOld365p.
   ...
   DEFINE VARIABLE vNameRekv AS CHARACTER NO-UNDO.

   IF NOT AVAILABLE acct THEN
   RETURN.

   ASSIGN
      vNameRekv = FGetSetting("Настройка_365П", 
                              "ПарамСчСтНом", 
                              "Счет_СтарНом")
...
PROCEDURE GetAcct365p.
    ...
    IF vRequestType = {&REQ-TYPE-ALL} THEN DO:
        ...
        vS = "FOR EACH " + vHBuffer:NAME                               +
             ...
             ( IF vAllFilials
              THEN ""
              ELSE ("acct.filial-id = " + QUOTER(shFilial) + " AND ")) +
        ...
        WHEN {&INFO-TYPE-ZNO} THEN DO:
            IF vRequestKind = {&REQ-KIND-TICLAM} THEN DO:
                ...
                IF ERROR-STATUS:ERROR THEN DO:
                    ...
                    RETURN.
                END.
                vInitDate = DATE(FGetSetting("Дата_НР", "", "")) NO-ERROR.
...
PROCEDURE ValidatePNOOpAttrs.
    ...
    ASSIGN
        ...
        iDocNum    = ""
                     WHEN iDocNum = ?
        vLimit     = FGetSetting("Настройка_365П", "КонтрДатыПоруч", "")
        vPriority  = FGetSetting("Настройка_365П", "КонтрОчерПл", "*")
    .
    IF vDelta = ? THEN DO:
...
PROCEDURE SetAttrValue365p.
    ...
    ELSE
        vMessage = "Неизвестный класс объекта: " +
                   QUOTER(GetNullStr(vClassName)).
    iHObject:BUFFER-FIELD(GetMangledName(iAttrName)):BUFFER-VALUE = iAttrValue.
...
PROCEDURE CheckZNOOpLimit365p.
    ...
    IF iRequestKind = {&REQ-KIND-TICLAM} THEN DO:
        vMaxNumDocs = {&ZNO-MAX-PARTS} *
                      INT64(FGetSetting("Настройка_365П", "КолДок", "0"))
                      NO-ERROR.
...
PROCEDURE GetBalanceTypeManual365p.
    ...
    ASSIGN
        oBalType       = {&BALANCE-CF}
        vBalTypeConfig = FGetSetting("Настройка_365П", "ТипОстатка", "")
        vSyntaxError   = NO
    .
...
PROCEDURE WarnIfRedSaldo365p.
    ...
    IF vRedSaldo THEN DO:
        ...
        ASSIGN
            ioPos         = 0
            vCheckLimOver = (FGetSetting("Настройка_365П",
                                         "ПровОврд",
                                         "Нет") = "Да")
        .
...
/* $LINTENV ='dpl' */
/* $LINTVSS ='$/ws3-dpl/bq' */
/* $LINTDATE='09/06/2015 12:23:34.847+04:00' */
/* $LINTUSER='krok' */
/* $LINTMODE='1' */
/* $LINTFILE='core365p.pro' */
/*prosignxm3l85JkAruljdJLl5lV4A*/
/* --- core365p.pro was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:07am --- */
