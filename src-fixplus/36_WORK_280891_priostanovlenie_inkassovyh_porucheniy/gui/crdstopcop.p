...
/* +++ crdstopcop.p was humbly modified by (c)blodd converter v.1.09 on 6/30/2016 1:15pm +++ */
...
DO:
   ...
   DO TRANSACTION:
      ...
      IF {&LAST_KEY} NE KEYCODE("ESC") AND pick-value NE ? THEN
...
{update.i &THIS_FRAME = "fAcct" &EXFILE = "crdstopcop.p.st1" {&*}} .

HIDE FRAME fAcct.

IF {&LAST_KEY} EQ KEYCODE("ESC") OR NOT {assigned mAcct} THEN
   RETURN.
...
DO:
   RUN Fill-AlertSysMes IN h_tmess("","",-1,"Документы на картотеке не обнаружены").
