...
/* +++ pb_k2kbs-1.p was humbly modified by (c)blodd converter v.1.09 on 6/30/2016 1:16pm +++ */
...
cMess = "====== Урегулирование К2 - КБС на счете " + iAcct.
RUN LogIt(cMess, iLog).
cUser = IF (iUser EQ "") THEN USERID('bisquit') ELSE iUser.

/* Остаток на л/с (с учетом незавершенных списаний) */
RUN acct-pos IN h_base(iAcct, iCurr, TODAY, TODAY, "Ф").
nAcPos    = - ( IF (iCurr EQ "") THEN sh-bal ELSE sh-val). /* Только для печати */
nAcPosTmp = nAcPos.
            ...
            ELSE DO:                                            /* и р/с и картотека - рублевые */
                ...
                ELSE DO:
                    ...
                    THEN DO:
                        ...
                        RUN LogIt(cMess, iLog).
                        RUN pb_mail.p ("a.borisov", "Ошибка К2-КБС", cMess, SEARCH("sysmess.log")).
                        ttKauCrt.eerror = ttKauCrt.eerror + ( IF (ttKauCrt.eerror EQ "") THEN "" ELSE "|") + cErrMsg.
