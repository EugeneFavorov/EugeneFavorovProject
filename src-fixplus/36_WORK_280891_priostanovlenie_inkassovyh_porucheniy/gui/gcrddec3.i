...
/* +++ gcrddec3.i was humbly modified by (c)blodd converter v.1.09 on 6/30/2016 1:16pm +++ */
...
/*ВНЕБАЛАНСОВЫЙ ДОКУМЕНТ КАРТОТЕКИ*/
   ...
   IF GetXAttrValueEx("op", STRING(o_op.op), "ПриостСпис", "") EQ "Да" THEN
   DO:
       vStrTMP = IF AVAILABLE b_op THEN b_op.doc-num
                                   ELSE vOpBalChar.
       IF {assigned vStrTMP} THEN
       vStrTMP = "N " + vStrTMP.

       ASSIGN
          opErrLogc  = YES
          opMessChar = "По документу " + vStrTMP + " списание приостановлено"
       .
       RETURN.
   END.
