...
if error-status:error then do:
   message color ERROR
           "Неверно задано значение настроечного параметра ~"ДатаЗо~"."
   ...
   IF op-spis THEN DO:
      IF {assigned op-entry.kau-cr} THEN DO:
         RUN processing-kau (op-entry.acct-cr,
                             ...
                             op-entry.kau-cr,
                             "cr").
         IF RETURN-VALUE EQ "UNDO" THEN DO:
            OK = -1.
            UNDO  doc, LEAVE doc.
         END.
      END.
...
/*         END. */
   ...
   DO ON ERROR  UNDO,     RETRY
      ...
      DO:
         ...
         END.
         SET    op.doc-type         WHEN work-op.op EQ ? OR NUM-ENTRIES(op.doc-type) GT 1
      ...
      AND {assigned mDocType}
         ...
         THEN DO:
         MESSAGE "Тип документа не соответствует"
                 IF mVidSpis EQ 1 THEN "полному" ELSE "частичному"
                 "скорректировать на код <" mDocType ">?"
         VIEW-AS ALERT-BOX BUTTONS YES-NO UPDATE mBtnQues.
      ...
      IF NOT is-pack THEN DO:         
         ...
         IF GetSysConf("Карт2ФормаНал") NE "NO" AND NOT is-pack THEN DO:
            ...
            IF RETURN-VALUE EQ "ESC" THEN UNDO, RETRY.
         END.
      end.
   ...
   IF in-kau = '*' THEN DO:
      ...
      IF AVAIL acct THEN DO:
         if is-pack eq no then do:
            ...
            IF KEYFUNCTION(LASTKEY) EQ "END-ERROR" THEN RETURN "UNDO".
               ...
               DO:
                  ...
                  DO:
                     ...
                     FOR EACH skau WHERE skau.acct EQ ekau.acct
                         ...
                         DO:
                             MESSAGE "Нельзя списать этот документ, т.к. имеется документ с большим приоритетом"
                         ...
                         DO:
                             MESSAGE "Нельзя списать этот документ, т.к. имеется документ с ранней датой"
                                 VIEW-AS ALERT-BOX INFO BUTTONS OK.
                             UNDO  sss.
                         END. 
                         END.
                         ...
                         DO:
                             MESSAGE "Нельзя списать этот документ, т.к. документ с типом 06 и 09 идут в последнюю очередь"
                                 VIEW-AS ALERT-BOX INFO BUTTONS OK.
                             UNDO  sss.
                         END.
                         END.       
                         ...
                         DO:
                             MESSAGE "Нельзя списать этот документ, т.к. имеется документ с меньшим номером"
                                 VIEW-AS ALERT-BOX INFO BUTTONS OK.
                             UNDO  sss.
                         END.
                         END.
   ...
   IF AVAIL kau THEN DO:
      ...
      IF kau.currency NE "" AND kau.currency NE ? THEN DO:
         ...
         IF NOT AVAIL instr-rate THEN DO:
            MESSAGE "Не установлен курс валюты '" work-op.currency "'" SKIP
                    "На дату : " STRING(IF work-op.value-date NE ? THEN work-op.value-date
                                                            ELSE op-entry.value-date)
