...
DO:
   RUN Fill-AlertSysMes IN h_tmess("","",-1,"Не верно заданы входящие параметры" + CHR(32) + "~n" + CHR(32) + "Формат: <маска_счетов>;<статус_документа>" + CHR(32) + "~n").
...
DO:
   ...
   RUN Fill-AlertSysMes IN h_tmess("","",4,"Печатать уведомления, по уже отправленым документам?").
