...
/* +++ g-trig.i was humbly modified by (c)blodd converter v.1.09 on 10/16/2015 7:15am +++ */
  ...
  ON "F7" of frame opreq ANYWHERE DO:
    if is_cycle then do:
      DO:
         ...
         RUN Fill-AlertSysMes IN h_tmess("","",4,"Прекратить потоковый ввод?").
         DEFINE VARIABLE g_Ok AS LOG NO-UNDO.
         g_Ok = (pick-value = "YES").
      ...
      DO:
         mblodd_char_Tmp01 = pick-value.
         RUN Fill-AlertSysMes IN h_tmess("","",4,"Начать потоковый ввод?").
         g_Ok = (pick-value = "YES").
         pick-value = mblodd_char_Tmp01.
      END.
...
&IF DEFINED(likeDiasoft) &THEN
   ...
   IF LAST-EVENT:EVENT-TYPE = "KEYPRESS" THEN lbl = KEY_LABEL({&LAST_KEY}).
&ENDIF
   IF {&KEY_FUNCTION}({&LAST_KEY}) = "END-ERROR" THEN RETURN "LEAVE".
   ...
   IF LOOKUP(self:name + '-leave-post', THIS-PROCEDURE:INTERNAL-ENTRIES) <> 0 THEN DO:
      ...
      IF {&RETURN_VALUE} = "POST-ERROR" THEN {return_no_apply.i}
   END.
run OnLeaveAssignBuff.
&IF DEFINED(likeDiasoft) &THEN
...
ON " " OF op-entry.prev-year IN FRAME opreq DO:
  {&FRAME_VALUE} = ( IF {&FRAME_VALUE} = "" then "ЗО" else "").
  {return_no_apply.i}
END.
&ENDIF
...
&ENDIF
  if lookup("ProcessDetails", this-procedure:internal-entries) > 0 then do:
   ...
   &IF DEFINED(ie) = 0 &THEN SELF:SCREEN-VALUE &ELSE op.det &ENDIF = t-details.
    IF {&LAST_EVENT_FUNCTION} <> "GO" THEN
       return.
  end.
...
PROCEDURE OnF6_Frame.opreq:
   IF {&FRAME_NAME} = "opreq" THEN DO:
      ...
      THEN DO:
         DO:
            mblodd_char_Tmp01 = pick-value.
            RUN Fill-AlertSysMes IN h_tmess("","",4,'Переключить банк плательщика <-> банк получателя?').
            DEFINE VARIABLE opreg_choice AS LOG NO-UNDO.
   ...
   THEN DO:
      ...
      THEN DO WITH FRAME opreq: /* делаем пересчет поля "сумма в рублях" из поля "сумма в валюте" */
         ...
         IF AVAIL instr-rate THEN DO:
            IF instr-rate.rate-instr ne ? THEN DO:
               ...
               DO:
                  ...
                  THEN
                     DO:
                        mblodd_char_Tmp01 = pick-value.
                        RUN Fill-AlertSysMes IN h_tmess("","",4,"Рублевая сумма не соответствует рублевому эквиваленту по курсу ЦБ." + CHR(32) + "~n" + CHR(32) + "Пересчитать?").
                        DEFINE VARIABLE vOK AS LOGICAL NO-UNDO.
                        vOK = (pick-value = "YES").
                        pick-value = mblodd_char_Tmp01.
                     END.
            ...
            ELSE DO:
               &IF DEFINED(SESSION-REMOTE) &THEN
               RUN Fill-AlertSysMes IN h_tmess("","",1,{&EGMBadInstrRate}).
         ...
         ELSE DO:
            &IF DEFINED(SESSION-REMOTE) &THEN
            RUN Fill-AlertSysMes IN h_tmess("","",1,{&EGMNotInstrRate}).
...
PROCEDURE OnAny-Printable_.Vmfo:
  APPLY KEYCODE({rus2lat.i keylabel({&LAST_KEY})}) TO vmfo IN FRAME opreq.
  RETURN "NO-APPLY".
END PROCEDURE.
&ENDIF
...
PROCEDURE OnF1_Op.Doc-Type:
  ...
  IF ({&LAST_EVENT_FUNCTION} = "GO" OR {&LAST_EVENT_FUNCTION} = "RETURN") AND pick-value <> ? THEN
  ...
  DO:
     IF    mbank-code-type GT ""
       ...
       AND ENTRY(1, pick-value) NE mbank-code-type THEN DO:
           RUN Fill-AlertSysMes IN h_tmess("","",-1,"Выбран неверный тип идентификатора!" + CHR(32) + "~n" + CHR(32) + 'Правильный тип "' + STRING(mbank-code-type) + '".').
...
PROCEDURE OnLeave_Op.order-pay:
   ...
   ELSE
      ...
      THEN DO:
         &IF DEFINED(SESSION-REMOTE) &THEN
         RUN Fill-AlertSysMes IN h_tmess("","",1,"Код очередности платежа недействителен на дату" + CHR(32) + INPUT FRAME opreq op.doc-date).
   ...
   IF NOT CAN-DO(vOrder-PayLst, INPUT FRAME opreq op.order-pay) THEN DO:
      &IF DEFINED(SESSION-REMOTE) &THEN
      RUN Fill-AlertSysMes IN h_tmess("","",1,"Разрешенные очередности платежа " + STRING(vOrder-PayLst)).
...
&IF DEFINED(DoLoan) &THEN
    ...
    PROCEDURE OnF1_.In-Cont-Code:
      ...
      END.
        ...
        if ({&LAST_EVENT_FUNCTION} = "go" or
            {&LAST_EVENT_FUNCTION} = "return") and
            pick-value <> ?
    ...
    PROCEDURE OnF1_.In-Doc-Ref:
        ...
        IF     ({&LAST_EVENT_FUNCTION} = "GO" 
            OR  {&LAST_EVENT_FUNCTION} = "RETURN") and
            pick-value <> ? then 
        DO:
           IF    NOT GetDBMode()            
...
PROCEDURE OnLeave_Op-Entry.symbol.
    ...
    DO WITH FRAME opreq:
        ...
        IF vErrorMsg <> "" THEN DO:
            RUN Fill-AlertSysMes IN h_tmess("","",-1,STRING(vErrorMsg)).
...
PROCEDURE OnLeave_Op.Doc-type:
   IF NUM-ENTRIES(op-templ.doc-type) > 1 AND LOOKUP(SELF:SCREEN-VALUE, op-templ.doc-type) = 0 THEN DO:
      &IF DEFINED(SESSION-REMOTE) &THEN
      RUN Fill-AlertSysMes IN h_tmess("","",1,"Недопустимый код расчетно-денежного документа").
   ...
   IF NOT AVAIL doc-type THEN DO:
      &IF DEFINED(SESSION-REMOTE) &THEN
      RUN Fill-AlertSysMes IN h_tmess("","",1,{&EGMBadDocType}).
...
&IF defined(likeDiaSoft) &THEN
  IF KEY_LABEL({&LAST_KEY}) = "SHIFT-TAB" OR KEY_LABEL({&LAST_KEY}) = "CURSOR-UP" THEN RETURN.
&ENDIF
/* Если вызов доп. формы ввода налоговых реквизитов - без проверок */
   IF KEY_LABEL({&LAST_KEY}) = "F2" THEN
      RETURN.
      ...
      DO:
         ...
         DO:
            ...
            DO:
               DO:
                  mblodd_char_Tmp01 = pick-value.
                  RUN Fill-AlertSysMes IN h_tmess("","",4,"~n" + CHR(32) + "Код валюты должен быть равен " + CHR(32) + FGetSetting("КодНацВал","","810") + CHR(32) + "! Продолжить?" + CHR(32) + FILL("~n",1)).
...
&ENDIF
      ...
      ELSE IF vFlag EQ 0 AND
           msg NE "" THEN DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,STRING(msg)).
...
&ENDIF
    ...
    DO:
      DO:
         mblodd_char_Tmp01 = pick-value.
         RUN Fill-AlertSysMes IN h_tmess("","",4,STRING(msg) + CHR(32) + "! Продолжить ввод?").
         choice = (pick-value = "YES").
         pick-value = mblodd_char_Tmp01.
      END.
    ...
    DO:
      IF op-templ.mfo-needed THEN DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,STRING(msg) + CHR(32) + "! Повторите ввод!").
   ...
   DO WITH FRAME opreq:
      ...
      &THEN 
         ...
         ** то выдаем ошибку. */
         ...
         THEN DO:
            IF {&LAST_EVENT_FUNCTION} NE "END-ERROR" THEN DO:
               &IF DEFINED(SESSION-REMOTE) &THEN
               RUN Fill-AlertSysMes IN h_tmess("","",1,"Нет такого счета или у Вас недостаточно прав.").
      ...
      THEN DO:
         ...
         THEN DO:
            ...
            THEN DO:
               ...
               &THEN
                  ...
                  RUN Fill-AlertSysMes IN h_tmess("","",-1,{&EGMMismatchAcctCur}).
         ...
         &IF DEFINED(ie) < 1
         &THEN
            IF {&RETURN_VALUE} NE "" THEN DO:
               RUN Fill-AlertSysMes IN h_tmess("","",1,{&RETURN_VALUE}).
    ...
    PROCEDURE OnLeave_.In-Cont-Code:
        ...
        do with frame opreq:
            ...
            then do:
                ...
                RUN Fill-AlertSysMes IN h_tmess("","",-1,"Введите номер договора!").
            ...
            then do:
                ...
                then do:
                    ...
                    RUN Fill-AlertSysMes IN h_tmess("","",-1,"С " + ( IF in-contract eq 'dps' then 'вкладом' else 'договором') + " кто-то-работает.Подождите!").
                ...
                RUN Fill-AlertSysMes IN h_tmess("","",-1,"Договор не найден!").
        ...
        then do:
            ...
            RUN Fill-AlertSysMes IN h_tmess("","",-1,'Договор уже закрыт!').
...
DEFINE VAR vLocCur AS CHAR NO-UNDO.
  ...
  DO WITH FRAME opreq:
    IF INPUT FRAME opreq in-cont-cur  EQ "" THEN DO:
       RUN Fill-AlertSysMes IN h_tmess("","",-1,"Введите валюту операции!").
    ...
    IF INPUT FRAME opreq in-cont-cur <> vLocCur AND 
       ...
       RUN Fill-AlertSysMes IN h_tmess("","",-1,"Такой валюты не существует в справочнике!").
    ...
    IF NOT AVAILABLE b-loan THEN DO:
       RUN Fill-AlertSysMes IN h_tmess("","",-1,"Договор" + CHR(32) + INPUT FRAME opreq in-doc-ref + CHR(32) + "не поддерживает операции в валюте" + CHR(32) + INPUT FRAME opreq in-cont-cur + CHR(32) + "!").
        ...
        if (loan.contract eq 'Кредит' or
            ...
            then do:
                ...
                then do:            /* плановая дата меньше даты пересчета договора */
                    RUN Fill-AlertSysMes IN h_tmess("","",1,"Плановая дата операции не может быть меньше даты пересчета договора").
...
PROCEDURE OnEnd-Error_Frame.opreq:
  ...
  IF t-scrval <> screen-value THEN DO:
    DO:
       mblodd_char_Tmp01 = pick-value.
       RUN Fill-AlertSysMes IN h_tmess("","",4,"Данные были изменены. Выйти без сохранения?").
       choice = (pick-value = "YES").
       pick-value = mblodd_char_Tmp01.
    END.
  ...
  DEF VAR t-details LIKE op.details NO-UNDO.
   ...
   &IF DEFINED(NOmfo) eq 0 &THEN
      ...
      DO:
         ...
         IF {&RETURN_VALUE} EQ "no-apply" THEN RETURN "NO-APPLY".
      END.
   ...
   &IF DEFINED(browse-entry) = 0 &THEN
      ...
      IF {&RETURN_VALUE} EQ "NO-APPLY" THEN
      DO:
   ...
   DO:   
      RUN OnLeave_Op.Inn.
      IF {&RETURN_VALUE} EQ "NO-APPLY":U THEN DO:
   ...
   THEN DO:
      &IF DEFINED(OFcash) = 0 &THEN
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Код очередности платежа недействителен на дату" + CHR(32) + op.doc-date:SCREEN-VALUE).
      ...
      &ELSE
         ...
         ELSE DO:
            RUN Fill-AlertSysMes IN h_tmess("","",-1,"Нет ни одного кода очередности платежа, действительного на дату" + CHR(32) + op.doc-date:SCREEN-VALUE).
      ...
      IF INPUT op.inn GT "" THEN DO:
         ...
         THEN DO:
            IF mchCorrectInnSign = "" THEN 
               DO:
                  mblodd_char_Tmp01 = pick-value.
                  RUN Fill-AlertSysMes IN h_tmess("","",4,"Неверная длина ИНН получателя." + CHR(32) + "~n" + CHR(32) + "Будете исправлять?").
                  choose-button = (pick-value = "YES").
                  pick-value = mblodd_char_Tmp01.
               END.
            ...
            ELSE
               DO:
                  mblodd_char_Tmp01 = pick-value.
                  RUN Fill-AlertSysMes IN h_tmess("","",4,"Последние цифры ИНН должны быть: ~"" + STRING(mchCorrectInnSign) + "~"" + CHR(32) + "~n" + CHR(32) + "Будете исправлять?").
                  choose-button = (pick-value = "YES").
                  pick-value = mblodd_char_Tmp01.
               END.
