...
*/
DEF VAR mNRDateStart  AS  DATE  NO-UNDO. /* НП НРДатаСтарта, если она пустая то НР ДатаНачКред/Дата_НР */

mNRDateStart = DATE(fGetSetting("НРДатаСтарта", ?, ?)) NO-ERROR.
IF mNRDateStart EQ ? THEN
   mNRDateStart = DATE(fGetSetting("ДатаНачКред", ?, ?)) NO-ERROR.
IF mNRDateStart EQ ? THEN
   mNRDateStart = DATE(fGetSetting("Дата_НР", ?, ?)) NO-ERROR.
IF mNRDateStart EQ ? THEN
...
/*=== операции по параметрам до даты возникновения просрочки ===*/
DEF NEW GLOBAL SHARED TEMP-TABLE all-sum-par NO-UNDO 
   FIELD par  AS INT64
   FIELD summ AS DEC
   FIELD date AS DATE
.
/*=== остатки на параметрах ===*/
DEF TEMP-TABLE each-par NO-UNDO    
   FIELD par   AS INT64
   FIELD total AS DEC
.

/*=== KMBIS: Облагородим немного код ===*/
/* Логика работы остается без изменений, меняется только оформление кода +  */
/* Добавляется анализ темпорированных рекизитов для мигрировавших договоров */

/*================================================================================================*/
/*=== Разбивает суммы гашения по КД по параметрам для обеспечения расчетов по методу FIFO ========*/
PROCEDURE FifoSum:
   DEF OUTPUT PARAM is-ok AS LOG  NO-UNDO. /* успешно ли выполнена ? */
/*
  Синтаксис : ФункцияДляПримера2 ( Параметр1 [, Параметр2 ] )
	  Если общий расчет, то:
		1 пар-р - Номер договора, 
                2 пар-р - Сумма гашения, 
		3 пар-р - текущий параметр, 
                4 пар-р - последовательность параметров.
	  Если по конкретному параметру, то 1 пар-р - текущий параметр.
  Автор     : MNG-SKU
*/
/*=== Пример вызова функции в транзакции ===*/
/*
FifoSum( ДогНом(), 
         Neg(ОстБлокСуммР(Дб(3),Вал(3),"v")), 
         377,    
         ConCat( _if(ПСумма(Кредит,3,ПД,LE) + ПСумма(Кредит,1,ПД,LE) > 0, "377," , "" ), 
                "10,210,48,248,233,229,7")
        ) 
*/
/* DEF VAR fiforesult  AS  DEC   NO-UNDO. */
DEF VAR pp-contcode AS  CHAR  NO-UNDO. /* 1 параметр парсера: сont-code договора      */
DEF VAR pp-total    AS  DEC   NO-UNDO. /* 2 параметр парсера: доступные средства      */
DEF VAR pp-par      AS  INT64 NO-UNDO. /* 3 параметр парсера: код итогового параметра */
DEF VAR pp-seq      AS  CHAR  NO-UNDO. /* 4 параметр парсера: список параметров       */
DEF VAR pp-k        AS  INT64 NO-UNDO. /* Счетчик для цикла                           */
DEF VAR pp-sum      AS  DEC   NO-UNDO.	
DEF VAR pp-sum1     AS  DEC   NO-UNDO EXTENT 2.
DEF VAR pp-mainsum  AS  DEC   NO-UNDO INIT 0.
DEF VAR vPrmCode    AS  CHAR  NO-UNDO. /* Коды темпорированных реквизитов             */
...
DEF BUFFER bPrmSign  FOR tmpsigns.
   ...
   /*=== Удаляем таблицу гашений ===*/
   {empty all-sum-par}
   /*=== Удаляем таблицу с общей суммуй параметров ===*/
   {empty each-par}
   /*=== Получаем параметры парсерки ===*/
   ASSIGN
   ...
   DO pp-k = 1 TO NUM-ENTRIES(pp-seq):        
      ...
      RUN STNDRT_PARAM in h_loan("Кредит",                 /* Назначение договора   */
                                 ...
                                 wop.con-date,             /* На дату документа     */
                                 OUTPUT pp-sum,            /* Общая сумма параметра */
                                 OUTPUT pp-sum1[1],        /* Сумма по дебету       */
                                 OUTPUT pp-sum1[2]).       /* Сумма по кредиту      */
      IF pp-sum GT 0 THEN
      DO:
         /*=== Параметр не нулевой ===*/
         CREATE each-par.
	 ASSIGN 
            ...
            /* Сразу формируем соответсвующие коды темпорированных реквизитов */
            vPrmCode       = SUBST("&1,ПРМ&2", vPrmCode, STRING(each-par.par))
   ...
   /*=== Параметры по датам их возникновения ===*/
   /*=== Берем только "нормальные" параметры, порожденные в ходе жизни договора (НП НРДатаСтарта) ===*/
   FOR EACH loan-int WHERE loan-int.cont-code                    EQ pp-contcode 
      ...
      DO:
         /*=== Создаем запись по параметру в разрезе дат ===*/
         CREATE all-sum-par.
         ASSIGN
   ...
   /*=== Учтем то что было до начала жизни договора в базе, задается через темпорированные реквизиты ===*/ 
   lTmpParamFnd:
   FOR FIRST bLoan WHERE bLoan.contract  EQ "КРЕДИТ"
      ...
      /* Нашли темпорированный реквизит с параметром, найдем для него запись в */
      /* таблице с общими остатками по параметром (не нулевыми)                */
      FOR FIRST each-par WHERE each-par.par   EQ INT64(SUBSTR(bPrmSign.code, 4))
                           AND each-par.total GT 0
                         EXCLUSIVE-LOCK:
         /*=== Ищем значение параметра ===*/
         vTmpDec = bPrmSign.dec-value.                    /* По-хорошему */
         IF vTmpDec EQ 0 OR vTmpDec EQ ? THEN
            vTmpDec = DEC(bPrmSign.code-value) NO-ERROR.  /* Чуть хуже, значит кто-то изменил определение реквизита */
         IF vTmpDec EQ 0 OR vTmpDec EQ ? THEN
            vTmpDec = DEC(bPrmSign.xattr-value) NO-ERROR. /* Совсем плохо: или поменяно определение реквизита       */
                                                          /*               или в реквизите записаны кривые данные   */
         IF     vTmpDec GT 0 
            AND vTmpDec NE ? 
         THEN
         DO:
   ...
   END. /* lTmpParamFnd: FOR FIRST bLoan WHERE bLoan.contract  EQ "КРЕДИТ" */

   /*=== Выбор операций для гашения ===*/
   FOR EACH all-sum-par BREAK BY all-sum-par.date                        /* Сортируем по дате возникновения                        */
                              BY LOOKUP(STRING(all-sum-par.par),pp-seq): /* Приоритет параметров задан положением параметра строке */

      IF all-sum-par.summ LE pp-total THEN 
         pp-total = pp-total - all-sum-par.summ. /* Уменьшаем средства на размер параметра */
      ELSE 
      DO:
         /*=== Сумма доступных средств меньше размера параметра ===*/
         ASSIGN
            all-sum-par.summ = pp-total /* Корректируем размер параметра */
            pp-total         = 0        /* Обнуляем доступные средства   */
         .
         VALIDATE all-sum-par.
   ...
   /*=== Начинаем основной подсчет: результат работы парсера ===*/
   pp-mainsum = 0. 
