...
DEFINE VARIABLE mAllowNegativeSaldo AS LOGICAL NO-UNDO.
DEFINE VARIABLE mAllowCardIndex     AS LOGICAL NO-UNDO.
...
ASSIGN
  ...
  type-bal         = FGetSettingEx("ТипБал",          ?,            "", no) =  "Да"
  Cart2Flag        = FGetSettingEx("Карт2уКлиента",   ?,         "Нет", no) =  "Да"
  saldo_check_sett = (TRIM(FGetSetting("СКонСальдо",?,"")) =  {&saldo_yes})
...
IF NUM-ENTRIES(vMaskDohRash,";") >  1 THEN
...
FUNCTION fmt-summ RETURNS CHARACTER (INPUT vSumm AS DECIMAL):
   ...
   DO mCount = 1 TO LENGTH(ENTRY(1,STRING(ABS(vSumm)),'.')):
      mFormat = "z" + mFormat.
      IF (mCount MODULO 3) =  0 THEN
         mFormat = "," + mFormat.
   END.
   mFormat = TRIM(mFormat,",") + ".99".
   IF      vSumm >  0 THEN RETURN (STRING(ABS(vSumm),mFormat) + "Дб").
   ELSE IF vSumm <  0 THEN RETURN (STRING(ABS(vSumm),mFormat) + "Кр").
...
FUNCTION OpIsSvod RETURNS LOG
   ...
   FIND FIRST op WHERE RECID(op) =  ipRecOp NO-LOCK NO-ERROR.
   IF AVAIL op THEN
      RETURN GetXAttrValue("op",STRING(op.op),"link-op-sum") <> "".
   ELSE
      RETURN NO.
END FUNCTION.
...
PROCEDURE Check-Op-Entry:
   ...
   IF (op-entry.currency <> "" AND op-entry.amt-cur =  0 AND op-entry.qty =  0) THEN
      RETURN.
...
PROCEDURE Anl-Stb.
  ...
  DEFINE VARIABLE vStat   AS CHARACTER   NO-UNDO.
   ...
   IF (op-entry.currency <> "" AND op-entry.amt-cur =  0) THEN
      RETURN.
  ...
  flager = 1.
  FIND ycode WHERE ycode.class =  "Статус"
               AND ycode.code  =  op-entry.op-status NO-LOCK NO-ERROR.
  {&FillProgressError}
...
*/
   ...
   DEF VAR vUserChoise    AS CHARACTER            NO-UNDO.
   ...
   &SCOPED-DEFINE BAD-MENU-CHOICE                              ~
      ...
      OR INT64 (pick-value)        =  NUM-ENTRIES (choose-list)  ~
      OR INT64 (pick-value)        =  0

   &SCOPED-DEFINE CONT-MENU-CHOICE ENTRY(INT64(pick-value),choose-list) =  choose-debit[{&ADD_CONT}] OR ~
                                   ENTRY(INT64(pick-value),choose-list) =  choose-credit[{&ADD_CONT}]
   pick-value = "".
   READKEY PAUSE 0.
   ...
   ASSIGN
      ...
      vNotBlokAcctDb = (GetSysConf("NotCheckBlockAcctDb") =  "Да")
      vNotBlokAcctCr = (GetSysConf("NotCheckBlockAcctCr") =  "Да")
      vUserChoise    = GetSysConf("Acct-bal-budget-ex")
   .
   ...
   IF     vNotChkChar                  <> ""
      AND vNotChkChar                  <> ?
      AND NUM-ENTRIES(vNotChkChar,"#") >= 2
      AND ENTRY(1,vNotChkChar,"#")     =  op.op-kind + "!" + STRING(RECID(op-entry))
   ...
   END.
    ...
    IF FGetSetting("СКонСПар", "", "НЕТ") <> "ДА"
      AND acct.contr-acct <> ""
      AND (   (    INDEX(cod-event,"ksa") <> 0
               AND acct.side              =  "А")
           OR (    INDEX(cod-event,"dsp") <> 0
              AND acct.side =  "П")
           ) THEN
   DO:
      flager = 0.
...
/* Из проверки на возможность постановки на картотеку выключена проверка
   ...
   IF setrassh            <> ""            AND /* контрольный статус         */
                                               /* установлен                 */
      op-entry.op-status  >= setrassh      AND
      (vStatusValue       =   ""           OR
       vStatusValue       =   "Да")        AND /* процедура соединения ?     */
...
/*----------------------------------------------------------------------------------------------*/
   ...
   THEN DO:
      ...
      IF fl-cardinc
         ...
         AND acct.acct-cat =  "b" THEN
      DO:

         RUN CheckCard2Entry(BUFFER cacct, BUFFER op).

         IF RETURN-VALUE = "" THEN /* вводимая проводка не связана с      */
         DO:                       /* внебалансовой списания с картотеки ?*/
            vNoOrdBlkLst = BlockAcctNoOrderPay(acct.acct + "," + acct.currency,
                                               DATETIME(TODAY, MTIME)).
            IF {assigned mBlTypes} AND 
               CAN-DO(mBlTypes,mAcctStatType) AND 
               vUserChoise <> "YES" AND 
               (Chk_OrderPay_For_CBLACCT(INPUT op.order-pay) OR 
                Chk_AcctRec_For_CBLACCT(IF {assigned op.ben-acct} THEN op.ben-acct 
      ...
      pick-value = STRING(NUM-ENTRIES(choose-list)). /* по умолчанию - отменить*/
        ...
        {&OpMess} IF LOOKUP(choose-debit[{&ADD_CRDB}],choose-list) =  0 THEN "core17" ELSE "core56",
      ...
      DO:
         ...
         ELSE DO:
            FIND op-kind WHERE
                 op-kind.op-kind =  choose-tran[{&ADD_CRD2}] NO-LOCK NO-ERROR.
            {&FillProgressError}
   ...
   IF cod-event =  "d_ksa" THEN              /* Кредитовое сальдо при дебетовании           */
   DO:                                       /* активного счета                             */
   ...
   DO:
      mAllowNegativeSaldo = NO.
      PUBLISH "IsNegativeSaldoAllowed" (op-entry.op,
                                        op-entry.op-entry,
                                        acct.acct,
                                        acct.currency,
                                        cod-event,
                                        OUTPUT mAllowNegativeSaldo).
      IF mAllowNegativeSaldo <> YES THEN DO:
      {additem.i choose-list choose-debit[{&ADD_CONT}]}    /* продолжить                        */
      {additem.i choose-list choose-debit[{&ADD_RJCT}]}    /* отменить                          */
...
CHECK_SALDO1:
      REPEAT WHILE recalc ON ENDKEY UNDO, RETURN
         ...
         ASSIGN
            ...
            trans-list  = ""
            recalc      = FALSE
            mess-saldo  = IF cod-event =  "d_dspb" THEN
                             "Нельзя списать блокированную сумму.~n"  +
                             "Блокировано " + fmt-summ(mbl-pos) + " " +
                             (IF sh-val <> 0 THEN Get_Val_Name(INPUT acct.currency,
                                                               INPUT mbl-pos) + "~n" +
         ...
         DO ON ERROR UNDO, RETRY:
            ...
            DO:
               ...
               DO:
                  ...
                  vNoOrdBlkLst = BlockAcctNoOrderPay(acct.acct + "," + acct.currency,
                                                     DATETIME(TODAY, MTIME)).
                  IF (CAN-DO(mLstBlk,mAcctStatType) AND 
                      vUserChoise NE "YES") OR
            ...
            DO:
               mAllowCardIndex = NO.
               PUBLISH "IsCardIndexAllowed" (op-entry.op,
                                             op-entry.op-entry,
                                             acct.acct,
                                             acct.currency,
                                             cod-event,
                                             vCartName,
                                             OUTPUT mAllowCardIndex).
               IF mAllowCardIndex THEN
                  LEAVE.
               ELSE DO:
                  if fl-cardinc then 
                  do:
                     ...
                     {&OpMess}"core20","","%s=" + ClueAcctCurr(acct.acct,acct.currency) +
                                          "%s=" + (IF vCartName =  "Карт2" THEN "2" ELSE (IF vCartName =  "КартБлСч" THEN "блокированных счетов" ELSE "")) +
                                          "|" + choose-list).
                  end.
                  else pick-value = "1".
               END.
            ...
            DO:
               str1 = "%s=" + ClueAcctCurr(acct.acct,acct.currency) +
                      "%s=" + (IF NOT {assigned acct.check-op}
                              ...
                              mess-saldo  + ' ' +  mes-dop.
               str1 = str1 +
                      (IF SUBSTRING(TRIM(str1),LENGTH(TRIM(str1))) =  "."
                       THEN ""
                       ELSE ".") +
               ...
               IF cod-event = "d_dspb" AND LOOKUP(choose-debit[{&ADD_CRDB}],choose-list) <> 0 THEN DO:
                  ...
                  ELSE DO:
                     ...
                     THEN DO:
                        IF  {assigned vAccessStatus} AND op-entry.op-status >= vAccessStatus THEN DO:
            ...
            ELSE
            DO:
               IF  ENTRY(INT64(pick-value),choose-list) =  choose-debit[{&ADD_CRDB}]  THEN DO:
               ...
               ELSE DO:
                  ...
                  FIND op-kind WHERE
                       op-kind.op-kind =  ENTRY(INT64(pick-value),trans-list)
                  NO-LOCK NO-ERROR.
                  {&FillProgressError}
                  ...
                  DO:
                     ...
                     RUN cli-pos IN h_base (acct.acct,
                                            ...
                                            gend-date,
                                            "П").
                     IF    (acct.currency <> "" AND sh-val >  lim-pos)
                        OR (acct.currency =  "" AND sh-bal >  lim-pos) THEN
                     DO:
                        recalc = YES.
                        NEXT CHECK_SALDO1.
                     END.
...
/*------------------------------------------------- Обработки при кредитовании счетов -----------*/
   ...
   IF cod-event =  "k_ksa" THEN              /* Кредитовое сальдо на активном счете при кредитовании */
   DO:
check_saldo2:
...
suggest2:
         DO ON ERROR UNDO, RETRY:
            ...
            END.
            ELSE DO:
               FIND op-kind WHERE op-kind.op-kind =  ENTRY(INT64(pick-value),trans-list)
                   NO-LOCK NO-ERROR.
               {&FillProgressError}
               ...
               DO:
                  ...
                  IF    (acct.currency =  ""  AND sh-bal <  lim-pos)
                     OR (acct.currency <> ""  AND sh-val <  lim-pos)
                     OR (acct.acct-cat =  "d" AND sh-bal <  lim-pos) THEN
                  DO:
                     recalc = YES.
                     NEXT check_saldo2.
                  END.
   ...
   IF cod-event =  "k_dsp" THEN             /* дебетовое сальдо пассивного счета кредита   */
   DO:
      ...
      IF lim-pos =  0 THEN
         mess-saldo  = (IF {assigned vAccessMask} AND CAN-DO(vAccessMask,Acct.acct) AND
                       {assigned vAccessContAcct} AND CAN-DO(vAccessContAcct,acct.contract)
                        ...
                        THEN "доступное " ELSE "") +
                       "сальдо " +
                       (IF sh-val <> 0 THEN fmt-summ(sh-val) + " " +
                                            Get_Val_Name(INPUT acct.currency,
                                                         INPUT sh-val) + "~n" +
                                            "(" + fmt-summ(sh-bal) + " " +
                                            Get_Val_Name("", INPUT sh-bal) + ")"
      ...
      IF {assigned vAccessStatus} AND op-entry.op-status <  vAccessStatus AND
...
/*------------------- Подкрепление депозита,возврат кредита при овердрафте,списание с картотеки.*/
   IF cod-event =  "k_ksp" THEN              /* кредитовое сальдо пассивного счета в кредите*/
   DO:
      IF fl-cardinc THEN
      DO:
         IF AVAIL cacct THEN
      ...
      IF fl-poddep THEN
      DO:
         FIND FIRST xop WHERE xop.op-transaction =  op.op-transaction
                          AND xop.acct-cat       =  "b"
                          AND CAN-FIND(FIRST op-entry
                                          WHERE op-entry.op       =  xop.op
                                            AND op-entry.acct-cr  =  buf-acct.acct
                                            AND op-entry.currency =  buf-acct.currency)
               NO-LOCK NO-ERROR.
      ...
      DO:
         FIND FIRST xop WHERE xop.op-transaction =  op.op-transaction
                          ...
                          AND CAN-FIND(FIRST op-entry
                                          WHERE op-entry.op       =  xop.op
                                            AND op-entry.acct-cr  =  buf-acct1.acct
                                            AND op-entry.currency =  buf-acct1.currency)
               NO-LOCK NO-ERROR.
...
check_saldo3:
      REPEAT WHILE recalc ON ENDKEY UNDO,RETURN ON ERROR UNDO,RETURN:
         ...
         DO:
            ...
            IF mBlSum + mLimSum - sh-bal <= 0.0 THEN fl-cardinc = NO.
            ...
            END.
         END.
         IF choose-list =  "" OR (NOT fl-poddep AND NOT fl-ovdr AND NOT fl-cardinc) THEN
         DO:
            flager = 0.
            {&RelOp}
            RETURN.
...
PROCEDURE Check-Bank:
   ...
   ELSE DO:
      ...
      ASSIGN
         bank-dis  = GetCodeMisc("bnk-real",bank-real,2) =  "ДА"
      ...
      IF    bank-dis
         OR (    vDateChk <> ?
             AND vDateChk <= gend-date + vDay) THEN /* или скоро запретят        */
      DO:
...
PROCEDURE Kau-Trigger:
   ...
   IF mess-mode AND (flag-error <> 0) THEN
      RUN Fill-SysMes IN h_tmess ("", vErrCode, "", vErrInfo).

   pick-value = STRING (flag-error =  0).
...
find first op-entry of op no-lock no-error.
   ...
   FIND FIRST xop WHERE
          ...
          AND CAN-FIND(FIRST op-entry WHERE
                         ...
                         AND (op-entry.amt-rub = vAmtRub OR vAmtRub =  0)
                         AND (op-entry.amt-cur = vAmtCur OR vAmtCur =  0)
                      )
   NO-LOCK NO-ERROR.
   ...
   DO:
      ...
      FIND FIRST xop WHERE xop.op-transaction =  op.op-transaction
                       AND xop.op-template    <  op.op-template
                       AND xop.acct-cat       =  "b"
                       AND CAN-FIND(FIRST op-entry WHERE op-entry.op       =  xop.op
                                                     AND op-entry.currency =  cacct.currency
                                                     AND op-entry.amt-rub  =  vAmtRub
                                                     AND op-entry.amt-cur  =  vAmtCur
                                   )
         NO-LOCK NO-ERROR.
...
PROCEDURE UpdStatusAfterFin:
   ...
   bl:
   DO:
      FIND FIRST op WHERE RECID(op) =  ipRecOp EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
      IF AVAIL op THEN
      DO:
         ...
         DO:
            ...
            IF ERROR-STATUS:ERROR THEN DO:
               IF RETURN-VALUE <> "" THEN
                  opError = RETURN-VALUE.
               LEAVE bl.
            END.


            IF op.user-id =  USERID("bisquit") THEN
...
/*   DEF PARAMETER BUFFER op     FOR op.   /*буффер документа*/ */
   ...
   ASSIGN
      ...
      opKodPS = ""
   .
   FIND FIRST op WHERE op.op =  in-opop NO-LOCK NO-ERROR.
...
PROCEDURE pGet117ReqN:
   ...
   opKodVO = GetXAttrValueEx("op",
                             ...
                             "КодОпВал117",
                             "").
   IF    opKodVO =  "" /* для валютных тоже можно указать в назначении */
      OR NUM-ENTRIES(opKodVO) <= 5 THEN
   DO:
      IF opKodVO =  "" THEN
      DO:
         vI = INDEX(op.details,"~{VO") + 3.
         ...
         DO:
            ...
            .
            IF    vI1 =  0          /* нет PS          */
               OR vI2 <  vI1 THEN   /* или за скобками */
               vI1 = vI2.
            IF vI2 >  0 THEN        /* есть что писать */
            DO:
               ASSIGN
                  opKodVO = SUBSTRING(op.details,vI,vI1 - vI)
                  opKodPS = IF vI2 >  vI1 THEN SUBSTRING(op.details,vI1 + 2,vI2 - vI1 - 2)
                                          ELSE ""
               .
            END.
         END.
         ...
         IF vI2 <= 0 OR ERROR-STATUS:ERROR OR LENGTH(opKodVO) <> 5 THEN
            RETURN "3".
      END.
      ELSE DO:
         vtt-handle:BUFFER-CREATE.
         ASSIGN
            opKodPS = IF NUM-ENTRIES(opKodVO) >  1 THEN
                         ENTRY(2,opKodVO)
   ...
   ELSE DO: /* для валюты может быть много записей в реквизитах */
      ...
      DO vI = 1 TO NUM-ENTRIES(opKodVO,"|"):
         ...
         /* Убираем ссылку на фрагмент КОП, если есть */
         opKodPS = ENTRY(1, opKodPS, "$").
         vtt-handle:BUFFER-CREATE.
         IF NUM-ENTRIES(opKodPS) >= 5 THEN
         DO:
            ASSIGN
            ...
            .
            IF     vtt-handle:BUFFER-FIELD("VC-cu"):BUFFER-VALUE =  ""
               AND vtt-handle:BUFFER-FIELD("VC-su"):BUFFER-VALUE <> 0 THEN
               vtt-handle:BUFFER-FIELD("VC-cu"):BUFFER-VALUE = "643".
            IF NUM-ENTRIES(opKodPS) >  5 THEN
               vtt-handle:BUFFER-FIELD("op2su"):BUFFER-VALUE = DECIMAL(ENTRY(6,opKodPS)).
            IF NUM-ENTRIES(opKodPS) >  6 THEN
               ASSIGN
               ...
               .
            ELSE DO:
               FIND FIRST tmpop-entry OF op WHERE tmpop-entry.acct-db <> ?
               ...
               FIND FIRST tmpop-entry OF op WHERE tmpop-entry.acct-cr <> ?
            ...
            END.
         END.
         IF NUM-ENTRIES(opKodPS) >= 10 THEN
         DO:
            ASSIGN
...
PROCEDURE pGet117ReqB:
   ...
   FOR EACH op-entry OF OP WHERE (    op-entry.acct-db <> ?
                                  AND NOT CAN-DO(mBalExs,op-entry.acct-db))
                              OR (    op-entry.acct-cr <> ?
      ...
      ASSIGN
         ...
         vtt-handle:BUFFER-FIELD("VC-cu"):BUFFER-VALUE =
            IF NUM-ENTRIES(opSumValCon) >  1 THEN ENTRY(2,opSumValCon)
                                             ELSE ""
         vtt-handle:BUFFER-FIELD("db-cr"):BUFFER-VALUE =
            IF op-entry.acct-db <> ? THEN "Д"
                                     ELSE "К"
      .
      IF     op-entry.acct-db <> ?
      ...
      DO:
         ...
         IF op-entry.currency <> "" THEN     /*смотрим не рублевый ли счет */
         DO:
            {find-act.i
         ...
         RUN GetCountryIdCli IN h_Acct (op-entry.acct-db,
                                        ...
                                        OUTPUT mCustCat).
         IF     mCustCat <> "Б"
            AND (   macctcountry =  "RUS"
                 OR macctcountry =  "643") THEN
      ...
      DO:
         ...
         DO:
            ...
            ELSE
               vAcctCurr = "".
         END.
         IF vtt-handle:BUFFER-FIELD("KodPS"):BUFFER-VALUE =  "" THEN
         DO:
...
:
   ...
   DO WHILE mParent <> 0:
      FIND FIRST op-trans WHERE op-trans.op-trans =  mParent NO-LOCK NO-ERROR.
   ...
   IF AVAILABLE op-trans THEN DO:
      ...
      DO WHILE TRUE:
         mLevel = mLevel + 1.
         mNumNodes = 0.
         FOR EACH tt-xtree WHERE tt-xtree.level =  mLevel:
            FOR EACH op-trans WHERE op-trans.parent =  tt-xtree.op-trans NO-LOCK:
            ...
            END.
         END.
         IF mNumNodes =  0 THEN LEAVE.
      END.

      /* собственно поиск любой другой операции, принадлежащего дереву */
      find-another-op:
      FOR EACH tt-tree:
         FOR EACH op WHERE op.op-trans =  tt-tree.op-trans
                       AND op.op       <> iOp
         NO-LOCK:
            mResult = op.op.
...
FUNCTION Chksgnopint RETURNS LOGICAL
   ...
   FIND FIRST signs WHERE
              signs.FILE-NAME =  "op-int"
          AND signs.code =  "ДокОтр47423"
...
FUNCTION IsOpConv RETURNS LOGICAL (INPUT  iOp     AS INT64,
   ...
   FOR EACH op-entry WHERE
      ...
      IF op-entry.acct-db = ? THEN DO:
         ...
         IF vCurrency =  op-entry.currency THEN DO:
            IsOpConv = NO.
            LEAVE.
         END.
         ASSIGN
            ...
            oAmtCr    = IF op-entry.currency <> "" THEN op-entry.amt-cur ELSE op-entry.amt-rub
         .
      END.
      IF op-entry.acct-cr = ? THEN DO:
         ...
         ASSIGN
            ...
            vCurrency = op-entry.currency
            oAmtDb    = IF op-entry.currency <> "" THEN op-entry.amt-cur ELSE op-entry.amt-rub
         .
      END.
   END.
...
FUNCTION CountDocNum RETURNS CHARACTER (INPUT iDefCount AS INT64,
   ...
   */
   IF mNameCount =  "" THEN RETURN STRING(iDefCount).
   ...
   IF mCountVal =  ? THEN RETURN "ERROR".
...
DO ON ERROR UNDO MAIN, LEAVE MAIN:
   vCheck = (fGetSetting ("СтандТр", "CheckRedSaldo", "Нет") =  "Да") NO-ERROR.
   IF NOT vCheck THEN
      LEAVE MAIN.
   {find-act.i
      &acct = iAcct
   ...
   IF NOT ((CAN-DO(vContMask, acct.contract)) OR (CAN-DO(vAcctMask, acct.acct))) THEN
      LEAVE MAIN.
   IF (acct.side     =  "П"     AND iSide         =  "db" AND
      acct.check-op <> "ДбОст" AND acct.check-op <> "ДбКрОст") OR
     (acct.side     =  "А"     AND iSide         =  "cr" AND
      acct.check-op <> "КрОст" AND acct.check-op <> "ДбКрОст")
       THEN
   DO:
      vLastClDay = Get_Date_Cat(acct.acct-cat, ?).

      /* В закрытых днях вероятное красное сальдо не проверяется */
      IF vLastClDay <> ? AND vLastClDay >= iOpDate THEN oError = NO.
      ELSE DO:
         ...
         vBal = IF acct.currency =  "" THEN sh-bal ELSE sh-val.
         IF iStat >= CHR(251) THEN
            vBal = IF acct.currency =  "" THEN (vBal + iAmtRub) ELSE (vBal + iAmtCur).
         vDbA = IF acct.currency =  "" THEN sh-db ELSE sh-vdb.
         IF iStat >= CHR(251) THEN
            vDbA = IF acct.currency =  "" THEN (vDbA + iAmtRub)  ELSE (vDbA + iAmtCur).
         RUN acct-pos IN h_base (acct.acct, acct.currency, vLastClDay + 1, iOpDate, "П").
         vDbP = IF acct.currency =  "" THEN sh-db ELSE sh-vdb.
         IF iStat >= "П" THEN
            vDbP = IF acct.currency =  "" THEN (vDbP + iAmtRub) ELSE (vDbP + iAmtCur).
         ASSIGN
   ...
   DO ON ERROR UNDO MAIN, LEAVE MAIN:
      ...
      ELSE DO:
         ...
         IF iCrd THEN DO:
            ...
            IF vOAcct <> ? THEN
               FIND FIRST oacct WHERE RECID(oacct) =  vOAcct NO-LOCK NO-ERROR.
            IF AVAIL(oacct) THEN
         ...
         ELSE DO:
            ...
            END.
            ASSIGN
               oBal = sh-bal + IF acct.side <> "П" THEN (sh-db0  - sh-db - vSumcrd2 - vBldb) ELSE (sh-cr  - sh-cr0 + vSumCrd2 - vBlcr) /* только акцептованнные документы зачисления (дебет активных счетов, кредит пассивных)*/
               oVal = sh-val + IF acct.side <> "П" THEN (sh-vdb0 - sh-vdb - vBldb) ELSE (sh-vcr - sh-vcr0 - vBlcr)
            .
         END.
      END.
   END.
...
/* $LINTUSER='seei' */
/* $LINTDATE='04/07/2017 11:09:34.487+03:00' */
/*prosignExZTUsl/MGHCTT6mul3dVA*/