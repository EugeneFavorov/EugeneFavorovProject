...
ON  LEAVE , GO OF in-cont-code IN FRAME opreq DO :
  ...
  if not avail loan then do :
   message 'Вклад не существует или уже закрыт' view-as alert-box  error .
   return no-apply.
  end.
...
&IF DEFINED(recond) = 0 &THEN 
  ...
  MESSAGE "Пролонгацию нельзя делать раньше  даты закрытия вклада " + string(loan.end-date)  view-as alert-box error .
  return no-apply.
 end.
 if in-op-date <  loan.end-date and input cred >  loan.end-date then do :
  message 'При проведении пролонгации до даты окончания договора плановая дата должна совпадать с датой окончания договора'   view-as alert-box error .
  return no-apply.
 end.
&ELSE
    IF vHeirRole = "" THEN
    DO:
      ...
      IF pick-value = "" THEN
         RETURN NO-APPLY "Не еыбран наследник".
      ELSE
...
&ENDIF
 ...
 DO: 
    ...
    DO:
  message 'При проведении пролонгации после даты окончания договора плановая дата должна быть не больше даты опердня'   view-as alert-box error .
  return no-apply.
    END.
 END.
       ...
       RUN ProcessDetails (RECID(wop), INPUT-OUTPUT op.details).
        ...
        if avail op-entry then do :
         ...
         SET
            op.doc-num
            op.doc-type
            op-entry.amt-cur  VALIDATE((IF OP-ENTRY.AMT-RUB =  0 THEN input op-entry.amt-cur <> 0 AND input op-entry.amt-cur <> ? ELSE TRUE),
                                      "Сумма не может быть равна нулю")
            op-entry.currency WHEN op-template.currency =  ?
            op-entry.symbol
            op-entry.acct-db WHEN op-templ.acct-db =  ? OR op-templ.acct-db =  ""
                               OR modify-flag-db
                             VALIDATE(modify-flag-db OR CAN-FIND(LAST acct WHERE acct.acct =  input op-entry.acct-db),
                                      "Счет по дебету не найден")
            op-entry.acct-cr WHEN op-templ.acct-cr =  ? OR op-templ.acct-cr =  ""
                               OR modify-flag-cr
                             VALIDATE(modify-flag-cr OR CAN-FIND(LAST acct WHERE acct.acct =  input op-entry.acct-cr),
                                      "Счет по кредиту не найден")
            op.details
          WITH FRAME opreq.
         end.
...
&IF DEFINED(recond) = 0 &THEN
   ...
   RUN put_kd.p (op-kind.op-kind,
                 ...
                 "loan-op-kind",
                  ...
                  output in-op-kind).
   IF fl-o =  -1 THEN DO:
      MESSAGE 'С этим договором кто-то работает'
      VIEW-AS ALERT-BOX ERROR BUTTONS OK.
  ...
  /* здесь подключается метод для определения новых-старых реквизитов вклада */
   ...
   if nameproc <> ? then  do:
     ...
     IF RETURN-VALUE <> "" THEN DO:
        IF RETURN-VALUE = "UNDO_STS" THEN modify_loan = recid(loan).
