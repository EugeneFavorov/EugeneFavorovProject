...
/* +++ f-person.p was humbly modified by (c)blodd converter v.1.11 on 8/28/2017 8:43am +++ */
...
&IF DEFINED(IF_DEFINED_THEN_THIS_BLOCK_CAN_BE_DELETED) =  0 &THEN
CREATE WIDGET-POOL.
&ENDIF
/* ***************************  Definitions  ************************** */
...
DO:
   ...
   THEN DO:
      ...
      IF {&LAST_EVENT_FUNCTION} <> "END-ERROR"
      THEN DO:
...
DO:
   ...
   IF {assigned tt-person.inn} THEN DO:
      ...
      DO:
        ...
        DO:
          RUN Fill-AlertSysMes IN h_tmess("","",1,'В ИНН запрещены только нули.').
      ...
      DO:
        RUN Fill-AlertSysMes IN h_tmess("","",1,'В ИНН должно быть 5, 10 или 12 цифр, введено ' + TRIM(STRING(vLen)) + '.').
      ...
      IF vTempVal <> ? THEN DO:
         ...
         THEN DO:
           DO:
              ...
              RUN Fill-AlertSysMes IN h_tmess("","",4,"Запись с такими реквизитами уже существует в таблице:" + CHR(32) + SUBSTR(xperson.name-last + " " + xperson.first-names, 1, 80) + CHR(32) + "!" + CHR(32) + "~n" + CHR(32) + "OK - Исправить, Cancel - Пропустить").
              vfl = (pick-value = "YES").
      ...
      IF vLen <> 5 AND NOT fValidInnSignature(SELF:SCREEN-VALUE, mTempVal) THEN DO:
        DO:
           mblodd_char_Tmp01 = pick-value.
           RUN Fill-AlertSysMes IN h_tmess("","",4,("Последние 2 цифры ИНН (ключ) должны быть: ~"" + STRING(mTempVal) + "~"") + CHR(32) + "~n" + CHR(32) + "OK - Исправить, Cancel - Пропустить").
           vfl = (pick-value = "YES").
           pick-value = mblodd_char_Tmp01.
        END.
   ...
   THEN DO TRANSACTION:
      ...
      IF AVAIL bcident THEN DO:
         ...
         DO:
             ...
             ELSE {return_no_apply.i '{&RET-ERROR}'}
         END.
...
&ENDIF
   ...
   ** в зависимости от режима открытия */
   RUN EnableDisable.
&IF DEFINED(IF_DEFINED_THEN_THIS_BLOCK_CAN_BE_DELETED) &THEN
   RETURN ERROR.
&ENDIF
...
&GLOBAL-DEFINE CustIdntSurr    cust-ident.cust-code-type + ',' +                                cust-ident.cust-code      + ',' +                                STRING(cust-ident.cust-type-num)
...
/* $LINTUSER='seei' */
/* $LINTDATE='19/04/2017 07:48:02.940+03:00' */
/*prosigntPH5HOreVL9wbUhyK998/w*/
/* --- f-person.p was humbly modified by (c)blodd converter v.1.11 on 8/28/2017 8:43am --- */
