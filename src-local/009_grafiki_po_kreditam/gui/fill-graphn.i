...
/* +++ fill-graphn.i was humbly modified by (c)blodd converter v.1.09 on 7/26/2016 7:46am +++ */
...
*/
    ...
    FIND FIRST ttReportTable
        WHERE ttReportTable.tf_payment-date EQ iDate
        NO-LOCK NO-ERROR.
    /* Если не нашли, тогда надо создать запись на дату */
    IF NOT AVAIL ttReportTable
    THEN DO:
        CREATE ttReportTable.
        ttReportTable.tf_id = 0.    /* Потом перенумеруем */
        tf_payment-date = iDate.
    END.

    /* Проценты присутствуют в каждой строке, остальные могут отсутствовать * /
    IF iNum EQ 1
    THEN ttReportTable.tf_id = iID. */

    /* Разбираемся с видом операции по ее коду и накапливаем сумму */
    CASE iNum:
        WHEN 1 THEN     /* Проценты */
            ttReportTable.tf_sum-percent        = ttReportTable.tf_sum-percent        + iSumma.
        WHEN 2 THEN     /* Остатки */
            ttReportTable.tf_rest-debts         = ttReportTable.tf_rest-debts         + iSumma.
        WHEN 3 THEN     /* Обязательства по погашению основного долга */
            ttReportTable.tf_basic-sum-loan     = ttReportTable.tf_basic-sum-loan     + iSumma.
        WHEN 4 THEN     /* Дополнительные суммы 1 -  */
            ttReportTable.tf_additional-charge1 = ttReportTable.tf_additional-charge1 + iSumma.
        WHEN 400 THEN   /* Дополнительные суммы */
            ttReportTable.tf_additional-charge2 = ttReportTable.tf_additional-charge2 + iSumma.
        WHEN 5 THEN     /* Страховые и прочие суммы */
            ttReportTable.tf_actual-payment     = ttReportTable.tf_actual-payment     + iSumma.
    END CASE.
