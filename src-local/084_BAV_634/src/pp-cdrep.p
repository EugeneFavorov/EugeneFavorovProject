...
PROCEDURE StartInterface.
   ASSIGN
      mNew316  = FGetSetting("Форма316", "ДосрПог", "") =  "ПОЛ(ГОД)".
   .
   RETURN.
END PROCEDURE.
/* &GLOB IS-DEBUG true */
...
FUNCTION LN_GetAcct RETURNS CHAR
   ...
   RUN RE_L_ACCT IN h_Loan(iContract,
                           iContCode,
                           GetMainAcctRole(iClassCode, iContract),
                           iDate,
                           BUFFER loan-acct).
...
PROCEDURE LN_GetPrsDateEx: 
   ...
   DO:
      ASSIGN       
         oProcFl = NO   
         vPosCntrPrmO = IF NUM-ENTRIES(iPosCntrPrmO,":") =  2 THEN ENTRY(2,iPosCntrPrmO,":")
      ...
      REPEAT vCnt = 1 TO NUM-ENTRIES(vPosCntrPrmO, ";"):
         ...
         .
         IF vBracketL >  0 AND vBracketR >  0 THEN
            ASSIGN
      ...
      REPEAT:
         ...
         DO:
            ...
            FOR EACH ttPrmPog:
               ...
               END.
               ELSE 
                  IF iPosCntrPrmT =  "cont" THEN
                  DO:
                     ttPrmPog.summPog = ttPrmPog.summPog + ttPrmPog.vunos.
                     
                     IF ttPrmPog.vunos <> 0 THEN
                        IF SESSION:DEBUG-ALERT THEN
                        DO: 
                           {putfile.i  &of="/*" 
...
/* Конец вставки Плюс банк */
      ...
      DO:
         ...
         .
            /* Если нужно только %% */
         IF     GetSysConf("Просрочка%%") =  "Да"
            AND vDatePrP                  <> ? THEN
            oResDate = vDatePrP.
            /* Если нужно только ОД */
         ELSE IF GetSysConf("ПросрочкаОД") =  "Да"
            AND vDatePr                    <> ? THEN
            oResDate = vDatePr.
...
/* Замена Плюс банк
      IF iCurrDay AND oResDate =  iDate THEN 
*/    IF vFlag THEN 
...
FUNCTION LN_GetProcPeriod RETURNS INT64
   ...
   DO vI = 1 TO NUM-ENTRIES (vS):
      ...
      FIND FIRST chowhe WHERE chowhe.id-op =  INT64(ENTRY(vI, vS)) NO-LOCK NO-ERROR.
      IF AVAIL chowhe THEN
      DO:
         ...
         IF AVAIL loan-int THEN
            IF vDate =  ? OR loan-int.mdate > vDate THEN vDate = loan-int.mdate.
      END.
   END.
   ...
   IF AVAIL term-obl THEN
      IF vDate =  ? OR term-obl.end-date > vDate THEN vDate = term-obl.end-date.
...
PROCEDURE Ln_BefPogLoan.
   ...
   vDate = IF loan.open-date >  iBegDate
                        THEN loan.open-date
                        ELSE iBegDate.
...
&ENDIF
   ...
   FIND LAST bterm-obl-hist WHERE
             bterm-obl-hist.contract  =  loan.contract
         AND bterm-obl-hist.cont-code =  loan.cont-code
         AND bterm-obl-hist.idnt      =  3
         AND bterm-obl-hist.olap      =  "1"
         AND bterm-obl-hist.since     <= vDate
   NO-LOCK NO-ERROR.
   ...
   DO:
      ...
      BY term-obl.end-date:
         ...
         IF term-obl.end-date >  iEndDate THEN
            LEAVE T-OBL.
      END.
      &IF DEFINED(IS-DEBUG) <> 0 &THEN
         OUTPUT TO "cdrep.log" APPEND.
   ...
   DO:
      ...
      FOR EACH tobl-hist-amt WHERE
               tobl-hist-amt.tobl-id  =  bterm-obl-hist.tobl-id
      NO-LOCK
      BY tobl-hist-amt.end-date:
         ...
         vHistSince = bterm-obl-hist.since.
            /* Необходимо собирать графики до даты расчета 
               плюс еще один, чтобы учесть погашения в 
               последнюю дату планового погашения */
         IF tobl-hist-amt.end-date >  iEndDate THEN
            LEAVE T-OBL_HIST.
      END.
      &IF DEFINED(IS-DEBUG) <> 0 &THEN
         OUTPUT TO "cdrep.log" APPEND.
...
/*----------------------------------------------------------------------------------------------------------------------*/
   ...
   IF YEAR(loan.open-date) <  YEAR(iBegDate) THEN
   DO:
      vDate2 = iBegDate - 1.
      LOOP_TO:
      FOR EACH tt-hist-amt WHERE
               tt-hist-amt.end-date <= vDate2
      NO-LOCK:
      ...
      NO-LOCK:
         ...
         DO:
            ...
            IF vNumCr =  0 THEN
               LEAVE BLK_50.
            IF vNumDb <> vNumCr THEN
               LEAVE BLK_50.
            vDolg = vDolg - loan-int.amt-rub.
         END.
            /* 5 операция */
         IF loan-int.id-d  =  1 AND loan-int.id-k =  2 THEN
         DO:
            vDolg = vDolg - loan-int.amt-rub.
         END.
      END.
   ...
   DO:
      LOOP_TO:
      FOR EACH tt-hist-amt WHERE
               tt-hist-amt.end-date >= vDate1 /* если кого-то смущает, что переменная используется внутри не волнуйтесь на запрос это не влияет */
      NO-LOCK:
         vNoper = vNoper + 1.
         IF tt-hist-amt.end-date >  iEndDate THEN
            vDate2 = iEndDate.
         ELSE
         ...
         NO-LOCK:
            ...
            DO:
               IF NOT CAN-FIND (FIRST bloan-int WHERE
                                      bloan-int.op        =  loan-int.op
                                  AND bloan-int.op-entry  =  loan-int.op-entry
                                  AND bloan-int.contract  =  loan.contract
                                  AND bloan-int.cont-code =  loan.cont-code
                                  AND bloan-int.id-d      =  0
                                  AND bloan-int.id-k      =  3
                                NO-LOCK) THEN
               DO:
                  vDolg = vDolg  - loan-int.amt-rub.
                  IF vDolg <  0 THEN
                  DO:
                     ...
                     ASSIGN
                        ...
                        vSumPogRub = CurToBase("Учетный", 
                                               loan.currency, 
                                               loan-int.mdate, 
                                               vSumPogCur)  WHEN loan.currency <> ""
                        vSumPogRub = vSumPogCur             WHEN loan.currency =  ""
                     .
                     IF iCrOpEn THEN
                     DO:
                        CREATE tt-OpEntry.
         ...
         IF vHistSince <> ? THEN 
            ...
            FIND FIRST loan-cond WHERE loan-cond.contract  =  loan.contract
                                   ...
                                   AND loan-cond.since     =  tt-hist-amt.end-date
                                   AND loan-cond.since     <= vHistSince
   ...
   DO:
      LOOP_TO:
      FOR EACH tt-hist-amt WHERE
               tt-hist-amt.end-date >= vDate1
           AND tt-hist-amt.end-date <= iEndDate
      NO-LOCK:
         vDolg = vDolg + tt-hist-amt.amt-rub.
...
PROCEDURE Ln_BefPogByAcct.
   ...
   NO-LOCK:
      ...
      FIND FIRST xloan-acct WHERE
                 xloan-acct.contract  =  loan-acct.contract
             AND xloan-acct.cont-code =  loan-acct.cont-code
             AND xloan-acct.acct-type =  loan-acct.acct-type
             AND xloan-acct.since     >  loan-acct.since
      NO-LOCK NO-ERROR.
...
PROCEDURE GetCollatSinglByParam.
   ...
   vListOper = vListOperDb + (IF vListOperCr <> "" AND vListOperDb <> "" THEN "," ELSE "") + vListOperCr.
   ...
   DO vI = 1 TO NUM-ENTRIES(vListOper):
      FIND FIRST chowhe WHERE
                 chowhe.id-op =  INT64(ENTRY(vI, vListOper))
      NO-LOCK NO-ERROR.
      IF AVAIL chowhe THEN
      DO:
         IF chowhe.id-d <> ?
...
PROCEDURE GetVidOb.
   ...
   DO:
      ...
      DO:
         ...
         IF NUM-ENTRIES(oVidObespech) <> 1 THEN
            oVidObespech = "".
      END.
   END.
      /* если код определен */
...
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTUSER='soav' */
/* $LINTDATE='07/03/2017 11:27:43.925+03:00' */
/*prosignV5h3oRrauecED0M1WnNTIw*/