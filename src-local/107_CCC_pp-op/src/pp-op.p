   ...
   ASSIGN
      ...
      DO:
         ...
         RUN SetFastCache IN h_cache (PROGRAM-NAME(1),
                                      ...
                                      {unknown vSymbol},"").
      END.
      IF FGetSetting("СтандТр","ОпСрокДокум","Нет") EQ "Да" THEN
      DO:
...
PROCEDURE vo__op:
   ...
   DEFINE VARIABLE vFlError  AS LOGICAL    NO-UNDO.
   DEFINE VARIABLE vTxtUndo  AS CHARACTER  NO-UNDO.
   DEFINE VARIABLE vPrevStat AS CHARACTER NO-UNDO.
   ...
   RUN CheckNPS_OP IN h_nps (vop,
                             IF {assigned vPrevStat} AND 
                             vPrevStat < iObject:BUFFER-FIELD("op-status"):BUFFER-VALUE 
                             THEN 'Акцепт'
                             ELSE 'Ввод',
                             OUTPUT vFlError,
                             OUTPUT vError,
                             OUTPUT vTxtUndo).
   IF vFlError THEN DO:
      IF vTxtUndo = "WARNING" THEN DO:
         RUN Fill-SysMes IN h_tmess ("","","4",vError).
         IF   pick-value = "yes" OR 
              pick-value = "2"   OR 
              pick-value = ? 
         THEN DO:
            RETURN 'ERROR'.
   ...
   vChv2 = FALSE.
        ...
        IF USERID("bisquit") begins 'SERV' THEN DO:
        end.
        ELSE do:
