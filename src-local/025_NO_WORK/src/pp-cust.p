...
FUNCTION GetIntegrationID RETURNS INT64:
   ...
   IF LENGTH(STRING(vIntegrationID)) > LENGTH(vIdFormat) THEN DO:
      MESSAGE "Формат интеграционных номеров клиентов (integrationID) для класса person" 
              "составляет" STRING(LENGTH(vIdFormat)) "символов, невозможно"
              "поместить следующее значение " quoter(STRING(vIntegrationID))
              "!" SKIP
   ...
   DO:
      ...
      IF LENGTH(vUnk) > LENGTH(vUnkFormat) THEN DO:
         MESSAGE "Формат уникальных номеров клиентов (УНК) для класса" iFile
                 ...
                 "поместить следующее значение " quoter(string(vNextUnk))
