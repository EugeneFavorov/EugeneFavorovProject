...
/* +++ pp-cust.p was humbly modified by (c)blodd converter v.1.09 on 9/19/2016 7:56am +++ */
...
/*
   ...
   Parameters: нет
     ...
     Modified: 03/03/2014 sasa (0161083) доработана процедура NewUnk.
     Modified: 04/03/2014 zaka (221688) добавлена ф-я GetIntegrationID
     Modified: 30.10.2015 13:11 MAXIMUS 
     Modified: 03/12/2015 seei (0249745) - В функции GetCustIDoc введено 
                                           ограничение на закрытые удостоверения
                                           личности клиента, теперь они не 
                                           отбираются для номера паспорта
*/

{pfuncdef
 &DefLib="cust" 
 &Description="Библиотека работы с клиентами"}
...
/* Возвращает КПП внешнего клиента по БИКу и счету и ИНН */
   ...
   DEFINE OUTPUT PARAMETER oValue     AS CHARACTER NO-UNDO.
   DEFINE BUFFER client-recepient FOR client-recepient.
   ...
   FOR EACH client-recepient NO-LOCK
...
/*получение интеграционного номера*/

FUNCTION GetIntegrationID RETURNS INT64:
   DEFINE VARIABLE vIntegrationID   AS INT64 NO-UNDO.
   DEFINE VARIABLE vID AS CHARACTER NO-UNDO.
   DEFINE VARIABLE vIdFormat AS CHARACTER NO-UNDO.
   DEFINE VARIABLE vOut AS CHARACTER NO-UNDO.
   vIntegrationID = GetCounterNextValue("Integration",?).
   vIdFormat = GetXAttrEx("person","integrationid","data-format").
   IF LENGTH(STRING(vIntegrationID)) > LENGTH(vIdFormat) THEN DO:
      RUN Fill-AlertSysMes IN h_tmess("","",-1,"Формат интеграционных номеров клиентов (integrationID) для класса person" + CHR(32) + "составляет" + CHR(32) + STRING(LENGTH(vIdFormat)) + CHR(32) + "символов, невозможно" + CHR(32) + "поместить следующее значение " + CHR(32) + quoter(STRING(vIntegrationID)) + CHR(32) + "!" + CHR(32) + "~n" + CHR(32) + "Обратитесь к администратору!").

      RETURN ?.
   END.
   RETURN INT64(vIntegrationID).
END FUNCTION.
...
FUNCTION NewUnk RETURNS DECIMAL (iFile AS CHARACTER):
   ...
   DO:
      ...
      IF LENGTH(vUnk) > LENGTH(vUnkFormat) THEN DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Формат уникальных номеров клиентов (УНК) для класса" + CHR(32) + STRING(iFile) + CHR(32) + "составляет" + CHR(32) + STRING(LENGTH(vUnkFormat)) + CHR(32) + "символов, невозможно" + CHR(32) + "поместить следующее значение " + CHR(32) + quoter(string(vNextUnk)) + CHR(32) + ( IF LENGTH(vUnkBeg) > 0 THEN "с префиксом '" + STRING(vUnkBeg) + "'" ELSE "") + CHR(32) + "!" + CHR(32) + "~n" + CHR(32) + "Обратитесь к администратору!").
      ...
      IF NOT vUnk BEGINS vUnkBeg 
         ...
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Закончились доступные уникальные номера клиентов (УНК)!" + CHR(32) + "~n" + CHR(32) + "Обратитесь к администратору!").
   ...
   DO:
      ...
      REPEAT:
         ...
         IF CAN-FIND(FIRST code WHERE code.code    = "CNTUNK"
                                  AND code.class   = "Counters"
                                  AND code.parent  = "Counters"
                                  AND code.misc[3] = STRING({&CNT_BY_FILIAL}))  
         THEN vNextUnk = GetCounterNextValue("CNTUNK",?) NO-ERROR.
         ELSE
         vNextUnk = DECIMAL(SendData("COUNTER;CNTUNK_" + vUnkBeg + "_0,CNTUNK,NEXT,0", 5)) NO-ERROR.

         
         IF vNextUnk = ? OR ERROR-STATUS:ERROR THEN DO:
            RUN Fill-AlertSysMes IN h_tmess("","",-1,"Закончились доступные уникальные номера клиентов (УНК) " + CHR(32) + ( IF LENGTH(vUnkBeg) > 0 THEN "с префиксом '" + STRING(vUnkBeg) + "'" ELSE "") + CHR(32) + "!" + CHR(32) + "~n" + CHR(32) + "Обратитесь к администратору!").
...
PROCEDURE ChkUpdDate-out.
   ...
   DEF VAR vLoanList AS CHAR   NO-UNDO. /* Классы договоров без контроля */
   ...
   ELSE DO:
      ...
      THEN 
      DO:   
      vLoanList = FGetSetting("ЗакрКл","ДогБезКонтр","").
      FOR FIRST loan WHERE
         ...
         AND NOT CAN-DO(vLoanList,loan.Class-Code) 
         NO-LOCK:
         FIND FIRST class WHERE class.Class-Code EQ loan.Class-Code NO-LOCK NO-ERROR.
         vErrMsg = "У клиента есть открытые договоры '" 
                 + ( IF CAN-DO("АХД,sf-in,sf-out",loan.contract)
                    THEN loan.doc-num
                    ELSE loan.cont-code) 
                 + "' с классом '" + ( IF AVAIL Class THEN class.NAME ELSE "")  
                 + "("             + loan.Class-Code 
                 + ")'.".
      END.
      END.
...
PROCEDURE SetCustAdr.
   ...
   NO-LOCK:
      ...
      FIND FIRST cust-ident WHERE cust-ident.cust-code-type EQ ttCustAddress.fTypeAdr
                         ...
                         AND cust-ident.cust-type-num EQ ttCustAddress.fCustNum 
...
PROCEDURE SyncAdrSubjToCident.
   ...
   THEN DO ON ERROR  UNDO, LEAVE
      ...
      IF AVAIL bCustIdent THEN DO:
         ...
         IF    ERROR-STATUS:ERROR
            OR {&RETURN_VALUE} GT "" THEN
            RETURN ERROR ( IF {&RETURN_VALUE} GT ""
                          THEN {&RETURN_VALUE}
   ...
   CASE ipCustCat:
      ...
      WHEN "Ю" THEN DO:
         FIND FIRST cust-corp 
            WHERE cust-corp.cust-id = ipCustId NO-LOCK NO-ERROR.
         IF AVAIL cust-corp 
      ...
      WHEN "Б" THEN DO:
         FIND FIRST banks WHERE banks.bank-id = ipCustId NO-LOCK NO-ERROR.
          IF AVAIL banks 
             THEN DO:
...
PROCEDURE CreateUpdateRecipient:
   ...
   DEFINE BUFFER code-1 FOR code.

   &SCOPED-DEFINE LOCK_RECIPIENT IF NOT vInWrite THEN DO: ~
   FIND CURRENT xxcode EXCLUSIVE-LOCK NO-WAIT NO-ERROR. vInWrite = YES. END.
   ...
   DO TRANSACTION ON ERROR  UNDO, RETURN ERROR
      ...
      FIND FIRST xxcode WHERE xxcode.class = "recipient"
                          AND xxcode.code  = iBIK + "," + iAcct +
                                             ( IF iINN <> "" THEN "," + iINN ELSE "")
      NO-LOCK NO-ERROR.
      IF  NOT AVAIL xxcode
      THEN FIND FIRST xxcode WHERE xxcode.class = "recipient"
                               AND xxcode.code  = iBIK + "," + iAcct +
      ...
      IF NOT AVAIL xxcode THEN DO:
         ...
         ASSIGN
            ...
            xxcode.code   = iBIK + "," + iAcct + "," + iINN
            xxcode.val    = IF {assigned iINN} AND (vNew OR CAN-DO(iFldToUpd,"ИНН"))
                               THEN iINN ELSE ""
         .
      END.
      IF {assigned iName} THEN DO:
         ...
         END.
      END.
      IF {assigned iINN} AND
         NOT CAN-FIND(FIRST code-1 WHERE
                                     code-1.class = "recipient" AND
                                     code-1.code  = iBIK
                                                    + "," +
                                                    iAcct
                                                    + "," +
                      ...
                      NO-LOCK)
      THEN DO:
         IF xxcode.code <> iBIK + "," + iAcct + "," + iINN THEN DO:
            {&LOCK_RECIPIENT}
            xxcode.code = iBIK + "," + iAcct + "," + iINN.
         END.
         IF xxcode.val <> iINN
...
PROCEDURE SetUNKP.
   ...
   DO ON ERROR  UNDO MAIN, LEAVE MAIN
      ...
      DO:
         ...
         DO:
            FIND CURRENT bCustIdent EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
            IF AVAIL bCustIdent THEN
               ASSIGN
...
FUNCTION CalcSrokForDocument RETURNS DATE
   ...
   DEFINE VAR vFormula    AS CHAR  NO-UNDO.
   DEFINE VAR i AS INTEGER NO-UNDO.
   ...
   ELSE IF iFormula BEGINS "Р" THEN DO:
      vFormula = SUBSTRING(iFormula,2).
      DO i = 1 TO NUM-ENTRIES(vFormula,";"):
         vFirstSrok =  INT64(ENTRY(i,vFormula,";")) NO-ERROR.
         IF ERROR-STATUS:ERROR THEN DO:
            RUN Fill-SysMes IN h_tmess ("","","-1",
               "Неправильно задана формула расчета" + iFormula +
               " даты окончания документа ").
            LEAVE.   
         END.
         oDate1 = DATE(MONTH(iBirthDate),DAY(iBirthDate),YEAR(iBirthDate) 
                + vFirstSrok) NO-ERROR.
         IF ERROR-STATUS:ERROR THEN DO:  /* день рождения 29.02 */
            oDate1 = DATE(MONTH(iBirthDate),DAY(iBirthDate) - 1,
                     YEAR(iBirthDate) + vFirstSrok) 
                     NO-ERROR.
         END.
         IF iDateDoc LT oDate1 THEN DO:
            oDate = oDate1.
            LEAVE.
         END.
      END.
...
/* 0185863  с учетом ДР ФорматНаим на транзакции и процедуре*/
PROCEDURE GetCustNameFormatted:
   ...
   AND iCustId > 0 THEN DO:
      ...
      /*185863*/
     IF vEntry1 EQ ? AND vEntry2 EQ ? THEN DO:
         IF vOpKind EQ "" THEN DO: /*user-proc*/
            FIND FIRST user-proc WHERE RECID(user-proc) EQ vNumProc NO-LOCK NO-ERROR.
            IF AVAIL (user-proc) THEN DO:
               vFormatNaim = GetXAttrValueEx("user-proc", STRING(user-proc.public-number) , "ФорматНаим", "").
                  IF NOT {assigned vFormatNaim} THEN 
                     ASSIGN
                        vEntry1 = ?
                        vEntry2 = ?
                     .
     ...
     IF vEntry1 EQ ? AND vEntry2 EQ ? THEN DO:  /*op-template*/
            FIND FIRST op-kind WHERE op-kind.op-kind EQ vOpKind NO-LOCK NO-ERROR.
            IF AVAIL(op-kind) THEN DO:
               FIND FIRST op-template 
                  WHERE op-template.op-kind EQ vOpKind AND op-template.op-template EQ vTmplID NO-LOCK NO-ERROR.
               IF AVAIL (op-template) THEN DO:
                  vFormatNaim = GetXAttrValueEx("op-template", STRING(op-template.op-kind) + "," + STRING(op-template.op-template), "ФорматНаим", "").
                  IF NOT {assigned vFormatNaim}
                     THEN ASSIGN
                     vEntry1 = ?
                     vEntry2 = ?
...
/*----------------------------------------------------------------------------*/
/* Определяет категорию клиента по счету                                      */
/*----------------------------------------------------------------------------*/
{pfuncdef 
 &DefFunc="GetCatOnAcct"
 &Description="Определяет категорию клиента по счету   "
 &Parameters="iBalAcct - бал.счет,~n
iParams - "список ассоциаций <Категория:Маска БС> ("Б:30109,30110;П:40802;Ч:40817,40820;Ю:40702"),~n
iDefault - значение по умолчанию
 &Result="Категория клиента"
 &Sample="GetCatOnAcct('40817','П:40802','Ч')"}

FUNCTION GetCatOnAcct RETURNS CHARACTER (INPUT  iBalAcct AS CHARACTER,
   ...
   IF NOT {assigned iParams} THEN
      iParams = fGetSetting("СтандТр","CatOnAcct","").
   IF NOT {assigned iParams} THEN
      RETURN.
      
   vResult = iDefault.
   DO vI = 1 TO NUM-ENTRIES(iParams,";"):
       vMask = GetEntries(2,ENTRY(vI,iParams,";"),":","").
       IF CAN-DO(vMask,iBalAcct) THEN 
         vResult = GetEntries(1,ENTRY(vI,iParams,";"),":","").         
   END.
...
{pfuncdef 
   &DefProc     = "GetBankrupcyInfo"
   &Description = "Определение сведений о банкротстве клиента"
   &Parameters  = "iCustCat - тип клиента;~
                   iCustId  - идентификатор клиента;~
                   iDate    - дата, на которую проводить проверку.~
                   При iDate = ? подставляется gend-date."
   &Result      = "oIsBankrupt - является ли клиент банкротом;~
                   oStage      - стадия банкротства;~
                   oDocNum     - номер дела о банкротстве;~
                   oEndDate    - дата решения о банкротстве."
   &Sample      = "RUN GetBankrupcyInfo IN h_cust (vCustCat,~
                                                   vCustId,~
                                                   iOpDate,~
                                                   OUTPUT vIsBankrupt,~
                                                   OUTPUT vStage,~
...
PROCEDURE GetBankrupcyInfo:
   DEFINE INPUT  PARAMETER iCustCat    LIKE loan.cust-cat NO-UNDO.
   DEFINE INPUT  PARAMETER iCustId     LIKE loan.cust-id  NO-UNDO.
   DEFINE INPUT  PARAMETER iDate       AS   DATE          NO-UNDO.
   ...
   IF iDate = ? THEN
      iDate = gend-date.
   FIND-LOAN:
   /* SORT-ACCESS loan сортировка по open-date для выбора последнего договора */
   FOR EACH loan WHERE
      loan.cust-cat   =  iCustCat   AND
      loan.cust-id    =  iCustId    AND
      loan.class-code =  "bankrupt" AND
      loan.contract   =  "bankrupt" AND
      (loan.end-date = ? AND loan.open-date <= iDate OR
       loan.end-date <= iDate)
   NO-LOCK
   BY loan.open-date DESCENDING:
      oStage = GetXAttrValue("loan", Surrogate(BUFFER loan:HANDLE), "ТипБанкр").
      ASSIGN
         oIsBankrupt = CAN-DO(FGetSetting("БанкрКонтр", "СтадииБанкр", ""),
                              oStage)
         oDocNum     = loan.doc-num
         oEndDate    = loan.end-date
      .
      LEAVE FIND-LOAN.
   END.
END PROCEDURE.
...
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTUSER='shoi' */
/* $LINTDATE='24/03/2016 12:10:39.262+04:00' */
/*prosignr5no4kUyvmT4RSEY+H1aqA*/
/* --- pp-cust.p was humbly modified by (c)blodd converter v.1.09 on 9/19/2016 7:56am --- */
