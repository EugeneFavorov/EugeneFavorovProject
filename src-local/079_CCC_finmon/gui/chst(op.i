...
/* +++ chst(op.i was humbly modified by (c)blodd converter v.1.11 on 8/15/2017 6:11am +++ */
...
&ENDIF
   ...
   IF     vDocTypeDig <> "02"
      AND vDocTypeDig <> "06"
      AND NOT CAN-DO(FGetSetting("СтатусыАннул","",""),vstatus)
      AND (       (vOrderPay = ?)
           OR NOT (vOrderPay >= 1 AND vOrderPay <= 3)) THEN
      FOR EACH op-entry OF op NO-LOCK:
         ...
         DO:
            ...
            DO:
               ...
               RUN Fill-SysMes IN h_tmess ("", "acct43", "", "%s=" + vCustNameLine + "%s=" + acct.number).
               &IF DEFINED(open-undo) <> 0 &THEN
                  {&open-undo}.
               &ELSE
                  UNDO, RETRY.
...
/* Замена Плюс банк
                     op.op-status = bCode.misc[1].
*/                   IF (grCode.misc[1] NE "") THEN
                     op.op-status = grCode.misc[1].
                     validate op.
/* Конец замены Плюс банк */
                     vVisaRes = YES.                 
                  END.
...
/* Конец вставки Плюс банк */
   ...
   IF NOT CAN-DO("no-method,no-proc",{&RETURN_VALUE}) AND pick-value <> "yes" THEN DO:
      &IF DEFINED(open-undo) <> 0 &THEN
         {&open-undo}.
      &ELSE
         UNDO, RETRY.
      ...
      IF v-xattr-cr-I = 2 THEN DO: /* если они есть (и требуют редактирования) */
           ...
           &IF DEFINED(xattr-undo) <> 0 &THEN
              ...
              IF    CAN-DO("1,2",pick-value)
                 OR vSelectQuest =  "2" 
                 OR {&LAST_KEY} =  KEYCODE("ESC") THEN DO:
                 {&xattr-undo}.
              END.
           &ELSE
              UNDO,LEAVE.
...
/* $LINTUSER='nikk' */
/* $LINTDATE='29/05/2017 13:16:58.302+03:00' */
/*prosignTkJPFwkUwHfjg1G3oJbZ0A*/
/* --- chst(op.i was humbly modified by (c)blodd converter v.1.11 on 8/15/2017 6:11am --- */
