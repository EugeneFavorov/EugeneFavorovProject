...
/* +++ f-acct.p was humbly modified by (c)blodd converter v.1.11 on 6/22/2017 1:19pm +++ */
...
DO:
   ...
   IF {&RETURN_VALUE} EQ {&RET-ERROR}
      THEN {return_no_apply.i '{&RET-ERROR}'}
   {&END_BT_LEAVE}
END.
...
DO:
   ...
   /*ccc*/
   IF  iMode EQ {&MOD_ADD}
   AND {&LAST_EVENT_FUNCTION}  NE "END-ERROR" 
...
DO:
   ...
   IF {&RETURN_VALUE} EQ {&RET-ERROR} THEN {return_no_apply.i}
   ...
   ON QUIT  UNDO TR, LEAVE TR:
      ...
      IF ERROR-STATUS:ERROR
         ...
         RUN Fill-SysMes IN h_tmess ("","","-1",{&RETURN_VALUE}).
         {return_no_apply.i}
      END.
      ...
      ELSE DO:
         ...
         IF ERROR-STATUS:ERROR THEN DO:
            ...
            vErrStr = "".
            IF {assigned RETURN-VALUE} THEN vErrStr = SUBSTR({&RETURN_VALUE},( IF INDEX({&RETURN_VALUE},"pp-acct.p<-") > 0 THEN INDEX({&RETURN_VALUE},"pp-acct.p<-") + 11 ELSE 1)) + CHR(10).
...
DO:
   ...
   DO:
      ...
      IF {&RETURN_VALUE} EQ {&RET-ERROR} THEN {return_no_apply.i '{&RET-ERROR}'}
   END.
   {&END_BT_LEAVE}
END.
