...
/* +++ stmt-c.i was humbly modified by (c)blodd converter v.1.11 on 5/25/2017 6:48am +++ */
...
ELSE DO:
   ...
   ELSE IF acct.cust-cat NE "В" THEN DO:
      ...
      IF acct.cust-cat = 'Ю' AND
         NOT (vTmpName = LEFT-TRIM(mAcctDetails) AND name[2] = "")
      THEN DO:
         RUN GetCustInfo2 (13, acct.acct, acct.currency, OUTPUT NAME[1]).
...
/* Замена Плюс банк
   RUN acct-pos IN h_base (acct.acct, acct.currency, qopd, qopd, ?).
...
IF acct.acct-cat NE "d" THEN
/* Замена Плюс банк
   RUN acct-pos IN h_base (acct.acct, acct.currency, qopd + 1, dob, ?).
...
/* $LINTUSER='ches' */
/* $LINTDATE='10/01/2017 15:31:06.871+03:00' */
/*prosignzMzn2OoGdaISU5RoJbO/sg*/
/* --- stmt-c.i was humbly modified by (c)blodd converter v.1.11 on 5/25/2017 6:48am --- */
