...
/* +++ stmt-r.i was humbly modified by (c)blodd converter v.1.11 on 5/25/2017 6:49am +++ */
...
DEFINE VARIABLE mAcctDetails AS CHARACTER NO-UNDO .
...
RUN SetSysConf IN h_base ("stmtTmpDate",string(dob) ).
/*acct.details с учетом темпарированности */
mAcctDetails = GetTempXattrValueEx ("acct", acct.acct + "," + acct.currency,"details",dob,"").
IF not {assigned mAcctDetails} THEN mAcctDetails = acct.details .
...
ELSE DO:
   ...
   END.
   ELSE IF acct.cust-cat <> "В" THEN DO:
      ...
      IF acct.cust-cat = 'Ю' AND
         NOT (vTmpName = mAcctDetails AND name[2] = "")
      THEN DO:
         RUN GetCustInfo2 (13, acct.acct, acct.currency, OUTPUT NAME[1]).
...
FIND FIRST stmt WHERE
           stmt.op-date >= ( IF vZoBeg =  ? THEN beg ELSE MIN (beg, vZoBeg))
       AND stmt.op-date <= ( IF vZoEnd =  ? THEN dob ELSE MAX (dob, vZoEnd))
   NO-ERROR.
...
/* Замена Плюс банк
   RUN acct-pos IN h_base (acct.acct, acct.currency, beg - 1, beg - 1, ?).
...
IF acct.acct-cat <> "d" THEN
/* Замена Плюс банк
   RUN acct-pos IN h_base (acct.acct, acct.currency, beg, dob, ?).
...
IF NOT vEndOfYear                                         AND
   FGetSetting ("Выписки","ОстатокЗО","Текущие") =  "Все" AND
   NOT Flag-ZO                                            THEN
DO:

   IF Dob <  End-Of-ZO THEN
   DO:
      ...
      IF acct.acct-cat <> "d" THEN
/* Замена Плюс банк
         RUN acct-pos IN h_base (acct.acct,acct.curr, Dob + 1, End-Of-Zo, ?).
...
DO:
   FIND FIRST loan WHERE
         loan.contract  =  bLoanAcct.contract
   AND   loan.cont-code =  bLoanAcct.cont-code NO-LOCK NO-ERROR.
   IF AVAIL(loan) THEN
   ASSIGN
...
FIND FIRST _user WHERE
   _user._userid =  mOIuserid
NO-LOCK NO-ERROR.
IF AVAILABLE _user THEN
   mOIName = _User-Name.
/*Адрес */
FOR EACH cust-ident WHERE
         cust-ident.cust-cat       =  acct.cust-cat
   AND   cust-ident.cust-id        =  acct.cust-id
...
FOR EACH cust-ident WHERE
         cust-ident.cust-cat       =  acct.cust-cat
   AND   cust-ident.cust-id        =  acct.cust-id
   AND   cust-ident.cust-code-type =  "Паспорт"
...
/* */
ASSIGN
    prev-db = IF sh-in-bal >= 0 THEN STRING (  sh-in-bal, "{&in-format}") ELSE ""
    prev-cr = IF sh-in-bal <  0 THEN STRING (- sh-in-bal, "{&in-format}") ELSE ""
    cnt     = 0{&cnt}
.
...
REPEAT i =  ( IF ts.pgd AND NOT ts.dwidth THEN 1 ELSE 2)
         TO ( IF ts.cmode =  3 AND acct.cust-cat <> "В" THEN 3 ELSE 2)
...
IF iPrintProc <> "" THEN DO:
   ...
   IF ts.splitmode THEN DO:
      ...
      FOR EACH stmt BREAK BY stmt.op-date:
         IF stmt.op-date =  dob AND LAST-OF(stmt.op-date) THEN DO:
   ...
   ELSE DO:
      ...
      FOR EACH stmt BREAK BY stmt.acct:
         IF stmt.acct =  acct.acct AND LAST-OF(stmt.acct) THEN DO:
            {stmtotal.i &stmtrub="YES" &new="YES"}
         END.
      END.
   END.
...
&IF DEFINED(stmtmgx) <> 0 &THEN
    IF ts.pgdevery AND cur-page-str =  "" THEN
    cur-page-str = "Стр. 1".
&ENDIF

    FOR EACH op-date WHERE
             op-date.op-date >= Beg
         AND op-date.op-date <= Dob NO-LOCK:
       &IF DEFINED(dbcr) =  0 &THEN
    ...
    IF CAN-FIND ( FIRST stmt WHERE stmt.prev-year
                    AND stmt.op-date >= vZoBeg
                    AND stmt.op-date <= vZoEnd ) THEN DO:
       zo-prt = 0.
       FOR EACH op-date WHERE
             op-date.op-date >= vZoBeg
         AND op-date.op-date <= vZoEnd NO-LOCK:
         &IF DEFINED(dbcr) =  0 &THEN
    ...
    END.
IF iPrintProc =  "" THEN DO:
    IF i <> 1 AND ((flag-prt = 1 AND NOT ts.emptystmt) OR ts.emptystmt) THEN DO:
       IF flag-prt =  0 AND ts.emptystmt THEN DISPLAY "" @ stmt.doc-num.
...
/* $LINTUSER='seei' */
/* $LINTDATE='07/04/2017 12:15:49.276+03:00' */
/*prosign5Kz1hCV3h+H7tMyvCQikYg*/
/* --- stmt-r.i was humbly modified by (c)blodd converter v.1.11 on 5/25/2017 6:49am --- */
