...
ASSIGN 
   mDelimList =  fGetSetting("Стоп-листы","DelimSymbol","^")
...
PROCEDURE ChkStopListByNamePerc: 
   ...
   ASSIGN
      mPercLent = INT64(FGetSetting("ОпОтмыв","ПроцДлины","60"))
      mPercWord = INT64(FGetSetting("ОпОтмыв","ПроцСлов", "60")).
   FOR EACH DataLine WHERE
            DataLine.Data-ID EQ INT64(iDataID)
      ...
      ASSIGN
         iName  = ReplaceBad(REPLACE(REPLACE(iName,        "ё","е"),"Ё","Е"))
         vName1 = ReplaceBad(REPLACE(REPLACE(DataLine.Sym2,"ё","е"),"Ё","Е")).
...
PROCEDURE Fill-ttSL:
   ...
   IF vDataID EQ 0 OR
      vDataID EQ ? THEN DO:
      RUN Fill-SysMes("","","-1",
                      "Не существует ссылка из классификатора StopList"  + "~n" +
                      "на класс быстрого поиска. Контроль невозможен."   + "~n" +
                      "Выполните расчет класса StopList для исправления ошибки.").
      RETURN.
   END.
   CASE iChkType:
      WHEN "ФИО" THEN
...
PROCEDURE Cre-tt-sl:
   ...
   FOR EACH t-obj-sl NO-LOCK,
      ...
      DO:
         ...
         CASE iPar1:
            WHEN "person"    THEN tt-view-sl.fld1 = "Ч".
            WHEN "cust-corp" THEN tt-view-sl.fld1 = "Ю".
            WHEN "banks"     THEN tt-view-sl.fld1 = "Б".
         END CASE.
   
         ASSIGN
            tt-view-sl.fld2  = iPar2                                           /*код клиента */
            tt-view-sl.fld3  = iPar3                                           /*наименование клиента*/
            tt-view-sl.fld4  = IF {assigned iPar4 }  THEN  iPar4 ELSE " "      /*инн*/
            tt-view-sl.fld5  = IF {assigned iPar5 }  THEN  iPar5 ELSE " "      /*паспорт*/
   
         
         &ELSE
         ASSIGN
            tt-view-sl.fld1 = iPar1     /*номер документа*/
            tt-view-sl.fld2 = iPar2     /*дата документа*/
            tt-view-sl.fld3 = iPar3     /*критерий*/
            tt-view-sl.fld4 = iPar4     /*значение критерия*/
            tt-view-sl.fld5 = " "       /*заглушка*/
...
PROCEDURE ChkStopListOp:
   ...
   DEF VAR mClType   AS CHAR     NO-UNDO.  /*тип клиентов для проверки*/
   DEF VAR mSLtype   AS CHAR     NO-UNDO.  /*виды стоп-листов, по которым проверять клиентов*/
   DEF VAR mChkType  AS CHAR     NO-UNDO.  /*вид проверки*/
   ...
   DO: 
      ...
      FOR EACH op-entry WHERE op-entry.op EQ op.op NO-LOCK:
         ...
         DO:
            ...
            IF AVAIL acct   THEN DO:
               mKName = "Кр " + DelFilFromAcct(STRING(op-entry.acct-cr)).
         ...
         DO:
             mKName = "Дб " + DelFilFromAcct(STRING(op-entry.acct-db)).
         ...
         END.
      END.
      /*проверяем счета из др acct-send acct-rec*/
      ASSIGN 
      ...
      DO:
         RUN Fill-ttSL("ИНН", op.inn, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
      ...
      DO:
         RUN Fill-ttSL("ИНН", mInnSend, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
      ...
      DO:
         RUN Fill-ttSL("ИНН", mInnRec, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
      ...
      DO:
         RUN Fill-ttSL("ФИО_ДР", op.name-ben, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
      ...
      DO:
         RUN Fill-ttSL("ФИО_ДР", op.details, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
      ...
      DO:
         RUN Fill-ttSL("ФИО_ДР", mNameSend, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
      ...
      DO:
         RUN Fill-ttSL("ФИО_ДР", mNameRec, iSLType, INPUT-OUTPUT TABLE t-obj-sl).
         IF CAN-FIND(FIRST t-obj-sl) THEN
...
PROCEDURE PrintOpSLRep.
   ...
   DO:
      ...
      PUT STREAM rep " Документы с фигурантами справочника 'Стоп-листы'"  SKIP .
      PUT STREAM rep SKIP(1).
      PUT STREAM rep "┌──────┬────────────────┬──────────────┬─────────────────────────────┬──────────────────────────────┬───────────┬──────────────────────────────┬──────────────────────────────┬─────────────────┬────────────────────┬────────────┐" SKIP.
      PUT STREAM rep "│№ п/п │ Дата документа │ № документа  │   Критерий                  │    Наименование критерия     │  № п/п из │          Примечание          │      ФИО/Наименование        │      ИНН        │   Номер резолюции  │    Дата    │" SKIP.
      PUT STREAM rep "│      │                │              │                             │                              │ Стоп-листа│                              │       из Стоп-листа          │  из Стоп-листа  │                    │  резолюции │" SKIP.
      FOR EACH tt-view-sl NO-LOCK 
         ...
         PUT STREAM rep  "│" STRING(vNum)        FORMAT "X(6)"   +                                       /*номер*/
                         "│" tt-view-sl.fld2     FORMAT "X(16)"  +                                       /*дата документа*/
                         "│" tt-view-sl.fld1     FORMAT "X(14)"  +                                       /*номер документа*/
                         "│" tt-view-sl.fld3     FORMAT "X(29)"  +                                       /*критерий*/
                         "│" vKritval[1]         FORMAT "X(30)"  +                                       /*значение критерия*/
                         "│" tt-view-sl.fld6     FORMAT "X(11)"  +                                       /*номер стоп лита*/
                         "│" IF vSL[1]           NE "" THEN vSL[1]           ELSE " " FORMAT "X(30)"  +  /*вид стоп листа*/
                         "│" IF vNameFromSL[1]   NE "" THEN vNameFromSL[1]   ELSE " " FORMAT "X(30)"  +  /*наименование из стоп листа*/
                         "│" IF tt-view-sl.fld9  NE "" THEN tt-view-sl.fld9  ELSE " " FORMAT "X(17)"  +  /*инн из стоп листа*/
                         "│" IF vResolution[1]   NE "" THEN vResolution[1]   ELSE " " FORMAT "X(20)"  +  /*Номер резолюции */
                         "│" IF tt-view-sl.fld11 NE "" THEN tt-view-sl.fld11 ELSE " " FORMAT "X(12)"  +  /*дата резолюции */
                         "│"
         SKIP. 
...
PROCEDURE PrintClientSLRep.
   ...
   PUT STREAM rep " Клиенты, совпавшие с фигурантами справочника 'Стоп-листы'"  SKIP .
   PUT STREAM rep SKIP(1).
   PUT STREAM rep "┌──────┬─────┬──────┬──────────────────────────────┬─────────────┬───────────────┬───────────┬──────────────────────────────┬──────────────────────────────┬─────────────────┬───────────────┬──────────────────────────────┬────────────────────┬────────────┐" SKIP.
   PUT STREAM rep "│№ п/п │ Тип │ Код  │     ФИО/Наименование         │    ИНН      │ Серия и номер │ № п/п из  │          Примечание          │      ФИО/Наименование        │      ИНН        │ Дата рождения │    Место рождения            │  Номер резолюции   │    Дата    │" SKIP.
   PUT STREAM rep "│      │     │      │                              │             │   паспорта    │Стоп-листа │                              │       из Стоп-листа          │  из Стоп-листа  │ из Стоп-листа │    из Стоп-листа             │                    │  резолюции │" SKIP.
   ...
   FOR EACH tt-view-sl BY tt-view-sl.fld7 BY tt-view-sl.fld1 BY tt-view-sl.fld3:
      ...
      PUT STREAM rep  "│" STRING(vNum)                                                 FORMAT "X(6)"        +
                      "│" tt-view-sl.fld1                                              FORMAT "X(5)"        +  /*тип клиента*/
                      "│" tt-view-sl.fld2                                              FORMAT "X(6)"        +  /*номер клиента*/
                      "│" vName[1]                                                     FORMAT "X(30)"       +  /*наименование клиента*/
                      "│" tt-view-sl.fld4                                              FORMAT "X(13)"       +  /*инн*/
                      "│" tt-view-sl.fld5                                              FORMAT "X(15)"       +  /*паспорт*/
                      "│" tt-view-sl.fld6                                              FORMAT "X(11)"       +  /*номер стоп-листа*/
                      "│" IF vSL[1]           NE ""     THEN vSL[1]           ELSE " " FORMAT "X(30)"       +  /*вид стоп листа*/
                      "│" IF vNameFromSL[1]   NE ""     THEN vNameFromSL[1]   ELSE " " FORMAT "X(30)"       +  /*наименование из стоп-листа*/
                      "│" IF {assigned tt-view-sl.fld9} THEN tt-view-sl.fld9  ELSE " " FORMAT "X(17)"       +  /*инн из стоп-листа*/
                      "│  " IF tt-view-sl.fld12 NE ""   THEN tt-view-sl.fld12 ELSE " " FORMAT "X(13)"       +  /*дата рождения */
                      "│" IF vBirthPlace[1]   NE ""     THEN vBirthPlace[1]   ELSE " " FORMAT "X(30)"       +  /*место рождения */       
                      "│" IF vResolution[1]   NE ""     THEN vResolution[1]   ELSE " " FORMAT "X(20)"       +  /*Номер резолюции */
                      "│" IF tt-view-sl.fld11 NE ""     THEN tt-view-sl.fld11 ELSE " " FORMAT "X(12)"       +  /*дата резолюции */
                      "│"
      SKIP. 
...
FUNCTION ChkClientByStopList RETURN LOGICAL
  (iClass      AS CHAR,     
   ...
   /*Определяем виды стоп-листов по которым проверять и действие в случаи совпадения*/

   IF iMode EQ "OP" THEN
      vSLtype = iSLType.
   ELSE 
   DO: 
      vNPContInsEditClnt =   fGetSetting("Стоп-листы",iSLType,"").
      IF  NOT (    ENTRY(1,vNPContInsEditClnt, ";")     EQ "Да" 
      ...
      IF iSLType EQ "КонтрВводРед_Кл" THEN
      DO:    
   ...
   /*0248377*/
   vDopControl = fGetSetting("Стоп-листы","ДопКритПров","").
   vDopFIO = NO.
   DO vI = 1 TO NUM-ENTRIES(vDopControl,";"):
      vDopCriteria[vI] = ENTRY(vI,vDopControl,";").
      IF vDopCriteria[vI] EQ "ДР" THEN vDopBirthDay = YES.
      IF vDopCriteria[vI] EQ "МР" THEN vDopBirthPlace = YES.
      IF vDopCriteria[vI] EQ "ФИОпроц" THEN
      DO:
         vDopFIO = YES.
         RUN Fill-ttSL("ФИОпроц",iName,vSLtype,INPUT-OUTPUT TABLE t-obj-sl).
      END.
   END.
   /*0248377*/
      ...
      CASE vChkType: 
         ...
         OTHERWISE  DO:
            IF {assigned iDocument} THEN
               RUN Fill-ttSL ("Пасп", iDocument, vSLtype,INPUT-OUTPUT TABLE t-obj-sl).     /*cначала проверяем номер паспорта для физ. лиц*/
            ELSE
               RUN Fill-ttSL ("ФИО", iName , vSLtype,INPUT-OUTPUT TABLE t-obj-sl).
         END. 
      END CASE.  
   ...
   DO:
      ...
      CASE vChkType: 
         WHEN "ИНН_Пасп" THEN
            RUN  Fill-ttSL ("ИНН", iInn, vSLtype,INPUT-OUTPUT TABLE t-obj-sl).         /*инн для юр. лиц и банков*/
         WHEN "ФИО" THEN
            RUN Fill-ttSL ("ФИО", vCustName , vSLtype,INPUT-OUTPUT TABLE t-obj-sl).
         OTHERWISE DO:
   ...
   IF vChkType EQ "*"  THEN
      RUN  Fill-ttSL ("ФИО-ИНН",vCustName, vSLtype,INPUT-OUTPUT TABLE t-obj-sl).
...
FUNCTION StopListAction    RETURN LOGICAL
   ...
   DO:   
      FOR EACH tt-view-sl NO-LOCK,
         FIRST ttSLAction WHERE 
           ...
           AND ttSLAction.CreateAction EQ  "НеСохр"
      NO-LOCK:
         RETURN NO.
      END. 
      ...
      FOR EACH tt-view-sl NO-LOCK,
       EACH ttSLAction WHERE 
               CAN-DO( ttSLAction.StopListName, tt-view-sl.fld7) 
           AND ttSLAction.CreateAction EQ  "Вопрос"
      NO-LOCK : 
      ...
      DO:    
         pick-value = "NO".
         RUN Fill-SysMes IN h_tmess ("", "", "4",  "Внимание! Лицо совпадает с фигурантами справочника Стоп-листы -  " + QUOTER(vStr)  + "!  ~n Сохранить карточку клиента?").
   ...
   FOR EACH tt-view-sl NO-LOCK,
       FIRST ttSLAction WHERE 
             CAN-DO( ttSLAction.StopListName, tt-view-sl.fld7) 
         AND ttSLAction.EditAction EQ  "Блок"
   NO-LOCK:
      UpdateSignsEx(iClass,iCust-id,"Блок", "Да").
      RETURN vReturn.
   END.
   ...
   FOR EACH tt-view-sl NO-LOCK,
    EACH ttSLAction WHERE 
            CAN-DO( ttSLAction.StopListName, tt-view-sl.fld7) 
        AND ttSLAction.EditAction EQ  "Вопрос"
   NO-LOCK:
         vFindSl = TRUE.
         {additem.i vStr tt-view-sl.fld7}
   END.
   ...
   DO:    
      pick-value = "NO".
      RUN Fill-SysMes IN h_tmess ("", "", "4", "Внимание! Клиент совпадает с фигурантом справочника Стоп-листы -  " + QUOTER(vStr)  + "!  ~n  Блокировать счета клиента (установить ДР Блок=Да)?").
...
FUNCTION ChkStopListImpExp    RETURN LOGICAL (iOp AS INT64):
   ...
   vSLType =  FGetSetting("Стоп-листы","ЭкспИмп_Док","").
   
   IF ENTRY(1,vSLType, ";") EQ "Нет" THEN
      RETURN NO.
...
PROCEDURE ChkStopListWOp:
   ...
   DO:
      mKName = "Кр " + DelFilFromAcct(STRING(mAcctCR.acct)).
   ...
   DO:
      mKName = "Дб " + DelFilFromAcct(STRING(mAcctDB.acct)).
   ...
   DO:
      RUN Fill-ttSL("ИНН", hWOP::inn, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
   ...
   IF {assigned mInnSend} THEN
   DO:
      RUN Fill-ttSL("ИНН", mInnSend, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
  ...
  IF {assigned mInnRec} THEN
   DO:
      RUN Fill-ttSL("ИНН", mInnRec, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
   ...
   DO:
      RUN Fill-ttSL("ФИО_ДР", hWOP::name-ben, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
   ...
   DO:
      RUN Fill-ttSL("ФИО_ДР", hWOP::details, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
   ...
   DO:
      RUN Fill-ttSL("ФИО_ДР", mNameSend, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
   ...
   DO:
      RUN Fill-ttSL("ФИО_ДР", mNameRec, vSLType, INPUT-OUTPUT TABLE t-obj-sl).
      IF CAN-FIND(FIRST t-obj-sl) THEN
...
PROCEDURE  ChkStopLines:
   ...
   /*Сначала проверяем номер паспорта*/
   RUN Fill-ttSL ("Пасп", iString, "*",INPUT-OUTPUT TABLE t-obj-sl).        

   /*Если ничего не нашли то проверяем ИНН*/
   IF NOT  CAN-FIND(FIRST t-obj-sl) THEN     
      RUN  Fill-ttSL ("ИНН", iString, "*",INPUT-OUTPUT TABLE t-obj-sl).  

   /*Если опять ничего не нашли то проверяем ФИО*/
   IF NOT  CAN-FIND(FIRST t-obj-sl) THEN                                    
      RUN Fill-ttSL ("ФИО", iString, "*",INPUT-OUTPUT TABLE t-obj-sl).
