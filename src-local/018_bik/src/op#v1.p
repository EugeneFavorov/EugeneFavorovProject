...
FORM
  "ДОКУМЕНТ" op.doc-type ":" doc-type.name FORMAT "x(33)" "N" op.doc-num FORMAT "x(8)" "ОТ" at 63 op.doc-date at 69 SKIP
...
on "F9" of frame opreq anywhere do:
   ...
   IF AVAIL signs THEN 
   DO:
      MESSAGE "К документу  " op.doc-num " привязан документ по зачислению З/П" SKIP
              "Нельзя изменить, пока есть связанные документы"
...
do on endkey undo, leave on error undo, leave:
  ...
  WAIT-FOR WINDOW-CLOSE, GO OF FRAME opreq.
end.
