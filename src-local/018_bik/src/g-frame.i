...
&IF DEFINED(DoFrame) &THEN
   ...
   FORM
      ...
      "Клиент:" op.name-ben VIEW-AS FILL-IN SIZE 37 BY 1 "ИНН:" op.inn VIEW-AS FILL-IN SIZE 12 BY 1 
      "КПП:" mKpp-rec VIEW-AS FILL-IN SIZE 9 BY 1
      &ENDIF
      &ENDIF
      ...
      &IF DEFINED(NOben) = 0 &THEN
      SKIP "Клиент:" op.name-ben VIEW-AS FILL-IN SIZE 37 BY 1 
      "ИНН:" op.inn VIEW-AS FILL-IN SIZE 12 BY 1 
      "КПП:" mKpp-rec VIEW-AS FILL-IN SIZE 9 BY 1
      &ENDIF
      &ENDIF
...
&ELSEIF DEFINED(DoSet) &THEN
   ...
   flag-edit-acct = GetXAttrValueEx("op-template",
                                    ...
                                    "EditAcctFlag", &IF DEFINED(EdtFlg) =  0 &THEN "НЕТ" &ELSE {&EdtFlg} &ENDIF) =  "Да".
   flag-edit-summ = GetXAttrValueEx("op-template",
                                    op-template.op-kind + "," + STRING(op-template.op-template),
                                    "EditAmtFlag",
                                    &IF DEFINED(Ofsum) =  0 &THEN 
                                    &IF DEFINED(EdtFlg) =  0 &THEN "НЕТ" &ELSE {&EdtFlg} &ENDIF
                                    &ELSE "ДА" &ENDIF
                                   ) =  "ДА".
   &IF DEFINED(Ofsum) =  0 &THEN
      ASSIGN
