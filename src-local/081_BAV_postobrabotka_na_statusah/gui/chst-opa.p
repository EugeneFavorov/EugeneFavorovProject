...
/* +++ chst-opa.p was humbly modified by (c)blodd converter v.1.11 on 7/27/2017 7:11am +++ */
...
smf:
         DO ON ERROR UNDO, LEAVE smf ON ENDKEY UNDO , LEAVE smf WITH FRAME edt:
            ...
            {update.i &THIS_FRAME = "edt" &EXFILE = "chst-opa.p.st1" &NODISPLAY = "YES" {&*}} .
            HIDE FRAME edt.
         END.
      END.
      ...
      RELEASE op NO-ERROR.
      IF ERROR-STATUS:ERROR THEN DO:
         RUN putlog('Ошибка документа: ' + {&RETURN_VALUE}). 
         UNDO tt, RETRY tt.
      END.
...
IF iall = 1 THEN DO:
   ...
   IF lastmess <> "" THEN
      RUN Fill-AlertSysMes IN h_tmess("","",-1,STRING(lastmess)).
