...
/* +++ pck-chck.p was humbly modified by (c)blodd converter v.1.11 on 3/27/2017 7:41am +++ */
...
/*--------------------------------------------- Просмотр созданых сообщений --*/
      ...
      IF mCount GT 0 THEN DO:
         DO:
            ...
            RUN Fill-AlertSysMes IN h_tmess("","",4,"Отмечено" + CHR(32) + STRING(mCount) + CHR(32) + "сообщений." + CHR(32) + "~n" + CHR(32) + "Изменить статус отмеченным сообщениям на ~"" + STRING(mStateTrg) + "~"?" + CHR(32) + "~n").
         ...
         CASE vChoice:
            ...
            WHEN NO  THEN DO:
               DO:
                  mblodd_char_Tmp01 = pick-value.
                  RUN Fill-AlertSysMes IN h_tmess("","",4,"В шаблоне" + CHR(32) + STRING(mOpKind) + "." + STRING(iClass) + CHR(32) + "задано изменение статуса" + CHR(32) + "~n" + CHR(32) + "с ~"" + STRING(mStateSrc) + "~" на ~"" + STRING(mStateTrg) + "~"" + CHR(32) + "~n" + CHR(32) + "Не выбрано ни одного сообщения для изменения статуса." + CHR(32) + "~n" + CHR(32) + "Укажите вариант: " + CHR(32) + "~n" + CHR(32) + "Да - продолжить выполнение транзакции без изменения статуса" + CHR(32) + "~n" + CHR(32) + "Нет - прекратить выполнение транзакции " + CHR(32) + "~n" + CHR(32) + "ESC - повторить выбор сообщений " + CHR(32) + "~n").
                  DEFINE VARIABLE vChoice1 AS LOGICAL NO-UNDO.
                  vChoice1 = (pick-value = "YES").
                  pick-value = mblodd_char_Tmp01.
               END.
