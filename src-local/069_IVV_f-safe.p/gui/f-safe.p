...
/* +++ f-safe.p was humbly modified by (c)blodd converter v.1.11 on 4/27/2017 2:08pm +++ */
...
DO:
   ...
   IF ERROR-STATUS:ERROR THEN DO:
      RUN Fill-AlertSysMes IN h_tmess("","",1,"Вводимое Вами значение должно быть числовым").
...
DO:
   ...
   DO TRANSACTION:
      ...
      IF     {assigned pick-value}
         AND ({&LAST_KEY} = 10 OR {&LAST_KEY} = 13) THEN 
      DO:
         mNdsCm = pick-value.
         RUN GetNDSValue IN h_asset (mNdsCm,
...
DO:
   ...
   DO:
      RUN Fill-AlertSysMes IN h_tmess("","",1,"Нельзя уменьшать дату открытия договора").
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   ** с идентификатором SEPARATOR# с атрибутом VIES-AS TEXT */
   ...
   IF NOT THIS-PROCEDURE:PERSISTENT THEN
     {wait_for.i &THIS_FRAME = "fMain" &EXFILE = "f-safe.p.wf1" {&*}} .
 END.
...
------------------------------------------------------------------------------*/
    ...
    DO ON ERROR  UNDO, LEAVE
       ...
       {update.i &THIS_FRAME = "dataframe" &EXFILE = "f-safe.p.st1" {&*}} .
    END.                                       
