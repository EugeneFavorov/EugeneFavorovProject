...
/* +++ f-cust.p was humbly modified by (c)blodd converter v.1.11 on 2/28/2017 8:41am +++ */
...
DO:
   ...
   DO:
      ...
      DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Банк с кодом идентификатора ~"" + tt-cust-corp.bank-code-type:SCREEN-VALUE + "~"" + CHR(32) + "~n" + CHR(32) + "и идентификатором ~"" + tt-cust-corp.bank-code:SCREEN-VALUE + "~" не существует " + CHR(32) + "~n").
...
ON F1 OF tt-cust-corp.unk$ IN FRAME fMain /* УНК */
DO:
...
DO:
   ...
   DO:
      ...
      DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Внимание: Неверный ключ!").
...
DO:
   ...
   IF ERROR-STATUS:ERROR THEN DO:
      RUN Fill-AlertSysMes IN h_tmess("","",1,ERROR-STATUS:GET-MESSAGE(1)).
   ...
   DO:
      RUN Fill-AlertSysMes IN h_tmess("","",1,"Дата заведения клиента должна быть меньше даты закрытия").
...
DO:
   ...
   DO:
     ...
     IF vLen NE 5 AND
       ...
       IF vLen = 10 THEN 
           DO:
              ...
              RUN Fill-AlertSysMes IN h_tmess("","",4,("Последняя цифра ИНН (ключ) должна быть: ~"" + STRING(mTempVal) + "~"") + CHR(32) + "~n" + CHR(32) + "OK - Исправить, Cancel - Пропустить").
              vfl = (pick-value = "YES").
     ...
     IF mTempVal NE ? THEN DO:
       DO:
          mblodd_char_Tmp01 = pick-value.
          RUN Fill-AlertSysMes IN h_tmess("","",4,"Запись с такими реквизитами уже существует в таблице:" + CHR(32) + SUBSTR(mTempVal, 1, 80) + CHR(32) + "!" + CHR(32) + "~n" + CHR(32) + "OK - Исправить, Cancel - Пропустить").
          vfl = (pick-value = "YES").
          pick-value = mblodd_char_Tmp01.
       END.
...
&Scoped-define SELF-NAME tt-cust-corp.kpp$
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-cust-corp.kpp$ TERMINAL-SIMULATION
ON LEAVE OF tt-cust-corp.kpp$ IN FRAME fMain /*КПП*/
DO:
   ...
   {&BEG_BT_LEAVE}
   DO mCnt = 1 TO NUM-ENTRIES(SELF:SCREEN-VALUE): 
      mKPP = ENTRY(mCnt,SELF:SCREEN-VALUE).
      IF mKPP NE "" THEN
      DO:
         vLen = LENGTH(mKPP).
         DO i = 1 TO vLen:
            ...
            IF isAlpha THEN LEAVE.
         END.
         IF isAlpha EQ YES THEN
         DO:
            RUN Fill-AlertSysMes IN h_tmess("","",1,'В КПП должны быть только цифры.').

            {return_no_apply.i '{&RET-ERROR}'}
         END.
         IF (vLen NE 9) THEN
         DO:
            RUN Fill-AlertSysMes IN h_tmess("","",1,'В КПП должно быть 9 цифр, введено ' + TRIM(STRING(vLen)) + '.').
...
DO:
   ...
   DO:
      ...
      DO:
         ...
         DO:
            mblodd_char_Tmp01 = pick-value.
            RUN Fill-AlertSysMes IN h_tmess("","",4,STRING(mMessage) + "~nБудете исправлять?").
      ...
      DO mItem = 1 TO 2:
         ...
         FOR FIRST signs NO-LOCK WHERE signs.file-name EQ ENTRY(mItem, "banks,cust-corp")
            ...
            DO:
               mblodd_char_Tmp01 = pick-value.
               RUN Fill-AlertSysMes IN h_tmess("","",4,"Запись ОГРН=" + tt-cust-corp.ogrn$:SCREEN-VALUE + " уже существует~nБудете исправлять?").
               mYesNo = (pick-value = "YES").
               pick-value = mblodd_char_Tmp01.
            END.
...
&Scoped-define SELF-NAME tt-cust-corp.unk$
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-cust-corp.unk$ TERMINAL-SIMULATION
ON LEAVE OF tt-cust-corp.unk$ IN FRAME fMain /* УНК */
DO:
{&BEG_BT_LEAVE}
...
DO:
   ...
   IF iMode EQ {&MOD_VIEW} THEN DO:
      ...
      THEN DO:
         vDatePer = DATE(FGetSetting("ДатаОКВЭД2","",?)) NO-ERROR.

         FOR LAST tmpsigns WHERE                        
                  tmpsigns.code        EQ 'ОКВЭД'                    
              AND tmpsigns.file-name   EQ 'cust-corp'       
              AND tmpsigns.surrogate   EQ STRING(tt-cust-corp.cust-id)  
              AND tmpsigns.xattr-value EQ SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME}
         NO-LOCK:
            vDateSign = tmpsigns.since.
         END.
                        /* Определение классификатора в зависимости от даты ДР */
         vCodeName = IF vDateSign GE  vDatePer
                     THEN "ОКВЭД2"
                     ELSE "ОКВЭД".
         RUN formld.p("code",
                      STRING(vCodeName + ',' +  
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   IF mFlagUnk THEN DO:
      ...
      IF mTempVal NE FILL("9", LENGTH(mTempVal)) THEN DO:
         RUN Fill-AlertSysMes IN h_tmess("","",-1,"Формат УНК, заданный в метасхеме д.б. ~"999...~"!").
...
------------------------------------------------------------------------------*/
   ...
   IF      ((FGetSetting("ЗапросыЕГР","ЕГР_ВводЮЛ","") EQ "Да"
        OR FGetSetting("ЗапросыЕГР","ЕГР_ВводИП","")   EQ "Да")
       AND iMode EQ {&MOD_ADD})
        OR ((FGetSetting("ЗапросыЕГР","ЕГР_КоррЮЛ","") EQ "Да"
        OR FGetSetting("ЗапросыЕГР","ЕГР_КоррИП","")   EQ "Да")
   ...
   DO:
      ...
      DO:
         ...
         DO:
            ...
            IF LENGTH(TRIM(tt-cust-corp.inn)) EQ 10 THEN
            DO:
               RUN SetSysConf IN h_base("EgrField","ИНН,ЮЛ").
               RUN SetSysConf IN h_base("EgrValue",tt-cust-corp.inn).
            END.
            ELSE IF LENGTH(TRIM(tt-cust-corp.inn)) EQ 12 THEN
            DO:
               RUN SetSysConf IN h_base("EgrField","ИНН,ИП,cust-corp").
               RUN SetSysConf IN h_base("EgrValue",tt-cust-corp.inn).
            END.
            ELSE
               RUN Fill-SysMes IN h_tmess("","",1,"Неверная длина ИНН для отправки запроса!").
         END.
         IF     {&LAST_KEY} NE KEYCODE("ESC")
            AND NUM-ENTRIES(GetSysConf("EgrField")) GE 2 THEN
         DO:
            IF GetBaseOpDate() EQ ? THEN
               RUN InitBaseLibrary IN h_pbase (?,TODAY,?).
            IF pick-value EQ "1" THEN
            DO:
               IF ENTRY(2,GetSysConf("EgrField")) EQ "ЮЛ" THEN
               DO:
                  mTypeEgr = "1".
                  RUN RunTransaction IN h_pbase ("e-zaprksul").
                  RUN RunTransaction IN h_pbase ("i-egrksul").
               END.
               ELSE IF ENTRY(2,GetSysConf("EgrField")) EQ "ИП" THEN
               DO:
                  mTypeEgr = "2".
      ...
      DO:
         CASE mTypeEgr:
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-cust-corp.cust-stat      = ttRespKSIP.fCuststat
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-cust-corp.cust-stat      = ttRespVIPUl.fCuststat
                  ...
                  RUN SetSysConf IN h_base("EgrField",
                                         ...
                                         + ttRespVIPUl.fAddr   + ";"
                                         + ttRespVIPUl.fNumReg).
                  FIND LAST ttRespVIPOkved WHERE ttRespVIPOkved.fNum EQ ttRespVIPUl.fNum 
                     NO-LOCK NO-ERROR.
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-cust-corp.cust-stat      = ttRespVIPIP.fCuststat
                  ...
                  RUN SetSysConf IN h_base("EgrField",
                                         ...
                                         + ttRespVIPIP.fAddr   + ";"
                                         + ttRespVIPIP.fNumReg).
                  FIND LAST ttRespVIPOkved WHERE ttRespVIPOkved.fNum EQ ttRespVIPIP.fNum 
                     NO-LOCK NO-ERROR.
                  IF AVAIL ttRespVIPOkved THEN
                     tt-cust-corp.okvwed$ = 
                        SUBSTRING(ttRespVIPOkved.fOkved,1,LENGTH(ttRespVIPOkved.fOkved) - 1).
...
------------------------------------------------------------------------------*/
   ...
   DO:
      CASE mTypeEgr:
         ...
         DO:
            ...
            IF AVAIL ttRespKSUL THEN
            DO:
               UpdateSigns("cust-corp", STRING(vCustId), "ДатаОГРН", ttRespKSUL.fDateOGRN, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УставКап", ttRespKSUL.fUstSum, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "РегОрган", ttRespKSUL.fNameNO, ?).
            END.
         END.
         WHEN "2" THEN
         DO:
            ...
            IF AVAIL ttRespKSIP THEN
            DO:
               UpdateSigns("cust-corp", STRING(vCustId), "ДатаОГРН", ttRespKSIP.fDateOGRN, ?).
               IF ttRespKSIP.fTypeIP THEN
                  UpdateSigns("cust-corp", STRING(vCustId), "Предпр", "Предпр", ?).
            END.
         END.
         WHEN "3" THEN
         DO:
            ...
            IF AVAIL ttRespVIPUl THEN
            DO:
               UpdateSigns("cust-corp", STRING(vCustId), "ДатаОГРН",    ttRespVIPUl.fDateOGRN, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УчДокГР",     ttRespVIPUl.fSvidet, ?).
               ...
               UpdateSigns("cust-corp", STRING(vCustId), "РегОрган",    ttRespVIPUl.fNameNO, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "RegNum",      ttRespVIPUl.fRegNum, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "РеоргИНН",    ttRespVIPUl.fReorgInn, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "РеоргПолное", ttRespVIPUl.fReorgName, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "РеоргОГРН",   ttRespVIPUl.fReorgOgrn, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УставКап",    ttRespVIPUl.fUstSum, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УчДокДата",   ttRespVIPUl.fDateNO, ?).
               FOR EACH ttRespVIPLicence WHERE ttRespVIPLicence.fNum EQ ttRespVIPUl.fNum NO-LOCK:
                  CREATE cust-ident.
                  ASSIGN
                     cust-ident.class-code     = "cust-lic"
                     cust-ident.cust-cat       = "Ю"
                  ...
                  FIND FIRST cust-ident WHERE cust-ident.cust-cat   EQ "Ю"
                                          ...
                                          AND cust-ident.cust-code  EQ ttRespVIPLicence.NumbLic
                                          AND cust-ident.open-date  EQ ttRespVIPLicence.fDateBeg
                     NO-LOCK NO-ERROR.
                  IF AVAIL cust-ident THEN
               ...
               END.
               FOR EACH ttRespVIPPred WHERE ttRespVIPPred.fNum EQ ttRespVIPUl.fNum NO-LOCK:
                  CREATE cust-role. 
                  CASE ttRespVIPPred.fType:
                     WHEN "УправлФЛ" THEN
                        ASSIGN
                           cust-role.Class-Code     = "Представитель"
                           cust-role.surrogate      = STRING(vCustId)
                           cust-role.file-name      = "cust-corp"
                           cust-role.open-date      = 
                              IF ttRespVIPPred.fDateBeg EQ ? THEN TODAY
                     ...
                     WHEN "УправлРос" THEN
                        ASSIGN
                           ...
                           cust-role.inn            = ttRespVIPPred.fInnFL
                           cust-role.cust-name      = ttRespVIPPred.fName
                           cust-role.cust-code-type = "ОГРН"
                           cust-role.cust-code      = ttRespVIPPred.fOGRN
                        .  
                     WHEN "УчредитФЛ" THEN
                     DO:
                        ASSIGN
                           cust-role.Class-Code     = "Учредитель"
                           cust-role.surrogate      = STRING(vCustId)
                           cust-role.file-name      = "cust-corp"
                           cust-role.open-date      = 
                              IF ttRespVIPPred.fDateBeg EQ ? THEN TODAY
                        ...
                        UpdateSigns (cust-role.Class-Code,
                                     ...
                                     "ДоляКап",ttRespVIPPred.fProcPart,?). 
                     END.
                     WHEN "УчредитИН" THEN
                     DO:
                        ASSIGN
         ...
         DO:
            ...
            IF AVAIL ttRespVIPIP THEN
            DO:
               UpdateSigns("cust-corp", STRING(vCustId), "ДатаОГРН",    ttRespVIPIP.fDateOGRN, ?).
               IF ttRespVIPIP.fTypeIP THEN
                  UpdateSigns("cust-corp", STRING(vCustId), "Предпр", "Предпр", ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УчДокГР",     ttRespVIPIP.fSvidet, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "RegDate",     ttRespVIPIP.fRegDate, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "РегОрган",    ttRespVIPIP.fRegName, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "RegNum",      ttRespVIPIP.fRegNum, ?).
               UpdateSigns("cust-corp", STRING(vCustId), "УчДокДата",   ttRespVIPIP.fDateNo, ?).
               ...
               FOR EACH ttRespVIPLicence WHERE ttRespVIPLicence.fNum EQ ttRespVIPIP.fNum NO-LOCK:
                  CREATE cust-ident.
                  ASSIGN
                     cust-ident.class-code     = "cust-lic"
                     cust-ident.cust-cat       = "Ю"
               ...
               DO:
                  ...
                  FIND FIRST cust-ident WHERE cust-ident.cust-cat   EQ "Ю"
                                          ...
                                          AND cust-ident.cust-code  EQ ttRespVIPIP.fDocNum
                                          AND cust-ident.open-date  EQ ttRespVIPIP.fDocDateOp
                     NO-LOCK NO-ERROR.
                  IF AVAIL cust-ident THEN
                     UpdateSigns ("p-cust-ident",
                                  ...
                                  STRING(cust-ident.cust-code) + ',' +
                                  STRING(cust-ident.cust-type-num),
                                  "подразд", ttRespVIPIP.fDocBranch,?).
               END.
            END.
         END.
      END CASE.
   ...
   THEN DO: 
      ...
      vMode =   IF iMode EQ {&MOD_ADD} THEN "create" ELSE "edit".      
   ...
   IF  iMode EQ {&MOD_ADD} THEN
   DO:
      IF ENTRY(1,fGetSetting("Cl_TerrAdr","AddrRedact","")) EQ "Да"   
      THEN DO:
         ...
         FOR EACH bcident WHERE 
                  bcident.cust-cat       EQ "Ю"
              AND bcident.cust-id        EQ tt-cust-corp.cust-id
              AND bcident.Class-code     EQ "p-cust-adr"
              AND (   bcident.close-date EQ ? 
                   OR bcident.close-date GE TODAY)
              AND CAN-DO(vTerrAdrType,bcident.cust-code-type)
         NO-LOCK:
           {additem.i vChkAdresses bcident.cust-code-type}
         END.
         
         RUN chkterradred.p ("Ю",
                             STRING(tt-cust-corp.cust-id),
                             vChkAdresses).
      END.
   END.
   ...
   ASSIGN
      vTmp    = GetXattrValueEX("cust-corp", STRING(vCustId), "Предпр", "")
      vStatIP = FGetSetting("СтандТр","СтатусФЛЧП","")
   .
   IF FGetSetting("ОбменФМС","ФМСРед_КлЮ","") EQ "Да" AND
      (vTmp NE "" OR CAN-DO(vStatIP,tt-cust-corp.cust-stat)) 
   THEN DO:
      {cl-fmschk.i
         "'cust-corp'"
         "tt-cust-corp.cust-id"
...
/* $LINTUSER='muta' */
/* $LINTDATE='20/09/2016 14:50:48.453+03:00' */
/*prosignQ68MwGganrwmRc7vCaOBDg*/
/* --- f-cust.p was humbly modified by (c)blodd converter v.1.11 on 2/28/2017 8:41am --- */
