...
/* +++ otch_vipa.p was humbly modified by (c)blodd converter v.1.11 on 2/21/2017 10:30am +++ */
...
DO ON ERROR  UNDO, LEAVE
   ON ENDKEY UNDO, LEAVE:
    ...
    {update.i &THIS_FRAME = "frame_date_codes" &EXFILE = "otch_vipa.p.st1" &EDT = "YES" {&*}}         {&EDITING_WORD}

        {&READKEY_WORD}.
        IF {&LAST_KEY} EQ KEYCODE("ESC") THEN return.
        ...
        THEN DO:
            CASE {&FRAME_FIELD}:
                WHEN "beg-date" OR WHEN "end-date" THEN DO:
                    RUN calend.p.
                    IF ({&LAST_KEY} EQ 13 OR {&LAST_KEY} EQ 10) AND (pick-value NE ?)
                    THEN {&FRAME_VALUE} = STRING(DATE(pick-value), "99/99/9999").
                END.
            END CASE.
        END.
        ELSE APPLY {&LAST_KEY}.
