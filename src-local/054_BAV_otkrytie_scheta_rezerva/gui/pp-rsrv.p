...
/* +++ pp-rsrv.p was humbly modified by (c)blodd converter v.1.11 on 8/14/2017 11:03am +++ */
...
PROCEDURE FillTProv:
   ...
   BREAK BY bCode.code:
      ...
      DO:
         ...
         IF {&RETURN_VALUE} = "STOP" THEN LEAVE ACCT.
      END.
      ...
      ** ( только в случае, если роль счета указана в НП АвтСчКР ) иначе со счета */
      ...
      DO:         
         ...
         IF {assigned RETURN-VALUE} THEN DO:
            ...
            PUT STREAM sOut UNFORMATTED {&RETURN_VALUE} SKIP.
         END.
         ELSE
         DO:               
            PUT STREAM sOut UNFORMATTED
...
PROCEDURE SetStatus:
  ...
  DO:
    ...
    IF {&RETURN_VALUE} = "STOP" THEN RETURN "STOP".
  END.
...
PROCEDURE Report:
  ...
  DO:
     ...
     RUN Fill-AlertSysMes IN h_tmess("","",4,"Выводить полный отчет?").
     vL = (pick-value = "YES").
     pick-value = mblodd_char_Tmp01.
  END.


  OS-DELETE VALUE ( {&RELATIVE_2_ABSOLUTE}( pFName ) ).
