...
mProvINNKPP = FGetSetting("Проверки","ВклПровИННКПП","").
...
DO:
   ...
   ELSE DO:
      IF tt-cust-corp.bank-code-type:SCREEN-VALUE <> "" AND
         tt-cust-corp.bank-code-type:SCREEN-VALUE <> "ИНН"
      THEN DO:
         ASSIGN 
            tt-cust-corp.bank-code-type.
         DO TRANSACTION:
...
ON LEAVE OF tt-cust-corp.tel IN FRAME fMain /* Идентиф. банка */
DO:
   {&BEG_BT_LEAVE}

   IF FGetSetting("Проверки","ПровТелефон","") = "Да" THEN
      IF tt-cust-corp.tel:SCREEN-VALUE = "" THEN
         RUN Fill-SysMes IN h_tmess ("", "", "-1", "Телефон клиента не указан").
...
DO:
   ...
   DO:
      IF tt-cust-corp.bank-code:SCREEN-VALUE <> "ИНН" THEN
      DO:
...
DO:
   IF iMode <> {&MOD_VIEW} THEN DO:
      ...
      THEN DO:
         IF tt-cust-corp.bank-code-type:SCREEN-VALUE <> "ИНН" THEN
...
DO:
   ...
   DO:
      IF GetCodeVal("КодПредп", SELF:SCREEN-VALUE) =  ? THEN DO:
...
DO:
   ...
   DO:
   /*Проверка при совпадении ИНН и КПП*/
      IF GetXAttrValueEx("cust-corp", 
         STRING(b-cust-corp.cust-id), "КПП", "" ) = tt-cust-corp.kpp$:SCREEN-VALUE  THEN
      DO:
         RUN Fill-SysMes IN h_tmess ("", "", "-1", 
            "Запись с такими реквизитами ИНН и КПП уже существует в таблице:~n" + 
...
DO:
   ...
   vProvOGRN = FGetSetting("Проверки","ЖестПровОГРН","").
   vProvOGRNUL = FGetSetting("Проверки","ПровОГРНЮЛ","").
   ...
   DO:
      ...
      DO:
         ...
         IF mOGRN2 =  "** не прошло **" OR (iMode <> {&MOD_EDIT} AND mOGRN2 BEGINS "** ") THEN
         ...
         IF vProvOGRN = "Да" AND INDEX(mOGRN2,"Неверная длина") > 0 THEN
         DO:
            IF mPredPr =  "Предпр" THEN 
               RUN Fill-SysMes IN h_tmess ("","","-1",
                                           "Формат ОГРН для предпринимателя 15 символов").
            ELSE 
               RUN Fill-SysMes IN h_tmess ("","","-1","Формат ОГРН для юр.лиц 13 символов").
            RETURN NO-APPLY {&RET-ERROR}.
         END.
         ELSE 
         DO:
...
PROCEDURE MainAction PRIVATE :
   ...
   IF GetCode("КодАдр", mAdrType) =  "0"  THEN
...
------------------------------------------------------------------------------*/
   ...
   IF      ((FGetSetting("ЗапросыЕГР","ЕГР_ВводЮЛ","") =  "Да"
        OR FGetSetting("ЗапросыЕГР","ЕГР_ВводИП","")   =  "Да")
       AND iMode =  {&MOD_ADD})
        OR ((FGetSetting("ЗапросыЕГР","ЕГР_КоррЮЛ","") =  "Да"
   ...
   DO:
      ...
      DO:
         ...
         DO:
            ...
            DO:
               IF ENTRY(2,GetSysConf("EgrField")) =  "ЮЛ" THEN
               DO:
                  mTypeEgr = "1".
                  ...
                  END.                  
               END.
               ELSE IF ENTRY(2,GetSysConf("EgrField")) =  "ИП" THEN
               DO:
                  mTypeEgr = "2".
      ...
      DO:
         CASE mTypeEgr:
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-cust-corp.cust-stat      = GetCodeEx("КодПредп",
         ...
         IF {assigned vAddress} THEN DO:
            ...
            FIND LAST cust-ident WHERE cust-ident.class-code      =  "p-cust-adr"
                             AND cust-ident.cust-code-type  =  "АдрЮр"
            ...
            IF AVAIL cust-ident THEN DO:
               ...
               UpdateSigns("cust-ident", 
                            ...
                            "КодРегГНИ", tt-cust-corp.kodreggni$, YES).
               UpdateSigns("cust-ident", 
                            cust-ident.cust-code-type + "," + 
                            cust-ident.cust-code + "," + 
                            STRING(cust-ident.cust-type-num),
                            "КодРег", tt-cust-corp.kodreg$, YES).
            END.
            ELSE IF iMode = {&MOD_ADD} THEN DO:
               ...
               ASSIGN
                  ...
                  tt-p-cust-adr.cust-code-type =  "АдрЮр"
                  tt-p-cust-adr.country-id     = tt-cust-corp.country-id
               NO-ERROR.

               /* чтобы сохранились ДР */
...
------------------------------------------------------------------------------*/
         ...
         DO:
            ...
            DO:
               ...
               FOR EACH ttRespVIPLicence WHERE ttRespVIPLicence.fNum =  ttRespVIPUl.fNum NO-LOCK:
                  ...
                  .
                  FIND FIRST cust-ident WHERE cust-ident.cust-cat   =  "Ю"
   ...
   /*очищение таблиц при проверке по стоп-листам*/
   {empty tt-view-sl}
   {empty ttFindSL}
   {empty t-obj-sl N}
   {empty ttSLAction}

   IF FGetSetting("Стоп-листы", "ContrSub","") = "Да" THEN 
   DO:      
...
/*      END.                                                                                       */
   ...
   .
   IF FGetSetting("ОбменФМС","ФМСРед_КлЮ","") =  "Да" AND
   ...
   THEN DO:
      ...
      IF FGetSetting("ОбменФМС","ФМСВвод_КлЮ","") = "Да" THEN
      DO:
