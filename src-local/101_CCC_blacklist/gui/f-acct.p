...
DEFINE VARIABLE vMaskAcct2    AS CHARACTER   NO-UNDO.
...
DEF VAR vFrmh        AS HANDLE NO-UNDO.          /* Указатель на фрейм. */
   ...
   tt-acct.details = GetTempXattrValueEx("acct",tt-acct.acct + "," + tt-acct.currency,"Details",gend-date,"").
...
DEF VAR vOk          AS LOG    NO-UNDO. /* Флаг ошибки. */
   ...
   /* Вставка Плюс банк */
   /* Проверка действительности паспорта по базе PipeLine */
   IF tt-acct.cust-cat EQ "Ч" THEN
   DO:
      vMaskAcct2 = FGetSetting("ОткрСчетов","МаскИсклСч","").
      IF NOT CAN-DO(vMaskAcct2,acct.acct) THEN
      DO:
         FIND FIRST person WHERE
            person.person-id EQ tt-acct.cust-id 
         NO-LOCK NO-ERROR.
         IF AVAIL(person) THEN 
            ASSIGN
               vDocID = person.document-id
               vDoc   = person.document.
         IF vDocID EQ "Паспорт" THEN 
         DO:
            ...
            DO:
               MESSAGE SUBSTR("    ДОКУМЕНТ: " + SUBSTR(vDocID,1,12)
                                            + FILL(" ",45),1,45) skip
                    SUBSTR("СЕРИЯ, НОМЕР: " + SUBSTR(vDoc,1,25)
                                            + FILL(" ",45),1,45) skip (1)
   ...
   THEN DO:
      ...
      IF vFlChk THEN DO:
         ...
         IF FGetSetting("ОбменФМС","ФМСВвод_Сч","") = "Да" THEN
         DO:
            IF AVAILABLE tt-res THEN DO:
               IF CAN-DO({&KodIsNotValidDoc},tt-res.fChkKodSt) THEN
               DO:   
                  APPLY "ENTRY" TO tt-acct.branch-id IN FRAME fMain.
                  RETURN ERROR.
               END.    
            END.
         END.
   ...
   THEN DO:
      ...
      DO:
         ...
         DO:
            ...
            RUN SetSysConf IN h_base ("CHKPERSDOC_CALL_PROCEDURE","f-acct").
