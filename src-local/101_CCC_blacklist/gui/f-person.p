...
{intrface.get date}
{egr-upd.def NEW}        /* для работы с сервисом ЕГР. */
{wordwrap.def}
...
DEFINE VARIABLE mFirstNames     AS CHARACTER NO-UNDO.
DEFINE VARIABLE mLastName       AS CHARACTER NO-UNDO.
DEFINE VARIABLE mBirthDate      AS CHARACTER NO-UNDO.

DEFINE VARIABLE mProvFIO        AS CHARACTER NO-UNDO.

mProvFIO = FGetSetting("Проверки","ВклПровФИО","").
...
tt-person.country-id tt-person.inn tt-person.tax-insp tt-person.country-id2 tt-person.document-id ~
...
tt-person.vidsotr$ tt-person.name-last tt-person.first-names ~
tt-person.birthday tt-person.gender tt-person.BirthPlace ~
tt-person.country-id tt-person.country-id2 tt-person.inn tt-person.tax-insp tt-person.document-id ~
...
&Scoped-define List-4 tt-person.country-id2
...
DEFINE FRAME fMain
     ...
     tt-person.document
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 8 COL 33 COLON-ALIGNED
          &ELSE AT ROW 8 COL 33 COLON-ALIGNED &ENDIF HELP
          ""
          LABEL "N" FORMAT "x(35)"
          VIEW-AS FILL-IN 
     ...
     tt-person.Document4Date_vid
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 8 COL 56
          &ELSE AT ROW 8 COL 56 &ENDIF HELP
          ...
          &ELSE SIZE 10 BY 1 &ENDIF
     mIssue
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 9 COLON-ALIGNED
          &ELSE AT ROW 9 COL 9 COLON-ALIGNED &ENDIF HELP
          "Кем выдан"
     mKP
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 64 COLON-ALIGNED
          &ELSE AT ROW 9 COL 64 COLON-ALIGNED &ENDIF HELP
          "Код подразделения"
     separator1
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 10 COL 1
          &ELSE AT ROW 10 COL 1 &ENDIF NO-LABEL
     tt-person.kodreggni$
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 11 COL 12 COLON-ALIGNED
          &ELSE AT ROW 11 COL 12 COLON-ALIGNED &ENDIF HELP
          "Код региона ГНИ"
     ...
     vOblChar
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 11 COL 24 COLON-ALIGNED
          &ELSE AT ROW 11 COL 24 COLON-ALIGNED &ENDIF HELP
          "Район"
     vGorChar
...
DEFINE FRAME fMain
     vStrChar
          ...
          &ELSE AT ROW 13 COL 24 COLON-ALIGNED &ENDIF HELP
          "Строение"
     vKorpChar
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 13 COL 38 COLON-ALIGNED
          &ELSE AT ROW 13 COL 38 COLON-ALIGNED &ENDIF HELP
          "Корпус"
     vKvChar
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 13 COL 50 COLON-ALIGNED
          &ELSE AT ROW 13 COL 50 COLON-ALIGNED &ENDIF HELP
          "Квартира"
     tt-person.kodreg$
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 13 COL 67 COLON-ALIGNED
          &ELSE AT ROW 13 COL 67 COLON-ALIGNED &ENDIF HELP
          "Коды регионов"
          ...
          &ELSE SIZE 10 BY 1 &ENDIF
     vAdrIndInt
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 14 COL 11 COLON-ALIGNED
          &ELSE AT ROW 14 COL 11 COLON-ALIGNED &ENDIF HELP
          "Индекс"
     separator2
     ...
     tt-person.bank-code
          ...
          &ELSE AT ROW 16 COL 49 COLON-ALIGNED &ENDIF HELP
     ...
     tt-person.corr-acct
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 17 COL 11 COLON-ALIGNED
          &ELSE AT ROW 17 COL 11 COLON-ALIGNED &ENDIF HELP
...
DEFINE FRAME fMain
     tt-person.date-out
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 20 COL 7 COLON-ALIGNED
          &ELSE AT ROW 20 COL 7 COLON-ALIGNED &ENDIF HELP
          ...
          &ELSE SIZE 10 BY 1 &ENDIF
     tt-person.subw%ekt$
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 20 COL 27 COLON-ALIGNED
          &ELSE AT ROW 20 COL 27 COLON-ALIGNED &ENDIF HELP
          ""
     ...
     tt-person.passw_card
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 20 COL 55 COLON-ALIGNED
          &ELSE AT ROW 20 COL 55 COLON-ALIGNED &ENDIF HELP
...
/* SETTINGS FOR FILL-IN tt-person.country-id IN FRAME fMain
   1 2 3                                                                */
/* SETTINGS FOR FILL-IN tt-person.country-id2 IN FRAME fMain
   1 2 3                                                                */
...
ON LEAVE OF tt-person.name-last IN FRAME fMain 
DO:
   {&BEG_BT_LEAVE}
    ...
    DEFINE VARIABLE vBadSymb      AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE vCyrillic     AS LOGICAL     NO-UNDO INIT NO.
    DEFINE VARIABLE vRoman        AS LOGICAL     NO-UNDO INIT NO.


    vTmpStr =  tt-person.name-last:SCREEN-VALUE.
    RUN  Check-Cyrillic-Roman IN h_strng (INPUT vTmpStr, OUTPUT vCyrillic,OUTPUT vRoman).
    IF vCyrillic AND vRoman THEN
    DO:
       RUN Fill-SysMes IN h_tmess (  "", "", "-1",
           "Фамилия должна содержать символы только одного алфавита").
       RETURN NO-APPLY {&RET-ERROR}.
    END.
    RUN Check-Ascii-Set2 IN THIS-PROCEDURE (INPUT vTmpStr, INPUT "1,2", INPUT "-", OUTPUT vBadSymb).
    IF {assigned vBadSymb} THEN
    DO:
       RUN Fill-SysMes IN h_tmess (  "", "", "-1", 
         SUBST("Символ &1 является недопустимым для имени или фамилии физического лица", vBadSymb)).
       RETURN NO-APPLY {&RET-ERROR}.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-person.first-names TERMINAL-SIMULATION
ON LEAVE OF tt-person.first-names IN FRAME fMain 
DO:
   {&BEG_BT_LEAVE}
    ...
    vTmpStr =  tt-person.first-names:SCREEN-VALUE.
    RUN  Check-Cyrillic-Roman IN h_strng (INPUT vTmpStr, OUTPUT vCyrillic,OUTPUT vRoman).
    IF vCyrillic AND vRoman THEN
    DO:
       RUN Fill-SysMes IN h_tmess (  "", "", "-1",
           "Имя,отчество должны содержать символы только одного алфавита").
       RETURN NO-APPLY {&RET-ERROR}.
    END.
    RUN Check-Ascii-Set2 IN THIS-PROCEDURE (INPUT vTmpStr,INPUT "1,2",INPUT "-",OUTPUT vBadSymb).
    IF {assigned vBadSymb} THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-person.bank-code TERMINAL-SIMULATION
ON LEAVE OF tt-person.birthday IN FRAME fMain 
DO:
   {&BEG_BT_LEAVE}

   DEFINE VARIABLE vDiapazon1 AS DATE NO-UNDO.
   DEFINE VARIABLE vDiapazon2 AS DATE NO-UNDO.
   DEFINE VARIABLE vTmpStr AS CHAR NO-UNDO.
   DEFINE VARIABLE vTmpDate AS DATE NO-UNDO.
   DEFINE BUFFER bperson FOR person.
   DEFINE BUFFER cperson FOR person.

   /* Проверка даты рождения */
   vTmpStr = FGetSetting("Проверки","ДиапазонДР","").
   vDiapazon1 = DATE(GetEntries(1,vTmpStr,"/","")).
   vDiapazon2 = DATE(GetEntries(2,vTmpStr,"/",STRING(TODAY))).

   IF   {assigned vTmpStr} THEN
   DO:
      vTmpDate = DATE(tt-person.birthday:SCREEN-VALUE).
      IF vTmpDate < vDiapazon1  OR  vTmpDate > vDiapazon2 THEN
      DO:
         RUN Fill-SysMes IN h_tmess ("", "", "-1",
                                     "Дата рождения выходит за границы заданного диапазона").
         RETURN NO-APPLY {&RET-ERROR}.
      END.
   END.
   vTmpStr = "".

   /* Проверки на дубли ФИО и Др */
   IF FGetSetting("Проверки","ДубльФИО_ДР","") =  "Да" THEN
   DO:
      FIND FIRST bperson WHERE bperson.person-id <> tt-person.person-id
                      AND bperson.name-last = tt-person.name-last:SCREEN-VALUE
                      AND bperson.birthday = DATE(tt-person.birthday:SCREEN-VALUE)
      NO-LOCK NO-ERROR.
      IF AVAIL bperson THEN
      DO:
      vTmpStr = GetEntries(1,bperson.first-names," ","").
      IF GetEntries(1,tt-person.first-names:SCREEN-VALUE," ","") = vTmpStr  THEN
      DO:
            RUN Fill-SysMes IN h_tmess (  "", "", "4",
                  SUBSTITUTE(
                     "В базе уже есть клиент № &1 ~n &2, дата рождения &3.~n Продолжить ввод/редактирование клиента?",
                     bperson.person-id, bperson.name-last + " " + 
                     GetEntries(1,bperson.first-names," ",""), STRING(bperson.birthday,"99.99.9999")
         )).
      ...
      IF NOT AVAIL bperson THEN
      DO:
         /* WHOLE-INDEX person */
         FIND FIRST cperson WHERE cperson.first-names = tt-person.first-names:SCREEN-VALUE
                              AND cperson.birthday = DATE(tt-person.birthday:SCREEN-VALUE)
                              AND cperson.person-id <> tt-person.person-id
         NO-LOCK NO-ERROR.
         IF AVAIL cperson THEN
         DO:
            RUN Fill-SysMes IN h_tmess (  "", "", "4",
                SUBSTITUTE(
                  "В базе уже есть клиент № &1 ~n &2, дата рождения &3.~n Продолжить ввод/редактирование клиента?",
                  cperson.person-id, cperson.first-names, STRING(cperson.birthday,"99.99.9999")
                    )).
            IF pick-value = "NO" THEN 
            DO:
               APPLY "CLOSE":U TO THIS-PROCEDURE.
   ...
   END. /* Проверки на дубли ФИО и Др */
...
DO:
   ...
   {&BEG_BT_LEAVE}
   IF       LENGTH (tt-person.bank-code-type:SCREEN-VALUE) >  0
      AND   LENGTH (tt-person.bank-code     :SCREEN-VALUE) >  0  
   THEN DO:

      IF tt-person.bank-code-type:SCREEN-VALUE =  "МФО-9" THEN /* добить слева нулями */
         tt-person.bank-code:SCREEN-VALUE = fStrPadC (tt-person.bank-code:SCREEN-VALUE, 
                                                      9, NO, "0").

      IF tt-person.bank-code-type:SCREEN-VALUE =  "ИНН" THEN
      DO:
...
DO:
   ...
   IF (        iMode =  {&MOD_EDIT}
      ...
      AND tt-person.bank-code-type:SCREEN-VALUE <> ""
   THEN DO TRANSACTION:
...
&Scoped-define SELF-NAME tt-person.country-id2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-person.country-id2 TERMINAL-SIMULATION
ON U1 OF tt-person.country-id2 IN FRAME fMain /* Код страны */
DO:
                        /* Если кол-во возвращаемых элементов больш 2-х,
                        ** то берем только первый. */
   pick-value = ENTRY (1, pick-value).
...
DO:
   ...
   IF (  iMode =  {&MOD_ADD}
      OR iMode =  {&MOD_EDIT})
      AND (DATE (tt-person.date-out:SCREEN-VALUE) <  DATE (tt-person.date-in:SCREEN-VALUE))
   THEN DO:
...
DO:
   ...
   DEFINE VARIABLE vCitizen AS CHARACTER NO-UNDO.
   ...
   IF {assigned tt-person.country-id2} THEN
      vCountry = tt-person.country-id2.
   ELSE
      vCountry = tt-person.country-id.
   IF NOT {assigned vCountry} THEN
      vCitizen  = "Stateless".
   ELSE
      vCitizen = IF vCountry =  "rus" THEN "CitizenRF" ELSE "Foreigner".
   RUN SetSysConf IN h_base ("FPersonIsRezident",vCitizen).
   RUN SetSysConf IN h_base ("PBirthday",tt-person.birthday).
   ...
   END.
   RUN DeleteOldDataProtocol IN h_base  ("FPersonIsRezident").
...
DO:
   ...
   THEN DO:
      ...
      vHBuffer:FIND-FIRST ("WHERE
               cust-ident.cust-code-type =  '" + SELF:SCREEN-VALUE + "'
         AND   cust-ident.cust-cat       =  'Ч'
         AND   cust-ident.cust-id        =  '" + STRING (tt-person.person-id) + "'
      ...
      ELSE DO:
         ...
         .   
         RUN Fill-SysMes IN h_tmess ("", "", "0","У клиента нет ни одного действующего удостоверения личности.").
         RETURN NO-APPLY {&RET-ERROR}.
      END.
...
DO:
   ...
   IF {assigned tt-person.inn} THEN DO:
      ...
      DO i = 1 TO vLen:
       IF ASC(SUBSTRING(SELF:SCREEN-VALUE,i,1)) <  48 OR 
          ASC(SUBSTRING(SELF:SCREEN-VALUE,i,1)) >  57 THEN isAlpha = YES.
       ELSE isAlpha = NO.
      ...
      IF vTempVal <> ? THEN DO:
         ...
         END.
      END.
      IF vLen <> 5 AND NOT fValidInnSignature(SELF:SCREEN-VALUE, mTempVal) THEN DO:
...
DO:
   ...
   THEN DO TRANSACTION:
      FIND FIRST bcident WHERE bcident.cust-cat       =  "Ч"
                           ...
                           AND bcident.open-date      <= gend-date
                           AND ( bcident.close-date     =  ? OR 
                                 bcident.close-date     >  gend-date )  
      NO-LOCK NO-ERROR.
      RUN browseld.p("p-cust-adr", 
                     SUBSTITUTE("&1&2&3&4",
                                ...
                                CHR(1),
                                "cust-id", 
                                (IF AVAIL bcident THEN (CHR(1) + "cust-code-type") ELSE "")),
                     SUBSTITUTE("&1&2&3&4",
                                "Ч",
                                CHR(1),
                                STRING(tt-person.person-id), 
                                (IF AVAIL bcident THEN (CHR(1) + mAdrType) ELSE "")),
                     SUBSTITUTE("&1&2&3&4",
                                "cust-cat",
                                CHR(1),
                                "cust-id", 
      ...
      IF AVAIL bcident THEN DO:
         ...
         IF bcident.cust-code-type <> mAdrType THEN
         DO:
             pick-value = "YES".
             RUN Fill-SysMes IN h_tmess ("", 
                                         "", 
                                         "4", 
                                         "Подтвердить выбранный адрес в качестве основного?").
   ...
   THEN DO:
      ...
      THEN DO:
         ...
         IF tt-p-cust-adr-reg.cust-code-type <> mAdrType THEN
         DO:
             pick-value = "YES".
             RUN Fill-SysMes IN h_tmess ("", 
                                         "", 
...
ON LEAVE OF tt-person.unk$ IN FRAME fMain /* УНК */
DO:
{&BEG_BT_LEAVE}
   IF (FGetSetting("ГенУНК", "", "НЕТ") = "ДА" AND mBankClient) 
      OR FGetSetting("ГенУНК", "", "НЕТ") = "НЕТ" 
   THEN DO:
      RUN ChkUpdUnk$ IN h_cust("person",
                               STRING(tt-person.person-id),
                               tt-person.unk$:SCREEN-VALUE) NO-ERROR.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   {get-form-params.i
      ...
      mLastName   "CHARACTER"
      mFirstNames "CHARACTER"
      mBirthDate  "CHARACTER"
   }
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE tt-person THEN 
    DISPLAY tt-person.person-id tt-person.unk$ tt-person.vidsotr$ 
          ...
          tt-person.inn tt-person.tax-insp tt-person.country-id2 tt-person.document-id 
          tt-person.document tt-person.Document4Date_vid tt-person.kodreggni$ 
          tt-person.kodreg$ tt-person.bank-code-type tt-person.bank-code 
  ...
  ENABLE mBankClient tt-person.person-id tt-person.unk$ tt-person.vidsotr$ 
         tt-person.name-last tt-person.first-names tt-person.birthday 
         tt-person.gender tt-person.BirthPlace tt-person.country-id 
         tt-person.inn tt-person.tax-insp tt-person.country-id2 separator4 tt-person.document-id 
...
DEF VAR vXattrFormat AS CHAR   NO-UNDO. /* ФОрмат ДР УНК. */
   ...
   THEN DO:
      ...
      IF vXattrFormat <> FILL("9", LENGTH(vXattrFormat))
      THEN DO:
         RUN Fill-SysMes IN h_tmess (
            "", "", "0",
...
IF mFlagUnk
   ...
   IF       iMode                                  =  {&MOD_ADD}
      AND   NUM-ENTRIES (iInstanceList, CHR (3))   >= 2
      THEN  mFirstTabItemAll  =  IF mFlagUnk
                                    THEN tt-person.unk$:HANDLE
                                    ELSE tt-person.person-id:HANDLE.

   IF       iMode                                  =  {&MOD_ADD}
      AND   NUM-ENTRIES (iInstanceList, CHR (3))   >= 4
      THEN  tt-person.name-last:SCREEN-VALUE   = mLastName.
   IF       iMode                                  =  {&MOD_ADD}
      AND   NUM-ENTRIES (iInstanceList, CHR (3))   >= 5
      THEN  tt-person.first-names:SCREEN-VALUE = mFirstNames.
   IF       iMode                                  =  {&MOD_ADD}
      AND   NUM-ENTRIES (iInstanceList, CHR (3))   >= 6
      THEN  tt-person.birthday:SCREEN-VALUE    = mBirthDate.
...
PROCEDURE LocalSetObject :
   ...
   DEFINE VAR vControlType  AS CHARACTER NO-UNDO.

   DEFINE VARIABLE vHasIDCard AS LOGICAL NO-UNDO.

   DEFINE BUFFER code       FOR code.
   DEFINE BUFFER cust-ident FOR cust-ident.
 
   IF mOnlyForm AND iMode =  {&MOD_ADD} THEN
   DO:
      /* проверяем документы */
      FIND FIRST code WHERE code.class  =  "КодДокум"
                        AND code.parent =  "КодДокум"
                        AND code.code   =  tt-person.document-id NO-LOCK NO-ERROR.
      IF AVAIL code THEN
      DO:
         ...
         IF {assigned vAvtoSrok} THEN DO:
            IF tt-person.birthday <> ? THEN DO:
               vEndDate = CalcSrokForDocument(vAvtoSrok,tt-p-cust-ident.open-date,tt-person.birthday).
               vOldEndDate = tt-p-cust-ident.end-date.
   
               IF vEndDate <> ? AND vEndDate <> vOldEndDate THEN DO:
                  RUN Fill-SysMes IN h_tmess ("","","3",
                                              "Для документа [" + tt-p-cust-ident.cust-code-type + "]~n" +
                                              "скорректировать дату окончания на " + STRING(vEndDate,"99/99/9999") + "?" + "|Да,Нет").
                  IF pick-value =  "1" THEN
                     tt-p-cust-ident.end-date = vEndDate.
                  vOldEndDate = vEndDate.
               END.

               vControlType = FGetSetting("СтандТр","СрокДокум","Нет").
               IF     vOldEndDate  <  TODAY 
                  AND vControlType <> "Нет" 
               THEN DO:
                  RUN Fill-SysMes IN h_tmess ("",
                                              "comm15",
                                              IF vControlType = "Да" 
                                              ...
                                              "%s=" + tt-p-cust-ident.cust-code-type + "%s=" + STRING(vOldEndDate) + "|Продолжить,Отменить").
      ...
      END.
   END.
   ELSE IF iMode = {&MOD_EDIT} THEN DO:
      CI:
      FOR EACH cust-ident WHERE
         cust-ident.cust-cat = "Ч"                 AND
         cust-ident.cust-id  = tt-person.person-id AND
         CAN-DO(Ls-Class("p-cust-ident"), cust-ident.class-code)
      NO-LOCK:
         RUN IsValidIDCardCode IN h_cust (cust-ident.cust-code-type,
                                          OUTPUT vHasIDCard).
         IF vHasIDCard THEN
            LEAVE CI.
      END.
      IF vHasIDCard <> YES THEN
         RETURN ERROR "У клиента не указано ни одного документа, допустимого " +
...
tt-person.last-date = TODAY.
   ...
   .
   IF mOnlyForm AND iMode =  {&MOD_ADD} AND mBankClient:SCREEN-VALUE =  "клиент" THEN
   DO:   
      /* записываем УНКг */
      RUN GetNewUnkgNumber("person", YES, OUTPUT tt-person.unkg$).
      SetFormDefList('tt-person.kodyadresa$,tt-person.unikkodadresa$,tt-person.nommarwsrut$,tt-person.unkg$').
   END.
   ELSE
...
/* Конец вставки Плюс банк */
   ...
   IF      (FGetSetting("ЗапросыЕГР","ЕГР_ВводИП","") =  "Да"
       AND iMode =  {&MOD_ADD})
        OR (FGetSetting("ЗапросыЕГР","ЕГР_КоррИП","") =  "Да"
       AND iMode =  {&MOD_EDIT}) THEN
   DO:
      RUN Fill-SysMes IN h_tmess(
         "","",3,
         "Запросить сведения из ЕГРЮЛ/ЕГРИП?:|Краткие сведения,Выписка,Не запрашивать").
      IF    pick-value =  "1"
      ...
      DO:
         ...
         DO:
            RUN SetSysConf IN h_base("EgrClId",STRING(tt-person.person-id)).
            RUN SetSysConf IN h_base("EgrMode","Edit"). 
            IF LENGTH(TRIM(tt-person.inn)) =  12 THEN
            DO:
               RUN SetSysConf IN h_base("EgrField","ИНН,ИП,person").
               RUN SetSysConf IN h_base("EgrValue",tt-person.inn).
            END.
            ELSE
               RUN Fill-SysMes IN h_tmess("","",1,"Неверная длина ИНН для отправки запроса!").
         END.
         IF     LASTKEY <> KEYCODE("ESC")
            AND NUM-ENTRIES(GetSysConf("EgrField")) >= 2 THEN
         DO:
            {intrface.get egr}      /*Библиотека запросов ЕГР*/
            IF GetBaseOpDate() =  ? THEN
               RUN InitBaseLibrary IN h_pbase (?,TODAY,?).
            IF pick-value =  "1" THEN
            DO:
               IF ENTRY(2,GetSysConf("EgrField")) =  "ИП" THEN
               DO:
                  mTypeEgr = "1".
                  RUN RunTransaction IN h_pbase ("e-zaprksip").
                  RUN TimeOutEgr IN h_egr ("i-egrksip", OUTPUT mErrEgr) .
                  IF mErrEgr THEN
                     RUN Fill-SysMes IN h_tmess("","",1,"Нет ответа от СМЭВ!").
                  ELSE
                  DO:
            ...
            IF NOT mErrEgr THEN
            DO:
               RUN Fill-SysMes IN h_tmess("","",4,"Внести сведения в карточку клиента?").
               IF pick-value =  "YES" THEN
               DO:
                  mUpdEgr = YES.
      ...
      DO:
         FIND LAST cust-ident WHERE cust-ident.cust-cat        =  "Ч"
                                AND cust-ident.cust-id         =  tt-person.person-id
                                AND cust-ident.class-code      =  "p-cust-ident"
                                AND cust-ident.cust-code-type  =  tt-person.document-id
                                AND cust-ident.close-date      =  ? NO-LOCK NO-ERROR.
         IF AVAIL cust-ident THEN
            ASSIGN
               mIssue =  cust-ident.issue
               mKP    =  GetXattrValueEx("cust-ident", 
                                         cust-ident.cust-code-type + "," + 
                                         cust-ident.cust-code      + "," + 
                                         STRING(cust-ident.cust-type-num),"Подразд","") NO-ERROR.
      END.
   END.
   ...
   THEN DO:             
      ...
      THEN DO:
         ...
         IF pick-value =  "YES" THEN DO:
            IF (FGetSetting("ГенУНК", "", "НЕТ") =  "ДА" AND mBankClient) 
               OR FGetSetting("ГенУНК", "", "НЕТ") =  "НЕТ" THEN
      ...
      DO:
         CASE mTypeEgr:
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-person.name-last   = ttRespKSIP.fLastName
                     tt-person.first-names = ttRespKSIP.fFirstName
                     tt-person.inn         = ttRespKSIP.fInn
                     tt-person.tax-insp    = ttRespKSIP.fCodeNo
                  .
            ...
            DO:
               ...
               DO:
                  ASSIGN
                     tt-person.inn               = ttRespVIPIP.fInn  
                     tt-person.country-id        = ttRespVIPIP.fCountry
                     tt-person.name-last         = ttRespVIPIP.fLastName
                     tt-person.first-names       = ttRespVIPIP.fFirstName
                     tt-person.tax-insp          = ttRespVIPIP.fCodeNo
                     tt-person.gender            = ttRespVIPIP.fGender
                     tt-person.birthday          = DATE(ttRespVIPIP.fDateBir)
                     tt-person.birthplace        = ttRespVIPIP.fPlaceBir
                  NO-ERROR.
                  RUN SetSysConf IN h_base("EgrField",
                                           ttRespVIPIP.fRegGni + ";" 
                                         + ttRespVIPIP.fAddr   + ";"
                  ...
                  RUN SetSysConf IN h_base("EgrDocPerson",
                                           "YES"                          + ";"
                                         + ttRespVIPIP.fDocCodeVd         + ";" 
                                         + ttRespVIPIP.fDocNum            + ";"
                                         + ttRespVIPIP.fDocBranch         + ";"
                                         + STRING(ttRespVIPIP.fDocDateOp) + ";"
                                         + ttRespVIPIP.fDocIssue).
               END.
            END.
         END CASE.
      END.
   ...
   IF    iMode =  {&MOD_ADD}
      OR iMode =  {&MOD_VIEW} THEN
   DO:
                        /* Если указан тип документа,
                        ** то ищем последний действующий документ
                        ** и перебрасываем данные для отображения. */
      IF LENGTH (tt-person.document-id) >  0 THEN
      DO:
         LASTDOC:
         FOR EACH tt-p-cust-ident WHERE            
            tt-p-cust-ident.cust-code-type   =  tt-person.document-id  AND
            tt-p-cust-ident.close-date       =  ?
...
PROCEDURE PostSetObject :
   ...
   DEFINE VAR vXattrCode    AS CHAR      NO-UNDO.
   DEFINE VAR vXattrName    AS CHAR      NO-UNDO.
   ...
   DEFINE VAR vControlType  AS CHARACTER NO-UNDO.
   DEFINE VARIABLE vFIOCl   AS CHARACTER NO-UNDO EXTENT 3.
   DEFINE VARIABLE vChkAdresses AS CHARACTER   NO-UNDO.
   DEFINE VARIABLE vTerrAdrType AS CHARACTER   NO-UNDO.
...
/* Переписываем из датасета значение person-id в таблицу tt-person. */
   ...
   THEN DO vInt = 1 TO NUM-ENTRIES(mCrCustRole):
      ...
      FIND FIRST cust-role WHERE cust-role.Class-Code = vRole
                             ...
                             AND cust-role.surrogate  = (IF shMode THEN shFilial ELSE STRING(tt-person.person-id))
      NO-LOCK NO-ERROR.
      IF NOT AVAIL cust-role THEN DO TRANS:
         CREATE cust-role.
         ASSIGN
...
*/
   ...
   DO:
      CASE mTypeEgr:
         ...
         DO:
            ...
            IF AVAIL ttRespKSIP THEN
            DO:
               UpdateSigns("person", STRING(tt-person.person-id), "ДатаВПред", 
                           ttRespKSIP.fDateOGRN, ?).
               UpdateSigns("person", STRING(tt-person.person-id), "ОГРН", ttRespKSIP.fOGRN, ?).
               IF ttRespKSIP.fTypeIP THEN
                  UpdateSigns("person", STRING(tt-person.person-id), "Предпр", "Предпр", ?).
            END.
         END.
         WHEN "2" THEN
         DO:
            ...
            IF AVAIL ttRespVIPIP THEN
            DO:
               UpdateSigns("person", STRING(tt-person.person-id), "ДатаОГРН", 
                           ttRespVIPIP.fDateOGRN, ?).
               IF ttRespVIPIP.fTypeIP THEN
                  UpdateSigns("person", STRING(tt-person.person-id),"Предпр","Предпр", ?).
               UpdateSigns("person", STRING(tt-person.person-id), "ОГРН", 
                           ttRespVIPIP.fOGRN,?).
               UpdateSigns("person", STRING(tt-person.person-id), "УчДокГР", 
                           ttRespVIPIP.fSvidet,?).
               UpdateSigns("person", STRING(tt-person.person-id), "RegDate", 
                           ttRespVIPIP.fRegDate, ?).
               UpdateSigns("person", STRING(tt-person.person-id), "РегОрган",    
                           ttRespVIPIP.fRegName, ?).
               UpdateSigns("person", STRING(tt-person.person-id), "RegNum",      
                           ttRespVIPIP.fRegNum, ?).
               UpdateSigns("person", STRING(tt-person.person-id), "УчДокДата",   
                           ttRespVIPIP.fDateNo, ?).
               UpdateSigns("person", STRING(tt-person.person-id), "engl-name",   
                           ttRespVIPIP.fEngName, ?).
               FIND LAST ttRespVIPOkved WHERE ttRespVIPOkved.fNum =  ttRespVIPIP.fNum 
                  NO-LOCK NO-ERROR.
               IF AVAIL ttRespVIPOkved THEN
                  UpdateSigns("person", STRING(tt-person.person-id),"ОКВЭД", 
               ...
               FOR EACH ttRespVIPLicence WHERE ttRespVIPLicence.fNum =  ttRespVIPIP.fNum NO-LOCK:
                  CREATE bcident.
                  ASSIGN
                     bcident.class-code     = "cust-lic"
                     bcident.cust-cat       = "Ч"
                     bcident.cust-id        = tt-person.person-id
                     bcident.cust-code-type = ttRespVIPLicence.fVid
                     bcident.open-date      = ttRespVIPLicence.fDateBeg
                     bcident.close-date     = ttRespVIPLicence.fDateEnd
                     bcident.issue          = ttRespVIPLicence.fIssue
                     bcident.cust-code      = ttRespVIPLicence.NumbLic
                  .
                  FIND FIRST bcident WHERE bcident.cust-cat   =  "Ч"
                                       AND bcident.cust-id    =  tt-person.person-id
                                       AND bcident.class-code =  "cust-lic" 
                                       AND bcident.cust-code  =  ttRespVIPLicence.NumbLic
                                       AND bcident.open-date  =  ttRespVIPLicence.fDateBeg
                     NO-LOCK NO-ERROR.
                  IF AVAIL bcident THEN
                     UpdateSigns ("cust-lic",
                                  STRING(bcident.cust-code-type) + ',' +
                                  STRING(bcident.cust-code) + ',' +
                                  STRING(bcident.cust-type-num),
                                  "ВидЛиценз", ttRespVIPLicence.NameLic,?).
               END.
               RELEASE bcident.
            END.
         END.
   ...
   FOR EACH code WHERE code.class  =  "КодДокум"
                   AND code.parent =  "КодДокум"
   NO-LOCK:
      vAvtoSrok = GetXattrValueEx("code",code.class + "," + code.code,"АвтоСрок","").

    /*SORT-ACCESS cust-ident*/
      FOR EACH cust-ident /*USE-INDEX*/ USE-INDEX cust
                       WHERE cust-ident.cust-cat        =  "Ч"
                         AND cust-ident.cust-id         =  tt-person.person-id
                         AND cust-ident.class-code      =  "p-cust-ident"
                         AND cust-ident.cust-code-type  =  code.code
                         AND cust-ident.close-date      =  ?
      ...
      vControlType = FGetSetting("СтандТр","СрокДокум","Нет").
      IF REPLACE(tt-person.document-id,'__FORM~001','') = cust-ident.cust-code-type THEN
         IF     vOldEndDate  <  TODAY 
            AND vControlType <> "Нет" 
         THEN DO:
            RUN Fill-SysMes IN h_tmess ("","comm15",
                                        IF vControlType = "Да" 
                                        THEN "-1" 
                                        ELSE "",
                                        "%s=" + tt-person.name-last + " " + tt-person.first-names +
      ...
      FOR EACH bcident WHERE bcident.cust-code-type =   cust-ident.cust-code-type
                        AND  bcident.cust-code      =   cust-ident.cust-code
                        AND  bcident.class-code     =   cust-ident.class-code
                        AND  RECID(bcident)         <>  RECID(cust-ident)
                        AND  bcident.cust-cat       =   cust-ident.cust-cat
                        AND  bcident.cust-id        <>  cust-ident.cust-id
         NO-LOCK:
         RUN GetCustName IN h_base (bcident.cust-cat,
                                    bcident.cust-id,
                                    ?,
                                    OUTPUT         vFIOCl[1],
                                    OUTPUT         vFIOCl[2],
         ...
         IF bcident.open-date <> cust-ident.open-date THEN
         DO:
            pick-value = "NO".
            RUN Fill-SysMes IN h_tmess (  "", "", "4",
               SUBSTITUTE(
                  "Документ &1 с номером &2 уже введен для клиента ~n &3 (номер: &4). Изменить номер документа?",
                  bcident.cust-code-type, bcident.cust-code, 
                  vFIOCl [1], bcident.cust-id)).
            IF   pick-value =  "YES" THEN
            DO:   
               APPLY "ENTRY" TO  tt-person.document-id IN FRAME fMain. 
               RETURN ERROR "Документ не введен".
            END.
         END.
         ELSE DO:
            RUN Fill-SysMes IN h_tmess 
             ...
             "Документ &1 с номером &2 и датой выдачи &3 уже введен для клиента ~n &4 (номер: &5).",
                  bcident.cust-code-type, bcident.cust-code,
                  STRING(bcident.open-date,"99/99/9999"), vFIOCl [1],
                  bcident.cust-id)). 
            RETURN ERROR.
         END.
      END.
   ...
   IF LOGICAL(fGetSetting("CLI-EXPORT", "Person", "NO")) =  YES THEN
      RUN createxmlpers.p(tt-person.person-id) .

   /*проверка по стоп листам*/
   IF  ChkClientByStopList ('person' ,
                            STRING(tt-person.person-id),
                            tt-person.name-last + ' ' + tt-person.first-name,
                            "",
                            tt-person.inn,
                            tt-person.document,
                            tt-person.birthday,
   ...
   THEN DO:
      ...
      IF NOT StopListAction("person",STRING(tt-person.person-id), vMode) THEN
         RETURN ERROR. 
   END.

/* Вставка Плюс банк */
   /*Проверка на наличие Распоряжения на овердрафт по карте МИР*/
   vOk = No.
   RUN chk-over-pers.p (tt-person.person-id, OUTPUT vOk) NO-ERROR.
   IF NOT vOk
   THEN DO:
...
/* Конец вставки Плюс банк */
   IF  iMode =  {&MOD_ADD} THEN
   DO:
      IF ENTRY(1,fGetSetting("Cl_TerrAdr","AddrRedact","")) =  "Да"   
      THEN DO:
   ...
   IF FGetSetting("ОбменФМС","ФМСРед_Кл","") =  "Да"
   THEN DO:
      {cl-fmschk.i
         "'person'"
         "tt-person.person-id"
         "REPLACE(tt-person.document-id,'__FORM~001','')"
         "tt-person.document"
         "GetXattrValueEX('person', STRING(tt-person.person-id), 'Document4Date_vid', '')"
         "tt-person.name-last + ' ' + tt-person.first-names"
      }
      /*если установлен запрет на ввод клиента и 
        паспорт не действительный*/
      IF FGetSetting("ОбменФМС","ФМСВвод_Кл","") = "Да" THEN
      DO:
...
PROCEDURE Local_F9:
   IF iMode = {&MOD_VIEW} THEN 
   RUN MainAction IN THIS-PROCEDURE.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Check-Ascii-Set2 TERMINAL-SIMULATION
PROCEDURE Check-Ascii-Set2 PRIVATE:
   ...
   /* ibm866 */
   DO vj = 1 TO NUM-ENTRIES(iParam):
      CASE ENTRY(vj,iParam):
         WHEN "1" THEN /* 1. Проверка строки на кириллицу. */
         DO: 
            DO vi = 128 TO 175:
               {additem.i vCorrectlyStr STRING(vi)}
            END.
            DO vi = 224 TO 241:
               {additem.i vCorrectlyStr STRING(vi)}
            END.
   ...
   DO vi = 1 TO LENGTH(iOthersSymbol):
      vTmpChar = STRING(ASC(SUBSTRING(iOthersSymbol,vi,1))).
      {additem.i vCorrectlyStr vTmpChar}
   END.
   /* Сама проверка */
   DO vi = 1 TO LENGTH(iStr):
      IF CAN-DO(vCorrectlyStr,STRING(ASC(SUBSTRING(iStr,vi,1)))) THEN
         NEXT.
      ELSE DO:                   /* Недопустимые символы  */
         oResult = SUBSTRING(iStr,vi,1).
         LEAVE.
      END.
   END.
END PROCEDURE.
...
/* $LINTUSER='kuds' */
/* $LINTDATE='03/11/2017 09:58:42.602+03:00' */
/*prosignH2RUNuDh/s/Dzft/9ws67Q*/