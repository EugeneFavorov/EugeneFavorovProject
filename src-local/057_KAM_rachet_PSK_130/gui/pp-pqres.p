...
/* +++ pp-pqres.p was humbly modified by (c)blodd converter v.1.10 on 2/17/2017 6:39am +++ */
...
PROCEDURE StartInterface.
   ASSIGN
      ...
      mDelay         = FGetSetting("ПробегОплаты", "ПробегОплСсуды", "Нет")   EQ "Да"
      mAlgFIFO       = FGetSetting("Форма115П",    "УчПог115",       "ДА")    EQ "Да"
      ...
      mCurrDay       = ( IF {assigned mCurrDayStr} then
                          ...
                          fGetSetting("ПОС","ПОСПрТекД",  "НЕТ")) EQ "ДА"
      mPutProt       = FGetSetting("rsvrStream","", "") EQ "Да"
      ...
      mI139_Int         = fGetSetting("И139_8861","И139_8861НачПроц", "")
      mI139_PastDueInt  = fGetSetting("И139_8861","И139_8861ПрПроц", "")
      mProcPar139       = FGetSetting("110И","ПарПроср","")
   .
      IF mProcPar139 EQ "" THEN mProcPar139 = FGetSetting("ПОС", "ПОСКонтрПар", "").
         /* алгоритм не нужен (может взятся из ПОСКонтрПар) */
      mProcPar139 = IF NUM-ENTRIES(mProcPar139,":") EQ 2 THEN ENTRY(2,mProcPar139,":")
                                                         ELSE mProcPar139.
   RETURN.
END PROCEDURE.
...
PROCEDURE PREPARE_REAL_OPV:
   ...
   FOR EACH term-obl WHERE
      ...
      DO:
         ...
         IF    term-obl.dsc-beg-date LE vDate
            OR term-obl.dsc-beg-date EQ ? THEN
            vDate = IF term-obl.dsc-beg-date EQ ? THEN term-obl.end-date ELSE term-obl.dsc-beg-date.
      END.
      ELSE  /* Иначе (ПробегОплСсуды=Нет)                              */
      DO:
         /* Проверяется строго поле term-obl.end-date. */
         IF term-obl.end-date LE vDate THEN
            vDate = term-obl.end-date.
      END.
...
PROCEDURE GetProcentUnAccount :
   ...
   IF loan.class-code EQ "own-bill-liability"  OR
      loan.class-code  EQ "dsc-bill-asset"
     THEN DO:
   ...
   /* Ищем сумму начисленных за период %% */
      IF vBegDate NE vEndDate  THEN
   RUN get_loan_procent (vContract,
                         vContCode,
                         IF vBegDate LT vEndDate THEN vBegDate + 1 ELSE vBegDate,
                         vEndDate,
                         4,
                         1,
                         OUTPUT vPint ).
...
PROCEDURE GET_SUMMA_POG:
   ...
   FOR EACH ttSpis WHERE
      (iLScope EQ ? AND ttSpis.end-date LE iRScope) OR
      (iRScope EQ ? AND ttSpis.end-date GE iLScope) OR
      (iRScope NE ? AND iLScope NE ? AND
       ttSpis.end-date GE iLScope AND ttSpis.end-date LE iRScope)
   NO-LOCK:
      oSumma = oSumma + ttSpis.summa.
   END.
END PROCEDURE.
...
PROCEDURE PREPARE_SPIS_PROC:
   ...
   DEF VAR vPar      AS CHAR NO-UNDO INIT '33,29,10'.
...
PROCEDURE GET_SUMMA_DOLG :
   ...
   ASSIGN
      vDelay = IF iDelayFl NE ? THEN iDelayFl ELSE (FGetSetting("ПробегОплаты","ПробегОплСсуды","Нет") = "Да")
   ...
   DO :
      FIND LAST loan-cond  WHERE loan-cond.contract = iContract
         ...
         AND  loan-cond.since LE iBegDate NO-LOCK NO-ERROR.
      ...
      FOR EACH loan-cond WHERE loan-cond.contract = iContract
         ...
         AND   loan-cond.since LE iEndDate  NO-LOCK :
...
PROCEDURE GET_SUMMA_DOLG_RUB :
   ...
   IF     AVAIL loan
      AND loan.cont-type NE {&SOGL_TYPE} THEN
   DO:
         /* Учитывать пробег погашения, пролонгацию */
      ASSIGN
         vDelay   = IF iDelayFl NE ? THEN iDelayFl ELSE (FGetSetting("ПробегОплаты",
                                                                     "ПробегОплСсуды",
                                                                     "Нет") EQ "Да")
         vProl    = FGetSetting("УчитПрол", ?, "Нет") EQ "Да"
         vBegDate = iBegDate
      .
      ...
      DO:
         ...
         NO-LOCK NO-ERROR.
         IF AVAIL loan-cond THEN
            vBegDate = IF (iBegDate - loan-cond.delay1) LT loan-cond.since
                          THEN loan-cond.since
                          ELSE iBegDate - loan-cond.delay1.
            /* Запоминаем точку с первым значением пробега */
         CREATE ttSpis.
         ...
         FOR EACH loan-cond WHERE
                  loan-cond.contract  EQ iContract
            AND   loan-cond.cont-code EQ iContCode
            AND   loan-cond.since     GT iBegDate
            AND   loan-cond.since     LE iEndDate
         NO-LOCK:
            IF loan-cond.delay1 NE vDelayl THEN
            DO:
               CREATE ttSpis.
               ASSIGN
                  ttSpis.end-date = loan-cond.since
      ...
      DO WHILE AVAIL ttSpis:
         ...
         NO-LOCK:
            ...
            IF (term-obl.end-date + vDelayl) LT iBegDate THEN
               NEXT CICL.
               /* Это как понимать?  */
            IF      iEndDate LT loan.open-date
               OR (term-obl.end-date + vDelayl) GT iEndDate THEN
               NEXT CICL.
            ...
            DO:
               FIND FIRST pro-obl WHERE
                          pro-obl.contract   EQ iContract
                  AND     pro-obl.cont-code  EQ iContCode
                  AND     pro-obl.idnt       EQ term-obl.idnt
                  AND     pro-obl.new-nn     EQ term-obl.nn
                  AND     pro-obl.n-end-date EQ term-obl.end-date
                  AND     pro-obl.pr-date    LT term-obl.end-date
               NO-LOCK NO-ERROR.
               IF     AVAIL pro-obl
                  AND pro-obl.end-date LT iBegDate THEN
                  NEXT CICL.
            END.
            ...
            IF vSummaDolgN EQ 0 THEN NEXT CICL.
      ...
      DO vCount = 1 TO NUM-ENTRIES(vIdDList):
         ...
         FOR EACH loan-int OF loan WHERE
                  loan-int.id-d  EQ INT64(ENTRY(vCount, vIdDList))
            AND   loan-int.id-k  EQ INT64(ENTRY(vCount, vIdKList))
...
PROCEDURE GET_SUMMA_DOLG_RUB_NEW:
   ...
   DO:
      ...
      NO-LOCK NO-ERROR.
         ...
         /* Если тип договора не "Течение" */
      IF AVAIL loan AND
               loan.cont-type NE {&SOGL_TYPE} THEN
      DO:
         ...
         FIND LAST term-obl  WHERE
               ...
               AND term-obl.end-date  LE iBegDate - 1
         NO-LOCK NO-ERROR.
         ...
         FIND LAST term-obl WHERE
             ...
             AND   term-obl.cont-code EQ iContCode
             AND   term-obl.idnt      EQ 3
             AND   term-obl.end-date  LT iBegDate
         NO-LOCK NO-ERROR.
        ...
        /* Теперь идем по обязательствам, попадающим в период */
         ...
         NO-LOCK:
            ...
            IF term-obl.dsc-beg-date GT iEndDate THEN
               LEAVE CICL.

            IF term-obl.dsc-beg-date LT iBegDate THEN
               NEXT CICL.

            IF term-obl.sop-date NE ? THEN
               IF (term-obl.sop-date LE iBegDate) THEN
                  NEXT CICL.

            IF vSumm GE term-obl.amt-rub THEN
            DO:
         ...
         IF FGetSetting("Форма115П","УчПред","") EQ "ПОЛН" THEN
...
PROCEDURE LoanFromAcct.
   ...
   FOR EACH loan-acct WHERE
        ...
        AND loan-acct.curr  EQ iCurr
        AND loan-acct.since LE iSince
        AND NOT CAN-FIND (FIRST b1-lacct WHERE b1-lacct.acct       EQ loan-acct.acct
                                           AND b1-lacct.curr       EQ loan-acct.curr
                                           AND b1-lacct.contract   EQ loan-acct.contract
                                           AND b1-lacct.cont-code  EQ loan-acct.cont-code
                                           AND b1-lacct.acct-type  EQ loan-acct.acct-type
                                           AND b1-lacct.since      LE iSince
                                           AND b1-lacct.since      GT loan-acct.since)
      NO-LOCK:
      CREATE tt-oLoanAcct.
      ASSIGN
...
PROCEDURE GetSummDolg.
   ...
   DO:
      ...
      ELSE
      DO:
         FIND FIRST code WHERE code.class EQ vCodeName
                           AND code.name  EQ iAcctType
         NO-LOCK NO-ERROR.
      ...
      IF     iAcctType       EQ "КредВГар"
         AND loan.class-code EQ "loan-guarantee"
         AND NOT oByTerm
         THEN oByTerm = NO.
      ELSE
      IF iBegDate NE ?
         AND iEndDate NE ? THEN
         oByTerm = YES.
   END.
...
PROCEDURE SUMM_SPIS_RES_TYPE.
   ...
   DO:
      ...
      IF    (NUM-ENTRIES(iContCode," ") GT 1 AND NOT vOk)
      ...
      IF NUM-ENTRIES(iContCode," ") EQ 2
          AND CAN-FIND (FIRST bloan WHERE bloan.contract  EQ iContract
                                AND bloan.cont-code EQ ENTRY(1,iContCode," ")
                       NO-LOCK) THEN
      ...
      THEN DO:
         ...
         DO:
            ...
            OUTPUT STREAM sOut CLOSE.
         END.
         IF vBasaOst EQ 0 THEN oSummRsrv = 0.
         ELSE DO:
            ...
            ** рассчитан как доля общего резерва, соответствующая доле обязательств по договору */
            DO vi = 1 TO NUM-ENTRIES(vPrmList):
               ...
               IF iBegDate NE ? AND iEndDate NE ?
                   AND ENTRY(vi,vPrmList) EQ "21"
               THEN
            ...
            END.
            vTranshCount = 0.
            FIND FIRST tt-SumDolg WHERE tt-SumDolg.contract  EQ iContract
                                    AND tt-SumDolg.cont-code EQ ENTRY(1,iContCode," ")
                                    AND tt-SumDolg.AcctType  EQ vOsnRoleList
                                    AND tt-SumDolg.since     EQ iSince
               NO-LOCK NO-ERROR.
            ...
            ASSIGN
               vRate = IF vAllBasaOst EQ 0 THEN 0 ELSE (vBasaOst / vAllBasaOst)
            ...
            DO:
               ...
               FOR EACH tt-DataLoan WHERE
                            tt-DataLoan.contract  EQ iContract
                        AND tt-DataLoan.cont-code BEGINS ENTRY(1,iContCode," ")
                                                         + " "
                        AND tt-DataLoan.since     EQ iSince
               NO-LOCK :
                  ASSIGN
   ...
   /* возможно требуется разбиение резерва */
     ...
     /* ищем  счет */
      FIND LAST loan-acct WHERE loan-acct.contract  EQ iContract
                            ...
                            AND loan-acct.acct-type EQ iAcctType
                            AND loan-acct.since     LE iSince
           NO-LOCK NO-ERROR.
      ...
      FIND FIRST code WHERE code.class   EQ vCodeName
                        AND code.parent  EQ vCodeName
                        AND code.code    NE  iAcctType
                        AND code.misc[3] EQ vPrmList
            NO-LOCK NO-ERROR .
...
PROCEDURE LnRsrvByType.
   ...
   /* Анализируем тип роли счета */
            /* Вексель */
   IF      iAcct BEGINS "51"
      AND (   iAcctType EQ "КредТ"
           OR iAcctType EQ "КредДискДН"
           OR iAcctType EQ "КредПр%"
           OR iAcctType EQ "КредДискПр")
   THEN
   ...
   ELSE IF iAcctType EQ "КредТ" OR iAcctType EQ "КредТПр" THEN
   DO:
      ASSIGN
   ...
   ELSE IF iAcctType EQ "КредПр%" OR iAcctType EQ "КредПр%1" THEN
   DO:
      ASSIGN
         oRsrvProc = LnFormRsrvRoleSpis (iContract,
                                         iContCode,
   ...
   DO:
      ...
      oRsrvRate = LnFormRsrvCom (iContract,
                                 ...
                                 iAcctType).
   END.
   ELSE IF iAcctType EQ "КредПр" THEN
   DO:
       ASSIGN
...
PROCEDURE GetLoanAcctRsrv.
   ...
   DO:
      ...
      RUN RE_L_ACCT(iContract,iContCode,oRsrvRole,iSince,BUFFER loan-acct).
      ...
      DO vCounter = 1 TO NUM-ENTRIES (iSpisPar):
         ...
         DO:
            ...
            DO:
               ...
               FOR EACH bcode WHERE
                        bcode.class  EQ vCodeName
                    AND bcode.parent EQ vCodeName
               NO-LOCK:
                  FIND FIRST tt-ReservDog WHERE
                             tt-ReservDog.res_par EQ bcode.misc[3]
                  NO-LOCK NO-ERROR.
                  IF NOT AVAIL tt-ReservDog THEN
                  DO:
                     CREATE tt-ReservDog.
         ...
         DO:
            ...
            FIND FIRST tt-ResLoan WHERE
                       tt-ResLoan.res_par   EQ vPar
                   AND tt-ResLoan.cont-code EQ loan.cont-code
            NO-LOCK NO-ERROR.
            ...
            DO:
               ...
               FOR EACH bloan WHERE
                    ...
                    AND NUM-ENTRIES (bloan.cont-code, " ") GT 1 NO-LOCK:
               ...
               FIND FIRST bloan WHERE
                          bloan.contract  EQ loan.contract
                      AND bloan.cont-code EQ ENTRY(1, loan.cont-code, " ")
               NO-LOCK NO-ERROR.
               ...
               DO:
                  ...
                  FOR EACH tt-ResLoan WHERE
                       ...
                       AND NUM-ENTRIES(tt-ResLoan.cont-code, " ") GT 1
                  EXCLUSIVE-LOCK:
                     ASSIGN
                     ...
                     IF vMaxSum LT tt-ResLoan.sum_res THEN
                        ASSIGN
...
PROCEDURE GetAllFromLoan.
   ...
   IF iInclClsd EQ ? THEN iInclClsd = YES.
   ...
   DO:
      ...
      FOR EACH bloan-acct WHERE
               bloan-acct.acct     EQ iAcct
           AND bloan-acct.curr     EQ iCurrency
           AND bloan-acct.since    LE iSince
           AND bloan-acct.contract EQ "Кредит"
           ...
           AND NOT CAN-FIND (FIRST b1-lacct WHERE b1-lacct.acct       EQ bloan-acct.acct
                                              AND b1-lacct.curr       EQ bloan-acct.curr
                                              AND b1-lacct.contract   EQ bloan-acct.contract
                                              AND b1-lacct.cont-code  EQ bloan-acct.cont-code
                                              AND b1-lacct.acct-type  EQ bloan-acct.acct-type
                                              AND b1-lacct.since      LE iSince
                                              AND b1-lacct.since      GT bloan-acct.since)
         NO-LOCK:

         FIND LAST b1-lacct WHERE b1-lacct.contract  EQ bloan-acct.contract
                              AND b1-lacct.cont-code EQ bloan-acct.cont-code
                              AND b1-lacct.acct-type EQ bloan-acct.acct-type
                              AND b1-lacct.since     LE iSince NO-LOCK NO-ERROR.

         IF AVAIL b1-lacct AND b1-lacct.acct NE iAcct THEN
            NEXT.
      ...
      FOR EACH bloan-acct WHERE
           ...
           AND bloan-acct.curr     EQ iCurrency
           AND bloan-acct.since    LE iSince
           AND bloan-acct.contract EQ "АРЕНДА"
           AND NOT CAN-FIND (FIRST b1-lacct WHERE b1-lacct.acct       EQ bloan-acct.acct
                                              AND b1-lacct.curr       EQ bloan-acct.curr
                                              AND b1-lacct.contract   EQ bloan-acct.contract
                                              AND b1-lacct.cont-code  EQ bloan-acct.cont-code
      ...
      FOR EACH tt-LoanAcct,
         FIRST bloan FIELDS (contract cont-code currency close-date end-date)
                     WHERE bloan.contract   EQ tt-LoanAcct.contract
                       AND bloan.cont-code  EQ tt-LoanAcct.cont-code
                       AND (   bloan.close-date EQ ?
                            OR bloan.close-date GT iSince
                            OR iInclClsd)
      NO-LOCK:

         IF bloan.contract EQ "Кредит" THEN
         DO:
            IF mPutProt THEN
            DO:
                  /* поиск счета резерва */
            ...
            RUN GetSummDolg (tt-LoanAcct.contract,
                             tt-LoanAcct.cont-code,
                             tt-LoanAcct.acct-type,
                             iSince,
                             iBegDate,
           ...
           /* значение сформированного резерва (с учетом сроков) */
            RUN SUMM_SPIS_RES_TYPE (tt-LoanAcct.contract,
                                    tt-LoanAcct.cont-code,
                                    tt-LoanAcct.acct-type,
                                    iSince,
                                    iBegDate,
            ...
            DO:
               ...
               ASSIGN tt-DataLoan.rid                 = RECID(bloan)
                      ...
                      tt-DataLoan.grsrv               = IF vGRsrv EQ ? THEN 1
                                                                       ELSE vGRsrv
                      tt-DataLoan.rsrv-rate           = IF vRsrvRate EQ ? THEN 0
                                                                          ELSE vRsrvRate
                      tt-DataLoan.rsrv-summ           = vRsrvSumm
                      tt-DataLoan.rsrv-rate-clc       = vRRsrvClc
                      tt-DataLoan.rsrv-summ-clc       = vRSummClc
         ...
         ELSE
         IF bloan.contract EQ "АРЕНДА" THEN
         DO:
            ...
            DO:
               ...
               FOR EACH bloan-cond WHERE
                  ...
                  AND bloan-cond.since     GT iSince
               NO-LOCK
               BY bloan-cond.since:
               ...
               IF vEndDate NE ?
                  AND (   vEndDate LT iBegDate
                       OR vEndDate GT iEndDate) THEN
                  vSummDolg = 0.
            END.
            ELSE
            DO:
...
PROCEDURE GetLoanEndDate.
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      NO-LOCK BY term-obl.end-date DESC:
         ...
         pb:
         FOR EACH t-pro-obl OF loan WHERE t-pro-obl.idnt       EQ 3
                                      AND t-pro-obl.n-end-date EQ term-obl.end-date
         ...
         FIND LAST pro-obl WHERE
                   pro-obl.contract   EQ iContract
            AND    pro-obl.cont-code  EQ iContCode
            AND    pro-obl.pr-date    GE loan.open-date
            AND    pro-obl.pr-date    LE iEndDate
         NO-LOCK NO-ERROR.

         IF     AVAIL pro-obl
            AND CAN-FIND (FIRST bterm-obl WHERE bterm-obl.contract  EQ pro-obl.contract
                                            AND bterm-obl.cont-code EQ pro-obl.cont-code
                                            AND bterm-obl.idnt      EQ pro-obl.idnt
                                            AND bterm-obl.end-date  EQ pro-obl.n-end-date
                                            AND bterm-obl.nn        EQ pro-obl.new-nn)
         THEN
         DO:
            ...
            FIND FIRST b-pro-obl WHERE
                       b-pro-obl.contract   EQ iContract
               AND     b-pro-obl.cont-code  EQ iContCode
               AND     b-pro-obl.pr-date    GT iEndDate
            NO-LOCK NO-ERROR.
            IF AVAIL b-pro-obl THEN
               oEndDateLast = b-pro-obl.end-date.
         END.
         /* Если не нашли записей о пролонгации, то поищем по ДР условий */
         IF oEndDateLast EQ ? THEN
         DO:
            BLK:
            FOR EACH loan-cond WHERE
                     loan-cond.contract  EQ iContract
            ...
            NO-LOCK BY loan-cond.since DESCENDING:
               ...
               IF GetXAttrValue("loan-cond", vSurr, "ВидРеструкт") EQ "ПролОбСрДог" THEN
               DO:
                  vDateString = GetXAttrValueEx("loan-cond", vSurr, "CondEndDate", "").
                  IF vDateString NE "" THEN
...
PROCEDURE GetLoanPrQDay.
   ...
   DO ON ERROR UNDO, RETRY:
      ...
      IF AVAIL loan AND loan.open-date LT mDateNR THEN
...
PROCEDURE GetQRestrLoan PRIVATE.
   ...
   IF iTypeRestr EQ "ПролОбСрДог" THEN
   DO:
      FOR EACH pro-obl WHERE
               pro-obl.contract   EQ iContract
         AND   pro-obl.cont-code  EQ iContCode
         AND   pro-obl.pr-date    GE iBegDate
         AND   pro-obl.pr-date    LE iEndDate
      NO-LOCK:
         oQuant = oQuant + 1.
      END.
   ...
   IF     oQuant EQ 0
      AND iIndex LE NUM-ENTRIES(iRestrQuan) THEN
...
PROCEDURE GetQRestrLoanCond PRIVATE.
   ...
   FOR EACH loan-cond WHERE
            loan-cond.contract  EQ iContract
        AND loan-cond.cont-code EQ iContCode
        AND loan-cond.since     GE iBegDate
        AND loan-cond.since     LE iEndDate
...
PROCEDURE GetSumPlanProc.
   ...
   IF AVAIL loan THEN
   DO:
      IF NOT (loan.class-code  EQ "own-bill-liability"
           OR loan.class-code  EQ "dsc-bill-asset") THEN
      DO:
         IF FGetSetting("Форма125", "ПроцУчИстГраф", "ДА")   EQ "ДА" THEN
            vUseHist = CAN-FIND(
               FIRST term-obl-hist WHERE
                 ...
                 AND term-obl-hist.since     GE iEndF125).
   ...
   IF oSumProc LT 0 OR oSumProc EQ ? THEN oSumProc = 0.
      ...
      DO:
         ...
         REPEAT vCounter = 1 TO NUM-ENTRIES(vList-Comm):
            ...
            FIND FIRST comm-rate WHERE
                       comm-rate.acct       EQ "0"
                   AND comm-rate.commission EQ vComm
         ...
         END.
         FOR EACH loan WHERE
                  loan.contract         EQ "Страх"
            AND   loan.parent-cont-code EQ iContCode
            AND   loan.parent-contract  EQ iContract
            AND   loan.open-date        LE iDate
            AND  (loan.close-date       GE iDate
         ...
         DO:
            ...
            DO:
               vCalcEPS = CAN-FIND (LAST bloan-acct WHERE
                                         bloan-acct.acct      EQ loan-acct.acct
...
FUNCTION fCalcEpsSpec RETURNS DEC
   ...
   FIND LAST comm-rate WHERE
             comm-rate.kau        EQ iContract + "," + iContCode
         AND comm-rate.commission EQ "%ОткрЛим"
         AND comm-rate.since      LE iDate
         AND comm-rate.acct       EQ "0"
   NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
   ...
   FIND LAST comm-rate WHERE
             comm-rate.kau        EQ iContract + "," + iContCode
         AND comm-rate.commission EQ "КомСч"
         AND comm-rate.since      LE iDate
         AND comm-rate.acct       EQ "0"
   NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
...
/* Ф117ПроцЭПС  ЭПС рассчитывается по следующей формуле %Кред + %Выд  Ф117ПроцЭПС */
FUNCTION fCalcEpsSpec1 RETURNS DEC
   ...
   FIND LAST comm-rate WHERE
             comm-rate.kau        EQ iContract + "," + iContCode
         AND comm-rate.commission EQ "%Выд"
         AND comm-rate.since      LE iDate
         AND comm-rate.acct       EQ "0"
   NO-LOCK NO-ERROR.
   /* Правка Плюс банк */
   IF AVAIL comm-rate THEN DO:
      ...
      IF comm-rate.rate-fixed THEN
         vOpenLim = vOpenLim / LN_GetLoanSumma(iContract, iContCode) * 100.
   END.
   oSumEPS = vCredRate + vOpenLim .
   RETURN oSumEPS.
END FUNCTION.

/* Ф117ПроцЭПС   ЭПС рассчитывается по следующей формуле %Кред + %Выд*365/колДнейДоговора */
...
FUNCTION fCalcPrpsSpec RETURNS DEC
   ...
   FIND LAST comm-rate WHERE
      comm-rate.kau        EQ iContract + "," + iContCode
      AND comm-rate.commission EQ "%КрПр"
      AND comm-rate.since      LE iDate
      AND comm-rate.acct       EQ "0"
      NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
   ...
   FIND LAST comm-rate WHERE
      comm-rate.kau        EQ iContract + "," + iContCode
      AND comm-rate.commission EQ "%КомЛим"
      AND comm-rate.since      LE iDate
      AND comm-rate.acct       EQ "0"
      NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
   ...
   FIND LAST comm-rate WHERE
      comm-rate.kau        EQ iContract + "," + iContCode
      AND comm-rate.commission EQ "Пеня-К"
      AND comm-rate.since      LE iDate
      AND comm-rate.acct       EQ "0"
      NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
   ...
   FIND LAST comm-rate WHERE
      comm-rate.kau        EQ iContract + "," + iContCode
      AND comm-rate.commission EQ "ПеняКомСч"
      AND comm-rate.since      LE iDate
      AND comm-rate.acct       EQ "0"
      NO-LOCK NO-ERROR.
   IF AVAIL comm-rate THEN
...
FUNCTION fCalcMarga RETURNS DEC
   ...
   DO:
      ...
      IF NOT AVAIL acct THEN
         LEAVE main.
      FOR EACH loan WHERE
               loan.cust-cat EQ acct.cust-cat
           AND loan.cust-id  EQ acct.cust-id
           AND loan.contract EQ "Позиция"
      NO-LOCK:
...
PROCEDURE CalcDebt115b.
   ...
   ASSIGN
      mTmpParz1 = ( IF iParz1 GT "" THEN iParz1 ELSE mParz1)
      mTmpParz2 = ( IF iParz2 GT "" THEN iParz2 ELSE mParz2)
      mTmpParz3 = ( IF iParz3 GT "" THEN iParz3 ELSE mParz3)
      mTmpParz4 = ( IF iParz4 GT "" THEN iParz4 ELSE mParz4)
      mTmpParp1 = ( IF iParp1 GT "" THEN iParp1 ELSE mParp1)
      mTmpParp2 = ( IF iParp2 GT "" THEN iParp2 ELSE mParp2)
      mTmpParp3 = ( IF iParp3 GT "" THEN iParp3 ELSE mParp3)
      mTmpParp4 = ( IF iParp4 GT "" THEN iParp4 ELSE mParp4)
      mTmpProcPar = ( IF iProcPar GT "" THEN iProcPar ELSE mProcPar)
   ...
   FIND FIRST loan NO-LOCK where
    ...
    /* Сумма  задолженности */
      ...
      IF ioProcFl EQ ? THEN /* Определение наличие просрочки - ОДИН раз для кредитной линии */
      DO:
         ...
         DO:
            /* признак наличия просрочки по договору (любой даже по внебалансу) */
            FOR FIRST bbloan where bbloan.contract  EQ iContract
                               and bbloan.cont-code EQ entry(1,iContCode ," " ) NO-LOCK :
               IF  LN_GetParamsInteres (iContract,
                                        bbloan.Cont-Code,
                                        vTmpProcPar1,
                                        iDate) GT 0
         ...
         DO:
            FOR FIRST bbloan where bbloan.contract  EQ iContract
                               and bbloan.cont-code EQ iContCode NO-LOCK :
               IF  LN_GetParamsInteres (iContract,
                                        bbloan.Cont-Code,
                                        vTmpProcPar1,
                                        iDate) GT 0
         ...
         DO:
            ...
            DO:
               FOR EACH loan-int WHERE loan-int.contract  EQ iContract
                                   AND loan-int.cont-code EQ entry(1,iContCode ," ")
                                   AND loan-int.mdate     LE iDate
                                 NO-LOCK:
...
FUNCTION GetPercntLoanDR RETURNS DECIMAL
   ...
   FOR EACH bclass WHERE
      bclass.PROGRESS-CODE EQ loan.class-code
      NO-LOCK,
      FIRST xattr WHERE
         xattr.Class-Code EQ bclass.CLASS-CODE
         AND xattr.Xattr-Code EQ iDR
         NO-LOCK:
...
PROCEDURE GET_SUMM_BS2:
   ...
   NO-LOCK,
       ...
       EACH loan-acct WHERE
        ...
        AND NOT CAN-FIND(LAST bloan-acct WHERE
                              bloan-acct.contract  EQ loan.contract
   ...
   NO-LOCK BY loan-acct.since:
      ...
      oBalance  = oBalance + IF loan.currency EQ "" THEN sh-bal
...
PROCEDURE Soot:
   ...
   DO:
      ...
      DO:
         ...
         vAcct   = ENTRY(1, LN_GetAcctSurr (loan.contract, loan.cont-code, loan.open-date)).
         ...
         FOR EACH term-obl WHERE
                  (    term-obl.contract   EQ loan.contract
                              ...
                              AND term-obl.idnt       EQ 5
                              AND term-obl.end-date   GT loan-int.mdate
                              AND term-obl.fop-date   LE loan-int.mdate )
                          OR
                             (    term-obl.contract   EQ loan.contract
                              AND term-obl.cont-code  EQ ENTRY(1,loan.cont-code," ")
                              AND term-obl.idnt       EQ 5
            ...
            DO:
               ...
               DO:
                  ...
                  FOR EACH insurance WHERE
                           insurance.contract         EQ "СТРАХ"
                       AND insurance.parent-contract  EQ term-obl.contract
                       AND insurance.parent-cont-code EQ term-obl.cont-code
                  NO-LOCK,
                     FIRST signs WHERE
                           signs.file-name   EQ "loan"
                       AND signs.surrogate   EQ insurance.contract + "," +
                                                insurance.cont-code
                       AND signs.code        EQ "Страхобесп"
                       AND signs.xattr-value EQ vSurrTObl
                  NO-LOCK:
      ...
      DO:
         ...
         FOR EACH term-obl WHERE
                  (    term-obl.contract   EQ loan.contract
                   ...
                   AND term-obl.end-date   GT loan-int.mdate
                   AND term-obl.fop-date   LE loan-int.mdate )
               OR (    term-obl.contract   EQ loan.contract
                   AND term-obl.cont-code  EQ ENTRY(1,loan.cont-code," ")
                   AND term-obl.idnt       EQ 5
                   AND term-obl.end-date   EQ ?
                   AND term-obl.fop-date   LE loan-int.mdate )
...
PROCEDURE GetSummGarInRes:
   ...
   DO vI = 1 TO NUM-ENTRIES(vListOp):
      FOR FIRST chowhe WHERE
                chowhe.id-op EQ INT64(ENTRY(vI,vListOp))
      NO-LOCK:
   ...
   FOR EACH term-obl WHERE
        ...
        AND term-obl.cont-code EQ loan.cont-code
        AND term-obl.idnt      EQ 5
        AND term-obl.fop-date  LE vPrevDate
        AND (   term-obl.sop-date  EQ ?
             OR term-obl.sop-date  GT vPrevDate )
   NO-LOCK:
...
:
   ...
   FOR EACH loan WHERE
           (loan.cust-cat   EQ iCust-cat
        AND loan.cust-id    EQ iCust-id
        AND loan.contract   EQ "Кредит"
        AND loan.close-date GT iDate)
        OR
           (loan.cust-cat   EQ iCust-cat
        AND loan.cust-id    EQ iCust-id
        AND loan.contract   EQ "Кредит"
...
/* $LINTVSS ='*' */
/* $LINTUSER='pase' */
/* $LINTDATE='27/10/2016 20:22:46.921+04:00' */
/*prosignhodnRYrflPqQFo4Z8aqq2w*/
/* --- pp-pqres.p was humbly modified by (c)blodd converter v.1.10 on 2/17/2017 6:39am --- */
