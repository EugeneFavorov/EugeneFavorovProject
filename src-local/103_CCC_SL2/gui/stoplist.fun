   ...
   DO vI = 1  TO NUM-ENTRIES( iName, CHR(1)):
      ...
      IF {assigned vName1} THEN
         ...
         NO-LOCK:
            ...
            {stoplist.funi}   /*если закрыт - NEXT*/
...
PROCEDURE PrintOpSLRep.
   ...
   DO:
      ...
      PUT STREAM rep "┌──────┬────────────────┬──────────────┬──────────────────────────────┬─────────────────────────────┬──────────────────────────────┬───────────┬──────────────────────────────┬─────────────────┬────────────────────┬────────────┐" SKIP.
      PUT STREAM rep "│№ п/п │ Дата документа │ № документа  │          Примечание          │   Критерий                  │    Наименование критерия     │  № п/п из │      ФИО/Наименование        │      ИНН        │   Номер резолюции  │    Дата    │" SKIP.
...
PROCEDURE PrintClientSLRep.
   ...
   PUT STREAM rep SKIP(1).
   PUT STREAM rep "┌──────┬─────┬──────┬──────────────────────────────┬──────────────────────────────┬─────────────┬───────────────┬───────────┬──────────────────────────────┬─────────────────┬───────────────┬──────────────────────────────┬────────────────────┬────────────┐" SKIP.
   PUT STREAM rep "│№ п/п │ Тип │ Код  │     ФИО/Наименование         │          Примечание          │    ИНН      │ Серия и номер │ № п/п из  │      ФИО/Наименование        │      ИНН        │ Дата рождения │    Место рождения            │  Номер резолюции   │    Дата    │" SKIP.
...
FUNCTION ChkClientByStopList RETURN LOGICAL
  (iClass      AS CHAR,     
   ...
   IF vChkType EQ "*"  THEN
      RUN  Fill-ttSL ("ФИО-ИНН",vCustName + CHR(1) + iInn,vSLtype,INPUT-OUTPUT TABLE t-obj-sl).
