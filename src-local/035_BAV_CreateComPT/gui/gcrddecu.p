...
/* +++ gcrddecu.p was humbly modified by (c)blodd converter v.1.09 on 11/3/2016 9:33am +++ */
...
gen:
do transaction on error  undo gen,leave gen
...
/*------------------------------------------ создание балансового документа --*/
   find first op-entry where recid(op-entry) eq out-rid  no-lock no-error.
   ...
   IF {assigned op-entry.kau-cr} THEN DO:
      FIND FIRST op WHERE op.op EQ INT64(ENTRY(1,op-entry.kau-cr)) 
      NO-LOCK NO-ERROR.
      FIND FIRST kau WHERE kau.acct     EQ op-entry.acct-cr
   ...
   if {assigned op-entry.kau-db} THEN DO:
      FIND FIRST op WHERE op.op EQ INT64(ENTRY(1,op-entry.kau-db)) 
      NO-LOCK NO-ERROR.
      FIND FIRST kau WHERE kau.acct     EQ op-entry.acct-db
                       AND kau.currency EQ op-entry.currency
   ...
   if not available(op-template) then LEAVE gen.
   ...
   if avail op then do:
      ...
      IF tmp-sign ne "" THEN
         FIND FIRST kau-op WHERE kau-op.op EQ INT64(TMP-SIGN) NO-LOCK NO-ERROR.
      ELSE
...
PROCEDURE FILL-ACCT:
    ...
    FIND FIRST acct WHERE RECID(acct) EQ in-rec NO-LOCK NO-ERROR.
    FIND FIRST bal-acct OF acct NO-LOCK NO-ERROR.
    IF NOT AVAIL acct THEN RETURN.
    tmp-kau-id = IF   acct.kau-id NE ? AND acct.kau-id NE "" THEN acct.kau-id
                 ELSE IF bal-acct.kau-id NE ? AND bal-acct.kau-id NE "" THEN bal-acct.kau-id
...
*/
    END.
    if num-entries(pick-value) = 5 and entry(3, pick-value) = "Суммы" then do:
       find first oba where recid(oba) = INT64(ENTRY(2, pick-value)) 
       no-lock no-error.
...
PROCEDURE FILL-KAU:
    ...
    DEF BUFFER code FOR code.
    FIND FIRST kau WHERE RECID(kau) EQ in-rec NO-LOCK NO-ERROR.
