...
/* +++ chst(op.i was humbly modified by (c)blodd converter v.1.09 on 8/2/2016 1:53pm +++ */
...
&ENDIF
   ...
   IF vstatus BEGINS "А" THEN DO:
      ...
      IF GetXattrValue("op",STRING(op.op),"КодВидаПлатежа") EQ "Казначейство" THEN DO:
   ...
   FIND code WHERE code.class EQ "Статус"
   ...
   FOR EACH bcode WHERE
            bCODE.class   EQ "Визы" AND
            bCODE.parent  EQ "Визы"
            NO-LOCK:
   ...
   IF {assigned vVisaAll} THEN DO: 
      ...
      IF {assigned vProcContr} AND {assigned vVisaStat} AND (CAN-DO(GetCode("Статус", vStatus), vPrevStat)  OR vPrevStat EQ vVisaStat) THEN 
         hist: 
         DO:
         FOR EACH history WHERE
            ...
            IF vPos GT 0 THEN DO:
               ...
               IF vHistStat EQ vStatus THEN DO:   /* убедимся, что документ уже был в этом статусе */
   ...
   IF {assigned vProcContr} AND NOT vstatus BEGINS "А" THEN DO:

      FIND FIRST bCode WHERE
                 bCODE.class EQ "ПроцедурыКонтр"
      ...
      IF NOT {assigned vVisaStat} THEN DO:  /* Если статус для визирования не указан, 
         ...
         ELSE DO:   /* Группа процедур */
            ...
            FOR EACH grCode WHERE 
                     grCode.class  EQ "ПроцедурыКонтр"
      ...
      ELSE DO:   /* механизм виз */
         ...
         IF {assigned bCode.val} AND GetXattrValue("op",STRING(op.op),bCode.misc[2]) NE "Утверждена" THEN DO: /* Процедура */
         ...
         ELSE DO:
            ...
            FOR EACH grCode WHERE 
               ...
               IF vVisa NE "Утверждена" AND NOT (vVisa EQ "Требуется" AND op.op-status EQ bCode.misc[1]) THEN DO:
   ...
   IF NOT CAN-DO("no-method,no-proc",{&RETURN_VALUE}) AND pick-value NE "yes" THEN DO:
      &IF DEFINED(open-undo) NE 0 &THEN
         {&open-undo}.
      &ELSE
         UNDO, RETRY.
   ...
   RUN SetSysConf IN h_base("NoFmsDocChk","YES").
                        /* Проверка документа. */
   IF       code.misc[1]         EQ ""
      OR    TRIM(code.misc[1])   EQ "да"  THEN 
   DO:
      ...
      RUN Check-Op IN h_op (RECID(op),
                            ...
                            "{&OFcur-bal}",
                            "{&chkupd}",
                            IF vstatus EQ "А" THEN "Off-VO" ELSE "").
      ...
      IF v-xattr-cr-I = 2 THEN DO: /* если они есть (и требуют редактирования) */
           ...
           &IF DEFINED(xattr-undo) NE 0 &THEN
              ...
              IF    CAN-DO("1,2",pick-value)
                 ...
                 OR {&LAST_KEY} EQ KEYCODE("ESC") THEN DO:
                 {&xattr-undo}.
              END.
           &ELSE
              UNDO,LEAVE.
...
END.
      ...
      END.
      IF op-entry.op-status GE CHR(251) and op-entry.kau-db begins "Депоз" AND NUM-ENTRIES(op-entry.kau-db) GE 2 THEN DO:
   ...
   DO:
      DO vCount2 = 1 TO 3:
         ...
         IF vCodeMes NE 0 THEN PAUSE 6 MESSAGE "Передается сообщение в Золотую Корону.".
      END.
      IF {assigned vMes} THEN DO:
         ...
         RUN Fill-AlertSysMes IN h_tmess("","",1,STRING(vMes)).
