...
/* +++ card2inc.p was humbly modified by (c)blodd converter v.1.09 on 1/31/2017 9:23am +++ */
...
DO ON ENDKEY UNDO tit,RETURN
   ...
   IF {&RETURN_VALUE} EQ "ERROR" THEN UNDO tit,RETURN.
 ...
 END.
   ...
   IF NOT AVAIL bAcct THEN DO:
      RUN Fill-AlertSysMes IN h_tmess("","",-1,"Ошибка шаблона транзакции!").
   ...
   /* Плюс банк: пакетная обработка без вывода в/б документа на экран */
   RUN g-call2.p (OUTPUT flager,
                  in-op-date,
                  RECID(work-op),
                  RECID(op-templ),
                  OUTPUT out-rid).
