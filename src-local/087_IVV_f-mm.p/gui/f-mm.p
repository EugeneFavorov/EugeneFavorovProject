...
DEFINE TEMP-TABLE tt-commrate NO-UNDO LIKE comm-rate
       FIELD comm-rate-curr  AS CHARACTER /* Валюта тарифа */
       FIELD local__template AS LOGICAL   /* Признак шаблон/не шаблон */
       FIELD local__rowid    AS ROWID     /* ROWID записи в БД        */
       FIELD local__id       AS INT64   /* Идентификатор записи     */
       FIELD local__upid     AS INT64   /* Ссылка на запись в аггрегирующей таблице */
...
DEFINE TEMP-TABLE tt-loan NO-UNDO LIKE loan
       ...
       FIELD poslpdzapo$ AS LOG /* ДР ПослПдЗаПо */
       FIELD local__template AS LOGICAL   /* Признак шаблон/не шаблон */
       FIELD local__rowid    AS ROWID     /* ROWID записи в БД        */
       FIELD local__id       AS INT64   /* Идентификатор записи     */
       FIELD local__upid     AS INT64   /* Ссылка на запись в аггрегирующей таблице */
...
DEFINE TEMP-TABLE tt-loan-cond NO-UNDO LIKE loan-cond
       ...
       FIELD bgminsrok$ AS INT64 /* ДР БГМинСрок БГ:Контр.срока: Минимальный срок */
       FIELD bgmakssrok$ AS INT64 /* ДР БГМаксСрок БГ:Контр.срока: Максимальный срок */
       FIELD bgsrokr$ AS CHAR /* ДР БГСрокР БГ:Размерность срока действ. гар. */
       FIELD bggarstop$ AS DATE /* ДР БГГарСтоп БГ:Дата окончания выдачи гарантий */
       FIELD bgvalwutymbg$ AS CHAR /* ДР БГВалютыМБГ БГ:Перечень одобренных валют для МБГ */
       FIELD bgspisbezakc$ AS LOG /* ДР БГСписБезАкц БГ:Признак безакцепт. списания комиссии */
       FIELD local__template AS LOGICAL   /* Признак шаблон/не шаблон */
       FIELD local__rowid    AS ROWID     /* ROWID записи в БД        */
       FIELD local__id       AS INT64   /* Идентификатор записи     */
       FIELD local__upid     AS INT64   /* Ссылка на запись в аггрегирующей таблице */
...
DEFINE VARIABLE mDayTr       AS INT64      NO-UNDO.
DEFINE VARIABLE mErrSaveHist AS LOGICAL    NO-UNDO. /* Признак ошибки при сохранении условий*/
DEFINE VARIABLE mChangeMCG   AS LOGICAL    NO-UNDO.
DEFINE VARIABLE mOldgend-date AS DATE      NO-UNDO INIT ?.
   /* Индикатор того, что по стоп-листам выбрали ответ Yes.
      Чтобы не спрашивать у пользователя по два раза одно и тоже. */
DEFINE VARIABLE mStLPressYes AS LOG NO-UNDO INIT ?. 
DEFINE VARIABLE mInStopList  AS LOG NO-UNDO INIT ?.
...
/* ivv begin */
DEFINE VARIABLE str_s  AS char   NO-UNDO .
DEFINE VARIABLE bSrok  AS int64   NO-UNDO .
...
&Scoped-define FIELDS-IN-QUERY-br-comm tt-comm-rate.commission /* tt-comm-rate.local__id */ tt-comm-rate.rate-fixed ENTRY(1, GetBufferValue( "commission", "WHERE commission.commission = '" + tt-comm-rate.commission + "'", "name-comm"),CHR(1)) @ mNameCommi tt-comm-rate.rate-comm tt-comm-rate.min_value tt-comm-cond.commission tt-comm-cond.since tt-comm-rate.comm-rate-curr   
...
&Scoped-define QUERY-STRING-br-comm PRESELECT EACH tt-comm-rate WHERE tt-comm-rate.since =  tt-loan-cond.since SHARE-LOCK, LAST tt-comm-cond OUTER-JOIN WHERE tt-comm-cond.commission =  tt-comm-rate.commission AND tt-comm-cond.since <= tt-loan-cond.since SHARE-LOCK BY tt-comm-rate.since INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-comm OPEN QUERY {&SELF-NAME} PRESELECT EACH tt-comm-rate WHERE tt-comm-rate.since =  tt-loan-cond.since SHARE-LOCK, LAST tt-comm-cond OUTER-JOIN WHERE tt-comm-cond.commission =  tt-comm-rate.commission AND tt-comm-cond.since <= tt-loan-cond.since SHARE-LOCK BY tt-comm-rate.commission INDEXED-REPOSITION.
...
tt-loan-cond.isklmes$ tt-loan-cond.poslpd$
...
tt-loan.comment tt-loan-cond.poslpd$ ~
tt-loan-cond.bgminsrok$ tt-loan-cond.bgmakssrok$ tt-loan-cond.bgsrokr$ ~
tt-loan-cond.bggarstop$ tt-loan-cond.bgvalwutymbg$ tt-loan-cond.bgspisbezakc$
...
tt-loan-cond.annuitkorr$ mBag tt-loan-cond.grperiod$ tt-loan-cond.poslpd$ ~
tt-loan-cond.bgminsrok$ tt-loan-cond.bgmakssrok$ tt-loan-cond.bgsrokr$ ~
tt-loan-cond.bggarstop$ tt-loan-cond.bgvalwutymbg$ tt-loan-cond.bgspisbezakc$
...
mLimit  mLimitRest tt-loan-cond.annuitkorr$tt-loan-cond.poslpd$
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD CheckCli TERMINAL-SIMULATION
FUNCTION CheckCli RETURNS LOGICAL
...
DEFINE FRAME fMain
   tt-loan-cond.bgminsrok$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 10 COLON-ALIGNED
      &ELSE AT ROW 6 COL 10 COLON-ALIGNED &ENDIF
      LABEL "Мин.срок"
      VIEW-AS FILL-IN
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 4 BY 1
      &ELSE SIZE 4 BY 1 &ENDIF
      FORMAT ">>>9" 
   tt-loan-cond.bgmakssrok$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 26 COLON-ALIGNED
      &ELSE AT ROW 6 COL 26 COLON-ALIGNED &ENDIF
      LABEL "Макс.срок"
      VIEW-AS FILL-IN
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 4 BY 1
      &ELSE SIZE 4 BY 1 &ENDIF
      FORMAT ">>>9"
   tt-loan-cond.bgsrokr$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 32 COLON-ALIGNED
      &ELSE AT ROW 6 COL 32 COLON-ALIGNED &ENDIF
&IF DEFINED(MANUAL-REMOTE) &THEN
      VIEW-AS COMBO-BOX INNER-LINES 3 DROP-DOWN-LIST  
&ELSE
      NO-LABEL FORMAT "Д/М"  
...
&ENDIF
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 3 BY 1
      &ELSE SIZE 3 BY 1 &ENDIF
   tt-loan-cond.bggarstop$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 42 COLON-ALIGNED
      &ELSE AT ROW 6 COL 42 COLON-ALIGNED &ENDIF
      LABEL "Стоп"
      VIEW-AS FILL-IN
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 10 BY 1
      &ELSE SIZE 10 BY 1 &ENDIF
      FORMAT "99/99/9999"
   tt-loan-cond.bgvalwutymbg$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 62 COLON-ALIGNED
      &ELSE AT ROW 6 COL 62 COLON-ALIGNED &ENDIF
      LABEL "ВалГ"
      VIEW-AS FILL-IN
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 14 BY 1
      &ELSE SIZE 14 BY 1 &ENDIF
      FORMAT "x(14)"
   tt-loan-cond.bgspisbezakc$
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 7 COL 10 COLON-ALIGNED
      &ELSE AT ROW 7 COL 10 COLON-ALIGNED &ENDIF
      LABEL "БезАкц"
      VIEW-AS FILL-IN /*TOGGLE-BOX*/
      &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 20 BY 1
      &ELSE SIZE 20 BY 1 &ENDIF
      FORMAT "Да/Нет"
   WITH 1 DOWN KEEP-TAB-ORDER OVERLAY
...
DEFINE FRAME fMain
     ...
     mLimit
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 16 COL 61 COLON-ALIGNED
     ...
     mLimitRest
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 17 COL 61 COLON-ALIGNED
...
DEFINE FRAME fMain
     ...
     tt-loan-cond.poslpd$
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 19 COL 59
          &ELSE AT ROW 19 COL 59 &ENDIF HELP
...
DEFINE VARIABLE vRez AS LOGICAL NO-UNDO .
  ...
  IF tt-loan.rewzim$:SCREEN-VALUE  <> ? AND
     tt-loan.rewzim$:SCREEN-VALUE  <> ""  THEN DO:
...
&Scoped-define SELF-NAME mLimitGraf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL mLimitGraf TERMINAL-SIMULATION
ON F2 OF FRAME fMain ANYWHERE
DO:
   str_s = "".
   DEFINE VARIABLE vRez AS LOGICAL NO-UNDO .
/* Первое условие */
   for each loan-cond WHERE loan-cond.contract  EQ tt-loan.Contract
                        AND loan-cond.cont-code EQ tt-loan.Cont-Code
   NO-LOCK.

      str_s = str_s  + "Ограничения по договору действующие с " + string(loan-cond.since).
      tDate = loan-cond.since.
      FIND LAST comm-rate  WHERE  comm-rate.commission EQ "МинДопВ"
                            ...
                            AND  comm-rate.kau       eq tt-loan.Contract   + "," + tt-loan.Cont-Code 
                            AND comm-rate.min-value  EQ 0
                            AND comm-rate.period     EQ 0
                            AND comm-rate.since      LE tDate
                            no-lock no-error.
      
      if avail comm-rate then do:
         IF tt-loan.currency EQ "" THEN do:
            str_s = str_s + chr(10) + " Cумма взноса должна быть не менее " + STRING(comm-rate.rate-comm).
         end.
         ELSE do:
            str_s = str_s + chr(10) + " Cумма взноса должна быть не менее " + STRING(comm-rate.rate-comm) + "(" + tt-loan.currency + ")".
         END.
      end.
      ...
      if avail comm-rate then do:
         IF tt-loan.currency EQ "" THEN do:
            str_s = str_s + chr(10) + " Минимальная сумма снятия должна быть не менее " + STRING(comm-rate.rate-comm).
         end.
         ELSE do:
            str_s = str_s + chr(10) + " Минимальная сумма снятия должна быть не менее " + STRING(comm-rate.rate-comm) + "(" + tt-loan.currency + ")".
         END.
      end.
      ...
      if avail comm-rate then do:
         IF tt-loan.currency EQ "" THEN do:
            str_s = str_s + chr(10) + " Неснижаемый остаток должен быть не менее " + STRING(comm-rate.rate-comm).
         end.
         ELSE do:
            str_s = str_s + chr(10) + " Неснижаемый остаток должен быть не менее " + STRING(comm-rate.rate-comm) + "(" + tt-loan.currency + ")".
         END.
      end.
      FIND LAST comm-rate  WHERE  comm-rate.commission EQ "ПополНеПоздней"
                            AND comm-rate.acct       EQ "0"
                            AND comm-rate.currency   EQ tt-loan.currency 
                            AND  comm-rate.kau       eq tt-loan.Contract   + "," + tt-loan.Cont-Code 
                            AND comm-rate.min-value  EQ 0
      ...
      if avail comm-rate then do:
         bSrok = tt-loan.end-date - tt-loan.open-date.
         bSrok = int(bSrok * comm-rate.rate-comm).
         iDate = tt-loan.open-date + bSrok.
         str_s = str_s + chr(10) + " Дополнительные взносы по договору разрешены до " + STRING(idate).
      end.
      str_s = str_s + chr(10) + chr(10).
   end.
/*    Конец обработки первого условия */
/* Второе условие */

/*
   FIND LAST loan-cond WHERE loan-cond.contract  EQ tt-loan.Contract
                         AND loan-cond.cont-code EQ tt-loan.Cont-Code
   NO-LOCK NO-ERROR.

   IF tDate <> loan-cond.since then do:
      tDate = loan-cond.since.
      str_s = str_s  + chr(10) + chr(10) + "Ограничения по договору действующие с " + string(loan-cond.since).
      FIND LAST comm-rate  WHERE  comm-rate.commission EQ "МинДопВ"
                            AND comm-rate.acct       EQ "0"
                            AND comm-rate.currency   EQ tt-loan.currency 
                            AND  comm-rate.kau       eq tt-loan.Contract   + "," + tt-loan.Cont-Code 
  ...
  */
/* конец обработки второго условия */



   Message str_s view-as alert-box.
END.
...
DO:
   ...
   DO:
      ...
      DO:
         ...
         IF AVAIL acct THEN
         DO:
            IF acct.branch-id <> tt-loan.branch-id:SCREEN-VALUE
            THEN DO:
               RUN Fill-SysMes IN h_tmess (
                  "", "", "4",
                  "Код подразделения договора не совпадает~nс кодом подразделения " +
                  "выбранного счета.~nПродолжить?"
               ).
               IF pick-value <> "YES"
               THEN DO:
                  RUN Fill-SysMes IN h_tmess (
         ...
         ** на аналитику проверяется роль счета */
         ASSIGN
            ...
            tt-loan-acct-main.acct-type = "Кредит"   WHEN tt-loan-acct-main.acct:SCREEN-VALUE <> "?"
            tt-loan-acct-cust.acct-type = "КредРасч" WHEN tt-loan-acct-cust.acct:SCREEN-VALUE <> "?"
         .
      END.
      ELSE
         ASSIGN
            tt-loan-acct-main.acct-type = "" WHEN tt-loan-acct-main.acct:SCREEN-VALUE =  "?"
            tt-loan-acct-cust.acct-type = "" WHEN tt-loan-acct-cust.acct:SCREEN-VALUE =  "?"
         .
   END.
   {&END_BT_LEAVE}
END.
...
DO:
   IF SELF:INPUT-VALUE <> 0 THEN ASSIGN
...
DO:
   IF NOT (   tt-loan.rewzim$:SCREEN-VALUE =  "ЛимВыдЗад"
           OR tt-loan.rewzim$:SCREEN-VALUE =  "ВозобнЛиния"
           OR tt-loan.rewzim$:SCREEN-VALUE =  "НевозЛиния") THEN
      ASSIGN
...
DO:
   ...
   DO:
      IF CAN-FIND(FIRST b-comm-rate WHERE
                  b-comm-rate.commission =  pick-value AND
                  b-comm-rate.since      =  tt-loan-cond.since )  THEN
      DO:
         MESSAGE
            COLOR MESSAGES
   ...
   FIND FIRST tt-comm-rate WHERE tt-comm-rate.since =  tt-loan-cond.since
   ...
   FIND FIRST tt-comm-cond WHERE tt-comm-cond.since =  tt-loan-cond.since
   ...
   IF NOT CAN-DO(mKomisNacVal,tt-comm-rate.commission) THEN 
      tt-comm-rate.comm-rate-curr = tt-loan.currency:SCREEN-VALUE.
   ...
   FIND FIRST commission WHERE commission.commission =    tt-comm-rate.commission
                           AND commission.currency   =    tt-loan.currency:SCREEN-VALUE NO-LOCK NO-ERROR.
   ...
   DO:
      DISP
         ...
         WITH BROWSE br-comm.
      IF tt-comm-rate.commission =  "" THEN
         DISP
            "" @ mNameCommi
            WITH BROWSE br-comm.
   END.
...
ON VALUE-CHANGED OF tt-comm-rate.rate-comm IN BROWSE br-comm DO:
   IF tt-comm-rate.commission =  "%ВыдМБГ" THEN
   DO:
      FIND FIRST ttRateCondMCGTransh WHERE
                 ttRateCondMCGTransh.fCommission =  tt-comm-rate.commission
      EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
   
      IF NOT AVAIL ttRateCondMCGTransh THEN
      DO:
         CREATE ttRateCondMCGTransh.
         ASSIGN
            ttRateCondMCGTransh.fCommission  = tt-comm-rate.commission
         .
      END.
      
      ttRateCondMCGTransh.fPermMinRate = 
         DEC(tt-comm-rate.min_value:screen-value IN 
                BROWSE br-comm ) NO-ERROR .
   END.
END.
...
ON LEAVE OF tt-comm-rate.min_value IN BROWSE br-comm DO:
 tt-comm-rate.min_value = DEC (tt-comm-rate.min_value:screen-value IN BROWSE br-comm ) NO-ERROR .
   ...
   DO:
      ...
      ttRateCondMCGTransh.fPermMinRate = tt-comm-rate.min_value.
   END.
END.
...
ON VALUE-CHANGED OF tt-comm-rate.rate-comm IN BROWSE br-comm DO:
   ...
   DO:
      ...
      ttRateCondMCGTransh.fPermRate = 
         DEC(tt-comm-rate.rate-comm:screen-value IN 
                BROWSE br-comm ) NO-ERROR .
   END.
END.

ON LEAVE OF tt-comm-rate.rate-comm IN BROWSE br-comm DO:
   IF tt-comm-rate.rate-comm:screen-value IN BROWSE br-comm  =  "?" THEN 
   ASSIGN 
      tt-comm-rate.rate-comm:screen-value IN BROWSE br-comm  = "0"
      tt-comm-rate.rate-comm  = 0
   . 
   ...
   DO:
      ...
      ttRateCondMCGTransh.fPermRate = tt-comm-rate.rate-comm.
   END.
END.
...
ON LEAVE OF tt-comm-rate.rate-fixed IN BROWSE br-comm  
DO:
   IF (iMode =  {&MOD_ADD} 
   OR iMode =  {&MOD_EDIT})
   AND (tt-comm-rate.rate-fixed:SCREEN-VALUE IN BROWSE br-comm = "%") THEN
   DO:
      ASSIGN
         tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = 
            tt-loan.currency:SCREEN-VALUE IN FRAME fMain
         tt-comm-rate.comm-rate-curr = tt-loan.currency:SCREEN-VALUE IN FRAME fMain
      .
...
&Scoped-define SELF-NAME br-comm
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br-comm TERMINAL-SIMULATION
ON CTRL-F4 OF br-comm IN FRAME fMain
ANYWHERE
DO:
   DEFINE VARIABLE vOk AS LOGICAL NO-UNDO. 
   DEF BUFFER DataBlock FOR DataBlock.
    
   IF     iMode =  {&MOD_ADD}
      AND tt-comm-rate.commission =  "%ВыдМБГ" THEN
   DO:
      IF     tt-loan.class-code =  "loan-garmc-tr-sim" THEN
      DO:
         RUN f-cratmcg.p (tt-comm-rate.commission,
                          INPUT-OUTPUT TABLE ttRateCondMCGTransh,
                          br-comm:ROW,
                          OUTPUT vOk). 
         ...
         ASSIGN
            tt-comm-rate.rate-comm:SCREEN-VALUE IN BROWSE br-comm = 
               STRING(ttRateCondMCGTransh.fPermRate)
            tt-comm-rate.min_value:SCREEN-VALUE IN BROWSE br-comm = 
               STRING(ttRateCondMCGTransh.fPermMinRate)
            tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = 
               ttRateCondMCGTransh.fCurrency
         . 
      END.
      ELSE
      IF tt-loan.class-code =  "loan-garmc-agr-sim" THEN
      DO:
         FIND LAST DataBlock WHERE
                   DataBlock.Branch-Id    =  ""
               AND DataBlock.DataClass-Id =  "МБГ"
               AND DataBlock.Beg-Date     <= gend-date
               AND DataBlock.End-Date     <= gend-date
         NO-LOCK NO-ERROR.

         DO TRANSACTION:
            RUN browseld.p ("IndicationLine",
                            SUBSTITUTE("&1&2&3",
                                       "Data-Id",
                                       CHR(1),
                                       "cont-code"),
                            STRING(DataBlock.Data-Id) + CHR(1) + tt-loan.cont-code,
                            SUBSTITUTE("&1&2&3",
                                       "Data-Id",
                                       CHR(1),
                                       "cont-code"),
...
DO:
   ...
   DO:
      ...
      ** на договоре. Поэтому расчитывать из ставок договора смысла нет */
      IF tt-comm-cond.source =  "Общие ставки" THEN
      DO:
         RUN CalcFloatRate (tt-loan.contract,
                            tt-loan.cont-code,
                            tt-comm-cond.source,
                            tt-comm-cond.BaseCode,
                            tt-comm-rate.comm-rate-curr, /* Код валюты базовой комиссии */
                            DATE(tt-loan.open-date:SCREEN-VALUE),
                            tt-comm-cond.action,
                            tt-comm-cond.BaseChange,
                            tt-comm-cond.firstdelay,
                            tt-comm-cond.NegBaseRate <> "Нет",
                            OUTPUT vDatePS,
                            OUTPUT vBaseRatePS,
                            OUTPUT vRatePS).
         IF vRatePS <> ? AND vRatePS <> 0 THEN
         DO:
            RUN GET_COMM_BUF (tt-comm-cond.BaseCode, /* Код комиссии. */
                     ?,                              /* Идентификатор счета. */
                     tt-comm-rate.comm-rate-curr,               /* Код валюты. */
            ...
            IF AVAIL t-comm-rate 
               AND t-comm-rate.since <> vDatePS
            THEN DO:
               RUN Fill-SysMes IN h_tmess ("", "", "0", 
              "На дату опердня " + STRING(vDatePS) + " не найдено значение ставки по коду " +
              STRING(tt-comm-cond.BaseCode) + " !~n Последнее изменение ставки " + STRING(t-comm-rate.since) + "." ).
            END.

            FIND FIRST tt-comm-rate WHERE tt-comm-rate.commission =  tt-comm-cond.commission
                                      AND tt-comm-rate.since      =  tt-comm-cond.since
            NO-ERROR.
...
DO:
   ...
   IF RETURN-VALUE = {&RET-ERROR} THEN
      RETURN NO-APPLY.
   IF FOCUS:NAME =  "rate-comm" THEN
   DO:
      vRow = ROWID(tt-comm-rate).
      APPLY "CURSOR-DOWN" TO br-comm.
      IF vRow =  ROWID(tt-comm-rate) THEN
         APPLY "ENTRY" TO vH.
   END.
   ELSE IF FOCUS:NAME =  "min_value" THEN
   DO:
      vRow = ROWID(tt-comm-rate).
      APPLY "CURSOR-DOWN" TO br-comm.
      IF vRow =  ROWID(tt-comm-rate)
         THEN APPLY "TAB"        TO SELF.
         ELSE APPLY "BACK-TAB"  TO SELF.
   END.
   ELSE IF FOCUS:NAME =  "comm-rate-curr" THEN
   DO:
      vRow = ROWID(tt-comm-rate).
      APPLY "CURSOR-DOWN" TO br-comm.
      IF vRow =  ROWID(tt-comm-rate) 
...
DO:
   ...
   IF SELF:NAME =  "br-comm" THEN
   DO:
      vLoanCurr = tt-loan.currency:SCREEN-VALUE.
      IF iMode =  {&MOD_ADD} THEN
      DO:
         RUN CopySinceTT IN THIS-PROCEDURE.
         FOR EACH tt-comm-rate:
            IF    (   {assigned vLoanCurr}
                   OR tt-comm-rate.comm-rate-curr <> vLoanCurr)
              AND NOT mIsProcStCh
              AND tt-loan.class-code <> "loan-garmc-tr-sim"
            THEN
               IF NOT CAN-DO(mKomisNacVal,tt-comm-rate.commission) THEN 
               ASSIGN
                  tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = 
                     vLoanCurr
                  tt-comm-rate.comm-rate-curr = vLoanCurr
               .
               ELSE 
               ASSIGN
                  tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = ""
      ...
      RUN PutHelp(
         "F1" + (IF iMode =  {&MOD_ADD} THEN "│Ctrl-F2 - Добавить│Ctrl-F3 - Удалить│Ctrl-F9 - Редактировать" ELSE (IF mSwitchF9 AND iMode =  {&MOD_VIEW} THEN "│F9" ELSE "")),
         FRAME {&MAIN-FRAME}:HANDLE).
   END.
   IF     iMode = {&MOD_VIEW}
      AND tt-loan.class-code <> "loan-garmc-tr-sim" THEN
      FOR EACH tt-comm-rate:
         IF tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm =  "?" THEN
            tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = "".
      END.
END.
...
DO:
   DEF VAR vLoanCurr     AS CHAR NO-UNDO.
   IF SELF:NAME =  "comm-rate-curr" THEN
   DO:
      vLoanCurr = tt-loan.currency:SCREEN-VALUE.
      IF {assigned vLoanCurr} THEN
      DO:
         IF SELF:SCREEN-VALUE =  vLoanCurr THEN
            SELF:SCREEN-VALUE = "".
         ELSE
...
DO:
   DEF VAR vLoanCurr     AS CHAR NO-UNDO.
   DEF VAR vCommRateCurr AS CHAR NO-UNDO.
   ASSIGN
      vLoanCurr     = tt-loan.currency:SCREEN-VALUE
      vCommRateCurr = tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm
   .
   IF     tt-loan.Class-Code <> "loan-garmc-tr-sim"
      AND {assigned vLoanCurr} 
      AND {assigned vCommRateCurr}
      AND vCommRateCurr <> vLoanCurr THEN
   DO:
      RUN Fill-SysMes IN h_tmess ("","","0",
         "Для договоров в валюте комиссии могут" +
         " быть либо в валюте договора, либо в нац.валюте.").
      APPLY 'ENTRY' TO tt-comm-rate.comm-rate-curr IN BROWSE br-comm.
      RETURN NO-APPLY.
   END.
   IF CAN-DO(mKomisNacVal,tt-comm-rate.commission)
      AND {assigned vCommRateCurr} THEN
   DO:
      IF LASTKEY <> KEYCODE("ESC") THEN
      DO:
         IF FOCUS:NAME =  "comm-rate-curr" OR LASTKEY <> KEYCODE("ENTER") THEN 
            RUN Fill-SysMes IN h_tmess ("","","0",
      ...
      END.
   END.
   /* Пока только индивидуальные комиссии у нас могут быть в другой валюте */
   DO mI = 1 TO EXTENT(lrate):

      IF     lrate[mI] =  tt-comm-rate.commission
         AND tt-comm-rate.comm-rate-curr:SCREEN-VALUE <> 
             tt-loan.currency:SCREEN-VALUE
         AND NOT CAN-DO(mKomisNacVal,tt-comm-rate.commission)
      THEN
      DO:
         RUN Fill-SysMes IN h_tmess ("","","0",
            "Только по индивидуальным комиссиям" +
            " валюта может отличаться от валюты договора.").
         APPLY 'ENTRY' TO tt-comm-rate.comm-rate-curr IN BROWSE br-comm.
         RETURN NO-APPLY.
      END.
   END.
...
DO:                                                             
   tt-comm-rate.comm-rate-curr:READ-ONLY IN BROWSE br-comm 
      = tt-comm-rate.rate-fixed:SCREEN-VALUE IN BROWSE br-comm =  "%".
...
DO:
   ...
   IF iMode <> {&MOD_VIEW} AND
      ...
      SELF:SCREEN-VALUE =  "Течение" THEN
   DO:
...
&Scoped-define SELF-NAME tt-loan.cont-type
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-loan.cont-type TERMINAL-SIMULATION
ON "SHIFT-F3" OF tt-loan.cont-type IN FRAME fMain /* Тип */
DO:
   DEF VAR vDate AS DATE NO-UNDO.
    
   IF    iMode = {&MOD_ADD}
      OR iMode = {&MOD_VIEW} THEN RETURN.
      
   ASSIGN
      vDate     = gend-date
      gend-date = tt-loan.since 
   . 
   APPLY "SHIFT-F3" TO FRAME {&MAIN-FRAME}.  
...
DO:
  {&BEG_BT_LEAVE}
  IF  ( tt-loan-cond.cred-period:INPUT-VALUE =  "К"
    AND tt-loan-cond.cred-month:INPUT-VALUE >  3
    OR  tt-loan-cond.cred-period:INPUT-VALUE =  "ПГ"
    AND tt-loan-cond.cred-month:INPUT-VALUE >  6
    OR  tt-loan-cond.cred-period:INPUT-VALUE =  "Г"
    AND tt-loan-cond.cred-month:INPUT-VALUE >  12 )
    OR ((tt-loan-cond.cred-period:INPUT-VALUE =  "К"
    OR   tt-loan-cond.cred-period:INPUT-VALUE =  "ПГ"
    OR   tt-loan-cond.cred-period:INPUT-VALUE =  "Г")
    AND  tt-loan-cond.cred-month:INPUT-VALUE  <= 0) THEN
  DO:
...
DO:
   ...
   DO:
      ...
      vi = vi + 1.
      IF vi >  NUM-ENTRIES(mOffsetVld) THEN vi = 1.
...
DO:
   ...
   DO:
      ...
      ASSIGN
        ...
        tt-loan-cond.cred-offset:SCREEN-VALUE = IF SELF:SCREEN-VALUE =  mOffsetNone
                                           THEN {&CB-NONE}
                                           ELSE SELF:SCREEN-VALUE
        tt-loan-cond.cred-offset              = IF SELF:SCREEN-VALUE =  mOffsetNone
                                           THEN ""
                                           ELSE SELF:SCREEN-VALUE.
      {&BT_LEAVE}
      mChangedField = IF mHandCalcAnnuitet THEN NO ELSE mChangedField.
...
DO:
   /* записываем правильное значение в "настоящее" поле ({&CB-NONE} вместо "--") */
   ASSIGN
      tt-loan-cond.int-offset:SCREEN-VALUE = IF SELF:SCREEN-VALUE =  mOffsetNone
                                             THEN {&CB-NONE}
                                             ELSE SELF:SCREEN-VALUE
      tt-loan-cond.int-offset              = IF SELF:SCREEN-VALUE =  mOffsetNone
                                             THEN ""
                                             ELSE SELF:SCREEN-VALUE.
END.
...
DO:
   /* записываем правильное значение в "настоящее" поле ({&CB-NONE} вместо "--") */
   ASSIGN
      tt-loan-cond.delay-offset:SCREEN-VALUE = IF SELF:SCREEN-VALUE =  mOffsetNone
                                               THEN {&CB-NONE}
                                               ELSE SELF:SCREEN-VALUE
      tt-loan-cond.delay-offset              = IF SELF:SCREEN-VALUE =  mOffsetNone
                                               THEN ""
                                               ELSE SELF:SCREEN-VALUE.
END.
...
DO:
   /* записываем правильное значение в "настоящее" поле ({&CB-NONE} вместо "--") */
   ASSIGN
      tt-loan-cond.delay-offset-int:SCREEN-VALUE = IF SELF:SCREEN-VALUE =  mOffsetNone
                                                   THEN {&CB-NONE}
                                                   ELSE SELF:SCREEN-VALUE
      tt-loan-cond.delay-offset-int              = IF SELF:SCREEN-VALUE =  mOffsetNone
                                                   THEN ""
                                                   ELSE SELF:SCREEN-VALUE.
END.
...
DO:
   ...
   DEF VAR vCurrList AS CHAR NO-UNDO.
   ...
   DO:
      FIND LAST loan-cond WHERE
                loan-cond.contract  =  tt-instance.contract
            AND loan-cond.cont-code =  tt-instance.cont-code
            AND loan-cond.since     <= DATE(tt-loan.datasogl$:SCREEN-VALUE)
      NO-LOCK NO-ERROR.
      
      ASSIGN   
         vCurrList = GetXattrValueEx("loan-cond",
                                     SUBSTITUTE("&1,&2,&3",
                                                tt-instance.contract,
                                                tt-instance.cont-code,
                                                STRING(loan-cond.since)),
                                     "БГВалютыМБГ",
                                     "")
      .
      
      IF NOT CAN-DO(vCurrList, tt-loan.currency:SCREEN-VALUE) THEN
      DO:
         RUN Fill-SysMes IN h_tmess ("", "", "0", 
            "Валюта гарантии не соответствует допустимым валютам " + 
            vCurrList).
         RETURN NO-APPLY {&RET-ERROR}.  
      END.
   END.
   ...
   DO:
      ...
      IF RETURN-VALUE =  {&RET-ERROR}
         THEN RETURN NO-APPLY.
      IF LASTKEY = KEYCODE("ESC") OR iMode =  {&MOD_VIEW} THEN RETURN.
      ...
      tt-comm-rate.comm-rate-curr:READ-ONLY IN BROWSE br-comm = tt-loan.currency:SCREEN-VALUE =  "".
   END.
  {&END_BT_LEAVE}
END.
...
, tt-broker.cust-id
DO:
   IF tt-loan.alt-contract =  "mm" AND
      iMode <> {&MOD_VIEW} THEN
   DO:
      {f-fx.trg
         &F1-BROKER = YES
...
DO:
   ...
   IF FGetSetting("CheckCli", "", "") =  "YES" THEN
      IF CheckCli(tt-loan.cust-cat:SCREEN-VALUE,INT64(tt-loan.cust-id:SCREEN-VALUE),TODAY) THEN
         RUN Fill-SysMes IN h_tmess ("", "", "", 
                                     " Данный клиент является " +
                                     "поручителем по просроченному кредитному договору.").
   ...
   CASE mStLPressYes:
      WHEN ? THEN
      DO:
         /* Проверка клиента на стоп-листы */ 
         RUN ChkByStopList IN h_wchck (tt-loan.cust-cat:SCREEN-VALUE,
                                       INT64(tt-loan.cust-id:SCREEN-VALUE),
                                       "КонтрВводРед_КД",
                                       "CLNT",
                                       OUTPUT mInStopList).
         IF mInStopList THEN
         DO:
            ...
            RUN Fill-SysMes IN h_tmess ("","","4", 
                "Клиент является фигурантом справочника Стоп-листы!" + 
                " Продолжить ввод договора?").
            
            mStLPressYes = (pick-value = "YES").
      ...
      DO:
         RUN Fill-SysMes IN h_tmess ("","","1", 
             "Клиент является фигурантом справочника Стоп-листы!" + 
             " Выбрано - не продолжать ввод договора.").        
         RETURN NO-APPLY {&RET-ERROR}.
      END.     
   END CASE.
...
&Scoped-define SELF-NAME tt-loan.cust-id
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-loan.cust-id TERMINAL-SIMULATION
ON VALUE-CHANGED OF tt-loan.cust-id IN FRAME fMain /* Код */
DO:
   /* Если выбрали другого клиента, то сбрасываем флаг */  
   IF mStLPressYes <> ? THEN 
      mStLPressYes = ?.
END.
...
DO:
   DEF VAR vGarStop  AS DATE NO-UNDO. 
   DEF VAR vDataSogl AS CHAR NO-UNDO.
   DEF BUFFER loan-cond FOR loan-cond.
   ...
   DO:
      ...
      vDataSogl = tt-loan.datasogl$:SCREEN-VALUE.
      IF     iMode =  {&MOD_ADD}
         AND tt-loan.class-code =  "loan-garmc-tr-sim"
         AND {assigned vDataSogl} THEN
      DO:
         FIND LAST loan-cond WHERE
                   loan-cond.contract  =  tt-instance.contract
               AND loan-cond.cont-code =  tt-instance.cont-code
         ...
         ASSIGN   
            vGarStop = DATE(GetXattrValueEx("loan-cond",
                                            SUBSTITUTE("&1,&2,&3",
                                                       tt-instance.contract,
                                                       tt-instance.cont-code,
                                                       STRING(loan-cond.since)),
         ...
         IF DATE(tt-loan.datasogl$:SCREEN-VALUE) >  vGarStop THEN
         DO:
            RUN Fill-SysMes IN h_tmess ("", "", "0", 
               SUBSTITUTE("&1 &2 &3 &4",
                          "Выдача гарантии невозможна, так как",
                          "превышена дата окончания выдач!",
                          "См. реквизит 'БГГарСтоп' на условиях",
                          "соглашения на дату заключения транша.")).
            RETURN NO-APPLY {&RET-ERROR}. 
         END.
      END.
   END.
...
DO:
   ...
   IF INT64(SELF:SCREEN-VALUE) >  31 THEN SELF:SCREEN-VALUE = "31".
   IF INT64(SELF:SCREEN-VALUE) <  1  THEN SELF:SCREEN-VALUE = "1".
   ...
   ** больше номера дня начала платежного периода */
   IF     tt-loan-cond.cred-date:VISIBLE
      AND tt-loan-cond.cred-curr-next:SCREEN-VALUE =  ENTRY(1,tt-loan-cond.cred-curr-next:FORMAT,"/")
      AND INT64(tt-loan-cond.cred-date:SCREEN-VALUE) >  INT64(tt-loan-cond.DateDelay:SCREEN-VALUE)
   THEN DO:
...
DO:
   ...
   ** больше номера дня начала платежного периода */
   IF     tt-loan-cond.int-date:VISIBLE
      AND tt-loan-cond.int-curr-next:SCREEN-VALUE =  ENTRY(1,tt-loan-cond.int-curr-next:FORMAT,"/")
      AND INT64(tt-loan-cond.int-date:SCREEN-VALUE) >  INT64(tt-loan-cond.DateDelayInt:SCREEN-VALUE)
   THEN DO:
      RUN Fill-SysMes IN h_tmess ("", "", "", "Дата окончания платежного периода (день " +
...
DO:
   ...
   DO:
      ...
      END.
   END.
   IF iMode =  {&MOD_ADD} AND SELF:INPUT-VALUE <> -1 THEN
...
DO:
   ...
   IF CAN-FIND(FIRST loan WHERE
               loan.filial-id =  tt-loan.filial-id AND
               loan.contract =  tt-loan.contract AND
               loan.doc-ref =  SELF:INPUT-VALUE AND
               ROWID(loan) <> tt-loan.local__rowid) THEN
   DO:
      MESSAGE "Договор с таким номером уже существует"
         VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY {&RET-ERROR}.
   END.
   IF   INDEX(SELF:INPUT-VALUE,",") <> 0
     OR INDEX(SELF:INPUT-VALUE,";") <> 0 THEN
   DO:
...
DO:
   ...
   DEF VAR vDayOrMonth AS INT64 NO-UNDO.
   DEF VAR vTmp        AS CHAR  NO-UNDO.
   DEF VAR vSurr       AS CHAR  NO-UNDO.
   DEF VAR vSurrTmp    AS CHAR  NO-UNDO.
   DEF VAR vIndCount   AS INT64 NO-UNDO.
   DEF VAR vSrokMin    AS CHAR  NO-UNDO.
   DEF VAR vSrokMax    AS CHAR  NO-UNDO.
   DEF VAR vI          AS INT64 NO-UNDO.
   ...
   IF     mNewEndDate  <> ?
      AND mNewEndDate  <> DATE(tt-loan.end-date:SCREEN-VALUE)
   THEN DO:
      RUN Fill-SysMes IN h_tmess 
                      ("", "", "4", "Была изменена автоматически установленная дата окончания с "
   ...
   END.
   {&BEG_BT_LEAVE}
   IF tt-loan.end-date:SCREEN-VALUE =  "" THEN
      tt-percent.amt-rub:SCREEN-VALUE = "0".
   IF tt-loan.end-date:INPUT-VALUE <= tt-loan.open-date:INPUT-VALUE THEN
   DO:
      RUN Fill-SysMes IN h_tmess ("", "", "0", "Дата окончания не может быть меньше даты начала").
      RETURN NO-APPLY {&RET-ERROR}.
   END.
   IF AVAIL tt-instance AND
      tt-loan.end-date:INPUT-VALUE >  tt-instance.end-date THEN
   DO:
      IF tt-loan.class-code =  "loan-garmc-tr-sim" THEN
         RUN Fill-SysMes IN h_tmess ("", "", "0", 
             SUBSTITUTE("&1 &2",
                        "Дата закрытия договора не должна быть больше",
                        "даты закрытия соглашения по гарантиям!")).
      ELSE
   ...
   IF     iMode =  {&MOD_EDIT}
      AND tt-loan.end-date:INPUT-VALUE <  tt-loan.end-date
   THEN DO:
      /* Если есть пролонгации датой больше новой даты окончания договора */
      IF CAN-FIND(FIRST pro-obl WHERE pro-obl.contract   =  tt-loan.contract
                                  AND pro-obl.cont-code  =  tt-loan.cont-code
                                  AND pro-obl.idnt       =  3
                                  AND pro-obl.n-end-date >  tt-loan.end-date:INPUT-VALUE)
      THEN DO:
      ...
      IF CAN-FIND(FIRST loan-cond WHERE loan-cond.contract     =  tt-loan.contract
                                  AND   loan-cond.cont-code    =  tt-loan.cont-code
                                  AND   loan-cond.since        >  tt-loan.end-date:INPUT-VALUE)
      THEN DO:
   ...
   DO:
      ...
      IF vFlag <  0 THEN
      DO:
         IF vFlag =  -1 THEN vmess = "из-за потери соглашения".
         ELSE IF vFlag =  -2 THEN vmess = "из-за нарушения срока выдачи".
         ELSE IF vFlag =  -3 THEN vmess = "из-за наличия непогашенного транша".
         ELSE IF vFlag =  -4 THEN vmess = "из-за попадания в период действия др. транша".
         ELSE vmess = "".
         RUN Fill-SysMes IN h_tmess ("", "", "0", "Транш не прошел проверки " + vmess).
         RETURN NO-APPLY {&RET-ERROR}.
      END.
   END.
   IF tt-loan-cond.grperiod$ AND tt-loan-cond.grdatapo$ >  tt-loan.end-date:INPUT-VALUE THEN
   DO:
   ...
   DO:
      ...
      FOR EACH tt-comm-rate WHERE tt-comm-rate.rate-comm =  0.0 :
         ...
         IF vNewRate <> ?
            AND vNewRate <> tt-comm-rate.rate-comm THEN
            ASSIGN
               tt-comm-rate.rate-comm = vNewRate
         ...
         RUN getDefaultFixed IN hDefaultRate (
            ...
            OUTPUT vNewFixed).
         IF vNewFixed <> ?
            AND vNewFixed <> tt-comm-rate.rate-fixed THEN
            ASSIGN
               tt-comm-rate.rate-fixed = vNewFixed
               mChangedField           = YES      /* Для пересчета аннуитета */
            .
   ...
   DO:
      FIND LAST loan-cond WHERE
                loan-cond.contract  =  tt-instance.contract
            AND loan-cond.cont-code =  tt-instance.cont-code
            AND loan-cond.since     <= DATE(tt-loan.open-date:SCREEN-VALUE)
      NO-LOCK NO-ERROR.
      
      ASSIGN  
         vSurr    = SUBSTITUTE("&1,&2,&3",
                               tt-instance.contract,
                               tt-instance.cont-code,
                               STRING(loan-cond.since))
         vSrok    = GetXattrValueEx("loan-cond",
                                    vSurr,
                                    "БГСрокР",
                                    "")
         vSrokMin = GetXattrValueEx("loan-cond",
                                    vSurr,
                                    "БГМинСрок",
                                    "")
         vSrokMin = IF NOT {assigned vSrokMin} THEN
                       GetXAttrInit (loan-cond.class-code,
                                     "БГМинСрок")
                    ELSE
                       vSrokMin
         vSrokMax = GetXattrValueEx("loan-cond",
                                    vSurr,
                                    "БГМаксСрок",
                                    "")
         vSrokMax = IF NOT {assigned vSrokMax} THEN
                       GetXAttrInit (loan-cond.class-code,
                                     "БГМаксСрок")
                    ELSE
                       vSrokMax
      ...
      CASE vSrok:
         WHEN "Д" THEN
            vDayOrMonth = DATE(tt-loan.end-date:SCREEN-VALUE) - 
                          DATE(tt-loan.open-date:SCREEN-VALUE) + 1.
         WHEN "М" THEN
            vDayOrMonth = MonInPer(DATE(tt-loan.open-date:SCREEN-VALUE), 
                                   DATE(tt-loan.end-date:SCREEN-VALUE)).
      END CASE.  

      IF {assigned vSrokMin} THEN
      DO:
         IF vDayOrMonth <  INT64(vSrokMin) THEN
         DO:
            RUN Fill-SysMes IN h_tmess ("", "", "0", 
                                        SUBSTITUTE("&1 &2 &3 &4 &5!",
                                                   "Выдача гарантии невозможна,",
                                                   "так как нарушен минимальный",
      ...
      IF {assigned vSrokMax} THEN
      DO:
         IF vDayOrMonth >  INT64(vSrokMax) THEN
         DO:
            RUN Fill-SysMes IN h_tmess ("", "", "0",
                                        SUBSTITUTE("&1 &2 &3 &4 &5!",
                                                   "Выдача гарантии невозможна,",
                                                   "так как нарушен максимальный",
                                                   "срок действия гарантии в",
                                                   vSrokMax,
                                                   vSrok)).
            RETURN NO-APPLY {&RET-ERROR}.
      ...
      IF     NOT mChangeMCG
         AND AVAIL DataBlock THEN
      DO:         
         ASSIGN
            vSurr = SUBSTITUTE("&1,&2",
                               tt-instance.cont-code,
                               tt-loan.currency:SCREEN-VALUE)
            vIndCount = GetRefCrVal ("МБГ",
                                     "БГМинСрок",
                                     gEnd-Date,
                                     vSurr,
         ...
         FOR EACH ttIndicate WHERE
                  ttIndicate.fint <= vDayOrMonth:
            vTmp = vTmp + TRIM(STRING(DEC(ttIndicate.fint), ">>>9")) + ",".
         END.
         
         BLK2:
         DO vI = 1 TO NUM-ENTRIES(vTmp) - 1:
            ASSIGN
               vSurrTmp = SUBSTITUTE("&1,&2",
                                     vSurr,
                                     ENTRY(vI, vTmp))
               vIndCount =  GetRefCrVal  ("МБГ",
                                          "БГМаксСрок",
         ...
         DO:
            RUN browseld.p ("IndicationLine",
                            SUBSTITUTE("&1&2&3&2&4&2&5&2&6&2&7",
                                       "Data-Id",
                                       CHR(1),
                                       "cont-code",
                                       "currency",
                                       ...
                                       "RetFld",
                                       "RetRcp"),
                                       STRING(DataBlock.Data-Id) + CHR(1) + 
                                       tt-instance.cont-code + CHR(1) +
                                       tt-loan.currency:SCREEN-VALUE + CHR(1) + 
                                       STRING(vDayOrMonth) + CHR(1) + 
                                       STRING(vDayOrMonth) + CHR(1) + "RECID" + CHR(1) + 
            ...
            DO:
               FIND FIRST DataLine WHERE
                    ROWID(DataLine) =  ttRetVal.FileRowId
               NO-LOCK NO-ERROR.
               IF AVAILABLE DataLine THEN
               DO:
                  pick-value = DataLine.Txt.
               END.
            END.
         END.
         ELSE
            pick-value = GetRefVal ("МБГ",
                                    gEnd-Date,
                                    vSurr). 
         
         IF {assigned pick-value} THEN
            RUN GetTTRateCondMCGTransh IN THIS-PROCEDURE (pick-value, YES).
         ELSE
         DO:
            RUN Fill-SysMes IN h_tmess ("", "", "0", 
                                        SUBSTITUTE("&1 &2 &3 &4 &5",
                                                   "В тарифной сетке договора",
                                                   "соглашения отсутствуют",
                                                   "подходящие значения комиссии",
                                                   "'%ВыдМБГ'.",
                                                   "Параметры необходимо ввести вручную.")).
               /* Заполняем значениями по умолчанию и 
                  проставляем валюту оплаты комиссии как у договора */
            RUN GetTTRateCondMCGTransh IN THIS-PROCEDURE (
                SUBSTITUTE("%ВыдМБГ=&1,1,1,Д,0,0,0,,0,", 
                           tt-loan.currency:SCREEN-VALUE),
                YES).
         END. 
   ...
   {loan-trg.pro
     ...
     &OpenDate       = tt-loan.open-date:SCREEN-VALUE
     &EndDate        = tt-loan.end-date:SCREEN-VALUE
...
/*для счета Кредит проверим бал2 */
   ...
   DEF VAR vCopyTranshAcct AS CHAR NO-UNDO.
     
   vCopyTranshAcct = FGetSetting("CopyTranshAcct", "", "1").

   IF vCopyTranshAcct <> "0" THEN 
      IF     AVAIL tt-instance
         AND iMode =  {&MOD_ADD} THEN
      DO:
         ...
         ASSIGN
            ...
            vDTTerm = GetXAttrValueEx("loan",
                                      tt-instance.contract + "," + tt-instance.cont-code,
                                      "DTTerm",
                                      GetXAttrInit(tt-instance.class-code,"DTTerm"))
            pick-value = ?
         .
         FOR EACH code WHERE 
                  code.class  =  vDTTerm
              AND code.parent =  vDTTerm
         NO-LOCK:  
            ...
            DO:   
               ...
               /* при отсутствии подходящей строки по сроку в DecisionTable302
                  ищем следующий срок */
               IF pick-value <> ? THEN    
               LEAVE.
            END.
         ...
         vList = FGetSetting("КлассГарант", "", "").

         IF NOT CAN-DO(vList, tt-loan.class-code) THEN
         DO:
            IF pick-value =  ? THEN
               RUN Fill-SysMes IN h_tmess ("", "", "0", "Для транша не определен бал2 ").
            ELSE 
               IF tt-loan-acct-main.acct:SCREEN-VALUE =  "?"  
                  OR NOT tt-loan-acct-main.acct:SCREEN-VALUE BEGINS pick-value THEN
               DO:
                  ...
                  .
                  IF     vCopyTranshAcct =  "2" 
                     OR (   vCopyTranshAcct =  "1" 
                        AND GetXAttrValueEx("loan",
                                     tt-instance.contract + ',' + tt-instance.cont-code,
                                     "ТраншПерТип",
                                     "Да") =  "Нет" ) THEN
                     FOR EACH loan-acct WHERE 
                           loan-acct.contract  =  tt-instance.contract
                       AND loan-acct.cont-code BEGINS tt-instance.cont-code
                       AND loan-acct.acct-type =  tt-instance.contract
                       AND loan-acct.acct      BEGINS pick-value NO-LOCK,
                     FIRST acct WHERE 
                           acct.acct =  loan-acct.acct
                       AND acct.open-date <= DATE(tt-loan.open-date:SCREEN-VALUE)
                       AND (acct.close-date =  ?
                         OR acct.close-date >  DATE(tt-loan.end-date:SCREEN-VALUE))
                     NO-LOCK :
                        ASSIGN
...
DO:
   {&BEG_BT_LEAVE}
   IF tt-loan-cond.int-mode:INPUT-VALUE =  "ПериодДн" THEN
   DO:
   ...
   IF tt-loan-cond.int-mode:SCREEN-VALUE  =  "КоличДней"
...
DO:
  ...
  IF  ( tt-loan-cond.int-period:INPUT-VALUE =  "К"
    AND tt-loan-cond.int-month:INPUT-VALUE >  3
    OR  tt-loan-cond.int-period:INPUT-VALUE =  "ПГ"
    AND tt-loan-cond.int-month:INPUT-VALUE >  6
    OR  tt-loan-cond.int-period:INPUT-VALUE =  "Г"
    AND tt-loan-cond.int-month:INPUT-VALUE >  12 )
    OR ((tt-loan-cond.int-period:INPUT-VALUE =  "К"
    OR   tt-loan-cond.int-period:INPUT-VALUE =  "ПГ"
    OR   tt-loan-cond.int-period:INPUT-VALUE =  "Г")
    AND  tt-loan-cond.int-month:INPUT-VALUE  <= 0) THEN
  DO:
     RUN Fill-SysMes ("", "", "0", "В указанном периоде нет месяца с указанным номером.") NO-ERROR.
     {return_no_apply.i '{&RET-ERROR}'}
  END.
...
DO:
  {&BEG_BT_LEAVE}
  IF tt-loan-cond.int-period:INPUT-VALUE =  "ДН" THEN
  DO:
  ...
  {loan-trg.pro
   ...
   IF tt-loan-cond.int-date:SENSITIVE AND tt-loan-cond.int-date:VISIBLE THEN 
   DO:
      APPLY "ENTRY" TO tt-loan-cond.int-date IN FRAME {&MAIN-FRAME}.
      RETURN NO-APPLY.
   END.
   ELSE 
   DO:
      APPLY "ENTRY" TO int-offset_ IN FRAME {&MAIN-FRAME}.
      RETURN NO-APPLY.
   END.
...
DO:
   {&BEG_BT_LEAVE}
      /* валидация значения */
   IF INT64(SELF:SCREEN-VALUE) >= GetQntPer(DATE(tt-loan.open-date:SCREEN-VALUE),
                                          DATE(tt-loan.end-date:SCREEN-VALUE),
                                          IF tt-loan-cond.cred-date:VISIBLE =  YES
                                             THEN INT64(tt-loan-cond.cred-date:SCREEN-VALUE)
                                             ELSE 0,
                                          tt-loan-cond.cred-period:SCREEN-VALUE + ":" + tt-loan-cond.cred-month:SCREEN-VALUE,
                                          tt-loan-cond.since)
      AND INT64(SELF:SCREEN-VALUE) <> 0 THEN
   DO:
      RUN Fill-SysMes IN h_tmess ("", "", "0",
...
DO:
   ...
   IF INT64(SELF:SCREEN-VALUE) >= GetQntPer(DATE(tt-loan.open-date:SCREEN-VALUE),
                                          DATE(tt-loan.end-date:SCREEN-VALUE),
                                          IF tt-loan-cond.int-date:VISIBLE =  YES
...
DO:
   ...
   IF tt-loan-cond.grperiod$ AND tt-loan-cond.grdatas$ <> ? THEN
      vGrDateS = tt-loan-cond.grdatas$ - 1.
   ELSE
      vGrDateS = DATE(tt-loan.open-date:SCREEN-VALUE).
   IF tt-loan-cond.grperiod$ AND tt-loan-cond.grdatapo$ <> ? THEN
      vGrDatePo = tt-loan-cond.grdatapo$.
   ELSE
      vGrDatePo = DATE(tt-loan.end-date:SCREEN-VALUE).
   mPerCnt = GetQntPer (vGrDateS,
                        vGrDatePo,
                        IF tt-loan-cond.cred-date:VISIBLE =  YES THEN INT64(tt-loan-cond.cred-date:SCREEN-VALUE)
                                                              ELSE 0,
                        tt-loan-cond.cred-period:SCREEN-VALUE + ":" + tt-loan-cond.cred-month:SCREEN-VALUE,
...
DO:
   IF iMode = {&MOD_ADD} AND tt-loan.open-date:SCREEN-VALUE <> "?" AND NOT vEndRasch THEN DO:
...
DO:
   ...
   DO:
      IF tt-loan.open-date:INPUT-VALUE <  tt-instance.open-date THEN
      DO:
      ...
      IF iMode =  {&MOD_ADD} THEN
      DO:
         IF mDayTr >  0 AND tt-loan.end-date =  ? THEN
            ASSIGN
               mSrokChange = TRUE
               tt-loan.end-date:SCREEN-VALUE = STRING(DATE(DATE(tt-loan.open-date:SCREEN-VALUE) + mDayTr))
   ...
   DO:
      ...
      IF ENTRY(NUM-ENTRIES(vProdPog), vProdPog) =  "ДД" THEN
      DO:
         pick-value = ?.
         RUN Fill-SysMes ("", "", "3",
   ...
   FINALLY:
      IF iMode =  {&MOD_ADD} THEN
         gend-date = DATE(tt-loan.open-date:SCREEN-VALUE).
   END FINALLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tt-loan.currency TERMINAL-SIMULATION
ON VALUE-CHANGED OF tt-loan.currency IN FRAME fMain /* Валюта */
DO:
   IF tt-loan.class-code <> "loan-garmc-tr-sim" THEN
   DO:
      RUN CopySinceTT IN THIS-PROCEDURE.
      /* меняем сразу, чтобы пользователь не забыл потом поменять валюту, а изменял ее только в случае надобности */
      FOR EACH tt-comm-rate:
         IF     tt-comm-rate.comm-rate-curr <> tt-loan.currency:SCREEN-VALUE 
            AND NOT CAN-DO(mKomisNacVal,tt-comm-rate.commission)
         THEN
            ASSIGN
               tt-comm-rate.comm-rate-curr:SCREEN-VALUE IN BROWSE br-comm = 
...
DO:
   ...
   tt-loan-cond.grperiod$ = tt-loan-cond.grdatas$ <> ? OR tt-loan-cond.grdatapo$ <> ?.
...
DO:
   {&BEG_BT_LEAVE}
   IF     tt-loan-cond.poslpd$:SCREEN-VALUE <> ""
      AND DATE(tt-loan-cond.poslpd$:SCREEN-VALUE) <  
          DATE(tt-loan.end-date:SCREEN-VALUE) THEN
   DO:
      RUN Fill-SysMes  IN h_tmess ("", "", "0",
                                   "Последняя плановая дата " +
...
&GLOBAL-DEFINE EVENT_LIST "GO,END-ERROR"
MAIN-BLOCK:
...
/* Commented by KSV: Читаем данные */
   ...
   DO:
      ...
      DO:
         ...
         IF     tt-loan-cond.prodtrf$ <> ?
            AND tt-loan-cond.prodtrf$ <> ""
            AND tt-loan-cond.prodtrf$ <> "?" THEN
         DO:
               /* Сначала на "ПродЛин" ищем общий тариф продукта╨ */
            IF     tt-loan.prodkod$ <> ""
               AND tt-loan.prodkod$ <> "?"
               AND tt-loan.prodkod$ <> ? THEN
            DO:
               ...
               FIND LAST tmp-code WHERE
                         tmp-code.class      =  "ПродЛин"
                  AND    tmp-code.code       =  mProdCode
                  AND    tmp-code.beg-date   <= tt-loan.open-date
                  AND   (tmp-code.end-date   >= tt-loan.open-date
                     OR  tmp-code.end-date   =  ?)
               NO-LOCK NO-ERROR.
                  /* Если на подпродукте не задан базовый тариф, то ищем на родителях */
               REPEAT WHILE AVAIL tmp-code AND tmp-code.misc[7] =  "":
                  FIND FIRST code WHERE
                             code.class =  "ПродЛин"
                         AND code.code  =  mProdCode
                  NO-LOCK NO-ERROR.
                  IF AVAIL code AND code.parent <> "" THEN
                  DO:
               ...
               IF AVAIL tmp-code AND tmp-code.misc[7] <> "" THEN
         ...
         FOR EACH tt-commrate BREAK BY tt-commrate.commission:
            ...
            DO:
               FIND FIRST tt-comm-rate WHERE
                          tt-comm-rate.commission =  tt-commrate.commission AND
                          tt-comm-rate.since =  tt-loan-cond.since NO-LOCK NO-ERROR.
      ...
      FOR EACH tt-comm-rate:
         FOR EACH b-comm-cond BY b-comm-cond.Local__Id DESC:
             mI = b-comm-cond.Local__Id.
             LEAVE.
         END. 
         FIND FIRST tt-comm-cond WHERE tt-comm-cond.commission =  tt-comm-rate.commission
                                   AND tt-comm-cond.since      =  tt-comm-rate.since
         NO-ERROR.
         IF NOT AVAIL tt-comm-cond THEN
         DO:
            CREATE tt-comm-cond.
            ASSIGN
               ...
               tt-comm-cond.local__Id = MAX(GetInstanceId("tt-comm-cond"), mI) + 1
            .
 
            FIND LAST comm-cond WHERE comm-cond.contract   =  tt-loan.contract
                                     AND comm-cond.cont-code  =  tt-loan.cont-code
                                     AND comm-cond.commission =  tt-comm-cond.commission
                                     AND comm-cond.since      <= tt-comm-cond.since
               NO-LOCK NO-ERROR.
      ...
      DO:
         ...
         DO:
            DO vCounter = 1 TO NUM-ENTRIES(vCredPlav):
               ...
               FIND FIRST code WHERE code.class =  "ПлавСтПарам"
                                 AND code.code  =  ENTRY(vCounter,vCredPlav)
               NO-LOCK NO-ERROR.
               IF AVAIL code THEN
               DO:
                  FIND FIRST tt-comm-cond WHERE tt-comm-cond.commission =  ENTRY(2,code.misc[2])
                                            AND tt-comm-cond.since      =  tt-loan-cond.since
                  NO-ERROR.
                  ...
                  DO:
                     ASSIGN
                        ...
                        tt-comm-cond.NegBaseRate = ENTRY(3,code.misc[2]) 
                                                   WHEN NUM-ENTRIES(code.misc[2]) >= 3
                     .
                     ...
                     ** по указанным правилам */
                     ...
                     DO:
                        FIND FIRST tt-comm-rate WHERE 
                                   tt-comm-rate.commission =  tt-comm-cond.commission
                               AND tt-comm-rate.since      =  tt-comm-cond.since
                        NO-ERROR.
                        IF AVAIL tt-comm-rate THEN
                        DO:                       
   ...
   DO:
      ...
      FOR FIRST tt-amount WHERE tt-amount.amt-rub <> 0 BY tt-amount.end-date:
         LEAVE.
      END.
   ...
   ** не нулевого планового платежа */
   IF tt-loan-cond.since =  tt-loan.end-date THEN
   ...
   /* Берем с верхнего договора форму расчета и число месяца,
   когда осуществляется погашение кредита + пробег */
   IF     tt-loan.class-code =  "loan-tran-lin-ann"
      AND AVAIL tt-instance THEN
   DO:
      FIND LAST loan-cond WHERE loan-cond.contract  =  tt-instance.contract
                            AND loan-cond.cont-code =  tt-instance.cont-code
      NO-LOCK NO-ERROR.
      IF AVAIL loan-cond THEN
      ASSIGN
         tt-loan-cond.disch-type = loan-cond.disch-type
         tt-loan-cond.cred-date  = loan-cond.cred-date
         tt-loan-cond.delay1     = loan-cond.delay1
      .
   END.
   ...
   ** с идентификатором SEPARATOR# с атрибутом VIES-AS TEXT */
   ...
   FINALLY:
      IF mOldgend-date <> ? THEN 
         /* При закрытии окна восстанавливаем значение gend-date */
         gend-date = mOldgend-date.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE AfterGetInstance  TERMINAL-SIMULATION 
PROCEDURE AfterGetInstance PRIVATE:
/*------------------------------------------------------------------------------
Purpose: 
Parameters: <none>
Notes: 
------------------------------------------------------------------------------*/
   ...
   DEF VAR vInstCond  AS HANDLE NO-UNDO.
   DEF VAR vOk        AS LOG    NO-UNDO.
   DEF VAR vAggrClass AS CHAR   NO-UNDO.
   ...
   DO:
      ASSIGN
         vContract = mInstance:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD ("contract"):BUFFER-VALUE ()
         vContCode = mInstance:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD ("cont-code"):BUFFER-VALUE ()
         vSince    = mInstance:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD ("since"):BUFFER-VALUE ()
      .
      ...
      CASE mFindLoanCond:
         WHEN "ДАТАСОСТ" THEN
            FIND LAST bloan-cond WHERE
                      bloan-cond.contract  =  vContract
                  AND bloan-cond.cont-code =  vContCode
                  AND bloan-cond.since     <= vSince
            NO-LOCK NO-ERROR.
         WHEN "ПОСЛЕДНИЕ" OR
         WHEN "" THEN
            FIND LAST bloan-cond WHERE
      ...
      DO:
         vAggrClass = "".
            /* Собираем все агрегируемые классы */
         FOR EACH xattr WHERE 
                  xattr.data-type  =  "class" 
              AND xattr.Class-Code =  bloan-cond.class-code
              AND CAN-DO ("!,*", TRIM (xattr.Validation)) 
         NO-LOCK:
            {additem.i vAggrClass xattr.Xattr-Code}
         END.
          
         RUN PrepareInstance IN h_data (vAggrClass).
             /* Получаем контейнер с необходимым условием */
         RUN GetInstance IN h_data (bloan-cond.class-code,
                                    bloan-cond.contract + "," + 
                                    bloan-cond.cont-code + "," + 
                                    STRING(bloan-cond.since),
                                    OUTPUT vInstCond,
                                    OUTPUT vOk).
            /* Переприсваиваем на необходимое условие */                              
         mInstance:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD ("loan-cond"):BUFFER-VALUE = vInstCond.
      END.
...
------------------------------------------------------------------------------*/
   ...
   DO:
      ...
      IF GetXAttrValue("loan", b-loan.contract + "," + b-loan.cont-code, "СводГрАвто") =  "Да" THEN
      DO:
         bkl:
         DO:
               /* Ищем действующее условие соглашения */
            FIND LAST b-loan-cond WHERE
                      b-loan-cond.contract  =  b-loan.contract
               AND    b-loan-cond.cont-code =  b-loan.cont-code
               AND    b-loan-cond.since     <= b-loan.end-date
            NO-LOCK NO-ERROR.
            IF AVAIL b-loan-cond THEN
            DO:
                  /* Не Конец срока */
               IF tt-loan-cond.cred-period:SCREEN-VALUE IN FRAME {&frame-name} <> "Кс" THEN
               DO:
                     /* 1. Проверка совпадения параметров периодичности */
                  IF    tt-loan-cond.cred-period:SCREEN-VALUE IN FRAME {&frame-name} <> "П" AND
                       (b-loan-cond.cred-period <>       
                        tt-loan-cond.cred-period:SCREEN-VALUE IN FRAME {&frame-name}
                     OR b-loan-cond.cred-date   <> 
                        INT64(tt-loan-cond.cred-date:SCREEN-VALUE   IN FRAME {&frame-name})
                     OR b-loan-cond.cred-month  <> 
                        INT64(tt-loan-cond.cred-month:SCREEN-VALUE  IN FRAME {&frame-name}))
                  THEN DO:
               ...
               END.
                  /* Конец срока */
               ELSE IF tt-loan-cond.cred-period:SCREEN-VALUE IN FRAME {&frame-name} =  "Кс" THEN
               DO:
                     /* Ищем в графике соглашения проверяется наличие записи на дату окончания срока транша */
                  FIND FIRST b-term-obl WHERE
                             b-term-obl.contract     =  b-loan.contract    /* Данные */
                     AND     b-term-obl.cont-code    =  b-loan.cont-code   /* соглашения */
                     AND     b-term-obl.idnt         =  3
                     AND     b-term-obl.end-date     =  DATE(tt-loan.end-date:SCREEN-VALUE IN FRAME {&frame-name})      /* Окончание транша */
                  NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
   ...
   DEFINE BUFFER b-loan FOR tt-loan.
      FIND FIRST b-loan WHERE b-loan.local__ID =  1 NO-ERROR.
      IF AVAIL b-loan THEN DELETE b-loan.
   DEFINE BUFFER b-loan-cond FOR tt-loan-cond.
      FIND FIRST b-loan-cond WHERE b-loan-cond.local__ID =  1 NO-ERROR.
      IF AVAIL b-loan-cond THEN DELETE b-loan-cond.
   DEFINE BUFFER b-amount FOR tt-amount.
      FIND FIRST b-amount WHERE b-amount.local__ID =  1 NO-ERROR.
      IF AVAIL b-amount THEN DELETE b-amount.
   DEFINE BUFFER b-percent FOR tt-percent.
      FIND FIRST b-percent WHERE b-percent.local__ID =  1 NO-ERROR.
      IF AVAIL b-percent THEN DELETE b-percent.
   DEFINE BUFFER b-loan-acct-main FOR tt-loan-acct-main.
      FIND FIRST b-loan-acct-main WHERE b-loan-acct-main.local__ID =  1 NO-ERROR.
      IF AVAIL b-loan-acct-main THEN DELETE b-loan-acct-main.
   DEFINE BUFFER b-loan-acct-cust FOR tt-loan-acct-cust.
      FIND FIRST b-loan-acct-cust WHERE b-loan-acct-cust.local__ID =  1 NO-ERROR.
      IF AVAIL b-loan-acct-cust THEN DELETE b-loan-acct-cust.
   DEFINE BUFFER b-contragent FOR tt-contragent.
      FIND FIRST b-contragent WHERE b-contragent.local__ID =  1 NO-ERROR.
      IF AVAIL b-contragent THEN DELETE b-contragent.
   DEFINE BUFFER b-dealer FOR tt-dealer.
      FIND FIRST b-dealer WHERE b-dealer.local__ID =  1 NO-ERROR.
      IF AVAIL b-dealer THEN DELETE b-dealer.
   DEFINE BUFFER b-broker FOR tt-broker.
      FIND FIRST b-broker WHERE b-broker.local__ID =  1 NO-ERROR.
      IF AVAIL b-broker THEN DELETE b-broker.
   DEFINE BUFFER b-comm-rate FOR tt-comm-rate.
      FOR EACH b-comm-rate WHERE b-comm-rate.local__UpID =  1:
         DELETE b-comm-rate.
      END.

   FIND FIRST tt-loan WHERE tt-loan.local__UpId =  0 NO-ERROR.
   FIND FIRST tt-contragent WHERE tt-contragent.local__UpId =  0 NO-ERROR.
   FIND FIRST tt-dealer WHERE tt-dealer.local__UpId =  0 NO-ERROR.
   FIND FIRST tt-broker WHERE tt-broker.local__UpId =  0 NO-ERROR.
   FIND LAST tt-loan-cond WHERE tt-loan-cond.local__UpId =  0 NO-ERROR.
   FIND LAST tt-amount WHERE tt-amount.local__UpId =  0 NO-ERROR.
   FIND LAST tt-percent WHERE tt-percent.local__UpId =  0 NO-ERROR.
   FIND LAST tt-loan-acct-main WHERE tt-loan-acct-main.local__UpId =  0 NO-ERROR.
   FIND LAST tt-loan-acct-cust WHERE tt-loan-acct-cust.local__UpId =  0 NO-ERROR.
   ...
   ASSIGN
      ...
      b-loan.Local__Id    = 1
      b-loan.alt-contract = IF tt-loan.alt-contract =  "mm" THEN "mmap"
   ...
   END.
   FIND FIRST tt-term-obl NO-ERROR.
   IF AVAIL tt-term-obl THEN
      ASSIGN
         tt-term-obl.idnt        = 3
   ...
   DO:
      ...
      DO: /* tt-loan-acct-main.acct может быть без @ */
         FIND FIRST acct WHERE acct.filial-id =  tt-loan.filial-id
                           AND acct.number    =  ENTRY(1, tt-loan-acct-main.acct, "@")
                           AND acct.currency  =  tt-loan-acct-main.currency
            NO-LOCK NO-ERROR.
         IF AVAIL acct THEN
         DO:
            IF tt-loan-acct-main.acct <> acct.acct THEN
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE tt-loan-cond THEN
    DISPLAY tt-loan-cond.NDays tt-loan-cond.NMonthes tt-loan-cond.NYears
          ...
          tt-loan-cond.grperiod$ tt-loan-cond.poslpd$
          tt-loan-cond.bgminsrok$ tt-loan-cond.bgmakssrok$ tt-loan-cond.bgsrokr$ 
          tt-loan-cond.bggarstop$ tt-loan-cond.bgvalwutymbg$ tt-loan-cond.bgspisbezakc$
      WITH FRAME fMain IN WINDOW TERMINAL-SIMULATION.
  ...
  ENABLE tt-loan-cond.NDays tt-loan-cond.NMonthes tt-loan-cond.NYears
         ...
         mBag mSvod mLimit mLimitRest tt-loan-cond.grperiod$ tt-loan-cond.poslpd$
         tt-loan-cond.bgminsrok$ tt-loan-cond.bgmakssrok$ tt-loan-cond.bgsrokr$ 
         tt-loan-cond.bggarstop$ tt-loan-cond.bgvalwutymbg$ tt-loan-cond.bgspisbezakc$
      WITH FRAME fMain IN WINDOW TERMINAL-SIMULATION.
...
DEFINE VARIABLE vHPoslPD    AS HANDLE     NO-UNDO. /* хендл для поля poslpd$ */
DEFINE VARIABLE vHPoslPDLbl AS HANDLE     NO-UNDO. /* --||-- для метки poslpd$ */
DEFINE VARIABLE vHComment   AS HANDLE     NO-UNDO. /* --||-- comment */
DEFINE VARIABLE vLoanCurr   AS CHARACTER  NO-UNDO.
   IF     mNewEndDate <> ?
      AND GetFstWrkDay(tt-loan.Class-Code, tt-loan.user-id, mNewEndDate, 9, 1) <> mNewEndDate 
   THEN
      mNewEndDate = ?.
   IF mNewEndDate <> ? 
   THEN DO:
   ...
   DO:
      ...
      ASSIGN
         tt-loan-cond.bgminsrok$   :VISIBLE IN FRAME {&MAIN-FRAME} = FALSE
         tt-loan-cond.bgmakssrok$  :VISIBLE IN FRAME {&MAIN-FRAME} = FALSE
         tt-loan-cond.bgsrokr$     :VISIBLE IN FRAME {&MAIN-FRAME} = FALSE
         tt-loan-cond.bggarstop$   :VISIBLE IN FRAME {&MAIN-FRAME} = FALSE
         tt-loan-cond.bgvalwutymbg$:VISIBLE IN FRAME {&MAIN-FRAME} = FALSE 
         tt-loan-cond.bgspisbezakc$:VISIBLE IN FRAME {&MAIN-FRAME} = FALSE
      .
   END.
   ELSE
   DO:
      tt-loan-cond.bgsrokr$:LIST-ITEMS = "Д,М". 
   END.
   ...
   IF  tt-loan.class-code <> "loan_dbl_ann" AND
       (   FGetSetting("РасчСумОб","АннуитОбесп","?") <> "Да"
        OR tt-loan.class-code <> "loan_mortgage")  THEN
   ...
   IF tt-loan.contract = "Депоз" AND FGetSetting("СкрПол", "", ?) =  "ДА" THEN
      ASSIGN
   ...
   IF     AVAIL tt-loan
      AND       tt-loan.poslpdzapo$ THEN
      ASSIGN
         tt-loan-cond.poslpd$:VISIBLE   IN FRAME {&MAIN-FRAME} = TRUE
         tt-loan-cond.poslpd$:SENSITIVE IN FRAME {&MAIN-FRAME} = IF iMode =  {&MOD_ADD}
                                                                 THEN TRUE
                                                                 ELSE FALSE
      .
   ELSE
   ...
   IF AVAIL tt-instance AND iMode =  {&MOD_ADD} THEN
   DO:
      vNextStream = INT64(ENTRY(2,
         GetBuffersValue(
            "loan",
            "FOR EACH loan WHERE
                 loan.contract =  '" + tt-instance.contract + "' AND
                 loan.cont-code BEGINS '" + tt-instance.cont-code + " ' AND
                 NUM-ENTRIES(loan.cont-code,' ') =  2 NO-LOCK BY INT64(ENTRY(2,loan.cont-code,' ')) DESC",
            "loan.cont-code"),
         " ")) NO-ERROR.

      vNextStream = IF vNextStream = ? THEN 1 ELSE (vNextStream + 1).
      ...
      tt-loan.cont-type:SCREEN-VALUE = GetxAttrValueEx("loan",
                                                       ...
                                                       "ТипДогСогл",
                                                       "").
      IF tt-loan.cont-type:SCREEN-VALUE <> "" THEN
   ...
   IF     iMode =  {&MOD_ADD}
      AND tt-loan.class-code =  "loan-garmc-tr-sim" THEN
   DO:
      tt-loan.currency:SENSITIVE = YES.  
   END.
   ...
   DO:
      ...
      FOR EACH term-obl OF tt-loan WHERE term-obl.idnt =  1 NO-LOCK:
         ACCUMULATE
      ...
      IF vCount >  1 AND tt-percent.amt-rub:VISIBLE THEN
      DO:
      ...
      END.
   END.
   /* т.к. поле comment большое и закрывает поле poslpd$,
   ** уменьшим его длину, чтобы можно было poslpd$ увидеть */
   IF     tt-loan.poslpdzapo$
      AND tt-loan.comment:VISIBLE THEN
   DO:
      ASSIGN
         vHPoslPD    = tt-loan-cond.poslpd$:HANDLE
         vHPoslPDLbl = vHPoslPD:SIDE-LABEL-HANDLE
         vHComment   = tt-loan.comment:HANDLE
      .

      IF     vHComment:ROW =  vHPoslPD:ROW
         AND vHComment:COL + vHComment:WIDTH >= (vHPoslPD:COL - vHPoslPDLbl:WIDTH + 1) THEN
      DO:
         vHComment:INNER-CHARS = vHComment:INNER-CHARS - 
                               (vHPoslPD:WIDTH + vHPoslPDLbl:WIDTH + 2).
      END.
   END.
   ...
   ** нужно поле tt-loan.trade-sys, а оно закрыто полем tt-loan.comment */
   ...
   DO:
      ...
      DO:
         ...
         IF GetXattrInit(tt-loan.class-code,"ОкругДоРуб") =  "Да" THEN
         ...
         IF    tt-loan.class-code =  "loan-transh-ann"
            OR tt-loan.class-code =  "loan-transh-lin-ann" THEN
            tt-loan-cond.annuitplat$ = ?.
   ...
   IF FGetSetting("Закрытие","","?") =  "нет" THEN
...
&IF DEFINED(MANUAL-REMOTE) &THEN
   ...
   IF tt-loan.ovrstopr$ <> ? THEN tt-loan.ovrstopr$:SCREEN-VALUE = tt-loan.ovrstopr$ .
&ENDIF 

    /* Лимиты выдачи */
   IF   ( iMode =  {&MOD_EDIT}
      OR  iMode =  {&MOD_VIEW} )
     AND ( tt-loan.rewzim$:SCREEN-VALUE =  "ЛимВыдЗад" OR
           tt-loan.rewzim$:SCREEN-VALUE =  "НевозЛиния" )THEN
   DO:
      ...
      ** и вычтем их из лимита - получим оставшийся лимит */
      FOR EACH loan-int WHERE loan-int.contract  =  tt-loan.contract
                          AND loan-int.cont-code =  tt-loan.cont-code
                          AND loan-int.id-d      =  0
                          AND loan-int.id-k      =  3
                          AND loan-int.mdate     <= tt-loan.since
      NO-LOCK:
         vOperSumm = vOperSumm + loan-int.amt-rub.
      END.
      IF tt-loan.cont-type =  "Течение" THEN
         FOR EACH loan-int WHERE loan-int.contract  =      tt-loan.contract
   ...
   IF   ( iMode =  {&MOD_EDIT}
      OR  iMode =  {&MOD_VIEW})
     AND tt-loan.rewzim$:SCREEN-VALUE =  "ВозобнЛиния" THEN
   DO:
   ...
   END.
   ELSE
      mSvodROnly =     iMode <> {&MOD_ADD}
                   AND CAN-FIND(FIRST bloan WHERE
                                      bloan.contract                    =      tt-loan.contract
                                  AND bloan.cont-code                   BEGINS tt-loan.cont-code + " "
                                  AND NUM-ENTRIES(bloan.cont-code, " ") >  1
                                )
                   OR (   tt-loan.class-code =  "loan-transh-ann"
                       OR tt-loan.class-code =  "loan-tran-lin-ann")
      .

      /* показываем, если редактирование, или просмотр
      ** и установлен платежный период = ПериодДн */
   IF iMode <> {&MOD_ADD} AND tt-loan-cond.cred-mode =  "ПериодДн" AND tt-loan-cond.shemaplat$ <> YES THEN
   DO:
         /* TOGGLE-BOXы потом не перерисрвываются, поэтому рисуем здесь */
...
/*====*/
   ii = ii + 1 .
   IF ii <= 1 THEN DO:
      SetHelpStrAdd(mHelpStrAdd + "│F6-Период доступности средств│F2-Ограничения" ).
      RUN PutHelp("",FRAME {&MAIN-FRAME}:HANDLE).
   END.
    /* Для валютных договоров инициализируем валюту */
    vLoanCurr = tt-loan.currency:SCREEN-VALUE.
    FOR EACH tt-comm-rate:
       IF iMode =  {&MOD_ADD} THEN
       DO:
          IF    NOT {assigned vLoanCurr} 
             OR tt-comm-rate.comm-rate-curr =  "?" THEN
             tt-comm-rate.comm-rate-curr = "".
          IF     {assigned vLoanCurr} 
             AND NOT CAN-DO(mKomisNacVal,tt-comm-rate.commission)
          THEN
            ASSIGN 
               tt-comm-rate.comm-rate-curr:SCREEN-VALUE = tt-loan.currency:SCREEN-VALUE
               tt-comm-rate.comm-rate-curr = tt-loan.currency:SCREEN-VALUE
            .
       END.
       ELSE
          IF tt-comm-rate.comm-rate-curr =  ? OR 
             tt-comm-rate.comm-rate-curr =  "?" THEN
             tt-comm-rate.comm-rate-curr = "".
   END.
      
   IF     tt-loan.contract  = "Кредит" 
      AND tt-loan.cont-type = "Течение" THEN
      DISABLE tt-loan.cont-type WITH FRAME {&MAIN-FRAME}.
      
      /* Для классов течений с ИндРезервТранш = НЕТ
         запрещаем редактирование КК и Риска */
   IF     NUM-ENTRIES(tt-loan.cont-code, " ") = 2
      AND CAN-FIND (FIRST loan WHERE 
                          loan.contract  =  tt-loan.contract
                      AND loan.cont-code =  ENTRY(1, tt-loan.cont-code, " ")
                      AND loan.cont-type =  "Течение"
                    NO-LOCK)
      AND GetXattrInit(tt-loan.Class-Code, "ИндРезервТранш") = "НЕТ" THEN
   DO:
      DISABLE mGrRiska WITH FRAME {&MAIN-FRAME}.
      DISABLE mRisk    WITH FRAME {&MAIN-FRAME}.
   END.   
...
------------------------------------------------------------------------------*/
   ...
   DEFINE VARIABLE vErrorLimit AS CHARACTER NO-UNDO.
   DEFINE VARIABLE vCheckLimit AS CHARACTER NO-UNDO.
   
   DEF BUFFER loan       FOR loan.
   DEF BUFFER xloan      FOR loan.
   DEF BUFFER xcomm-rate FOR comm-rate.

      /* Проверка длительности транша */
   DL_TR:
   DO:
   IF FGetSetting("ДлитТранша",?,"Да") =  "Да" THEN
      IF (mDayTr <> 0) AND (mDayTr <  INT64(tt-loan.end-date - tt-loan.open-date)) THEN
      DO: 
            /* если дата окончания попала на следующий рабочий день, то выдаем предупреждение */
         IF GetFstWrkDay(tt-loan.Class-Code, 
                         tt-loan.user-id, 
                         tt-loan.open-date + mDayTr, 
                         9, 1) =  tt-loan.end-date THEN
         DO: 
            pick-value = "".
            RUN Fill-SysMes IN h_tmess (
               "", "", "4",
               "В результате сдвига вперед даты окончания договора длительность транша"+
               " получилась больше, чем максимальная длительность, установленная на" +
               " реквизите 'ОврПр' - " + STRING(mDayTr,">>>9") + " дней.~nПродолжить?"
            ).
            IF pick-value =  "YES" THEN
               LEAVE DL_TR.
         END.
         RETURN ERROR "Длительность транша больше, чем максимальная длительность," +
                      " установленная на реквизите 'ОврПр' - " + 
                      STRING(mDayTr,">>>9") + " дней.".
      END. 
   END.
   ...
   ** не объектами полями, нужно добавить их в список след. образом : */
   SetFormDefList(GetFormDefList() +
                  ...
                  ",tt-loan.LimitGrafDate,tt-loan-cond.grdatas$,tt-loan-cond.grdatapo$,tt-loan.fend-date").
   /* Проверка ДатаПо лимитов */
      IF tt-loan.rewzim$ <> ? AND
         tt-loan.rewzim$ <> ""  AND
         tt-loan.LimitGrafDate =  ?
      THEN DO:
         tt-loan.LimitGrafDate = tt-loan.end-date .
      END.
      IF tt-loan.rewzim$ =  ? OR
         tt-loan.rewzim$ =  ""
      THEN DO:
         tt-loan.LimitGrafDate = ? .
      END.
   ...
   DO WITH FRAME {&MAIN-FRAME}:
      ...
      NO-ERROR.
      IF tt-loan.class-code =  "loan-tran-lin-ann" THEN
      DO:
         FIND FIRST xloan WHERE xloan.contract  =  tt-loan.contract
                            AND xloan.cont-code =  ENTRY(1,tt-loan.cont-code," ")
         NO-LOCK NO-ERROR.
         IF AVAIL xloan THEN
            FIND LAST  xcomm-rate WHERE
                       xcomm-rate.commission =  "%Кред"
                   AND xcomm-rate.acct       =  "0"
                   AND xcomm-rate.kau        =  xloan.contract + "," + xloan.cont-code
                   AND xcomm-rate.since      <= tt-loan-cond.since
            NO-LOCK NO-ERROR.
       END.
      ...
      ** поэтмоу переискиваем нужные даты */
      IF iMode =  {&MOD_EDIT} THEN
      DO:
         FIND LAST xxloan-cond WHERE xxloan-cond.contract  =  tt-loan.contract
                                 AND xxloan-cond.cont-code =  tt-loan.cont-code
                                 AND xxloan-cond.since     <= DATE(tt-loan.end-date:SCREEN-VALUE)
         NO-LOCK NO-ERROR.
         FIND FIRST tt-amount WHERE tt-amount.contract  =  tt-loan.contract
                                AND tt-amount.cont-code =  tt-loan.cont-code
                                AND tt-amount.idnt      =  2
                                AND tt-amount.end-date  <= xxloan-cond.since
         NO-LOCK NO-ERROR.
      END.
      ...
      RUN CalcAnnuitet(
                       tt-loan.contract,
                       tt-loan.cont-code,
                       IF iMode =  {&MOD_EDIT} THEN xxloan-cond.since
     ...
     END.
         ...
         ** класса "loan_mortgage" и "loan-tran-lin-ann", для других классов пропускается */
   ...
   DO:
      FOR EACH b-comm-rate WHERE b-comm-rate.since =  tt-loan-cond.since BY b-comm-rate.Local__ID:
         IF iMode =  {&MOD_ADD} THEN
         DO:
            mChangedField = IF mHandCalcAnnuitet THEN NO ELSE mChangedField.
            {loan-trg.pro
   ...
   DO:
      ...
      IF tt-loan.cont-code =  "" THEN
         tt-loan.cont-code = ?.

      IF tt-loan.doc-ref =  "" THEN
         tt-loan.doc-ref = ?.

      IF tt-amount.amt-rub <> 0 THEN ASSIGN
         tt-amount.fop-date = tt-loan.open-date
         tt-amount.end-date = tt-loan.open-date.
      IF tt-percent.amt-rub <> 0 THEN ASSIGN
      ...
      DO:
         ...
         DO: /* tt-loan-acct-cust.acct может быть без @ */
            FIND FIRST acct WHERE acct.filial-id =  tt-loan.filial-id
                              AND acct.number    =  ENTRY(1, tt-loan-acct-cust.acct, "@")
                              AND acct.currency  =  tt-loan.currency
               NO-LOCK NO-ERROR.
            IF AVAIL acct THEN
            DO:
               IF tt-loan-acct-cust.acct <> acct.acct THEN
   ...
   DO:
      ...
      IF iMode =  {&MOD_ADD} THEN /* создание течения */
      DO:
   ...
   IF iMode =  {&MOD_ADD} AND AVAIL tt-instance THEN
   DO:
      mUniformBag = GetXAttrValueEx("loan", tt-instance.contract + "," + tt-instance.cont-code, "UniformBag", "").
      IF tt-instance.cont-type =  "Течение" AND mUniformBag <> "" THEN
      DO:
         pick-value ="".
   ...
   IF iMode =  {&MOD_ADD} THEN
      tt-loan.FEnd-date = tt-loan.end-date.

   /* Обработка лимитов при наличии комиссии "%МинОД" */
   FIND FIRST tt-comm-rate WHERE
              tt-comm-rate.commission =  "%МинОД"
   NO-ERROR.
   ...
   IF tt-loan.cont-type <> "Течение" THEN
      ASSIGN
         tt-loan.svodgrafik$ = IF tt-loan.svodgrafik$ =  NO THEN ? ELSE tt-loan.svodgrafik$
         tt-loan.svodskonca$ = IF tt-loan.svodskonca$ =  NO THEN ? ELSE tt-loan.svodskonca$
         tt-loan.svodgravto$ = IF tt-loan.svodgravto$ =  NO THEN ? ELSE tt-loan.svodgravto$
         tt-loan.svodspostr$ = IF tt-loan.svodspostr$ =  NO THEN ? ELSE tt-loan.svodspostr$
      .
      /* для ставок, которые были созданы для корректного отображения в браузере
      br-comm, устанавливаем признак удаления */
   IF iMode <> {&MOD_ADD} THEN
      FOR EACH tt-comm-rate WHERE tt-comm-rate.local__template =  YES:
   ...
   FOR EACH tt-comm-cond:
      ...
      FIND LAST comm-cond WHERE
                comm-cond.contract   =  tt-comm-cond.contract
            AND comm-cond.cont-code  =  tt-comm-cond.cont-code
            AND comm-cond.commission =  tt-comm-cond.commission
            AND comm-cond.since      <= tt-comm-cond.since
      NO-LOCK NO-ERROR.
      IF AVAIL comm-cond THEN
      DO:
         IF comm-cond.since =  tt-comm-cond.since
         THEN
            tt-comm-cond.local__rowid = ROWID(comm-cond).
         ELSE IF    comm-cond.FloatType =  tt-comm-cond.FloatType
                 OR comm-cond.Action    =  tt-comm-cond.Action
   ...
   DO:
      ...
      DO:
         ...
         IF vCheckLimit = "Да" THEN
         DO:
            RUN proc-check-limit.p ("LocalSet",
                                    tt-loan.class-code,
                                    tt-loan.Contract,
                                    tt-loan.Cont-Code,
                                    DATE(tt-loan.open-date:screen-value),
            ...
            DO:
               RUN Fill-SysMes IN h_tmess ("", "", "1", 
                                           vErrorLimit).
               RETURN ERROR "Ошибка сохранения карточки договора".
            END. 
            ELSE IF {assigned vErrorLimit} THEN
               RUN Fill-SysMes IN h_tmess ("", "", "", 
                                           vErrorLimit).
         END. 
      END.
...
IF NOT AVAIL tt-loan-cond THEN
   ...
   FOR EACH tt-comm-cond WHERE tt-comm-cond.user__mode <> {&MOD_DELETE}:
   ...
   IF     iMode =  {&MOD_ADD}
      AND tt-loan.class-code =  "loan-garmc-tr-sim" THEN
   DO:
      RUN chk-lim-mcg.p (tt-instance.contract,
                         tt-instance.cont-code,
                         tt-loan.open-date,
                         tt-loan.end-date,
                         tt-loan.currency,
...
PROCEDURE Local_F9:
   ...
   IF     USERID("bisquit") <> tt-loan.user-id
...
------------------------------------------------------------------------------*/
   ...
   DEFINE VARIABLE vDopPrmComm   AS CHARACTER  NO-UNDO.
   DEFINE VARIABLE vCurrList     AS CHARACTER  NO-UNDO.
   DEFINE VARIABLE vDiffCond     AS LOGICAL    NO-UNDO.
   DEFINE VARIABLE vSchemCond    AS CHARACTER  NO-UNDO.
   ...
   DEF BUFFER bloan-cond FOR loan-cond. /* Локализация буфера. */

   /* Сохраняем старое значение gend-date. 
      При выходе из формы будем восстанавливать */
   
   IF iMode =  {&MOD_ADD} THEN
         mOldgend-date = gend-date.

   IF     AVAIL tt-Instance
      AND iMode =  {&MOD_ADD} THEN
      IF tt-Instance.cont-type =  "Течение" THEN 
      DO:
   ...
   IF     AVAIL tt-Instance THEN 
      mDayTr = INT64(GetXAttrValueEx("loan",
                                     tt-instance.contract + ',' + tt-instance.cont-code,
                                    "ОврПр",
                                    "0")).
   ELSE 
   ...
   IF iMode <> {&MOD_ADD} THEN
      tt-loan.cont-type = GetTempXAttrValueEx("loan",
                                              tt-loan.contract + "," + tt-loan.cont-code,
                                              "cont-type",
                                              tt-loan.since,
                                              tt-loan.cont-type).
   ...
   DO:
      ...
      CASE tt-loan.contract:
         WHEN "Кредит" THEN
         DO:
               /* Для различных схем условий 
                  в карточке договора отображаем только первое */
            vDiffCond = NO.
            BLOAN-COND:
            FOR EACH bloan-cond WHERE
                     bloan-cond.contract  = tt-loan.contract
                 AND bloan-cond.cont-code = tt-loan.cont-code
            NO-LOCK
            BREAK BY bloan-cond.contract 
                  BY bloan-cond.since:
               IF FIRST-OF(bloan-cond.contract) THEN
                  vSchemCond = GetXAttrValueEx("loan-cond",
                                               bloan-cond.contract + "," + 
                                               bloan-cond.cont-code + "," + 
                                               STRING(bloan-cond.since),
                                               "СхемаПлат",
               ...
               ELSE
               DO:
                  IF vSchemCond <> GetXAttrValueEx("loan-cond",
                                                   bloan-cond.contract + "," + 
                                                   bloan-cond.cont-code + "," + 
                                                   STRING(bloan-cond.since),
                                                   "СхемаПлат",
      ...
      CASE mFindLoanCond:
         WHEN "ПЕРВЫЕ" THEN
            FIND FIRST tt-loan-cond WHERE
                       tt-loan-cond.cont-code =  tt-loan.cont-code
                   AND tt-loan-cond.contract  =  tt-loan.contract
            NO-LOCK NO-ERROR.
         WHEN "ДАТАСОСТ" THEN
            FIND LAST tt-loan-cond WHERE
                      tt-loan-cond.cont-code =  tt-loan.cont-code
                  AND tt-loan-cond.contract  =  tt-loan.contract
                  AND tt-loan-cond.since     <= tt-loan.since
            NO-LOCK NO-ERROR.
      END CASE.
      ...
      DO:
         ...
         FIND FIRST bterm-obl WHERE
                    bterm-obl.contract  =  tt-loan.contract
                AND bterm-obl.cont-code =  tt-loan.cont-code
                AND bterm-obl.end-date  =  tt-loan-cond.since
   ...
   DO:
      tt-loan.open-time = TIME.
         /* дату согл-я заполняем для всех, кроме траншей */
      IF NOT AVAIL tt-instance THEN
      ...
      DO:
         ...
         IF {assigned vAcctType} AND
            ...
            /* для траншей инициализируем дату окончания
            ** датой окончания соглашения (если ДатОкончСТеч = Да) */
         IF     tt-Instance.cont-type =  "Течение"
            AND GetXattrInit(tt-loan.Class-Code,"ДатОкончСТеч") =  "Да" THEN
         DO:
            ASSIGN
               tt-loan.end-date = tt-Instance.end-date
               mSrokChange      = YES
            .
            IF     tt-loan.open-date <> ?
               AND tt-loan.end-date  <> ? THEN
            DO:
               ASSIGN
                  mChangedField = YES
               .
               {loan-trg.pro
                  &RecalcDMY      = YES                       
                  &SrokChange     = mSrokChange
                  &OpenDate       = STRING(tt-loan.open-date)
                  &EndDate        = STRING(tt-loan.end-date)
                  &HandCalcAnnuitet = mHandCalcAnnuitet
                  &LogVarChanged  = mChangedField
                  &tt-loan        = tt-loan
                  &tt-loan-cond   = tt-loan-cond
...
/* ivv begin */     
      ...
      if not {assigned vProd} then do:
         if GetSysConf("gl-g-trans") <> ? then do:
            vProd = GetSysConf("gl-g-trans").
         end.
      end.
      IF {assigned vProd}
      THEN DO:
...
/* ivv end */
         ...
         IF NUM-ENTRIES(vProd, ";") >  0 THEN
         DO:
            FIND LAST tmp-code WHERE
                         tmp-code.class      =  "ПродЛин"
                  AND    tmp-code.code       =  ENTRY( 1, vProd, ";")
                  AND    tmp-code.beg-date   <= tt-loan.open-date
                  AND   (tmp-code.end-date   >= tt-loan.open-date
                     OR  tmp-code.end-date   =  ?)
            NO-LOCK NO-ERROR.
            ...
            ASSIGN
               ...
               tt-loan-cond.NDays      = IF vProdType =  "Д" THEN INT64(ENTRY( 4, vProd, ";")) ELSE tt-loan-cond.NDays
               tt-loan-cond.NMonthes   = IF vProdType =  "М" THEN INT64(ENTRY( 4, vProd, ";")) ELSE tt-loan-cond.NMonthes
               tt-loan-cond.NYears     = IF vProdType =  "Г" THEN INT64(ENTRY( 4, vProd, ";")) ELSE tt-loan-cond.NYears
               tt-loan-cond.prodtrf$   = ENTRY( 7, vProd, ";")
               tt-loan.cont-type       = ENTRY( 8, vProd, ";")
              .
               /* ОД */
            IF NUM-ENTRIES (vProdPogOD) >= 9 THEN
            DO:
               ASSIGN
                  ...
                  tt-loan-cond.delay1       = INT64(ENTRY(8,vProdPogOD))
                  delay-offset_             = ENTRY(9,vProdPogOD)
                  tt-loan-cond.delay-offset = ENTRY(9,vProdPogOD)
               .
               IF ENTRY(6,vProdPogOD) = "КоличДней" THEN 
                  tt-loan-cond.cred-work-calend = LOGICAL(ENTRY(7,vProdPogOD),
                                                          "Календ/Рабоч").
               IF ENTRY(6,vProdPogOD) = "ДатаОконч" THEN 
                  tt-loan-cond.cred-curr-next = LOGICAL(ENTRY(7,vProdPogOD),
                                                        "Текущ/След").
            END.  
               /* %% */
            IF NUM-ENTRIES (vProdPogPr) >= 9 THEN
            DO:
               ASSIGN
                  ...
                  tt-loan-cond.delay            = INT64(ENTRY(8,vProdPogPr))
                  delay-offset-int_             = ENTRY(9,vProdPogPr)
                  tt-loan-cond.delay-offset-int = ENTRY(9,vProdPogPr)
               .
               IF ENTRY(6,vProdPogPr) = "КоличДней" THEN 
                  tt-loan-cond.int-work-calend = LOGICAL(ENTRY(7,vProdPogPr),
                                                         "Календ/Рабоч").
               IF ENTRY(6,vProdPogPr) = "ДатаОконч" THEN 
                  tt-loan-cond.int-curr-next   = LOGICAL(ENTRY(7,vProdPogPr),
                                                         "Текущ/След").
            END.
            ...
            IF tt-loan.class-code = "loan_dbl_ann" 
                AND NUM-ENTRIES(vProd, ";") >= 18 THEN
            DO:
                ASSIGN
                    tt-loan-cond.PartAmount  = DEC(ENTRY(18 , vProd, ";"))
                    tt-loan-cond.FirstPeriod = INT64(ENTRY(17 , vProd, ";"))
                    .
   ...
   DO WITH FRAME {&MAIN-FRAME}:
      IF    tt-loan.open-date <> ?
        AND tt-loan.end-date  <> ?
        AND ( tt-loan-cond.NDays  =  0
        AND  tt-loan-cond.NMonth =  0
        AND  tt-loan-cond.NYears =  0 )
         OR ( tt-loan-cond.NDays =  ?
         OR tt-loan-cond.NMonth =  ?
         OR tt-loan-cond.NYears =  ? ) THEN
      DO:
   ...
   END.
      /* Свод */
   ASSIGN
      tt-loan.svodgrafik$ = IF tt-loan.svodgrafik$ =  ? THEN NO ELSE tt-loan.svodgrafik$
      tt-loan.svodskonca$ = IF tt-loan.svodskonca$ =  ? THEN NO ELSE tt-loan.svodskonca$
      tt-loan.svodgravto$ = IF tt-loan.svodgravto$ =  ? THEN NO ELSE tt-loan.svodgravto$
      tt-loan.svodspostr$ = IF tt-loan.svodspostr$ =  ? THEN NO ELSE tt-loan.svodspostr$
   ...
   IF tt-loan-cond.grperiod$ =  ? THEN
      tt-loan-cond.grperiod$ = NO.
   
   FIND FIRST bloan WHERE
              bloan.contract  =  tt-loan.contract
        AND bloan.cont-code =  tt-loan.cont-code
   NO-LOCK NO-ERROR.
   
      /* установим валюту комиссии, если не установлена */
   FOR EACH tt-comm-rate:
      IF tt-comm-rate.comm-rate-curr = ? THEN
         tt-comm-rate.comm-rate-curr = GetCommRateCurrency (BUFFER comm-rate,
                                                            BUFFER bloan,
                                                            tt-comm-rate.commission,
                                                            tt-comm-rate.since).
   END.
   ...
   DO:
      ...
      vDopPrmComm = GetXAttrValueEx ("loan-cond",
                                     SUBSTITUTE("&1,&2,&3",
                                                tt-loan.contract,
                                                tt-loan.cont-code,
      ...
      RUN GetTTRateCondMCGTransh IN THIS-PROCEDURE (vDopPrmComm, NO).
   END.
   
   IF     iMode =  {&MOD_ADD}
      AND tt-loan.class-code =  "loan-garmc-tr-sim" THEN
   DO:
      IF tt-loan.datasogl$ = ? THEN
         FIND LAST loan-cond WHERE
                   loan-cond.contract  =  tt-instance.contract
               AND loan-cond.cont-code =  tt-instance.cont-code
               AND loan-cond.since     <= DATE(tt-loan.open-date:SCREEN-VALUE)
      ...
      RUN SetSysConf IN h_base ("СписВалМБГ", vCurrList).
   END.   
...
------------------------------------------------------------------------------*/
   ...
   DEFINE VARIABLE vRet           AS CHAR    NO-UNDO.
   DEFINE VARIABLE vRet1          AS CHAR    NO-UNDO.

   DEFINE VARIABLE vChangeSumm    AS LOG     NO-UNDO.
   DEFINE VARIABLE vChangePr      AS LOG     NO-UNDO.
   DEFINE VARIABLE vChangeDate    AS LOG     NO-UNDO.
   DEFINE VARIABLE vChangePer     AS LOG     NO-UNDO.
   DEFINE VARIABLE vGrRisk        AS INT64   NO-UNDO.
   DEFINE VARIABLE vListGrRiska   AS CHAR    NO-UNDO.

   DEFINE VARIABLE fl-error       AS INT64   NO-UNDO.
   DEFINE VARIABLE vObespTran     AS CHAR    NO-UNDO.
   DEFINE VARIABLE vOk            AS LOG     NO-UNDO.
   DEFINE VARIABLE vCondCount     AS INT64   NO-UNDO.
   DEFINE VARIABLE vCheckLimit    AS CHAR    NO-UNDO.

   DEFINE VARIABLE vGrRiskVn      AS INT64   NO-UNDO.
   DEFINE VARIABLE vMinRate       AS DEC     NO-UNDO.
   DEFINE VARIABLE vPayDateMove   AS CHAR    NO-UNDO. /* ДР РасчОкончСдвиг */

   DEFINE VARIABLE vAcctTypeList  AS CHAR    NO-UNDO.
   DEFINE VARIABLE vCounter       AS INT64   NO-UNDO.
   DEFINE VARIABLE vPrefLimDate   AS DATE    NO-UNDO.
   DEFINE VARIABLE vRetLim        AS CHAR    NO-UNDO.
   DEFINE VARIABLE vLCRecNew      AS RECID   NO-UNDO.
   DEFINE VARIABLE vLCSummNew     AS DEC     NO-UNDO.
   DEFINE VARIABLE vRaschInstr    AS CHAR    NO-UNDO.
   DEFINE VARIABLE vSigns         AS CHAR    NO-UNDO.
   DEFINE VARIABLE vCodePS        AS CHAR    NO-UNDO.
   DEFINE VARIABLE vVal           AS CHAR    NO-UNDO.
   DEFINE VARIABLE vI             AS INT64   NO-UNDO.
   DEFINE VARIABLE vEps           AS DEC     NO-UNDO.
   DEFINE VARIABLE vEpsStr        AS CHAR    NO-UNDO.   
   DEFINE VARIABLE vEPSTypeDVyd   AS CHAR    NO-UNDO.
   DEFINE VARIABLE vDopPrmComm    AS CHAR    NO-UNDO.
   DEFINE VARIABLE vDopPrmOneComm AS CHAR    NO-UNDO.
   DEFINE VARIABLE vSummComm      AS DEC     NO-UNDO.
   DEFINE VARIABLE vErrorLimit    AS CHAR    NO-UNDO.
   DEFINE VARIABLE vGrRiskaSogl   AS CHAR    NO-UNDO.
   DEFINE VARIABLE vSummProc      AS DEC     NO-UNDO.
   DEFINE VARIABLE vAcctCreate    AS LOGICAL NO-UNDO.
   ...
   DEFINE BUFFER buf_comm-rate FOR comm-rate.

   IF iMode =  {&MOD_ADD} 
   THEN DO:
      
      vSigns = GetRefVal("ДрПродукт", 
                         tt-loan.open-date,
                         trim(tt-loan.prodkod$)).
      DO vI =1 TO NUM-ENTRIES(vSigns):
         ASSIGN
            vCodePS = ENTRY(1,ENTRY(vI,vsigns),":")
            vVal    = ENTRY(2,ENTRY(vI,vsigns),":")
         .
         UpdateSigns(tt-loan.Class-Code,
                     ...
                     ?).
      END.
      RUN SetSysConf IN h_Base ("RUN_ON_f_mm_MOD_ADD", "YES"). 
   END.
   /* Сохраняем сумму по договору */
   IF    iMode =  {&MOD_ADD}
      OR iMode =  {&MOD_EDIT} THEN
   DO:                
         UpdateSigns(tt-loan.Class-Code,
                     tt-loan.contract + "," + tt-loan.cont-code,
                     "СуммаДог",
   ...
   IF   (iMode =  {&MOD_ADD}
      OR iMode =  {&MOD_EDIT})
     AND tt-loan.contract =  "Кредит"
     AND tt-loan.cust-cat:SCREEN-VALUE IN FRAME {&MAIN-FRAME} =  "Ч"
     AND GetXAttrValueEx("person",LEFT-TRIM(tt-loan.cust-id:SCREEN-VALUE IN FRAME {&MAIN-FRAME},"0"),"Субъект","") =  "ФЛП" 
   THEN
   DO:                
      REPEAT:
         RUN Fill-SysMes("","","3","Выберите тип|Физическое лицо,Физ.лицо-предприниматель").
         IF pick-value <> "0" THEN LEAVE.
      END.
      UpdateSigns(tt-loan.Class-Code,
                  tt-loan.contract + "," + tt-loan.cont-code,
                  "Субъект",
                  IF pick-value =  "1" THEN "ФЛ" ELSE "ФЛП",
                  ?).
   END.

   IF     AVAIL tt-Instance
      AND iMode =  {&MOD_ADD} THEN /* копирование ДР с охватывающего договора */
      IF     tt-Instance.Cont-code =  ENTRY(1, tt-loan.Cont-code, " ")
         AND tt-Instance.cont-type =  "Течение" THEN 
      DO:
   ...
   IF iMode =  {&MOD_ADD} THEN /* проверка на соответствие счетов клиенту */
   DO:
      ...
      FIND FIRST loan WHERE loan.contract  =  tt-loan.contract
      ...
      IF INDEX(tt-loan.cont-code, " ") =  0 THEN
         ...
         IF AVAILABLE tt-instance THEN DO:
            ...
            REPEAT vCounter = 1 TO NUM-ENTRIES(vAcctTypeList):
               FIND LAST bloan-acct WHERE
                        bloan-acct.contract  =  tt-instance.contract
                     AND bloan-acct.cont-code =  tt-instance.cont-code
                     AND bloan-acct.acct-type =  ENTRY(vCounter, vAcctTypeList)
                     AND bloan-acct.since     <= tt-loan.open-date
               NO-LOCK NO-ERROR.
               IF NOT AVAIL bloan-acct THEN
                  NEXT.
               FIND FIRST loan-acct WHERE
                        loan-acct.contract  =  tt-loan.contract
                     AND loan-acct.cont-code =  tt-loan.cont-code
                     AND loan-acct.acct-type =  bloan-acct.acct-type
                     AND loan-acct.since     =  tt-loan.open-date
               NO-LOCK NO-ERROR.
      ...
      DO:
         ...
         RUN Fill-SysMes IN h_tmess ("","","4", 
             "Клиент является фигурантом справочника Стоп-листы!" + 
             " Открывать счета для договора?").
             
         vAcctCreate = (pick-value = "YES").
      END. 
      ...
      END.
         ...
         vObespTran = GetXAttrInit(loan.class-code,"Op-Kind_Obesp").
         IF {assigned vObespTran} THEN
         DO:
            FIND FIRST Op-Kind WHERE
                       Op-Kind.Op-Kind = vObespTran
            NO-LOCK NO-ERROR.
      ...
      /* Сохранение лимитов */
         IF tt-loan.rewzim$:SCREEN-VALUE <> ? AND
            tt-loan.rewzim$:SCREEN-VALUE <> ""  AND
            tt-loan.LimitGrafDate =  ?
         THEN DO:
            tt-loan.LimitGrafDate = tt-loan.end-date .
         END.
      ...
      DO:
         ...
         DO:
            ...
            .
            ELSE ASSIGN
               vChangeSumm = mAmount <> mSumma-sd
               vChangePr   =
                  mCredPeriod <> tt-loan-cond.Cred-Period OR
                  mCredDate <> tt-loan-cond.Cred-Date OR
                  mDelay1 <> tt-loan-cond.Delay1 OR
                  mCredOffset <> tt-loan-cond.cred-offset OR
                  mCountPer <> tt-loan-cond.kollw#gtper$
               vChangeDate = mEndDate <> tt-loan.end-date
               vChangePer =   mIntPeriod  <> tt-loan-cond.int-Period
                           OR mIntDate    <> tt-loan-cond.int-Date
            .
         
            IF tt-loan.class-code = "loan_dbl_ann" THEN
            DO:
               FIND FIRST loan WHERE RECID(loan) =  vRecLoan NO-LOCK NO-ERROR .
               FIND FIRST loan-cond WHERE
                          loan-cond.contract  =  loan.contract
                      AND loan-cond.cont-code =  loan.cont-code 
               NO-LOCK NO-ERROR.
            ...
            ELSE DO:
                ...
                IF    tt-loan.class-code =  "loan-transh-ann"
                   OR tt-loan.class-code =  "loan-tran-lin-ann" THEN
                DO:
                   /* перенос графиков */
                   RUN loansvodgr.p(tt-loan.contract,ENTRY(1,tt-loan.cont-code," "),tt-loan-cond.since,"get").
                   /* для каждого транша */
                   FOR EACH tloan WHERE (tloan.contract  =  tt-loan.contract
                                     AND tloan.cont-code BEGINS ENTRY(1,tt-loan.cont-code, " ") + " "
                                     AND INDEX(tloan.cont-code," ") <>  0)
                                    OR  (tloan.contract  =  tt-loan.contract
                                     AND tloan.cont-code =  ENTRY(1,tt-loan.cont-code," "))
                       NO-LOCK,
                       
                       LAST tloan-cond WHERE tloan-cond.contract  =  tloan.contract
                                         AND tloan-cond.cont-code =  tloan.cont-code
                      ...
                      END.
                      /* пересчет графиков */
                      IF NUM-ENTRIES(tloan.cont-code, " ") =  1 THEN
                         RUN SetSysConf IN h_Base("НЕ ВЫВОДИТЬ ГРАФИКИ НА ЭКРАН","ДА").
                      IF     NUM-ENTRIES(tloan.cont-code, " ") >  1 
                         AND tloan.cont-code <> tt-loan.cont-code THEN
                         FIND FIRST term-obl OF tloan WHERE term-obl.idnt =  2 NO-LOCK NO-ERROR.
                      RUN mm-to.p(RECID(tloan),
                                  RECID(tloan-cond),
                                  (IF tloan.cont-code =  tt-loan.cont-code OR NOT AVAIL term-obl THEN tt-amount.amt-rub ELSE term-obl.amt-rub),
                                  (IF tloan.cont-code =  tt-loan.cont-code THEN iMode ELSE {&MOD_EDIT}),
                                  vChangeSumm,
                                  vChangePr  ,
                                  vChangeDate,
                                  vChangePer,
                   ...
                   IF NUM-ENTRIES(tt-loan.cont-code, " ") >= 1 THEN
                   DO:
                      ...
                      DO:
                         ...
                         DO TRANSACTION:
                            FIND FIRST loan-cond WHERE 
                                       loan-cond.contract  =  tt-loan.contract
                                   AND loan-cond.cont-code =  ENTRY(1,tt-loan.cont-code," ") 
                            EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
                            IF AVAIL loan-cond THEN
                            DO:
                               ASSIGN
            ...
            RUN SaveGraphToHistory IN h_loan (tt-loan.Contract,
                                              tt-loan.Cont-Code,
                                              tt-loan-cond.since,
                                              if iMode =  {&MOD_ADD} then "ВВУСЛ" else "РУСЛ",
                                              (if iMode =  {&MOD_ADD} then "Ввод нового условия :" else "Редактирование условия:") + tt-loan-cond.contract + "," + tt-loan-cond.cont-code + "," + STRING(tt-loan-cond.since),
                                               NO,
                                               ?,
                                               NO,
                                               OUTPUT mErrSaveHist).
         ...
         DO:
            ...
            DO:
               ...
               IF vCheckLimit = "Да" THEN
               DO:
                  RUN proc-check-limit.p ("PostSet",
                                          tt-loan.class-code,
                                          tt-loan.Contract,
                                          tt-loan.Cont-Code,
                                          DATE(tt-loan.open-date:screen-value),
            ...
            END.
         END.    
         END. /* IF tt-loan.class-code <> "loan-garmc-tr-sim" THEN */
            IF iMode =  {&MOD_ADD} AND tt-loan.alt-contract =  "mm" THEN
            DO:
            ...
            IF vRet =  {&RET-ERROR} OR vRet1 =  {&RET-ERROR} THEN
            DO:
               mRetVal = {&RET-ERROR}.
         ...
         END.
         IF iMode =  {&MOD_ADD} AND
            tt-loan.stream-show =  YES AND
            tt-loan.cont-type =  "Течение" THEN
         DO TRANS:
         ...
         IF NOT AVAILABLE comm-rate
            AND GetXAttrValueEx("loan",
                             tt-loan.contract + "," + tt-loan.cont-code,
                             "ИндРезервТранш",
                             GetXAttrInit(tt-loan.class-code,
                                          "ИндРезервТранш")
                             ) <> "нет"
         ...
         DO:
             RUN GET_COMM_LOAN_BUF IN h_Loan (tt-loan.contract,
                                             ...
                                             tt-loan.since,
                                             BUFFER comm-rate).
             /* Если % резерва на охватывающем договоре не равен тому, что ввели на
             транше, добавляем новую запись */
             IF     AVAIL comm-rate
                AND comm-rate.rate-comm <> mRisk THEN
             DO TRANSACTION:
                FIND LAST bcomm-rate WHERE 
                          bcomm-rate.kau = tt-loan.contract + "," +
                                           ENTRY (1, tt-loan.cont-code, " ")
                      AND bcomm-rate.commission = "%Рез"
                NO-LOCK NO-ERROR.
                /* создаем на охватывающем договоре %Рез от даты течения */
                CREATE comm-rate.
                ASSIGN
                   comm-rate.kau        = tt-loan.contract + "," + 
                                          ENTRY (1, tt-loan.cont-code, " ")
                   comm-rate.currency   = tt-loan.currency
                   comm-rate.rate-comm  = mRisk
                   comm-rate.acct       = "0"
                   comm-rate.commission = "%Рез"
                   comm-rate.min-value  = 0
                   comm-rate.period     = 0
                   comm-rate.since      = IF AVAIL bcomm-rate THEN 
                                             MAX(bcomm-rate.since + 1,tt-loan.open-date)
                                          ELSE tt-loan.open-date.
                VALIDATE comm-rate. 
             END.
         END.
         ...
         FOR EACH tt-comm-rate NO-LOCK:
            ...
            msurrcr2 = STRING(tt-comm-rate.commission) + "," +
                       STRING(tt-comm-rate.acct) + "," +
                       STRING(tt-loan.currency) + "," +
                       PushSurr(STRING(tt-loan.contract) + "," + 
                                STRING(tt-loan.cont-code)) + "," +
                       string(tt-comm-rate.min-value) + "," +
                       STRING(tt-comm-rate.period) + "," +
                       STRING(tt-loan-cond.since)
                  .
            FIND FIRST buf_comm-rate NO-LOCK WHERE
                    buf_comm-rate.commission = tt-comm-rate.commission    AND
                    buf_comm-rate.acct       = tt-comm-rate.acct          AND
                    buf_comm-rate.currency   = tt-loan.currency      AND
                    buf_comm-rate.kau        = tt-loan.contract + "," + 
                                               tt-loan.cont-code  AND
                    buf_comm-rate.min-value  = tt-comm-rate.min-value      AND
                    buf_comm-rate.period     = tt-comm-rate.period         AND
                    buf_comm-rate.since      = tt-loan-cond.since
                    NO-ERROR .

            UpdateSignsEx("comm-rate-loan",
                           GetSurrogateBuffer("comm-rate", (BUFFER buf_comm-rate:HANDLE)),
                          "МинЗнач",
                          STRING(tt-comm-rate.min_value)
               ) .
   
            IF tt-comm-rate.commission = "%Рез"
            THEN DO:
               ...
               THEN DO:
                  ...
                  THEN
                     UpdateSignsEx("comm-rate-loan",
                                   STRING(tt-comm-rate.comm-rate-id),
                                   "КатегорияКач",
                                   STRING(mGrRiska)).
         ...
         /* Если это Течение и на классе установлен ДР ИндРезервТранш = НЕТ,
            то копируем %Рез с охватывающего договора. */
      IF     iMode = {&MOD_ADD}
         AND NUM-ENTRIES (tt-loan.cont-code, " ") = 2
         AND CAN-FIND (FIRST loan WHERE 
                             loan.contract  = tt-loan.contract
      ...
      BLK_INDREZ:
      DO:
            /* Находим последний %Рез охватывающего договора на дату открытия
               течения и копируем его с датой открытия течения */
         FIND LAST comm-rate WHERE
                   comm-rate.kau        =  tt-loan.contract + "," + ENTRY(1, tt-loan.cont-code, " ")
               AND comm-rate.commission =  "%Рез"
               AND comm-rate.since      <= tt-loan.open-date
         NO-LOCK NO-ERROR.
         
         IF AVAIL comm-rate THEN
         DO TRANSACTION:
            CREATE bcomm-rate.
            BUFFER-COPY comm-rate EXCEPT kau comm-rate-id since TO bcomm-rate
            ASSIGN 
               bcomm-rate.kau   = tt-loan.contract + "," + tt-loan.cont-code
               bcomm-rate.since = tt-loan.open-date
            NO-ERROR. 
            vGrRiskaSogl = GetXAttrValueEx("comm-rate",
            ...
            UpdateSignsEx("comm-rate-loan",
                          GetSurrogateBuffer("comm-rate", 
                                             BUFFER bcomm-rate:HANDLE),
                          "КатегорияКач",
                          STRING(vGrRiskaSogl)).
         END.
            /* Если после даты открытия течения на охватывающем договоре
               есть еще %Рез, то и их копируем */
         ...
         NO-LOCK:
            CREATE bcomm-rate.
            BUFFER-COPY comm-rate EXCEPT kau comm-rate-id TO bcomm-rate
            ASSIGN 
               bcomm-rate.kau = tt-loan.contract + "," + tt-loan.cont-code 
            NO-ERROR.
            vGrRiskaSogl = GetXAttrValueEx("comm-rate",
                                           ...
                                           "").
            UpdateSignsEx("comm-rate-loan",
                          STRING(bcomm-rate.comm-rate-id),
                          "КатегорияКач",
                          STRING(vGrRiskaSogl)).
         END.
      END.
         ...
         DO ON ERROR  UNDO BLK, LEAVE BLK
            ...
            RUN LnGetRiskGrList IN h_i254 (mRisk,
                                           ...
                                           YES,
                                           OUTPUT vListGrRiska).
            FIND LAST loan-acct WHERE loan-acct.contract  =  tt-loan.contract
                                  AND loan-acct.cont-code =  tt-loan.cont-code
                                  AND loan-acct.acct-type =  "КредЛин"
                                  AND loan-acct.since     <= tt-loan.open-date
            NO-LOCK NO-ERROR.
            IF NOT AVAIL loan-acct THEN
               FIND LAST loan-acct WHERE loan-acct.contract  =  tt-loan.contract
                                     AND loan-acct.cont-code =  tt-loan.cont-code
                                     AND loan-acct.acct-type =  "КредН"
                                     AND loan-acct.since     <= tt-loan.open-date
               NO-LOCK NO-ERROR.
            ...
            IF     NOT AVAIL loan-acct
               ...
               AND IF IsTemporal("acct","ИндПрРез")
                    THEN (GetTempXAttrValueEx("acct",loan-acct.acct + ',' + loan-acct.currency,"ИндПрРез",tt-loan.open-date,"Нет") <> "Да")
                    ELSE (GetXAttrValue("acct",loan-acct.acct + ',' + loan-acct.currency,"ИндПрРез") <> "Да"))
            THEN LEAVE BLK.
            ...
            vGrRiskVn = IF CAN-DO(vListGrRiska,STRING(mGrRiska)) THEN mGrRiska ELSE vGrRisk.
                     /* Определяем минимальное для устанавливаемой категории качества значение к-та резервирования */
            RUN LnGetPersRsrvOnDate IN h_i254 (vGrRiskVn,tt-loan.open-date,OUTPUT vMinRate).

                     /* если мин.ставка для кат.кач-ва и установленная на счету различаются */
            IF vMinRate <> GET_COMM("%Рез",RECID(acct),acct.currency,"",0,0,tt-loan.open-date) THEN
            BLK2:
            DO ON ERROR  UNDO BLK2, LEAVE BLK2
               ...
               IF LAST-EVENT:FUNCTION =  "END-ERROR" THEN LEAVE BLK2.
   
               FIND FIRST bcomm-rate WHERE bcomm-rate.commission =  "%Рез"
                                       AND bcomm-rate.filial-id  =  shfilial
                                       AND bcomm-rate.branch-id  =  ""
                                       AND bcomm-rate.acct       =  acct.acct
                                       AND bcomm-rate.currency   =  acct.currency
                                       AND bcomm-rate.kau        =  ""
                                       AND bcomm-rate.MIN-VALUE  =  0
                                       AND bcomm-rate.period     =  0
                                       AND bcomm-rate.since      =  tt-loan.open-date
               EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
   
               IF NOT AVAIL bcomm-rate THEN
                  IF LOCKED bcomm-rate THEN
                  DO:
                     RUN wholocks2.p (RECID(bcomm-rate),
                                      (BUFFER bcomm-rate:TABLE),
                                      "Ошибка захвата записи с комиссией %Рез.").
      ...
      DO:
         FIND FIRST b-loan WHERE
                    b-loan.contract  =  tt-loan.contract
            AND     b-loan.cont-code =  ENTRY(1, tt-loan.cont-code, " ")
         NO-LOCK NO-ERROR.
         IF     AVAIL b-loan 
            AND b-loan.class-code <> "loan-transh-ann" THEN
         DO:
            IF GetXAttrValue("loan", b-loan.contract + "," + b-loan.cont-code, "СводГрАвто") =  "Да" THEN
            DO:
     ...
     /* Для ввода ПОС в новом договоре */
      ...
      IF iMode =  {&MOD_ADD} THEN
      DO:
         IF GetXAttrInit(tt-loan.class-code,"РасчЭПС") =  "ДА" THEN
         DO:          
            vEps = GetEpsLoan(tt-loan.contract,
                              tt-loan.cont-code,
                              tt-loan.open-date) * 100.
            vEpsStr = GetRoundEpsLoan (tt-loan.Class-Code,
                                       vEps).          
            UpdateTempSignsEx(tt-loan.Class-Code,
                        tt-loan.contract + "," + tt-loan.cont-code,
                        "ЭПС",
                        tt-loan.open-date,
                        vEpsStr,
            ...
            vEPSTypeDVyd = fGetSetting ("ЭПС", "ЭПСТипДВып", "").
         
            IF CAN-DO(vEPSTypeDVyd, tt-loan.cont-type) THEN
            DO:
               vEps = GetEpsLoanEx(tt-loan.contract,
                                   tt-loan.cont-code,
                                   tt-loan.open-date,
                                   "ЦБРеф") * 100.
               vEpsStr = GetRoundEpsLoanEx (tt-loan.Class-Code,
                                            vEps,
                                            "ПСКВыпДох").
               UpdateSigns(tt-loan.Class-Code,
                           tt-loan.contract + "," + tt-loan.cont-code,
   ...
   DO:
      vDopPrmComm = "".
      FOR EACH ttRateCondMCGTransh NO-LOCK:
         ASSIGN
            vDopPrmOneComm = SUBSTITUTE("&1=&2,&3,&4,&5,&6",
                                        ttRateCondMCGTransh.fCommission,
                                        ttRateCondMCGTransh.fCurrency,
                                        STRING(ttRateCondMCGTransh.fPrDateBeg),
                                        STRING(ttRateCondMCGTransh.fPermMethCalc),
                                        ttRateCondMCGTransh.fPermMethCalcPer,
                                        STRING(ttRateCondMCGTransh.fPermMethCalcCount))
            vDopPrmOneComm = SUBSTITUTE("&1,&2,&3,&4,&5,&6",
                                        vDopPrmOneComm,
                                        STRING(ttRateCondMCGTransh.fPermRate),
                                        STRING(ttRateCondMCGTransh.fPermMinRate),
                                        ttRateCondMCGTransh.fPermMinRateCurr,
                                        STRING(ttRateCondMCGTransh.fPostRate),
                                        ttRateCondMCGTransh.fPostCurr)
            vDopPrmComm    = vDopPrmComm + 
                             (IF {assigned vDopPrmComm} THEN
                                 "|"
                              ELSE
                                 "") +  
                             vDopPrmOneComm
      ...
      UpdateSigns (tt-loan-cond.class-code,
                   SUBSTITUTE("&1,&2,&3",
                              tt-loan.contract,
                              tt-loan.cont-code,
                              STRING(tt-loan-cond.since)),
      ...
      RUN clc-tobl-mcg-form.p (tt-loan.contract,
                               tt-loan.cont-code,
                               tt-loan-cond.since,
                               "%ВыдМБГ",
                               NO,
      ...
      IF iMode =  {&MOD_ADD} THEN
         UpdateSigns (tt-loan-cond.class-code,
                      SUBSTITUTE("&1,&2,&3",
                                 ...
                                 STRING(tt-loan-cond.since)),
                      "ДатаЗакл",
                      STRING(tt-loan.datasogl$),
                      ?).
   END.
   IF iMode =  {&MOD_ADD} AND tt-loan.contract = "ДЕПОЗ" THEN
   DO:
      ...
      FOR EACH term-obl WHERE
           ...
           AND term-obl.idnt = 1
      NO-LOCK:
         vSummProc = vSummProc + term-obl.amt-rub. 
      END.
      
      UpdateTempSignsEx(loan.class-code,
                        loan.contract + "," + loan.cont-code,
                        "ПСВСумПроц",
                        loan.open-date,
                        STRING(vSummProc),
   ...
   FINALLY:
      RUN DeleteOldDataProtocol IN h_base("RUN_ON_f_mm_MOD_ADD").
   END FINALLY.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
   ...
   IF     iTariffCode <> ?
      AND iTariffCode <> ""
      AND iTariffCode <> "?" THEN
   DO:
         /* Тянем все ставки по коду тарифа */
      cc:
      FOR EACH xbcomm-rate WHERE
               xbcomm-rate.kau =  "ПродТрф," + iTariffCode
      NO-LOCK BREAK BY xbcomm-rate.commission:
         IF FIRST-OF(xbcomm-rate.commission) THEN
         DO:
            FIND LAST comm-rate WHERE               
                      comm-rate.kau        =  xbcomm-rate.kau
                AND   comm-rate.commission =  xbcomm-rate.commission
                AND   comm-rate.currency   =  tt-loan.currency
                AND   comm-rate.since      <= tt-loan.open-date
            NO-LOCK NO-ERROR.
           IF   NOT AVAIL comm-rate 
             OR comm-rate.rate-comm <= 0 THEN
              NEXT cc.
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH tt-comm-rate:
      FIND LAST comm-rate WHERE
                comm-rate.kau        =  "Модификатор," + iProd
            AND comm-rate.commission =  tt-comm-rate.commission
            AND comm-rate.since      <= tt-loan.open-date
      NO-LOCK NO-ERROR.
      IF AVAIL comm-rate THEN DO:
...
FUNCTION GetQntPer RETURNS INT64 (
   ...
   DO WHILE mCurDate < iEndDate:
      ...
      IF (CAN-FIND (FIRST ttTermObl WHERE ttTermObl.contract  =  tt-loan.contract
                                     AND ttTermObl.cont-code =  tt-loan.cont-code
                                     AND ttTermObl.idnt      =  200
                                     AND INT64(ttTermObl.amt)  =  MONTH(mCurDate)
                                     AND ttTermObl.end-date  <= mCurDate
                   NO-LOCK)
         /* и месяца и года даты нет в списке ВКЛЮЧИТЬ */
         AND NOT CAN-FIND
                  (FIRST ttTermObl WHERE ttTermObl.contract   =  tt-loan.contract
                                     AND ttTermObl.cont-code  =  tt-loan.cont-code
                                     AND ttTermObl.idnt       =  201
                                     AND ttTermObl.sop-offbal =  1         /* ВКЛЮЧИТЬ */
                                     AND INT64(ttTermObl.amt)   =  MONTH(mCurDate)
                                     AND INT64(ttTermObl.sop)   =  YEAR (mCurDate)
                   NO-LOCK))
         ...
         OR CAN-FIND
                  (FIRST ttTermObl WHERE ttTermObl.contract   =  tt-loan.contract
                                     AND ttTermObl.cont-code  =  tt-loan.cont-code
                                     AND ttTermObl.idnt       =  201
                                     AND ttTermObl.sop-offbal <> 1         /* ИСКЛЮЧИТЬ */
                                     AND INT64(ttTermObl.amt)   =  MONTH(mCurDate)
                                     AND INT64(ttTermObl.sop)   =  YEAR (mCurDate)
                   NO-LOCK)
...
{pfuncdef 
&DefFunc="CheckCli"
&Description="Определяет, является ли клиент поручителем просроченного договора"}

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION CheckCli TERMINAL-SIMULATION
FUNCTION CheckCli RETURNS LOGICAL 
  ( INPUT iCustCat AS CHAR,
    INPUT iCustId  AS INT64,
    INPUT iDate    AS DATE ) :

   DEFINE VARIABLE vProcFlag    AS LOGICAL NO-UNDO.  
   DEFINE VARIABLE vProcMaxSrok AS INT64   NO-UNDO.
   DEFINE VARIABLE vSumSsud     AS DEC     NO-UNDO.  
   DEFINE VARIABLE vSumSsudProc AS DEC     NO-UNDO.
   ...
   CASE iCustCat:
      ...
      DO:
         FIND FIRST person WHERE
                    person.person-id =  iCustId
         NO-LOCK NO-ERROR.

         IF AVAIL person THEN
            FOR EACH term-obl WHERE
                     term-obl.symbol   =  iCustCat
                 AND term-obl.fop      =  person.person-id
            ...
            NO-LOCK:
               ...
               FIND FIRST loan WHERE
                          loan.contract  =  term-obl.contract
                      AND loan.cont-code =  term-obl.cont-code
                      AND loan.end-date  >= iDate
               NO-LOCK NO-ERROR.

               IF AVAIL loan THEN
               DO:

                  RUN CalcDebt115 IN h_pqres (loan.contract,       
                                              loan.cont-code,      
                                              iDate,               
                                              OUTPUT vProcFlag,    
                                              OUTPUT vProcMaxSrok, 
      ...
      DO:
         FIND FIRST cust-corp WHERE
                    cust-corp.cust-id =  iCustId
         NO-LOCK NO-ERROR.

         IF AVAIL cust-corp THEN
            FOR EACH term-obl WHERE
                     term-obl.symbol   =  iCustCat
                 AND term-obl.fop      =  cust-corp.cust-id
                 AND term-obl.contract =  "Кредит"
                 AND term-obl.idnt     =  5
                 AND term-obl.end-date >= iDate
            NO-LOCK:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE GetTTRateCondMCGTransh TERMINAL-SIMULATION
/* Процедура получения значения из ДР */
PROCEDURE GetTTRateCondMCGTransh PRIVATE:
   DEF INPUT PARAM iValue   AS CHAR NO-UNDO.
   DEF INPUT PARAM iRefresh AS LOG  NO-UNDO.

   DEF VAR vI             AS INT64 NO-UNDO.
   DEF VAR vDopPrmOneComm AS CHAR  NO-UNDO.
   
   DEF BUFFER btt-comm-rate FOR tt-comm-rate.
   
   {empty ttRateCondMCGTransh}
     
   IF {assigned iValue} THEN
   DO vI = 1 TO NUM-ENTRIES(iValue, "|"):
      vDopPrmOneComm = ENTRY(vI, iValue, "|").
       
      CREATE ttRateCondMCGTransh.
      ASSIGN
         ttRateCondMCGTransh.fCommission        = ENTRY(1, vDopPrmOneComm, "=")
         vDopPrmOneComm                         = ENTRY(2, vDopPrmOneComm, "=")
         ttRateCondMCGTransh.fCurrency          = ENTRY(1, vDopPrmOneComm)
         ttRateCondMCGTransh.fPrDateBeg         = INT64(ENTRY(2, vDopPrmOneComm))
         ttRateCondMCGTransh.fPermMethCalc      = INT64(ENTRY(3, vDopPrmOneComm))
         ttRateCondMCGTransh.fPermMethCalcPer   = ENTRY(4, vDopPrmOneComm)
         ttRateCondMCGTransh.fPermMethCalcCount = INT64(ENTRY(5, vDopPrmOneComm))
         ttRateCondMCGTransh.fPermRate          = DEC(ENTRY(6, vDopPrmOneComm))
         ttRateCondMCGTransh.fPermMinRate       = DEC(ENTRY(7, vDopPrmOneComm))
         ttRateCondMCGTransh.fPermMinRateCurr   = ENTRY(8, vDopPrmOneComm)
         ttRateCondMCGTransh.fPostRate          = DEC(ENTRY(9, vDopPrmOneComm))
         ttRateCondMCGTransh.fPostCurr          = ENTRY(10, vDopPrmOneComm)
      .
      
      FIND LAST btt-comm-rate WHERE 
                btt-comm-rate.commission =  ttRateCondMCGTransh.fCommission
            AND btt-comm-rate.since      =  tt-loan-cond.since
      EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
      
      IF AVAIL btt-comm-rate THEN
      ASSIGN
         btt-comm-rate.rate-comm = ttRateCondMCGTransh.fPermRate
         btt-comm-rate.min_value = ttRateCondMCGTransh.fPermMinRate
         btt-comm-rate.comm-rate-curr = ttRateCondMCGTransh.fCurrency
      .
...
/* Синхронизировать даты (иначе - сбой в запросе OPEN-QUERY-br-comm) */
PROCEDURE CopySinceTT PRIVATE:
   FOR EACH tt-comm-rate:
      tt-comm-rate.since = tt-loan-cond.since.
   END.
   FOR EACH tt-comm-cond:
...
/* $LINTUSER='daru' */
/* $LINTDATE='25/08/2017 16:49:28.734+03:00' */
/*prosignHLY8qQ8xU0KjEF1/sCHTWA*/