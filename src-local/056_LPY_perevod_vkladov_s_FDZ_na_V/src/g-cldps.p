...
DO:
   ...
   DO:
      MESSAGE SUBSTITUTE("Не найдена процедура дошаблонной обработки &1 !",
      vBefTranProc) VIEW-AS ALERT-BOX.
      RETURN.
   END.
   ELSE 
   DO:
...
for each op-template of op-kind where op-template.cr-class-code begins 'dep_person' no-lock :
  ...
  if in-currency = cur-n then in-currency = '' .
   ...
   DO:
      MESSAGE SUBSTITUTE("Не найдена процедура дошаблонной обработки &1 ~
      шаблона &2 !", vBefTranProc, op-template.op-template) VIEW-AS ALERT-BOX.
      NEXT cl.
   END. 
   ...
   DO WHILE TRUE  
     ...
     DO:
      ...
      /* Если вклад не закрыт - выводим информацию об этом */
       ...
       put stream err unformatted  'Вклад ' + loan.cont-code  skip return-value skip(1) .
      end.
     end.
   END. /*cl1  */
