...
PROCEDURE GetCust365p.
    ...
    NO-LOCK:
       IF iCheckCorpName = YES THEN DO:
          RUN VerifyPersName365p(person.person-id,
                                 vCustName,
                                 OUTPUT vL)
             NO-ERROR.
          IF vL <> YES THEN DO:
    ...
    FOR EACH cust-corp WHERE
        ...
        CAN-FIND(FIRST cust-role WHERE cust-role.cust-cat   EQ "Ю"                      
                  ...
                  NO-LOCK NO-WAIT)*/
    NO-LOCK:
        RUN dbgprint.p ("GetCust365p","~n" +
                        "vCustINN: " + GetNullStr(vCustINN) + "~n" +
                        "vCustName: " + GetNullStr(vCustName)
                        ).
       IF iCheckCorpName = YES THEN DO:
    ...
    IF vPersCnt + vCorpCnt + vBankCnt > 1 THEN DO:
        ...
        END.
    END.
/*MESSAGE "core365p.pro GetCust365p: vCustINN = " vCustINN              */
/*                               "; vCustName = " vCustName             */
/*                             "; vAllFilials = " vAllFilials           */
/*                             "; HAS-RECORDS = " oHCust365p:HAS-RECORDS*/
/*VIEW-AS ALERT-BOX.                                                    */
...
PROCEDURE ExtractOneCust365p.
    ...
    RUN dbgprint.p ("ExtractOneCust365p","~n" +
                   "iHCust365p:HAS-RECORDS: " + GetNullStr(STRING(iHCust365p:HAS-RECORDS))
                   ).
...
PROCEDURE VerifyPersName365p.
    DEFINE INPUT  PARAMETER iCustId   LIKE person.person-id  NO-UNDO.
    DEFINE INPUT  PARAMETER iCustName AS   CHARACTER         NO-UNDO.
    DEFINE OUTPUT PARAMETER oSuccess  AS   LOGICAL           NO-UNDO.
    ...
    IF NOT AVAILABLE person THEN
        RETURN ERROR "Не найден клиент " + getCustStr("Ч", iCustId).
    vName = SUBSTITUTE("&1 &2", person.name-last, person.first-names).
    RUN FixCustName365p IN THIS-PROCEDURE (INPUT-OUTPUT vName).
    vName = REPLACE(vName, ",", " ").
    oSuccess = (vName = iCustName).
END PROCEDURE.
...
/*MESSAGE "core365p.pro FindAcct365p: vAllFilials = " vAllFilials "iAcct = " iAcct*/
/*VIEW-AS ALERT-BOX.                                                              */
...
/*MESSAGE "core365p.pro FindAcct365p: " vClsFilLst*/
/*VIEW-AS ALERT-BOX.                              */
...
/*MESSAGE "core365p.pro FindAcct365p: vAcct = " vAcct*/
/*VIEW-AS ALERT-BOX.                                 */
...
PROCEDURE GetAcct365p.
    ...
    DEFINE VARIABLE vHQuery      AS   HANDLE    NO-UNDO.
    DEFINE VARIABLE vAcct        LIKE acct.acct NO-UNDO.
    DEFINE VARIABLE oAcct        LIKE acct.acct NO-UNDO.
    ...
    DEFINE VARIABLE vAcct401817  AS   CHARACTER NO-UNDO.
    
    DEFINE VARIABLE vCur         AS   CHARACTER NO-UNDO.
    DEFINE VARIABLE vOpenDate    AS   CHARACTER NO-UNDO.
    DEFINE VARIABLE vClosDate    AS   CHARACTER NO-UNDO.
    DEFINE VARIABLE vFindAcct    AS   LOGICAL   NO-UNDO.
    ...
    RUN dbgprint.p ("GetAcct365p","~n" +
                    "vRequestType: " + GetNullStr(STRING(vRequestType))
                    ).

    vAcct401817 = "40817*,40820*,423*,426*".
    ...
    END.
    RUN GetMFMode365p(OUTPUT vAllFilials) NO-ERROR.
    IF vRequestType = {&REQ-TYPE-ALL} THEN /*1*/ 
    DO:
        CREATE QUERY vHQuery.
        vHBuffer = iHCust365p:DEFAULT-BUFFER-HANDLE.
...
/*        vAcct401817 = "40817".*/
        ...
        vS = "FOR EACH " + vHBuffer:NAME                               +
             ...
             " AND NOT CAN-DO(" + QUOTER(vAcct401817) + ",acct.acct) AND "                +
             "acct.cust-id = " + vHBuffer:NAME + ".cust-id"            +
             " NO-LOCK".
        RUN dbgprint.p ("GetAcct365p","~n" +
                        "vFilialLst: " + GetNullStr(STRING(vFilialLst)) + "~n" +
                        "vHBuffer:NAME: " + GetNullStr(STRING(vHBuffer:NAME)) + "~n" +
                        "vS: " + GetNullStr(STRING(vS))
                        ).
        ...
        REPEAT:
            ...
            RUN ValidateAcct365p(vAcct,
                                 ...
                                 OUTPUT vFeature).
            RUN dbgprint.p ("GetAcct365p","~n" +
                        "1 vAcct: " + GetNullStr(STRING(vAcct))
                        ).
        ...
        &IF DEFINED(ExtBase) &THEN
        {empty ttExtAcct}
/*MESSAGE "core365p.pro GetAcct365p после (empty ttExtAcct) : перед RUN FindAcctEx"*/
/*VIEW-AS ALERT-BOX.                                                               */
        RUN FindAcctEx IN h_plbnk(iHReqInfo, "*", OUTPUT TABLE ttExtAcct) NO-ERROR.
        IF ERROR-STATUS:ERROR THEN DO:
            vS = RETURN-VALUE.
            RUN DeleteObject365p(oHAcct365p).
            IF vRecalcCust THEN
            ...
            RETURN ERROR vS.
        END.
        FOR EACH ttExtAcct WHERE
        NOT CAN-DO(vAcct401817,ttExtAcct.number) NO-LOCK:
           ...
           RUN dbgprint.p ("GetAcct365p","~n" +
                           "2 vAcct: " + GetNullStr(STRING(vAcct))
                           ).
        ...
        /*******************************************************************************************/
    END.
    ELSE IF vRequestType = {&REQ-TYPE-SELECTED} THEN   /*2*/
    DO:
        CREATE QUERY vHQuery.
        vHBuffer = iHReqAcct:DEFAULT-BUFFER-HANDLE.
        vHQuery:SET-BUFFERS(vHBuffer).
        ...
        REPEAT:
            ...
            RUN dbgprint.p ("GetAcct365p 1","~n" +
                        "vAcct: " + GetNullStr(STRING(vAcct))
                        ).
            
/*            vAcct = DelFilFromAcct(vAcct).*/
            
            RUN dbgprint.p ("GetAcct365p 2","~n" +
                            "vAcct: " + GetNullStr(STRING(vAcct))
                            ).
            ...
            ELSE DO:
                RUN ValidateAcct365p(vAcct,
                                     ...
                                     NO,
                                     OUTPUT vFeature).
                IF vFeature <> {&FEAT-NOT-FOUND} THEN
                DO:
                   RUN dbgprint.p ("GetAcct365p 3","~n" +
                        "vAcct: " + GetNullStr(STRING(vAcct))
                        ).
                   RUN FindAcct365p(BUFFER acct, vAcct) NO-ERROR.
                   RUN dbgprint.p ("GetAcct365p 4","~n" +
                        "vAcct: " + GetNullStr(STRING(vAcct))
                        ).
                   vFindAcct = IsAcctExist(vAcct,
                                   OUTPUT oAcct,
                                   OUTPUT vCur,
                                   OUTPUT vOpenDate,
                                   OUTPUT vClosDate).
                   IF vFindAcct THEN vAcct = oAcct.
                   RUN dbgprint.p ("GetAcct365p 5","~n" +
                        "vAcct: " + GetNullStr(STRING(vAcct))
                        ).
            ...
            RUN dbgprint.p ("GetAcct365p","~n" +
                            ...
                            "vFeature: " + GetNullStr(STRING(vFeature))
                            ).
            RUN AddAcctRecord365p(oHAcct365p, vAcct, vFeature) NO-ERROR.
            IF ERROR-STATUS:ERROR THEN DO:
                vS = RETURN-VALUE.
...
PROCEDURE EvaluateReqAcct365p.
    ...
    ELSE IF vRequestType = {&REQ-TYPE-SELECTED} THEN DO:
        ...
        REPEAT:
            ...
            END.
            RUN FindAcct365p(BUFFER acct, vAcct) NO-ERROR.
/*MESSAGE "AVAILABLE acct = " AVAILABLE acct*/
/*        "; vAcct = " vAcct                */
/*        "; vCur = " vCur                  */
/*        "; vOpenDate = " vOpenDate        */
/*        "; vOpenDate = " vOpenDate        */
...
/*VIEW-AS ALERT-BOX.   */
    ...
    ELSE
        vOK = YES.
    IF vOK <> YES THEN
        RETURN ERROR "Указанные в запросе номера счетов не соответствуют " +
...
/*MESSAGE                                            */
/*"core365p.pro ValidateAcct365p: iAcct = " iAcct ";"*/
/*" iBegDate = " iBegDate ";"                        */
/*" iEndDate = " iEndDate ";"                        */
/*" oFeature = " oFeature                            */
/*VIEW-AS ALERT-BOX.                                 */
...
PROCEDURE MainValidateRequest365p.
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        ...
        RETURN.
    END.
/*MESSAGE "core365p.pro MainValidateRequest365p: iCheckCorpName = " iCheckCorpName ";" TRNSettingValue("ExchSET-TAX", "Check33", "Наим")*/
/*VIEW-AS ALERT-BOX.                                                                                                                    */
    RUN GetCust365p(iHReqInfo,
                    iHReqAcct,
                    iCheckCorpName,
    ...
    RUN dbgprint.p ("MainValidateRequest365p before RUN GetAcctSplit365p","~n" + 
                    ":" + GetNullStr(STRING(""))
                    ).
   ...
   RUN dbgprint.p ("MainValidateRequest365p after RUN GetAcctSplit365p","~n" + 
                   "vHGoodAcct365p:HAS-RECORDS:" + GetNullStr(STRING(vHGoodAcct365p:HAS-RECORDS)) + "~n" +  
                   "vHBadAcct365p:HAS-RECORDS:"  + GetNullStr(STRING(vHBadAcct365p:HAS-RECORDS))
                   ).
...
PROCEDURE ValidatePNOOpAttrs.
    ...
    DEFINE VARIABLE oVidOp       AS   CHARACTER           NO-UNDO.
...
PROCEDURE GetAcctSplit365p.
    ...
    REPEAT:
        ...
        RUN dbgprint.p ("GetAcctSplit365p","~n" + 
                        "vHBuffer:NAME:" + GetNullStr(STRING(vHBuffer:NAME)) + "~n" +
                        "vAcct:" + GetNullStr(STRING(vAcct)) + "~n" +  
                        "vCnt:"  + GetNullStr(STRING(vCnt))
                        ).
        ...
        THEN DO:
            ...
            RUN dbgprint.p ("GetAcctSplit365p","~n" + 
                            "ERROR-STATUS:ERROR:" + GetNullStr(STRING(ERROR-STATUS:ERROR))
                            ).
            IF ERROR-STATUS:ERROR THEN DO:
                vHQuery:QUERY-CLOSE().
                DELETE OBJECT vHQuery.
...
PROCEDURE ValidateRequest365p.
    ...
    IF oErrorCode <> {&ERR-SUCCESS} THEN
        RETURN.
/*MESSAGE "core365p.pro ValidateRequest365p: vCheckCorpName = " vCheckCorpName*/
/*VIEW-AS ALERT-BOX.                                                          */
    RUN GetCust365p(iHReqInfo,
                    iHReqAcct,
                    vCheckCorpName,
...
PROCEDURE SetAttrValue365p.
    ...
    IF vValidAttr THEN DO:
       ...
       IF AVAILABLE class THEN DO:
          ...
          NO-LOCK NO-ERROR.
          IF NOT AVAILABLE xattr THEN DO:
             vOrigName = GetOriginalName(iAttrName).
             FIND FIRST xattr OF class WHERE
                 xattr.xattr-code = vOrigName
             NO-LOCK NO-ERROR.
          END.
          IF AVAILABLE xattr THEN DO:
             IF NOT {assigned iAttrValue} AND
                (xattr.mandatory OR
                 CAN-DO({&CONDITIONAL-ATTRS-365P}, xattr.xattr-code))
             THEN
                ASSIGN
                   iAttrValue = xattr.initial
                   vMessage   = SUBSTITUTE("Пустое значение обязательного " +
                                           "реквизита &1 класса &2. "       +
                                           "Использовано значение по "      +
                                           "умолчанию &3",
                                           QUOTER(xattr.xattr-code),
                                           QUOTER(class.class-code),
                                           QUOTER(xattr.initial))
                .
          END.
          ELSE
             vMessage = SUBSTITUTE("Неизвестный реквизит &1 класса &2",
                                   QUOTER(IF {assigned vOrigName}
                                          THEN vOrigName
                                          ELSE iAttrName),
                                   QUOTER(class.class-code)).
       END.
       ELSE
          vMessage = SUBSTITUTE("Неизвестный класс объекта: &1",
                                QUOTER(vClassName)).
       iHObject:BUFFER-FIELD(iAttrName):BUFFER-VALUE = iAttrValue.
    END.
    ELSE
       RETURN ERROR SUBSTITUTE("Неизвестный атрибут: &1",
                               GetOriginalName(iAttrName)).
...
PROCEDURE CreateObjectFromTemplate365p.
    ...
    DO vI = 1 TO oHObject:NUM-FIELDS:
        CASE oHObject:BUFFER-FIELD(vI):NAME:
            WHEN "ins-date" THEN
            DO:
               RUN dbgprint.p ("CreateObjectFromTemplate365p","~n" +
                               "1 NAME: " + GetNullStr(oHObject:BUFFER-FIELD(vI):NAME) + " = " +
                               "1 VALUE:" + GetNullStr(oHObject:BUFFER-FIELD(vI):BUFFER-VALUE)
                               ).
            END.
        END.
    END.
    ...
    END.
    RUN dbgprint.p ("CreateObjectFromTemplate365p","~n" + 
                            "1 vFieldList: " + GetNullStr(vFieldList)
                            ).
    DO vI = 1 TO oHObject:NUM-FIELDS:
        CASE oHObject:BUFFER-FIELD(vI):NAME:
            ...
            DO:
               RUN dbgprint.p ("CreateObjectFromTemplate365p","~n" +
                               "2 NAME: " + GetNullStr(oHObject:BUFFER-FIELD(vI):NAME) + " = " +
                               "2 VALUE:" + GetNullStr(oHObject:BUFFER-FIELD(vI):BUFFER-VALUE)
                               ).
            END.
        END.
    END.
    IF VALID-HANDLE(iHUpObject) THEN DO:
      vObjTableName = GetXClassProgress (iHUpObject:BUFFER-FIELD("class-code"):BUFFER-VALUE).
        ...
        DO vI = 1 TO NUM-ENTRIES(vObjTableSurr):
            ...
            RUN dbgprint.p ("CreateObjectFromTemplate365p","~n" + 
                            "vFieldName  : " + GetNullStr(vFieldName) + " = " + 
                            "BUFFER-VALUE: " + GetNullStr(iHUpObject:BUFFER-FIELD(vFieldName):BUFFER-VALUE))
                            ).
        END.
    END.
    RUN dbgprint.p ("CreateObjectFromTemplate365p","~n" + 
                    "2 vFieldList: " + GetNullStr(vFieldList)
                    ).
...
PROCEDURE FormattedAttrValue365p.
    DEFINE INPUT PARAMETER iHTable      AS HANDLE NO-UNDO.
    DEFINE INPUT PARAMETER iTegName     AS CHAR   NO-UNDO.
    DEFINE INPUT PARAMETER iTegFormat   AS CHAR   NO-UNDO.

    DEFINE VARIABLE vHQuery AS HANDLE NO-UNDO.
    IF NOT VALID-HANDLE(iHTable) THEN 
       RETURN.
    CREATE QUERY vHQuery.
    vHQuery:SET-BUFFERS(iHTable:DEFAULT-BUFFER-HANDLE).
    vHQuery:QUERY-PREPARE("FOR EACH "                        +
    ...
    REPEAT:
       ...
       IF vHQuery:QUERY-OFF-END THEN
          LEAVE.
       iHTable:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD("naimpp$"):BUFFER-VALUE = 
         TRIM(STRING( iHTable:DEFAULT-BUFFER-HANDLE:BUFFER-FIELD("naimpp$"):BUFFER-VALUE,
                 iTegFormat)).
...
/* $LINTENV ='1ut' */
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTDATE='22/10/2015 18:41:51.436+04:00' */
/* $LINTUSER='anba' */
/* $LINTMODE='1' */
/* $LINTFILE='core365p.pro' */
/*prosignutM7w0Uh6a5NYt1TGV77/Q*/