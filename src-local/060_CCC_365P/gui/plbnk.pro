...
/*MESSAGE "plbnk.pro FindAcctEx: vInn = " vInn           */
/*                            "; vKpp = " vKpp           */
/*                           "; vName = " vName          */
/*                           "; iAcct = " iAcct          */
/*                           "; mAcctFndEx = " mAcctFndEx*/
/*VIEW-AS ALERT-BOX.                                     */
...
/*MESSAGE "plbnk.pro FindAcctEx: after RUN VALUE(mAcctFndEx)"                                */
/*VIEW-AS ALERT-BOX.                                                                         */
/*                                                                                           */
/*For Each ttExtAcct No-Lock:                                                                */
/*   MESSAGE "plbnk.pro FindAcctEx: For Each ttExtAcct: ttExtAcct.number = " ttExtAcct.number*/
/*   VIEW-AS ALERT-BOX.                                                                      */
/*End.                                                                                       */
...
/*For Each ttExtAcct No-Lock:                                                             */
/*   MESSAGE "plbnk.pro AcctPosBuf1: For Each ttExtAcct: ttExtAcct.acct = " ttExtAcct.acct*/
/*   VIEW-AS ALERT-BOX.                                                                   */
/*End.                                                                                    */
...
/*For Each ttExtAcct No-Lock:                                                             */
/*   MESSAGE "plbnk.pro AcctPosBuf2: For Each ttExtAcct: ttExtAcct.acct = " ttExtAcct.acct*/
/*   VIEW-AS ALERT-BOX.                                                                   */
/*End.                                                                                    */
...
/*For Each ttExtAcct No-Lock:                                                             */
/*   MESSAGE "plbnk.pro AcctPosBuf3: For Each ttExtAcct: ttExtAcct.acct = " ttExtAcct.acct*/
/*   VIEW-AS ALERT-BOX.                                                                   */
/*End.                                                                                    */
...
DEF BUFFER bExtAcct FOR ttExtAcct.
   ...
   FIND FIRST bExtAcct WHERE bExtAcct.number EQ iAcct NO-LOCK NO-ERROR.
   ...
   IF NOT VALID-HANDLE(vHCustBuffer365p) THEN
   DO:
   ...
   FIND FIRST bExtAcct WHERE bExtAcct.number EQ iAcct NO-LOCK NO-ERROR.
/*MESSAGE                                          */
/*"plbank.pro ExtValidateAcct: iAcct = " iAcct ";" */
/*" iBegDate = " iBegDate ";"                      */
/*" iEndDate = " iEndDate ";"                      */
/*" bExtAcct.close-date = " bExtAcct.close-date ";"*/
/*" bExtAcct.open-date  = " bExtAcct.open-date ";" */
/*VIEW-AS ALERT-BOX.                               */
   IF iBegDate            NE ? AND
      bExtAcct.close-date NE ? AND
      bExtAcct.close-date LE iBegDate THEN
   DO:
...
DEF VAR vTmpDate        AS  DATE    NO-UNDO.
   ...
   RUN SetAttrValue365p(vHRepAcctBuffer,"nomswc$",vAcct)          {&RAISE-ERROR}.
/*   RUN SetAttrValue365p(vHRepAcctBuffer, "НомСч", vAcct)         {&RAISE-ERROR}.*/
   RUN SetAttrValue365p(vHRepAcctBuffer,"valswc$",vCurrency)      {&RAISE-ERROR}.
/*   RUN SetAttrValue365p(vHRepAcctBuffer, "ВалСч", vCurrency)     {&RAISE-ERROR}.*/
   ...
   DO:
      RUN SetAttrValue365p(vHRepAcctBuffer,"vidswc$", bExtAcct.contract) {&RAISE-ERROR}.
   END.
   ...
   CASE iRequestKind:
      ...
      DO:
          RUN SetAttrValue365p(vHRepAcctBuffer, "dataotkrswc$", date2str(bExtAcct.open-date))
              {&RAISE-ERROR}.
          IF bExtAcct.close-date <> ? THEN 
          DO:
             RUN SetAttrValue365p(vHRepAcctBuffer, "datazakrswc$", date2str(bExtAcct.close-date))
                 {&RAISE-ERROR}.
      ...
      DO:
         RUN SetAttrValue365p(vHRepAcctBuffer, "ostatok$", STRING(ABSOLUTE(vAmt)))
             {&RAISE-ERROR}.
      ...
      DO:
         ...
         RUN SetAttrValue365p(vHRepAcctBuffer,"datanawcala$",date2str(iBegDate))
             {&RAISE-ERROR}.
         RUN SetAttrValue365p(vHRepAcctBuffer,"datakonca$",date2str(iEndDate))
             {&RAISE-ERROR}.
         RUN SetAttrValue365p(vHRepAcctBuffer,"ostatnawc$",STRING(ABS(vAmtIn)))
             {&RAISE-ERROR}.                               
         RUN SetAttrValue365p(vHRepAcctBuffer,"ostatkon$",STRING(ABS(vAmt)))
             {&RAISE-ERROR}.                               
         RUN SetAttrValue365p(vHRepAcctBuffer,"summadeb$",STRING(ABS(vAmtDb)))
             {&RAISE-ERROR}.
         RUN SetAttrValue365p(vHRepAcctBuffer,"summakred$",STRING(ABS(vAmtCr)))
             {&RAISE-ERROR}.
         /* Заполняем данные из внешней системы */
         RUN FillOp IN THIS-PROCEDURE (vHRepOpBuffer,
                                       ...
                                       OUTPUT oNumDocs) 
                                      {&RAISE-ERROR}.
/*MESSAGE "plbnk.pro FillZNOExtAcct: oNumDocs = " oNumDocs*/
/*VIEW-AS ALERT-BOX.                                      */
