/*
   ...
   Parameters: NO-PARAMETRS
     ...
     Modified: 08/09/2015 boao (0261150) изменение контроля по параметру check33
*/
...
/*      vInfoType  = hExch:BUFFER-FIELD(GetMangledName("ТипИнф")):BUFFER-VALUE*/
   ...
   RUN dbgprint.p ("TAXAcctBlkFileImport begin","~n" +
                   "vPacketID: " + GetNullStr(STRING(vPacketID)) + "~n" +
                   "vSeanceID: " + GetNullStr(STRING(vSeanceID)) + "~n" +
                   "vInfoType: " + GetNullStr(vInfoType) + "~n" +
                   "vBIK     : " + GetNullStr(vBIK) + "~n" +
                   "vBlkType : " + GetNullStr(vBlkType) + "~n" +
                   "vStrTMP  : " + GetNullStr(vStrTMP) + "~n" +
                   "vErrClass: " + GetNullStr(vErrClass) + "~n" +
                   "vUserSel : " + GetNullStr(STRING(vUserSel)) + "~n" +
                   "vErrCode : " + GetNullStr(vErrCode) + "~n" +
                   "vErrText : " + GetNullStr(vErrText)
                  ). 
   ...
   IF vErrCode EQ "" OR             /* проверка не выполнялась */
      ...
      FOR EACH ttBlockAcct WHERE NOT {assigned ttBlockAcct.error-code} NO-LOCK:
          CASE vInfoType:
              ...
              DO:
                  ...
                  DO:
                     RUN PacketCreateLink IN h_pack (
                        vPacketID,
                        "acct",
      ...
      END.
       ...
       RUN dbgprint.p ("TAXAcctBlkFileImport","~n" +
                "mSuperPacketID: " + GetNullStr(STRING(mSuperPacketID)) + "~n" + 
                "vInfoType: " + GetNullStr(vInfoType)
                ).
      
      IF CAN-FIND(FIRST ttBlockAcct WHERE NOT {assigned ttBlockAcct.error-code})
      THEN 
      DO:
          ...
          CASE vInfoType:
              ...
              DO:
                  ...
                  RUN dbgprint.p ("TAXAcctBlkFileImport CreateBOSExch_RPO","~n" +
                                  "vErrCode: " + GetNullStr(vErrCode) + "~n" +
                                  "vErrText: " + GetNullStr(vErrText)
                                 ). 
   ...
   END.
   RUN dbgprint.p ("TAXAcctBlkFileImport","~n" +
                   "vErrClass: " + GetNullStr(vErrClass)
                   ). 
   IF {assigned vErrClass} THEN DO:
      ...
      DO:
          RUN TAXBlockAcctError(TABLE ttBlockAcct,
                                OUTPUT vErrCode,
                                OUTPUT vErrText).
          RUN dbgprint.p ("TAXAcctBlkFileImport TAXBlockAcctError","~n" +
                   "vErrCode: " + GetNullStr(vErrCode) + "~n" +
                   "vErrText: " + GetNullStr(vErrText)
                   ).
      END.  
      IF {assigned vErrCode} AND vErrCode <> "10" THEN
      DO:
...
PROCEDURE TAXAcctBlkImport.
   ...
   DEFINE VARIABLE hFile       AS HANDLE      NO-UNDO.
   DEFINE VARIABLE hMain       AS HANDLE      NO-UNDO.
   DEFINE VARIABLE hBlk        AS HANDLE      NO-UNDO.
   DEFINE VARIABLE hDataExch   AS HANDLE      NO-UNDO.
   DEFINE VARIABLE hResolutn   AS HANDLE      NO-UNDO.
   DEFINE VARIABLE vPacketID   AS INT64       NO-UNDO.
   ...
   ASSIGN
      ...
      vPacketID = hFile:BUFFER-FIELD("PacketID"):BUFFER-VALUE   /*разобраться с ?*/
      vInfoType = hFile:BUFFER-FIELD("tipinf$"):BUFFER-VALUE
      vAcct     = hExch:BUFFER-FIELD("nomswc$"):BUFFER-VALUE
      vCreateLink = LOGICAL(TRNSettingValue("", "CreateAcctLink", "YES"))
      vBegDate    = DATE(hMain:BUFFER-FIELD(GetMangledName("datanawc$")):BUFFER-VALUE)
   NO-ERROR.
   
   RUN dbgprint.p ("TAXAcctBlkImport begin","~n" +
                   "vPacketID   : " + GetNullStr(STRING(vPacketID)) + "~n" +
                   "vInfoType   : " + GetNullStr(vInfoType) + "~n" +
                   "vAcct       : " + GetNullStr(vAcct) + "~n" +
                   "vBegDate    : " + GetNullStr(STRING(vBegDate,"99/99/9999"))
                  ).   
   
   RUN SetMFMode365p IN THIS-PROCEDURE (hMain).
   RUN FindAcct365p IN THIS-PROCEDURE (BUFFER bAcct, vAcct) NO-ERROR.
   
   RUN dbgprint.p ("TAXAcctBlkImport 0","~n" +
                   "AVAIL(bAcct): " + GetNullStr(STRING(AVAIL(bAcct)))
                   ).
...
/*   vCreateLink = NO.*/
   ...
   DO:
      ...
      RUN dbgprint.p ("TAXAcctBlkImport 0","~n" +
                "bAcct.acct: " + GetNullStr(bAcct.acct)       + "~n" +
                "vInfoTypet: " + GetNullStr(vInfoType)        + "~n" +
                "vPacketID: " + GetNullStr(STRING(vPacketID))
                ).   
      
      IF vCreateLink 
      AND vPacketID NE ?
      AND vPacketID NE 0 THEN
      DO:
          ...
          DO:
             FIND FIRST PackObject WHERE
                ...
                AND PackObject.PacketID   EQ vPacketID
                AND PackObject.Surrogate  EQ bAcct.acct + "," + bAcct.currency
                AND PackObject.Kind       EQ vInfoType
             NO-LOCK NO-ERROR.
      ...
      ASSIGN
         ...
         vFindAcct = YES
      .
      RUN dbgprint.p ("TAXAcctBlkImport 1","~n" +
                      "AVAIL(bAcct): " + GetNullStr(STRING(AVAIL(bAcct))) + "~n" +
                      "vFindAcct   : " + GetNullStr(STRING(vFindAcct))
                     ).
   END.
   ELSE
   DO:
      ...
      DO:
         ...
         RUN dbgprint.p ("TAXAcctBlkImport 2","~n" +
                         "vFindAcct: " + GetNullStr(STRING(vFindAcct)) + "~n" +
                         "oAcct    : " + GetNullStr(oAcct) + "~n" +
                         "vOpenDate: " + GetNullStr(STRING(vOpenDate)) + "~n" +  
                         "vClosDate: " + GetNullStr(STRING(vClosDate))
                         ).
      END. /* IF     IsLessDateNR(vBegDate) */
      ELSE
      DO:
         /* Проверяем до даты начального решения       */
         vFindAcct = IsAcctExist(vAcct,
                                 OUTPUT oAcct,
                                 OUTPUT vCur,
                                 OUTPUT vOpenDate,
                                 OUTPUT vClosDate).
         IF vFindAcct THEN vAcct = oAcct.
         RUN dbgprint.p ("TAXAcctBlkImport 3","~n" +
                         "vFindAcct: " + GetNullStr(STRING(vFindAcct)) + "~n" +
                         "oAcct    : " + GetNullStr(oAcct) + "~n" +
                         "vOpenDate: " + GetNullStr(STRING(vOpenDate)) + "~n" +  
                         "vClosDate: " + GetNullStr(STRING(vClosDate))
         ...
         IF vFindAcct THEN
         DO:
            IF DATE(vClosDate) LT vBegDate THEN
            DO:
               vMsg = "Счет " + vAcct + " закрыт. Дата закрытия " + vClosDate + ".".
   ...
   ASSIGN
      ...
      ttBlockAcct.msg        = IF NOT vFindAcct THEN "Счет не найден." ELSE (IF {assigned vMsg} THEN vMsg ELSE "")
      ttBlockAcct.error-code = IF NOT vFindAcct THEN "32" ELSE (IF {assigned vMsg} THEN "32" ELSE "")
   ...
   RUN dbgprint.p ("TAXAcctBlkImport end","~n" +
                   "vAcct     : " + GetNullStr(vAcct) + "~n" +
                   "msg       : " + GetNullStr(ttBlockAcct.msg) + "~n" +
                   "error-code: " + GetNullStr(ttBlockAcct.error-code)
                  ).
   /* Очистка ТрФормы - необходима при загрузке нескольких счетов, т.к. используется только один инстанс */
   ASSIGN
...
/*   NO-LOCK NO-ERROR.                       */
   ...
   RUN dbgprint.p ("TAXBlockAcct begin","~n" +
   ...
   DO:
      ...
      RUN dbgprint.p ("TAXBlockAcct","~n" +
                      "iBlkType: " + GetNullStr(iBlkType) + "~n" +
                      "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct) + "~n" +
                      "ttBlockAcct.error-code: " + GetNullStr(ttBlockAcct.error-code) + "~n" +
                      "ttBlockAcct.msg: " + GetNullStr(ttBlockAcct.msg) + "~n" +
                      "AVAIL(BlockObject): " + GetNullStr(STRING(AVAIL(BlockObject)))
                      ).
...
/*             MESSAGE ttBlockAcct.msg VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
      ...
      ELSE
         ...
         .
      RUN dbgprint.p ("TAXBlockAcct end","~n" +
                      "iBlkType: " + GetNullStr(iBlkType) + "~n" +
                      "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct)
                      ).
   END.
...
/*   NO-LOCK NO-ERROR.                       */
   ...
   RUN dbgprint.p ("TAXUnblockAcct begin","~n" +
                   "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct) + "~n" +
                   "AVAIL(acct): " + GetNullStr(STRING(AVAIL(acct)))
                   ). 
   ...
   IF AVAILABLE acct THEN DO:
      ...
      RUN dbgprint.p ("TAXUnblockAcct","~n" +
                      "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct) + "~n" +
                      "ttBlockAcct.error-code: " + GetNullStr(ttBlockAcct.error-code) + "~n" +
                      "ttBlockAcct.msg: " + GetNullStr(ttBlockAcct.msg) + "~n" +
                      "AVAIL(BlockObject): " + GetNullStr(STRING(AVAIL(BlockObject)))
      ...
      END.
      RUN dbgprint.p ("TAXUnBlockAcct end","~n" +
                      "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct)
                      ). 
   END.
END PROCEDURE.
...
PROCEDURE TAXBlockAcctCheck.
    ...
    FOR EACH ttBlockAcct EXCLUSIVE-LOCK:
       ...
       DO:
          ...
          {find-act.i &acct   = ttBlockAcct.acct
	                   ...
	                   &filial = vFilial}
       END. 
       RUN dbgprint.p ("TAXBlockAcctCheck","~n" +
                       "ttBlockAcct.acct: " + GetNullStr(ttBlockAcct.acct) + "~n" + 
                       "vFilial: " + GetNullStr(vFilial) + "~n" +
                       "AVAIL(acct): " + GetNullStr(STRING(AVAIL(acct)))
                      ).
       ...
       END.
        ...
        IF {assigned vINNNP} AND vINN <> vINNNP THEN DO:
            ...
            NEXT.
        END.
        IF vCheck33 = "Наим" OR (NOT {assigned vCheck33}) THEN DO:
            {365p.get &obj = "iHExch"
                      &fld = "НаимНП"
                      &val = "vNaimNP"}
            {365p.get &obj = "iHExch"
    ...
    END.
    FOR EACH ttBlockAcct NO-LOCK:
       RUN dbgprint.p ("TAXBlockAcctCheck FOR EACH ttBlockAcct","~n" +
                       "ttBlockAcct.acct      : " + GetNullStr(ttBlockAcct.acct)       + "~n" + 
                       "ttBlockAcct.error-code: " + GetNullStr(ttBlockAcct.error-code) + "~n" +
                       "ttBlockAcct.msg       : " + GetNullStr(ttBlockAcct.msg)
                       ).
    END.
...
/*MESSAGE "pp-itax.p TAXOpPoruchFileImport: vCheckCorpName = " vCheckCorpName*/
/*VIEW-AS ALERT-BOX.                                                         */
...
/*MESSAGE "pp-itax.p TAXOpPoruchFileImport: mReqType = " mReqType*/
/*VIEW-AS ALERT-BOX.                                             */
         ...
         ELSE DO:
            ...
            IF CAN-DO("10,",vErrCode) THEN 
            DO:
/*MESSAGE "pp-itax.p TAXOpPoruchFileImport: 1 vErrCode = " vErrCode*/
/*VIEW-AS ALERT-BOX.                                               */
               mSuperPacketID = vPacketID.
...
/*MESSAGE "pp-itax.p TAXOpPoruchFileImport: 2 vErrCode = " vErrCode*/
/*VIEW-AS ALERT-BOX.                                               */
...
DO ON ERROR UNDO MAIN, RETRY MAIN:
   ...
   REPEAT:
      ...
      &IF DEFINED (IS-DEBUG) &THEN
         RUN dbgprint.p ("TAXFileIzv","vFormat:" + vFormat + " vLevel:" + GetNullInt(vLevel) +
                                        " vBuffer:" + vBuffer + " vFooter:" + GetNullStr(vFooter) +
                                      " mSpooling:" + STRING (mSpooling)).    
      &ENDIF
...
/* $LINTENV ='1ut' */
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTDATE='15/10/2015 19:35:34.135+04:00' */
/* $LINTUSER='kostik' */
/* $LINTMODE='1' */
/* $LINTFILE='pp-itax.p' */
/*prosignjWrAWlOP01VJ7t9isNC0zA*/