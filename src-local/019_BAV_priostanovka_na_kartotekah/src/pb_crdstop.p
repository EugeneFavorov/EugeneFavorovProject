...
DO:
    DO TRANSACTION:
        ...
        IF (LASTKEY NE KEYCODE("ESC")) AND (pick-value NE ?)
...
IF LASTKEY EQ KEYCODE("ESC") OR NOT {assigned mAcct}
THEN RETURN.
...
THEN DO:
    MESSAGE "Документы на картотеках не обнаружены" VIEW-AS ALERT-BOX ERROR.
    RETURN.
END.
/*
...
ON F3, F4 OF bOpBal RETURN NO-APPLY.
...
WAIT-FOR "ESC" OF FRAME fOpBal FOCUS bOpBal IN FRAME fOpBal.
HIDE FRAME fOpBal.
