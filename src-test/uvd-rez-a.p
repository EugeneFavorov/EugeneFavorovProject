...
DEFINE VARIABLE n2           AS int                NO-UNDO.   
...
n2 = today - beg-dt + 1.
do i1 = 1 to n2 :
   for each op where  (CAN-DO(list-bal-2, op.ben-acct) or (CAN-DO(list-bal-3, op.ben-acct) and (substr(trim(op.details),2,2) EQ "VO")))
                 and  op.op-date  = mtoday
                 and  op.op-status >= chr(251)
                 no-lock.
      find first op-entry of op.
      if  CAN-DO(list-bal, op-entry.acct-cr) 
...
*/
   ...
   file-name-out =  STRING(YEAR(mtoday) ,"9999")      + "-" + 
                ...
                + ".doc"
                .
   file-name-in = "/data/home2/bis/quit41d/src-test/uvd-rez.tpl".
   ...
   var-op-date   = string(op.op-date,"99.99.9999").
 ...
 /* Данные клиента */      
   ...
   RUN pb_mail.p ("v.ignatchenko,l.rusinova,o.shakin,A.Korobova,M.Vasileva,l.rusinova", "Izveshenie", "Izveshenie", file-name-out). 

   OS-COPY VALUE(file-name-out) VALUE("/data/home2/bis/quit41d/imp-exp/cur_cont/"  + file-name-out ). 

END PROCEDURE.
/*
   RUN pb_mail.p ("v.ignatchenko", "Izveshenie", "Izveshenie", file-name-out). 
