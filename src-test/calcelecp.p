...
FIND LAST datablock
  ...
  NO-LOCK NO-ERROR.
IF AVAILABLE(datablock) THEN
DO:
	FIND CURRENT datablock EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
        IF AVAILABLE(datablock) THEN
        DO:
           FOR EACH DataLine OF DataBlock EXCLUSIVE-LOCK:
/* run dbgprint.p("", dataline.sym1 + " - " + dataline.sym2). */
              DELETE dataline.
           END.
        DELETE datablock.
        END.
