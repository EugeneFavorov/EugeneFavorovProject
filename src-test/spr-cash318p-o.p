/*
   ...
   Parameters:
     ...
     sku При передаче денег от кассира к кассиру не используется смена хранилища,
         поэтому в справке заведующего кассой от входящего остатка отнимается
         сумма переданных денег подчиненному и не берется приход от другого
         кассира внутри одного дня.
...
DEFINE TEMP-TABLE ttSprCash NO-UNDO /* Данные для справки о наличности */
   ...
   INDEX idxAcct Acct
   INDEX idxU IS UNIQUE Acct Bal-acct Currency Dpr-Id User-Id vChBlank DocDate
   .
...
&SCOPED-DEFINE format-cur-name 20

ASSIGN
   mLines[ 1] = "┌─" + FILL("─",{&format-cur-name})                   + "─┬──────────────────────────────┬──────────────────────────────┬─────────────────────────────────┐"
   mLines[ 2] = "│ " + PADC("Наименование валюты",{&format-cur-name}) + " │            Приход            │             Расход           │             Фамилии             │"
   mLines[ 3] = "│ " + FILL(" ",{&format-cur-name})                   + " ├──────────┬───────────────────┼──────────┬───────────────────┤            и подписи            │"
   mLines[ 4] = "│ " + FILL(" ",{&format-cur-name})                   + " │количество│   сумма цифрами   │количество│   сумма цифрами   │          бухгалтерских          │"
   mLines[ 5] = "│ " + FILL(" ",{&format-cur-name})                   + " │ кассовых │                   │ кассовых │                   │            работников           │"
   mLines[ 6] = "│ " + FILL(" ",{&format-cur-name})                   + " │документов│                   │документов│                   │                                 │"
   mLines[ 7] = "├─" + FILL("─",{&format-cur-name})                   + "─┼──────────┼───────────────────┼──────────┼───────────────────┼─────────────────────────────────┤"
   mLines[10] = "│ " + PADC("1",{&format-cur-name})                   + " │     2    │         3         │     4    │         5         │                 6               │"
   ...
   FOR EACH bttSprCash
      ...
      DO:
   	...
   	/* Заполняем получено для совершения операций */
         PUT UNFORMATTED 
            vOtstup SPACE(3) "Получено для совершения операций в сумме: " SKIP(1). 
         ...
         FOR EACH ttSprCash WHERE ttSprCash.DocDate EQ bttSprCash.DocDate 
            ...
            DO:
               ...
               vIntStr[1] = STRING(ttSprCash.CurrName,"x({&format-cur-name})") + " " + 
                            STRING(ACCUM TOTAL {&BY-FLD2} ttSprCash.In-Bal,"->,>>>,>>>,>>9.99") + " / " + 
                            vIntStr[1] + " " + vDecStr.
...
/* sku        AND ttSprCash.QtyDb   NE 0
              AND ttSprCash.QtyCr   NE 0 */
            NO-LOCK
            BREAK {&BY-FLD4} {&BY-FLD1} {&BY-FLD2} {&BY-FLD3}:
            ...
            DO:
               ...
               PUT UNFORMATTED vOtstup "│ " ttSprCash.CurrName FORMAT "x({&format-cur-name})"  " │ "
                  vQtyDb                                      FORMAT ">>>>>>>9" " │ "
                  (ACCUM TOTAL {&BY-FLD3} ttSprCash.SummDb)   FORMAT "->>>>>,>>>,>>9.99" " │ " 
                  vQtyCr                                      FORMAT ">>>>>>>9" " │ "
                  (ACCUM TOTAL {&BY-FLD3} ttSprCash.SummCr)   FORMAT "->>>>>,>>>,>>9.99" " │ "
         ...
         PUT UNFORMATTED 
            ...
            vOtstup SPACE(3) "Остаток на конец дня в сумме: " SKIP(1).
   	/* Заполняем остаток на конец дня */
         FOR EACH ttSprCash WHERE ttSprCash.DocDate EQ bttSprCash.DocDate
            ...
            DO:
               ...
               /* Собираем в одну строку все, что хотим вывести на печать */
               vIntStr[1] = STRING(ttSprCash.CurrName,"x({&format-cur-name})") + " " + 
                            STRING(ACCUM TOTAL {&BY-FLD2} ttSprCash.Balance,"->,>>>,>>>,>>9.99") + " / " + 
                            vIntStr[1] + " " + vDecStr.
         ...
         RUN PrintFioAndPost(mFioInRep[1],mPostInRep[1],LENGTH(vOtstup)).
         RUN PrintFioAndPost(mFioInRep[2],mPostInRep[2],LENGTH(vOtstup)).
      END.      
   END.
