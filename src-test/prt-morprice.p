...
DEFINE VARIABLE vNaimCenn     AS CHARACTER NO-UNDO.
DEFINE VARIABLE mvdName as char no-undo.
...
FOR EACH ttop-entry NO-LOCK:
   ...
   ELSE DO:
      ...
      vCode = GetXAttrValue ("acct", acct.acct + "," + acct.currency,"form-type-code").
   END.
   vNaimCenn = GetXAttrValueEx("op",STRING(op.op),"НаимЦенн","").
   IF {assigned vNaimCenn} THEN
   DO:
      IF NOT CAN-FIND (FIRST ttCickleChek WHERE ttCickleChek.name EQ vNaimCenn) THEN DO:
         CREATE ttCickleChek.
         ttCickleChek.name = vNaimCenn.
      ...
      END.
      ELSE DO:
         FIND FIRST ttCickleChek WHERE ttCickleChek.name EQ vNaimCenn NO-LOCK NO-ERROR.
         IF {assigned STRING(ttop-entry.qty)} THEN 
      ...
      END.
   END.
   ELSE IF {assigned vCode} THEN DO:
...
mvdName = GetXAttrValueEx("op", STRING(op.op), "ФИО", "").
{find-act.i
   &acct = ttop-entry.acct-cr
}   
IF AVAIL(acct) THEN DO:
  IF op.name-ben NE ? AND TRIM(op.name-ben) NE "" THEN do:
	RUN Insert_TTName("Name",op.name-ben). /* VPA 0151828 */
	if substring(acct.acct,1,3) = '912' THEN DO:
	...
	ELSE DO:
	   find first bbacct where bbacct.acct = ttop-entry.acct-db no-lock no-error.
	   if avail bbacct then do:
	  	FIND FIRST branch WHERE branch.branch-Id EQ bbacct.branch-id NO-LOCK NO-ERROR. /* VPA 0151828 */
	        IF AVAIL(branch) THEN
 	        RUN Insert_TTName("bank", branch.name).
       end.
    END.
 ...
 END.  
END.
IF mvdName <> "" THEN RUN Insert_TTName("Name",mvdName).
