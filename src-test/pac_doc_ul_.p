...
FIND FIRST tmprecid NO-LOCK NO-ERROR.
    ...
    FIND FIRST cust-corp WHERE RECID(cust-corp) EQ tmprecid.id NO-LOCK NO-ERROR.
       ...
       /* IF mSignsV="" THEN DO:
        ...
        ELSE */
	   DO:       
            RUN Insert_TTName("RiskDa", IF CAN-DO("*высокий*,*большой*", GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "РискОтмыв","")) THEN "Х" ELSE "").
            RUN Insert_TTName("RiskNet", IF CAN-DO("низкий", GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "РискОтмыв","")) or (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "РискОтмыв",""))="" THEN "Х" ELSE "").
        END.
...
/*
	   RUN Insert_TTName("DP", IF (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "date_inspect",""))="" THEN "нет" ELSE          
        ...
        RUN Insert_TTName("ОргУправ", mSignsVal). 
		...
		RUN Insert_TTName("e-mail", IF (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "e-mail",""))="" THEN "нет" ELSE
                (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "e-mail",""))).
		...
		RUN Insert_TTName("OrgUpr_Da",  IF (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "ПрисутОргУправ", ""))="" THEN "" ELSE "Х").
		RUN Insert_TTName("OrgUpr_Net", IF(GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "ПрисутОргУправ", ""))="" THEN "Х" ELSE "").
       ...
       FIND FIRST person WHERE
            ...
            InfSignsV = person.name-last + " " + person.first-names + " " + REPLACE(STRING(person.birthday, "99/99/9999"), "/", ".") + " г.р., " .
             ...
             FIND country WHERE country.country-id EQ person.country-id NO-LOCK NO-ERROR.
                ...
                InfSignsV2 = InfSignsV2 + ", выд." + REPLACE(mSignsV, "/", ".") + ", " + person.issue.   
