...
DEF VAR cTrZay1         AS CHAR NO-UNDO. cTrZay1 = 'KASSZ'.          /*Транзакция создания кассовой заявки в БИСквите*/
DEF VAR cTrZay2         AS CHAR NO-UNDO. cTrZay2 = 'load-kassz_'.    /*Транзакция создания кассовой заявки по Банк-Клиенту*/
DEF VAR cTrZayString    AS CHAR NO-UNDO. cTrZayString = 'Создание кассовой заявки'.       /*Транзакция создания кассовой заявки*/

DEF VAR cPackUslSta     AS DEC  NO-UNDO. cPackUslSta = 100000.      /*Лимит пакета услуг Стартовый*/
...
DEF VAR cTime16_00      AS INT64 NO-UNDO.
cTime16_00 = 57600 - (TIMEZONE(GetXAttrValue("branch", shFilial, "timezone")) - SESSION:TIMEZONE) * 60. /* 16:00 Местного времени по Москве*/
...
DEF VAR in-Comm     AS CHAR NO-UNDO.

DEF VAR vNoKZ       AS LOG  NO-UNDO.   /*Флаг отсутсвия кассовой заявки*/

FUNCTION RaschKommParts RETURN DEC
   (INPUT MaxSummMonth  AS DEC,
    INPUT RealSummMonth AS DEC,
    INPUT SummThithDoc  AS DEC,
    INPUT SummFiDoc     AS DEC,
    INPUT KommNorm      AS DEC,
    INPUT KommBig       AS DEC ):
...
FIND FIRST acct WHERE acct.acct EQ vAcct NO-LOCK NO-ERROR.    
     IF AVAIL(acct)                             
      ...
      /*AND (acct.cust-id  EQ 8978    /* Эколайн */*/
      /* OR  acct.cust-id  EQ 26549)  /* Грифон  */*/
        AND  acct.cust-id  EQ 26549
     THEN cSummMonth = 800000.               
vUnAmt = 0.
vOvAmt = 0.



/* ivv  Поиск индивидуальных комиссий или сумм  . findcom33.i нужна, чтобы искались только индивидуальные тарифы.*/

FIND FIRST acct
    WHERE acct.acct = vAcct
    NO-LOCK NO-ERROR.
in-comm = "cSummMonth".
{findcom33.i
    &rcom=in-comm
...
IF AVAIL comm-rate
THEN cSummMonth = comm-rate.rate-comm.
...
IF AVAIL comm-rate
THEN  cAM_NKS_KZ = comm-rate.rate-comm.
...
FIND FIRST op-kind
    WHERE op-kind.op-kind EQ op.op-kind
    NO-LOCK NO-ERROR.
IF      AVAIL op-kind
    AND (op-kind.op-kind     EQ cTrZay1
      OR op-kind.op-kind     EQ cTrZay2
      OR op-kind.name-opkind EQ cTrZayString)
THEN vNoKZ = FALSE.
ELSE vNoKZ = TRUE.

/* Отсеиваем случай пакетов услуг*/
FIND FIRST acct
    WHERE acct.acct EQ vAcct
    NO-LOCK.
...
THEN DO:
    FOR EACH tmpsigns
        ...
        LEAVE.
    END.
    /*vProd = GetXattrValueEx("cust-corp", STRING(vCust-id), "Пакет", "").*/
END.

IF vCust-cat EQ 'Ч'
THEN DO:
    FOR EACH tmpsigns
        WHERE tmpsigns.file-name = 'person'
          AND tmpsigns.surrogate = STRING(vCust-id)
          AND tmpsigns.since le vOp-date
          AND tmpsigns.code = 'Пакет'
        NO-LOCK BY tmpsigns.since DESC:
        ...
        LEAVE.
    END.
    /*vProd = GetXattrValueEx("person", STRING(vCust-id), "Пакет", "").*/
END.

FIND FIRST op-entry
    WHERE op-entry.op       = vOpOp
      AND op-entry.op-entry = vOpEntry
    NO-LOCK NO-ERROR.
IF AVAIL op-entry AND vProd EQ ''
THEN DO:
    vKS = op-entry.symbol.
    vAmt = op-entry.amt-rub.
END.
ELSE IF AVAIL op-entry AND vProd NE ''
     THEN vKS = op-entry.symbol.
     ELSE MESSAGE "Ошибка в поиске документа!" VIEW-AS ALERT-BOX.

IF vProd NE ''
THEN DO:
    FOR EACH op-entry
        WHERE (((op-entry.op-date   >= vStartMonth)
            AND (op-entry.op-date   <= vOp-date)
            AND (op-entry.op-status BEGINS "√"))
           OR  ((op-entry.op-date   = vOp-date)
            AND (op-entry.op        EQ vOpOp)
            AND (op-entry.op-entry  < vOpEntry)))
    ...
    FOR EACH op-entry
        WHERE op-entry.op-date = vOp-date
          AND op-entry.acct-db EQ vAcct
          AND op-entry.acct-cr BEGINS '20202'
          AND op-entry.op EQ vOpOp
          AND op-entry.op-entry = vOpEntry
    ...
    IF vProd EQ 'Стартовый+'
        THEN DO:
                IF vSummPacUs >= cPackUslSta
                THEN DO:
                        MESSAGE 'Клиент превысил лимит пакета в ' + STRING(cPackUslSta) + 'руб на ' + STRING(vAmt) + 'руб' VIEW-AS ALERT-BOX.
                     END.
                ELSE IF vSummPacUs + vAmt <= cPackUslSta
                     THEN DO:
                             MESSAGE 'У клиента не превышен лимит пакета в ' + STRING(cPackUslSta) + 'руб' VIEW-AS ALERT-BOX.
                          komm = 0.
                         ...
                         RUN SetSysConf IN h_base ("кассзаявком", STRING(komm)).
                          ...
                          END.
                     ELSE DO:
                             vOvAmt = vAmt + vSummPacUs - cPackUslSta.
                             vUnAmt = vAmt - vOvAmt.
                             vAmt   = vOvAmt.
                             MESSAGE 'Клиент превысил лимит пакета в ' + STRING(cPackUslSta) + 'руб на ' + STRING(vAmt) + 'руб' VIEW-AS ALERT-BOX.
                          END.
    ...
    IF vProd EQ 'Базовый+'
        THEN DO:
                ...
                THEN DO:
                        MESSAGE 'Клиент превысил лимит пакета в ' + STRING(cPackUslBaz) + 'руб на ' + STRING(vAmt) + 'руб' VIEW-AS ALERT-BOX.
                     END.
                ELSE IF vSummPacUs + vAmt <= cPackUslBaz
                     THEN DO:
                             MESSAGE 'У клиента не превышен лимит пакета в ' + STRING(cPackUslBaz) + 'руб' VIEW-AS ALERT-BOX.
                          komm = 0.
                          RUN SetSysConf IN h_base ("кассзаявнедоком", STRING(0)).
                         RUN SetSysConf IN h_base ("кассзаявком", STRING(komm)).
                          RUN SetSysConf IN h_base ("кассзаявнетден", STRING(vNoKZ)).
    ...
    IF vProd EQ 'Оптимальный+'
        THEN DO:
                IF vSummPacUs >= cPackUslOpt
                THEN DO:
                        MESSAGE 'Клиент превысил лимит пакета в ' + STRING(cPackUslOpt) + 'руб на ' + STRING(vAmt) + 'руб' VIEW-AS ALERT-BOX.
                     END.
                ELSE IF vSummPacUs + vAmt <= cPackUslOpt
                     THEN DO:
                             MESSAGE 'У клиента не превышен лимит пакета в ' + STRING(cPackUslOpt) + 'руб' VIEW-AS ALERT-BOX.
                          komm = 0.
                          RUN SetSysConf IN h_base ("кассзаявнедоком", STRING(0)).
                         RUN SetSysConf IN h_base ("кассзаявком", STRING(komm)).
                          RUN SetSysConf IN h_base ("кассзаявнетден", STRING(vNoKZ)).
    ...
    THEN DO:
        ...
        THEN DO:
            MESSAGE 'Клиент превысил лимит пакета в ' + STRING(cPackUslPre) + 'руб на ' + STRING(vAmt) + 'руб' VIEW-AS ALERT-BOX.
        END.
        ELSE IF vSummPacUs + vAmt <= cPackUslPre
        THEN DO:
            MESSAGE 'У клиента не превышен лимит пакета в ' + STRING(cPackUslPre) + 'руб' VIEW-AS ALERT-BOX.
            komm = 0.
            RUN SetSysConf IN h_base ("кассзаявнедоком", STRING(0)).
            RUN SetSysConf IN h_base ("кассзаявком", STRING(komm)).
            RUN SetSysConf IN h_base ("кассзаявнетден", STRING(vNoKZ)).
...
FOR EACH op-entry
    WHERE op-entry.op-date <= vOp-date
      ...
      AND op-entry.acct-db EQ vAcct
      AND op-entry.acct-cr BEGINS '202'
      AND (op-entry.op-status BEGINS "√"
...
THEN DO:        /*Если кассовый символ со сниженной ставкой*/
    ...
    IF TIME <= cTime16_00
    THEN DO:
        IF NOT vNoKZ  /*Если по кассовой заявке*/
        THEN DO:
            IF vSummKS + vAmt + vUnAmt >= cSummMonthKS
            THEN DO:
                IF vSummKS >= cSummMonthKS /*Если превышено давно*/
                THEN komm = cAM_KS_KZ_P.
                ELSE DO:                           /*Если превышено в данной проводке*/
                    RUN dbgprint.p(program-name(1) + "cSummMonthKS:",STRING(cSummMonthKS)).
                    RUN dbgprint.p(program-name(1) + "vSummKS:",STRING(vSummKS)).
                    RUN dbgprint.p(program-name(1) + "vAmt:",STRING(vAmt)).
                    RUN dbgprint.p(program-name(1) + "vUnAmt:",STRING(vUnAmt)).
                    RUN dbgprint.p(program-name(1) + "cAM_KS_KZ:",STRING(cAM_KS_KZ)).

                    vSummKomm = RaschKommParts(cSummMonthKS, vSummKS, vAmt, vUnAmt, cAM_KS_KZ, cAM_KS_KZ).

                    RUN dbgprint.p(program-name(1) + "vSummKomm:",STRING(vSummKomm)).
                END.
            END.
            ELSE komm = cAM_KS_KZ.
        END.
        ELSE DO:    /*Если без кассовой заявки*/
            IF vSummKS + vAmt + vUnAmt >= cSummMonthKS
            THEN DO:
...
ELSE DO: /*Если кассовые символы с повышенной ставкой*/
    ...
    DO:
        ...
        END.
        ELSE DO: /*Если без кассовой заявки*/
            /* БЕЗ ЗАЯВКИ 400000 ; 801000 ; 0 ; 800000*/
            IF vSummNoKS + vAmt + vUnAmt >= cSummMonth
            THEN DO:
                IF vSummNoKS >= cSummMonth
...
IF (komm < 100) AND (komm NE 0) AND
   ((    CAN-DO(cKSymbols, vKS) AND TIME <= cTime16_00) OR
    (NOT CAN-DO(cKSymbols, vKS) AND TIME <= cTime16_00 AND vSummNoKS + vAmt <= cSummMonth)) THEN komm = 100.
