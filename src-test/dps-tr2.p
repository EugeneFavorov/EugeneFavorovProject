...
DO ON ERROR  UNDO, LEAVE
   ...
   &IF DEFINED(auto) = 0 &THEN
   {logg.i """Журнал ошибок при начислении процентов""" }
   &ELSE
   {logg.i """Журнал ошибок при начислении процентов""" "APPEND"}
   &ENDIF
   ...
   DO vi = 1 TO NUM-ENTRIES(t-type): 
      ...
      DO WHILE TRUE  
         ...
         DO:
            ...
            DO: 
		/*{get_date_kau.i}*/
/*для того, чтобы не перенастр. транз подключение контроля
                  дат начисл. оставлено по умолч. */
if loan.open-date > 06/03/2014 THEN DO:
/*message loan.cont-code " " loan.open-date VIEW-AS ALERT-BOX.*/
...
END.
   ...
   RUN PrintVed (in-op-date).
   
   &IF DEFINED(auto) = 0 &THEN
   
      /* Просмотр сохраненной информации по шаблонам, если необходимо */
      {prtfile.i &PREVIEWALL=YES}
   
      /* Ведомость ошибок */
      IF CAN-FIND (FIRST err) THEN DO:
         {setdest2.i
            &stream   = "stream err-ved"
            &filename = "_spool3.tmp"
            &cols     = 80
         }
         RUN ved_err ("ОШИБКИ ПРИ НАЧИСЛЕНИИ:").
        {preview2.i
             &stream   = "stream err-ved"
             &filename = "_spool3.tmp"
            &nodef    = "/*"
