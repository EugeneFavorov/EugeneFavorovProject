...
FOR EACH tmprecid,
   ...
   IF loan.since NE cessdate  THEN
   DO:
       message loan.since loan.cont-code view-as alert-box.
   END.
   ...
   lOK = hSAXWriter:START-ELEMENT("CDA_RATE"). /* различные ставки по договору */

   RELEASE comm-rate1.
   FOR EACH comm-rate1
      WHERE comm-rate1.kau EQ loan.contract + "," + loan.cont-code
      AND comm-rate1.commission EQ '%КРПР'
      AND comm-rate1.since LE cessdate
      NO-LOCK
      BY comm-rate1.since DESCENDING:
      LEAVE.
   ...
   IF AVAILABLE comm-rate1 THEN 
   DO:
      lOK = hSAXWriter:WRITE-FRAGMENT('	 <item id="interest_rate_on_overdue_debt">' + TRIM(string(comm-rate1.rate-comm,">>>>>>>>>>>9.99")) + '</item>\n'). 
   ...
   lOK = hSAXWriter:END-ELEMENT("CDA_RATE"). /* различные ставки по договору */
...
/*    lOK = hSAXWriter:WRITE-DATA-ELEMENT("DEBT",TRIM(string(comm-rate1.rate-comm,">>>>>>>>>>>9.99"))).       */
      lOK = hSAXWriter:WRITE-FRAGMENT('	 <item id="DEBT">' + TRIM(string(comm-rate1.rate-comm,">>>>>>>>>>>9.9999")) + '</item>\n'). 
   END.
...
/*    lOK = hSAXWriter:WRITE-DATA-ELEMENT("PERCENT",TRIM(string(comm-rate1.rate-comm,">>>>>>>>>>>9.99"))).       */
      lOK = hSAXWriter:WRITE-FRAGMENT('	 <item id="PERCENT">' + TRIM(string(comm-rate1.rate-comm,">>>>>>>>>>>9.9999")) + '</item>\n'). 
   END.
   ...
   FOR EACH term-obl 
      ...
      lOK = hSAXWriter:WRITE-DATA-ELEMENT("dtend",ISO-DATE(term-obl.end-date)).
...
/*
      ...
      /*================================================================  */
*/
      shortname = PERSON.NAME-LAST + " " + strtmp.
