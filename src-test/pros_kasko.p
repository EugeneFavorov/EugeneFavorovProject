/*
   ...
   Parameters:
      ...
      Used by:
      Created: 11/03/13
     Modified: 10/04/18 zss

     -добавление столбца Приз.проб
     -исключение из выборки КД те которые помечены признаком, цессии
*/
{globals.i}
{intrface.get loan}
{intrface.get i254}
...
DEFINE VARIABLE mPrproblem     AS CHARACTER NO-UNDO.
...
AND bloan.filial-id eq shFilial
 NO-LOCK:
	...
	and bbloan.filial-id eq shFilial
	 break by bbloan.end-date DESCENDING:
		IF CAN-DO("*ПК*", bbloan.parent-cont-code) THEN
			IF bbloan.open-date LT DATE(02,20,2013) OR GetXAttrValueEx("loan", STRING("СТРАХ," + bbloan.cont-code), "VidStr", "") begins "КАСКО"
			 AND NOT CAN-DO("!*ОВ*,*СОЮЗ*,*АЛЕКС*,*кол*",GetXattrValueEx("loan",bloan.contract + "," + bloan.cont-code,"priznak","")) THEN /* ориентировочно до 20.02.2013 г. все договора автострахования в банке были по умолчанию КАСКО, доп. реквизиты у них никто заполнять не собирается, поэтому так */
				IF bbloan.end-date GE beg-date																																																						/*zss добавление искл. условия на цессии*/				
    ...
    mPrproblem = GetXattrValueEx("loan",bloan.contract + "," + bloan.cont-code,"pr_problem","-").
    mcommrate = GET_COMM(
                          "%Кред",
                          ?,
                          bloan.currency,
