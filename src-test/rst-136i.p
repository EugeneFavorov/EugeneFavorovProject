/*
               Å†≠™Æ¢·™†Ô ®≠‚•£‡®‡Æ¢†≠≠†Ô ·®·‚•¨† Åàë™¢®‚
    Copyright: (C) 1992-2016 áÄé "Å†≠™Æ¢·™®• ®≠‰Æ‡¨†Ê®Æ≠≠Î• ·®·‚•¨Î"
     Filename: RST-136I.P
      Comment: ‡••·‚‡ ÆØ•‡†Ê®© 136-à
   Parameters: user-proc.params
     ...
     Modified: 05/05/2014 kraw (0223830) àß¨•≠•≠®• ß†£Æ´Æ¢™† ·Ø‡†¢™® Æ° Æ‚·„‚·‚¢®®...
*/
...
DEFINE VARIABLE mLines     AS CHARACTER   NO-UNDO EXTENT 32. /* í†°´®Á™† */
DEFINE VARIABLE mCLines    AS CHARACTER   NO-UNDO EXTENT 30. /* ë‚Æ´°•Ê ™Æ¨®··®© */
...
DEFINE VARIABLE mCommCol    AS LOGICAL   NO-UNDO. /* Ç·‚†¢´Ô‚Ï ·‚Æ´°•Ê ™Æ¨®··®© */
DEFINE VARIABLE mAlterForm  AS LOGICAL   NO-UNDO. /* Ä´Ï‚•‡≠†‚®¢≠†Ô ‰Æ‡¨† ®‚Æ£Æ¢ */
...
DEFINE VARIABLE mDispProxy  AS LOGICAL   NO-UNDO. /* é‚Æ°‡†¶†‚Ï ‡•™¢®ß®‚Î §Æ¢•‡•≠≠ÎÂ ´®Ê */
DEFINE VARIABLE mOperAcct   AS CHARACTER NO-UNDO.
DEFINE VARIABLE mOperCur    AS CHARACTER NO-UNDO.
 
DEFINE TEMP-TABLE ttRegister  NO-UNDO
   FIELD op       AS INT64     /* id §Æ™„¨•≠‚†          */
   FIELD doc-num  AS CHARACTER /* çÆ¨•‡ §Æ™„¨•≠‚†       */
   FIELD doc-time AS INT64     /* Ç‡•¨Ô ÆØ•‡†Ê®®        */
   ...
   FIELD dpr-id   AS INT64     /* id ·¨•≠Î §Æ™„¨•≠‚†    */
   FIELD forminfo AS CHARACTER /* à≠‰Æ‡¨†Ê®Ô ØÆ Á•™†¨ ≠† °„§„ÓÈ••  */
   FIELD vain     AS LOGICAL   /* è„·‚Æ© */
   FIELD curr-com AS CHARACTER /* Ç†´Ó‚† ™Æ¨®··®®       */
   FIELD amt-com  AS DECIMAL   /* ë„¨¨† ™Æ¨®··®®        */
...
DEFINE TEMP-TABLE ttReg-Det NO-UNDO
   ...
   FIELD Op       AS INT64      LABEL "çÆ¨•‡ ÆØ•‡†Ê®®"     
   ...
   THEN
      mStatus  = CHR(251).
ASSIGN
   mCodeRes   = FGetSetting("äÆ§ê•ß","","RUS")
   mAlterForm = (FGetSetting("ê••·‚‡136-à","Ä´Ï‚•‡≠îÆ‡¨†","çÖí") EQ "ÑÄ")
   mDispProxy = (FGetSetting("ê••·‚‡136-à","ÇÎ¢ÑÆ¢ã®Ê","Ñ†") EQ "Ñ†")
   mOperAcct  = FGetSetting("ê••·‚‡136-à","éØ•‡ä´ëÁ","51,52,53,54,59,60,61,62,63,64,65")
   mOperCur   = FGetSetting("ê••·‚‡136-à","éØ•‡Å•ßä„‡·†","55,57,60,63")
   mCommCol   = mLogComm AND (fGetSetting("Ç®§ê••·‚‡136®","","ç•‚") EQ "Ñ†").
   

ASSIGN
   mCLines[ 1] = IF mCommCol THEN "¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[ 2] = IF mCommCol THEN "≥         äÆ¨®··®Ô      " ELSE "".
   mCLines[ 3] = IF mCommCol THEN "≥                       " ELSE "".
   mCLines[ 4] = IF mCommCol THEN "≥       ¢ß®¨†•¨†Ô ß†    " ELSE "".
   mCLines[ 5] = IF mCommCol THEN "≥                       " ELSE "".
   mCLines[ 6] = IF mCommCol THEN "≥      Æ·„È. Ø•‡•¢Æ§†   " ELSE "".
   mCLines[ 7] = IF mCommCol THEN "≥                       " ELSE "".
   mCLines[ 8] = IF mCommCol THEN "≥                       " ELSE "".
   mCLines[ 9] = IF mCommCol THEN "≈ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[10] = IF mCommCol THEN "≥ äÆ§  ≥      ·„¨¨†     " ELSE "".
   mCLines[11] = IF mCommCol THEN "≥¢†´Ó‚Î≥                " ELSE "".
 /*mCLines[12] = IF mCommCol THEN "≥      ≥                " ELSE "".*/
 /*mCLines[13] = IF mCommCol THEN "≥      ≥                " ELSE "".*/
   mCLines[14] = IF mCommCol THEN "≈ƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[15] = IF mCommCol THEN "≥  16  ≥        17      " ELSE "".
   mCLines[21] = IF mCommCol THEN "≈ƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[22] = IF mCommCol THEN "≥      ≥                " ELSE "".
   mCLines[23] = IF mCommCol THEN "¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[24] = IF mCommCol THEN "¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[25] = IF mCommCol THEN "≈ƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[26] = IF mCommCol THEN "≈ƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[27] = IF mCommCol THEN "≥      ≥                " ELSE "".
   mCLines[28] = IF mCommCol THEN "¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
   mCLines[29] = IF mCommCol THEN "¬ƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ" ELSE "".
.
...
ASSIGN
   ...
   mLines[ 7] = IF CAN-DO('0516,0517', STRING(GetXattrValueEx("_user",STRING(_user._userid),"é‚§•´•≠®•",""))) THEN
                "≥        ≥(¢‡•¨Ô  ≥    ≥(™‡Æ··-≥                    ≥                    ≥    ≥¢†´Ó‚•                     ≥                    ≥     ≥·™Æ- ≥                    ≥                              ≥                         ≥                ≥      ≥"
                ELSE 
                "≥        ≥        ≥    ≥(™‡Æ··-≥                    ≥                    ≥    ≥¢†´Ó‚•                     ≥                    ≥     ≥·™Æ- ≥                    ≥                              ≥                         ≥                ≥      ≥"
   mLines[ 8] = IF CAN-DO('0516,0517', STRING(GetXattrValueEx("_user",STRING(_user._userid),"é‚§•´•≠®•",""))) THEN
                "≥        ≥é¨·™Æ•) ≥    ≥ -™„‡·)√ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒ≈ƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≥                    ≥     ≥£Æ   ≥                    ≥                              ≥                         ≥                ≥      ≥"
                ELSE
                "≥        ≥        ≥    ≥ -™„‡·)√ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒ≈ƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≥                    ≥     ≥£Æ   ≥                    ≥                              ≥                         ≥                ≥      ≥"
   ...
   mLines[22] = "≥        ≥        ≥    ≥       ≥ ,   ≥            , ≥     ≥              ≥    ≥      ≥     ≥              ≥                    ≥     ≥     ≥                    ≥                              ≥                         ≥                ≥      ≥"
   mLines[23] = "¿ƒƒƒƒƒƒƒƒ,ƒƒƒƒƒƒƒƒ¡ƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒŸ"
   mLines[24] = "¿ƒƒƒƒƒƒƒƒ¡ƒƒ ƒƒƒƒƒ¡ƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒŸ"
   ...
   mLines[26] = "√ƒƒƒƒƒƒƒƒ,ƒƒƒƒƒƒƒƒ≈ƒƒƒƒ≈ƒƒƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒ≈ƒƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒ¥"
   ...
   mLines[29] = "√ƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒ¬ƒƒƒƒ¬ƒƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒ¬ƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒ¥"
   mLines[30] = "≥        ≥        ≥    ≥       ≥     ≥              ≥     ≥              ≥    ≥      ≥     ≥              ≥                    ≥     ≥     ≥,                   ≥                              ≥                         ≥                ≥      ≥"
   mLines[31] = "√ƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒø                                                                                                                                                                                  "
   mLines[32] = "≥ ê•ß®§•≠‚ ≥ äÆ´-¢Æ ≥     à‚Æ£Æ ØÆ ê••·‚‡„                                                                                                                                                             "
...
PROCEDURE PrintRst.
   ...
   DEFINE VARIABLE vFio        AS CHARACTER   NO-UNDO EXTENT 10.
   DEFINE VARIABLE vPasport    AS CHARACTER   NO-UNDO EXTENT 10.
   DEFINE VARIABLE vAdress     AS CHARACTER   NO-UNDO EXTENT 10.
   DEFINE VARIABLE vBirthDay   AS CHARACTER   NO-UNDO EXTENT 10.
   ...
   FOR EACH ttRegister 
      ...
      DO:
         ...
         DO:
            PUT UNFORMATTED
               ...
               mSpace SPACE(57) "Ñ•≠Ï  å•·ÔÊ    ÉÆ§   " SKIP(2)


               PADC("ëèêÄÇäÄ éÅ éíëìíëíÇàà éèÖêÄñàâ ë çÄãàóçéâ àçéëíêÄççéâ ÇÄãûíéâ",110)                   SKIP(1)
         ...
         PUT UNFORMATTED
            ...
            mSpace SPACE(3) "èÆ‡Ô§™Æ¢Î© ≠Æ¨•‡ ê••·‚‡† ¢ ‚•Á•≠®• ‡†°ÆÁ•£Æ §≠Ô"      SPACE(7) vRstNum SKIP(3)
      ...
      IF CAN-DO(vOperAcct,ttRegister.doc-kind) AND NOT ttRegister.dover THEN do:
		   RUN GetFioByAcct(ttRegister.op,OUTPUT vFio[1],OUTPUT vPasport[1],OUTPUT vAdress[1],OUTPUT vBirthDay[1]).
                   /* ivv •·´®  vFio vAdress vBirthDay vPasport ≠• ß†ØÆ´≠•≠Î, ≠„¶≠Æ Ø´ÆØ‡Æ°Æ¢†‚Ï ¢ßÔ‚Ï ®Â · §Æ™„¨•≠‚†. ttRegister.country ß†ØÆ´≠Ô•¨, •·´® ≠• Ø„·‚Æ• ®¨Ô */
                   if  vFio[1]      = "" then do:
                   ...
                   if  vAdress[1]   = "" then vAdress[1]   = GetXAttrValue("op", STRING(ttRegister.op), "Ä§‡•·").
	   ...
	   ELSE 
		  ...
		  ELSE  do:
		    ...
		    .
                 ...
                 /*IF can-do(string(FGetSetting("à113","éØÉ‡15","")), ttRegister.doc-kind) and ttRegister.amt-cr < INT(FGetSetting("à113","ë„¨¨†1626","")) and ttRegister.amt-db < INT(FGetSetting("à113","ë„¨¨†1626","")) THEN vPasport[1] = "".*/
                 IF ((ttRegister.doc-kind EQ "01" AND ttRegister.amt-cr < INT(FGetSetting("à113","ë„¨¨†1626",""))) OR (ttRegister.doc-kind EQ "02" AND ttRegister.amt-db < INT(FGetSetting("à113","ë„¨¨†1626","")))) THEN vPasport[1] = "".
...
/*  4 */   " ≥"     IF CAN-DO(mOperCur, ttRegister.doc-kind) THEN "       " ELSE ttRegister.ChRate
/*  5 */    "≥ "    STRING(ttRegister.curr-db,"x(3)")
/*  6 */   " ≥"     IF ttRegister.doc-kind EQ "57" OR ttRegister.doc-kind EQ "63" THEN "              " ELSE STRING(ttRegister.amt-db, ">>>,>>>,>>9.99")
/*  7 */    "≥ "    STRING(ttRegister.curr-cr,"x(3)")
/*  8 */   " ≥"     IF ttRegister.doc-kind EQ "55" OR ttRegister.doc-kind EQ "60" THEN "              " ELSE STRING(ttRegister.amt-cr, ">>>,>>>,>>9.99")
...
/* 15 */    "≥ "    (IF ((ttRegister.doc-kind EQ "01" AND ttRegister.amt-cr < INT(FGetSetting("à113","ë„¨¨†1626",""))) OR (ttRegister.doc-kind EQ "02" AND ttRegister.amt-db < INT(FGetSetting("à113","ë„¨¨†1626","")))) THEN "   " ELSE STRING(ttRegister.country,"x(3)")) " "
...
*/
      ...
      DO:
         ...
         DO:
            ...
            DO:
               PUT UNFORMATTED 
                  mSpace ENTRY(1,mLines[28]) (IF mAlterForm THEN "¡" ELSE "ƒ") ENTRY(2,mLines[28]) SKIP
                  mSpace "≥ à‚Æ£® ØÆ ¢®§†¨ ÆØ•‡†Ê®© ¢ ‡†ß‡•ß• ™„‡·Æ¢                                                                                                                                                             " + FILL(" ",LENGTH(mCLines[1])) + "≥ " SKIP.
                  IF mAlterForm THEN
                     PUT UNFORMATTED
               ...
               FOR EACH ttRegItog WHERE 
                  ...
                  ELSE
                     PUT UNFORMATTED 
                        mSpace ENTRY(1,mLines[26]) (IF mAlterForm THEN "≈" ELSE "ƒ") ENTRY(2,mLines[26]) SKIP.
            ...
            "  ≥ "  IF CAN-DO(mOperCur, ttRegister.doc-kind) THEN "           " ELSE ttRegItog.ChRate
             " ≥ "  STRING(ttRegItog.curr-db,"x(3)")
            "  ≥ "  IF ttRegister.doc-kind EQ "57" OR ttRegister.doc-kind EQ "63" THEN "                  " ELSE STRING(ttRegItog.amt-db, ">>>,>>>,>>>,>>9.99")
             " ≥ "  STRING(ttRegItog.curr-cr,"x(3)")
            "  ≥ "  IF ttRegister.doc-kind EQ "55" OR ttRegister.doc-kind EQ "60" THEN "                  " ELSE STRING(ttRegItog.amt-cr, ">>>,>>>,>>>,>>9.99")
            ...
            "  ≥ "  (IF ttRegItog.amt-cheq NE 0 THEN STRING(ttRegItog.amt-cheq, ">>>,>>>,>>>,>>9.99") ELSE "                  ")
               ...
               PUT UNFORMATTED 
                  mSpace ENTRY(1,mLines[28]) (IF mAlterForm THEN "¡" ELSE "ƒ") ENTRY(2,mLines[28]) SKIP
                  mSpace "≥ à‚Æ£® ØÆ ¢®§†¨ ÆØ•‡†Ê®©                                                                                                                                                                              " + FILL(" ",LENGTH(mCLines[1])) + "≥ " SKIP.
            END.
...
/*  4 */   " ≥"   IF CAN-DO(mOperCur, ttRegItog.doc-kind) THEN "       " ELSE ttRegItog.ChRate
/*  5 */    "≥ "  STRING(ttRegItog.curr-db,"x(3)")
/*  6 */   " ≥"   IF ttRegItog.doc-kind EQ "57" OR ttRegItog.doc-kind EQ "63" THEN "              " ELSE STRING(ttRegItog.amt-db, ">>>,>>>,>>9.99")
/*  7 */    "≥ "  STRING(ttRegItog.curr-cr,"x(3)")
/*  8 */   " ≥"   IF ttRegItog.doc-kind EQ "55" OR ttRegItog.doc-kind EQ "60" THEN "              " ELSE STRING(ttRegItog.amt-cr, ">>>,>>>,>>9.99")
/*  9 10 */ "≥    ≥"  (IF ttRegItog.cheq-qty NE 0 THEN STRING(ttRegItog.cheq-qty, ">,>>>,>>>,>>9") ELSE "     ")
/* 11 */   " ≥"   STRING(ttRegItog.currcheq,"x(3)")
/* 12 */  "  ≥"   (IF ttRegItog.amt-cheq NE 0 THEN STRING(ttRegItog.amt-cheq, ">>>,>>>,>>>,>>9.99") ELSE "              ")
/* 13 */          ENTRY(3,mLines[27])
            ...
            END.
            PUT UNFORMATTED 
               mSpace ENTRY(1,mLines[23]) (IF mAlterForm THEN "¡" ELSE "ƒ") ENTRY(2,mLines[23]) SKIP(1).
         END.
         DO i = 1 TO mTotalSign:
            RUN PrintFioAndPost(CutFio(mFioInRep[i]),mPostInRep[i],length(mSpace)).
         END.
         ...
         DO:
            ...
            PUT UNFORMATTED
               ...
               SKIP(1)                                                                 
               SKIP mSpace "⁄ƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¬ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø"
               SKIP mSpace "≥  çÆ¨•‡  ≥         îàé ‰®ß®Á•·™Æ£Æ ´®Ê†         ≥ Ñ†‚† ® ¨•·‚Æ ‡Æ¶§•≠®Ô  ≥                      Ä§‡•· ‰®ß®Á•·™Æ£Æ ´®Ê†                     ≥         ç†®¨•≠Æ¢†≠®• §Æ™„¨•≠‚†, „§Æ·‚Æ¢•‡ÔÓÈ•£Æ ´®Á≠Æ·‚Ï         ≥"
               SKIP mSpace "≥         ≥                                      ≥ ‰®ß®Á•·™Æ£Æ ´®Ê†       ≥                                                                 ≥                                                                  ≥"
               SKIP mSpace "√ƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥"
            ...
            FOR EACH bttRegister WHERE
               ...
               DO:
                  ...
                  IF NOT bttRegister.dover AND
                     GetXAttrValue("op", STRING(bttRegister.op), "ÑÆ™„¨") NE ""
                  THEN
                     vPasport[1]  = GetPassport(bttRegister.op).
               END.
               ELSE
...
/*               END.                                                   */
            ...
            PUT UNFORMATTED
               SKIP mSpace "¿ƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ"
               SKIP(1)
               .
...
PROCEDURE CreateTemp-Table.
   ...
   DO i = 1 TO NUM-ENTRIES(mUsDprIDLst):
      ...
      DO: 
         ...
         IF AVAIL bsessions THEN
         DO:
            DO vI = 0 TO ((IF bsessions.dpr-status EQ "áÄäêõíÄ" THEN bsessions.dpr-close ELSE TODAY) - bsessions.dpr-open):
               IF NOT CAN-FIND(FIRST ttRegister WHERE 
...
PROCEDURE GetCountry.
   ...
   vVidOpNalV = GetXAttrValue("op",STRING(op.op), "Ç®§éØç†´Ç").

   IF (GetXAttrValue("op",STRING(op.op), "ÑÆ¢•‡") EQ "Ñ†") AND
...
/* oCountry = GetXAttrValue("op",STRING(op.op), "country-pers"). */
   ...
   vDocum = string(GetXAttrValue("op", string(op.op), "ÑÆ™„¨")).
   vFIO2  = string(GetXAttrValue("op", string(op.op), "îàé")).
...
/*=============================================================*/
   ...
   ELSE
      ...
      DO:
         oResident = IF oCountry EQ mCodeRes THEN "ÑÄ" ELSE "çÖí".
...
/* ivv ØÆ·™Æ´Ï™„ ·‚‡Æ™† §Æ™„¨•≠‚† ØÆ´≠Æ·‚ÏÓ ·‰Æ‡¨®‡Æ¢†≠† ¢ ‚‡†≠ß†™Ê®®, ‚Æ Ì‚Æ ´®Ë≠••.
