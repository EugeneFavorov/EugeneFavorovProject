...
DEFINE OUTPUT PARAM oMess        AS CHAR  NO-UNDO INIT "".
 ...
 IF in-cont-date GT mLim-date THEN DO:
   ...
   FIND LAST kau-entry
    WHERE kau-entry.kau begins "dps," + loan.cont-code + ",НачПр" 
      AND kau-entry.debit EQ 0 AND kau-entry.kau-id EQ 'loan-dps-int' NO-LOCK NO-ERROR.
   mSum = (IF AVAIL kau-entry THEN (IF kau-entry.currency NE '' THEN kau-entry.amt-cur ELSE kau-entry.amt-rub) ELSE 0).
   FOR EACH xkau-entry
    WHERE xkau-entry.kau begins "dps," + loan.cont-code + "," 
       AND xkau-entry.debit EQ 0 AND xkau-entry.kau-id EQ 'loan-dps-t' NO-LOCK BY xkau-entry.op-date DESC:
     IF AVAIL kau-entry AND AND xkau-entry.op-date < kau-entry.op-date THEN LEAVE.
     mSum = mSum - (IF xkau-entry.currency NE '' THEN xkau-entry.amt-cur ELSE xkau-entry.amt-rub).
     IF mSum <= 0 THEN LEAVE.
   END.
 IF (AVAIL op-entry AND mSum > mAmount) OR mSum <= 0 THEN DO:
      oMess =  "Нельзя делать довнесение на этот вклад позднее " + 
           STRING(mLim-date) + " в сумме более начисленных процентов".
      RETURN.     
 END.
 END.
 IF in-cont-date GT mLim-date THEN DO:
