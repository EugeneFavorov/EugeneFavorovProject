...
iCount = 0.
dSum = 0.
fName = 'plus_810_' + STRING(DAY(end-date),"99") + STRING(MONTH(end-date),"99") + STRING(YEAR(end-date),"9999") + ".txt".
...
FOR EACH op WHERE
    op.op-date = end-date 
    AND (op.op-kind = 'pernorved' or op.op-kind = 'PerechN' )
...
sName = 'Реестр на сумму ' + STRING(dSum, ">>>>>>>>>>>9.99") + CHR(10) + ' Документов ' + STRING(iCount) + CHR(10) + 'Выгружаем в Норвик'.
...
END.
ELSE DO:
	ffname = "/data/home2/bis/quit41d/imp-exp/norvik/out/" + fname.
/*	   ffname = "/home2/bis/quit41d/imp-exp/norvik/out/" + fname. */
