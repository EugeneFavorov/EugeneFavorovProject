...
DEFINE VARIABLE mAgentName    AS CHARACTER NO-UNDO.
DEFINE VARIABLE mAgentCountry AS CHARACTER NO-UNDO.
...
DEFINE TEMP-TABLE tt-117 NO-UNDO
   ...
   FIELD agent  AS CHARACTER /* Наименование контрагента, указанное в договоре */
   FIELD cagent AS CHARACTER /* Код страны контрагента */
   FIELD prizn  AS CHARACTER /* Признак представления резидентом документов */
   INDEX tsurr tsurr
.
...
&ENDIF
            ...
            mAgentName = GetXattrValueEx("op",STRING(op.op),"agent-name","").
            tt-117.agent = mAgentName.
            mAgentCountry = GetXattrValueEx("op",STRING(op.op),"agent-country","").
            tt-117.cagent = mAgentCountry.
            mPriznak = GetXattrValueEx("op",STRING(op.op),"ppdoc","").
            tt-117.prizn = mPriznak.
            /*ccc*/
