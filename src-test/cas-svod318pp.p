...
/*
     for each ttRepOp where  ttRepOp.Acct     eq bttRepOp.Acct
...
PROCEDURE AddRecInTemp-Table:
   ...
   /* ivv для исключения задвоения документов кассовых заявок пришеднших по клиент банку проводим поиск ниже по таблице ttRepOp */

      find first op-entry where op-entry.op = kau-entry.op 
                            and op-entry.op-entry = kau-entry.op-entry 
                            no-lock no-error.
      FIND FIRST ttRepOp WHERE ttRepOp.currency   = kau-entry.currency
                           AND ttRepOp.OpDate     = kau-entry.op-date
                           AND ttRepOp.Acct       = op-entry.acct-db
                           AND ttRepOp.doc-num    = bop.doc-num
                           AND ttRepOp.user-id    = bop.user-id
                           AND ttRepOp.doc-type   = bop.doc-type
                           AND ttRepOp.op-kind    = bop.op-kind
                           NO-ERROR.

      IF NOT AVAIL ttRepOp THEN do:
         CREATE ttRepOp.
         ASSIGN
            ttRepOp.currency = kau-entry.currency
            ttRepOp.op       = kau-entry.op
            ttRepOp.OpDate   = op-entry.op-date
            ttRepOp.Acct     = op-entry.acct-db
            ttRepOp.amt      = op-entry.amt-rub
            ttRepOp.doc-num  = bop.doc-num
            ttRepOp.user-id  = bop.user-id
            ttRepOp.doc-type = bop.doc-type
            ttRepOp.op-kind  = bop.op-kind
         .
         vFirstEntryInOp  = YES.

      end.
      else do:
         ttRepOp.amt      = ttRepOp.amt + op-entry.amt-rub.
      end.
     ...
     /* end  ivv */
      ...
      IF NOT AVAIL bttSprCash OR NOT mPodschKolDoc THEN
         IF kau-entry.debit THEN
            assign 
               ttSprCash.QtyDb = ttSprCash.QtyDb + 1 WHEN vFirstEntryInOp
            .
         ELSE
         DO:
            ...
            assign
               ttSprCash.QtyCr = ttSprCash.QtyCr + 1  WHEN vFirstEntryInOp
           . 
         END.
