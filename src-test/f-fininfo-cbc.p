...
DEFINE VARIABLE mAnkInterPay        AS LOGICAL   NO-UNDO FORMAT "Да/Нет"  INITIAL ? . 
...
&Scoped-define FIELDS-IN-QUERY-fMain tt-cust-ident. ~
...
&Scoped-Define ENABLED-OBJECTS vTarget ~
fCustName separator 
...
&Scoped-Define DISPLAYED-OBJECTS vTarget vTargetDetails ~
fCustName separator 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-1 vTarget ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
&Scoped-define List-2 vTarget ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
&Scoped-define List-3 vTarget ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
...
DEFINE VARIABLE vTarget AS CHARACTER FORMAT "X(20)":U 
     LABEL "Код цели" 
     VIEW-AS FILL-IN
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 57 BY 4
     &ELSE SIZE 57 BY 1 &ENDIF NO-UNDO.

DEFINE VARIABLE vTargetDetails AS CHARACTER FORMAT "X(500)":U 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vTarget TERMINAL-SIMULATION
ON F1 OF vTarget IN FRAME fMain /* Код цели */
    DO:
        DO TRANS:
            RUN browseld.p ("code",  /* Класс объекта. */
                "class" + CHR(1) + "parent" + CHR(1) + "title",
              /* Поля для предустановки. */
                "ДелОтнКл" + CHR(1) + "ДелОтнКл" + CHR(1) + "Цели и характер деловых отношений между клиентом и Банком",
        ...
        DO:
            ...
            IF vTarget:SCREEN-VALUE EQ "Иное" THEN
            DO:
                MESSAGE "Для вввода данных нажмите F2"  VIEW-AS ALERT-BOX .    
                mStrTMP = entry(1,GetXAttrValueEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_other",""),";") NO-ERROR.
                vTargetDetails:SCREEN-VALUE = "(F2) " + GetCodeDesc("ДелОтнКл",SELF:SCREEN-VALUE,1,"") + ": " + mStrTMP.
            END. 
            ELSE
                vTargetDetails:SCREEN-VALUE =  GetCodeDesc("ДелОтнКл",SELF:SCREEN-VALUE,1,"") .
                
            IF vTarget:SCREEN-VALUE EQ "РасчМеждунар" THEN /*RUN OpenWinMR. */
            
            RUN messmenu.p
               (
                 INPUT 7
               , INPUT "[ Проведение международных расчетов ]"
               , INPUT "Будут ли производиться переводы денежных средств на счета лиц-нерезидентов, не являющихся резидентами Республики Беларусь или Республики Казахстан и действующих в своих интересах или по поручению третьих лиц...?"
               , INPUT "Нет,Да"
               ) .
                   
          IF pick-value EQ "1" THEN 
            UpdateSignsEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_inter_pay","Нет").
          else         
            UpdateSignsEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_inter_pay","Да"). 
        END.        
          
    END.

ON F2 OF vTarget IN FRAME fMain /* Код цели */
    DO:    
        If vTarget:SCREEN-VALUE EQ "Иное" THEN 
        Do: 
            RUN  OpenWin.   
            mStrTMP = entry(1,GetXAttrValueEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_other",""),";").
            vTargetDetails:SCREEN-VALUE = "(F2) " + GetCodeDesc("ДелОтнКл",SELF:SCREEN-VALUE,1,"") + ": " + mStrTMP.   
        END.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vTarget TERMINAL-SIMULATION
ON LEAVE OF vTarget IN FRAME fMain /* Код цели */
DO:
  {&BEG_BT_LEAVE}
     FIND FIRST CODE WHERE
         CODE.class  = "ДелОтнКл" AND
         CODE.parent = "ДелОтнКл" AND
...
------------------------------------------------------------------------------*/ 
mStrTMP = entry(1,GetXAttrValueEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_other",""),";") NO-ERROR.
 If vTarget EQ "Иное" THEN  vTargetDetails = "(F2) " + vTargetDetails + ": " + mStrTMP.
 else vTargetDetails = "".
IF iMode EQ {&MOD_ADD} THEN
    tt-cust-ident.open-date = today.
  
  DISPLAY vTarget vTargetDetails fCustName separator 
...
------------------------------------------------------------------------------*/
   DO WITH FRAME fMain:
      ASSIGN
         vTarget        = vTarget:SCREEN-VALUE
         vTargetDetails = vTargetDetails:SCREEN-VALUE
         tt-cust-ident.open-date
      NO-ERROR.
      ASSIGN
         tt-cust-ident.cust-code      = vTarget + mSymbol + STRING(tt-cust-ident.open-date)
         tt-cust-ident.cust-code-type = "FinInfoChar"
         .
   END.
END PROCEDURE.
...
ELSE DO:
   ASSIGN
      vTarget        = ENTRY(1,tt-cust-ident.cust-code,mSymbol)
      vTargetDetails = GetCodeDesc("ДелОтнКл",vTarget,1,"")
      .
END.
END PROCEDURE.
...
PROCEDURE OpenWin :   
    ...
    IF mTmpStr3 NE "" THEN
    DO:
        mTmpStr = ENTRY(1,mTmpStr3,";") NO-ERROR.      
        IF  mTmpStr = "" THEN 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
/*prosignTNcTwStpAHbQBBUrb3f83g*/


/*IF NUM-ENTRIES(person.issue,",") LT 2 THEN DO:    entry(2, str, "|")
*/