...
&Scoped-define FIELDS-IN-QUERY-fMain tt-cust-ident.cust-code-type tt-cust-ident.open-date tt-cust-ident.close-date tt-cust-ident.cust-id tt-cust-ident.cust-cat 
&Scoped-define ENABLED-FIELDS-IN-QUERY-fMain tt-cust-ident.cust-code-type tt-cust-ident.open-date tt-cust-ident.close-date tt-cust-ident.cust-id tt-cust-ident.cust-cat 
...
&Scoped-Define ENABLED-FIELDS tt-cust-ident.cust-code-type tt-cust-ident.open-date tt-cust-ident.close-date tt-cust-ident.cust-id tt-cust-ident.cust-cat 
&Scoped-define ENABLED-TABLES tt-cust-ident
&Scoped-define FIRST-ENABLED-TABLE tt-cust-ident
&Scoped-Define ENABLED-OBJECTS vSource vSourceDesc separator 
&Scoped-Define DISPLAYED-FIELDS tt-cust-ident.cust-code-type tt-cust-ident.open-date tt-cust-ident.close-date tt-cust-ident.cust-id tt-cust-ident.cust-cat 
&Scoped-define DISPLAYED-TABLES tt-cust-ident
&Scoped-define FIRST-DISPLAYED-TABLE tt-cust-ident
&Scoped-Define DISPLAYED-OBJECTS vSource vSourceDesc fCustName separator 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-1 vSource vSourceDesc tt-cust-ident.open-date tt-cust-ident.close-date 
&Scoped-define List-2 vSource vSourceDesc tt-cust-ident.open-date tt-cust-ident.close-date  
&Scoped-define List-3 vSource vSourceDesc tt-cust-ident.open-date tt-cust-ident.close-date 
...
DEFINE VARIABLE vSourceDesc AS CHARACTER 
...
DEFINE FRAME fMain
    ...
    vSourceDetails
          ...
          &ELSE AT ROW 4 COL 16 COLON-ALIGNED &ENDIF NO-LABEL
     vSourceDesc
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 5 COL 21
          &ELSE AT ROW 5 COL 21 &ENDIF HELP
          "Описание информации" NO-LABEL
     tt-cust-ident.open-date
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 16 COLON-ALIGNED
          &ELSE AT ROW 9 COL 16 COLON-ALIGNED &ENDIF HELP
     ...
     tt-cust-ident.close-date
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 45 COLON-ALIGNED
          &ELSE AT ROW 9 COL 45 COLON-ALIGNED &ENDIF HELP
    ...
    APPLY "VALUE-CHANGED" TO SELF.
        ...
        IF vSource:SCREEN-VALUE EQ "Иное" THEN
            DO:
                ...
                mStrTMP = entry(3,GetXAttrValueEx('cust-corp',STRING(tt-cust-ident.cust-id),"ank_other",""),";") NO-ERROR. 
                vSourceDetails:SCREEN-VALUE = "(F2) " + GetCodeDesc("ИстДенСредств",SELF:SCREEN-VALUE,1,"") + ": " + mStrTMP.
            END. 
            ELSE
                vSourceDetails:SCREEN-VALUE =  GetCodeDesc("ИстДенСредств",SELF:SCREEN-VALUE,1,"") .
   END.
...
DO:
  {&BEG_BT_LEAVE}
  IF mClass-code EQ "ИстДенСредств" THEN DO:
     FIND FIRST CODE WHERE
         CODE.class  = "ИстДенСредств" AND
         CODE.parent = "ИстДенСредств" AND
         CODE.code   = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
     IF NOT AVAIL CODE THEN DO:
         RUN Fill-SysMes IN h_tmess ("", "", "-1", "Значения реквизита " + SELF:SCREEN-VALUE + 
                                     " нет в классификаторе ИстДенСредств.").
         RETURN NO-APPLY {&RET-ERROR}.
     END.
     ELSE
  do:
  ...
  ASSIGN
     ...
     .
  IF vSource EQ "" THEN DO:
     RUN Fill-SysMes IN h_tmess ("", "", "-1","Реквизит 'Источник' обязательный").
     RETURN NO-APPLY {&RET-ERROR}.
  END.
  {&END_BT_LEAVE}
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   RUN GetObject IN THIS-PROCEDURE.
   ...
   RUN DispCustName IN THIS-PROCEDURE.
   
   /* Заполняем COMBO-BOX'ы данными из метасхемы */
   RUN FillComboBox IN THIS-PROCEDURE (FRAME {&MAIN-FRAME}:HANDLE).
...
IF iMode EQ {&MOD_ADD} THEN
  ...
  DISPLAY vSource vSourceDetails vSourceDesc fCustName separator 
      WITH FRAME fMain IN WINDOW TERMINAL-SIMULATION.
  IF AVAILABLE tt-cust-ident THEN 
    DISPLAY tt-cust-ident.open-date tt-cust-ident.close-date tt-cust-ident.cust-id tt-cust-ident.cust-cat 
      WITH FRAME fMain IN WINDOW TERMINAL-SIMULATION.
  ENABLE vSource vSourceDetails vSourceDesc tt-cust-ident.open-date tt-cust-ident.close-date fCustName 
...
------------------------------------------------------------------------------*/
   DO WITH FRAME fMain:
      ASSIGN
         vSource     = vSource:SCREEN-VALUE
         vSourceDesc = vSourceDesc:SCREEN-VALUE
      NO-ERROR.
      ASSIGN
         tt-cust-ident.cust-code = vSource + mSymbol + STRING(TODAY) + STRING(TIME)
         tt-cust-ident.issue     = vSource + mSymbol + vSourceDesc
         .
      END. 
    RETURN.
END PROCEDURE.
...
IF iMode NE {&MOD_ADD} THEN
   ASSIGN 
      ...
      vSourceDesc = ENTRY(2,tt-cust-ident.issue,mSymbol)
      .
END PROCEDURE.
...
PROCEDURE OpenWin :   
    ...
    IF mTmpStr3 NE "" THEN
    DO:
        mTmpStr = entry(3,mTmpStr3,";") NO-ERROR.      
        if  mTmpStr = "" THEN
            mTmpStr2 = entry(1,mTmpStr3,";") + ";" + entry(2,mTmpStr3,";") + ";" + mStr + ";" + entry(4,mTmpStr3,";") + ";" + entry(5,mTmpStr3,";") + ";" + entry(6,mTmpStr3,";") + ";".
        else                                                           
...
/* $LINTENV ='1ut' */
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTDATE='30/12/2015 15:39:01.200+04:00' */
/* $LINTUSER='kozv' */
/* $LINTMODE='1' */
/* $LINTFILE='f-fininfo-source.p' */
/*prosignc7IrHif3eVtPbVQhAkO86w*/