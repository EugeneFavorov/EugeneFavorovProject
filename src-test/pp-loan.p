...
DEFINE TEMP-TABLE ttIndicate1 NO-UNDO LIKE ttIndicate.
DEFINE TEMP-TABLE ttIndicate2 NO-UNDO LIKE ttIndicate.


DEFINE VARIABLE mDelayProc AS LOGICAL NO-UNDO.

PROCEDURE StartInterface:
   mDelayProc = FGetSetting("ПробегОплаты","ПробегОпл%","Нет") = "Да".
   RETURN.
END PROCEDURE.
...
{form.def}
{lshpr.pro}
{lshpr-buff.pro} /*дубль процедур из lshpr.pro вместо вхоных параметров 
                   iContract iContCode передаются buffer loan*/
{loan.pro}
{loan-buff.pro}  /*дубль процедур из loan.pro вместо вхоных параметров 
                  iContract iContCode передаются buffer loan*/
{loanint.pro}
{loan-srk.pro}
{loan-srk-buff.pro}  /*дубль процедур из loan-srk.pro вместо вхоных параметров 
                       iContract iContCode передаются buffer loan*/
{jloan_counters.i}
...
{pfuncdef 
   &DEFPROC     = "GetTTReportTable"
   &DESCRIPTION = "Возвращает ТТ ttReportTable"
   &PARAMETERS  = "OUTPUT ttReportTable"
   &RESULT      = "ТТ ttReportTable"
   &SAMPLE      = "RUN GetTTReportTable IN h_loan (OUTPUT ttReportTable)."
} 
PROCEDURE GetTTReportTable:
   DEFINE OUTPUT PARAM TABLE FOR ttReportTable.
END PROCEDURE. 
...
FUNCTION GetFstWrkDay RETURNS DATE (
   ...
   vResDate = IF vResDate =  ? THEN in-date ELSE vResDate.
   RETURN vResDate.
END FUNCTION.
...
DEFINE VAR vRoleList AS CHAR NO-UNDO INIT "".
   ...
   FOR EACH b-acct      WHERE b-acct.bal-acct      =  INT64(iOrigBalAcct) NO-LOCK,
       ...
       END.
       ELSE DO:
          IF NOT CAN-DO(GetMainAcctRole(b-loan.Class-Code, b-loan.contract),
...
FUNCTION GET_MAIN_COMM_RATE RETURNS DEC
  (INPUT iContract  AS CHAR,   /* Идентификатор */
   ...
   RUN GetMainLnId IN THIS-PROCEDURE (INPUT vRec, OUTPUT vRec) .
...
FUNCTION getNextCommDateTransh RETURNS DATE
   ...
   IF vRetVal =  ? AND NUM-ENTRIES (iContCode," ") >  1 THEN
      vRetVal = GET_NEXT_COMM_DATE_LOAN (iContract,
                                         ENTRY (1, iContCode, " "),
                                         iComm,
                                         iBegDate,
...
/* Метод создает привязку проводки к текущему договору, если это возможно 
   ошибки возвращаются через RETURN-VALUE 
*/
PROCEDURE LinkOpEntryMeth:
   ...
   DEF VAR vErrMess       AS CHAR NO-UNDO.   /* Сообщение об ошибке */

   FIND FIRST op-entry WHERE RECID(op-entry) = INT64(iOpEntry) NO-LOCK NO-ERROR.
   ...
   IF AVAIL loan THEN
      FIND LAST op WHERE op.op =  op-entry.op NO-LOCK NO-ERROR.
   
   IF AVAIL op 
      AND IsBindEarlier(op.contract-date, loan.since)
   ...
   DO TRANSACTION
      ...
      RUN UpdContract IN THIS-PROCEDURE (
      ...
      RUN ConfirmAction IN THIS-PROCEDURE (
      ...
      RUN SetLIntDate IN THIS-PROCEDURE (vContractDate).

      /* Пересчет договора на подтвержденную плановую дату. */
      IF vContractDate <  loan.since THEN 
         RUN LoanCalc IN THIS-PROCEDURE (
            iContract,       /* Назначение договора. */
            iContCode,       /* Номер договора. */
      ...
      ** что-бы проверки были после подтверждения суммы и плановой даты */
      RUN RunChkMethod IN THIS-PROCEDURE (
         iContract,        /* Назначение договора. */
         iContCode,        /* Номер договора. */
      ...
      DO:
         IF vErrMess = "-" THEN
            RUN Fill-SysMes IN h_tmess ("","","0", "Ошибка привязки проводки.").
         ELSE
            RUN Fill-SysMes IN h_tmess ("","","0", vErrMess).
         UNDO MAIN, LEAVE MAIN.
      END.
      ...
      ** Определение суммы и валюты операций. */
      ...
      DO:
         ...
         UNDO MAIN, LEAVE MAIN.
      END.
                        /* Если операции подобраны,
                        ** но все нулевые,
                        ** то сообщение пользователю и аналитику не создаем*/
      IF NOT CAN-FIND (FIRST lInt WHERE lInt.Amt > 0)   THEN
      DO:
         vErrMess =  "Не могу создать операцию " + QUOTER (iCodeInt)  +
                     " по документу " + QUOTER (op.doc-num) + " за " + QUOTER (op.op-date) +
                     " по договору " + QUOTER (DelFilFromLoan (iContCode))   +
                     ": нулевая сумма операции.".
         RUN Fill-SysMes IN h_tmess ("","","0",vErrMess).
         UNDO MAIN, LEAVE MAIN.
      END.

      /* Создание операции. */
      RUN CreLInt IN THIS-PROCEDURE (
         iContract,       /* Назначение договора. */
         iContCode).      /* Номер договора. */
      ...
      RUN CreEntryKau IN THIS-PROCEDURE(
         RECID(op-entry), /* Идентификатор проводки. */
         iContract,        /* Назначение договора. */
         iContCode,        /* Номер договора. */
         iCodeInt,         /* Код вида операции. */
      ...
      IF {assigned vErrMess} THEN
      DO:
         IF vErrMess BEGINS "lock" THEN
         DO:
                         
            IF vErrMess =  "lock_op" THEN
               RUN wholocks2.p(RECID (op),"op","").
            ELSE
               RUN wholocks2.p(RECID(op-entry),"op-entry","").
...
PROCEDURE UnLinkOpEntryMeth.
   ...
   DO TRANSACTION
      ...
      IF op.contract-date <  loan.since
      ...
      THEN
      DO:
         IF RETURN-VALUE BEGINS "lock" THEN 
            RUN Fill-SysMes IN h_tmess ("","","0", "Ошибка отвязки проводки из-за блокировки.").
         ELSE
...
PROCEDURE GetNumLoan.
   ...
   DO:
      ...
      END.
      ASSIGN
         iBranch-id    = IF iBranch-id =  ? THEN "" ELSE iBranch-id         /* Проверим на "пустое" значение кода филиала */
      ...
      IF vObType <> "" THEN DO:    
         ...
         IF INDEX(oNum,"с") <> 0 THEN 
         DO:                              /* тэг "с" порядковый номер */
            vTmpStrT = SUBSTRING (oNum, INDEX(oNum, "[с") + 1, 
                                              INDEX(oNum, "с]") - INDEX(oNum, "[с")).
            IF FGetSetting("НомДогОб","","") = "2" THEN
            DO:
               ASSIGN
                  vTmpStr  = SUBSTRING (oNum, 1, INDEX(oNum, "[с") - 1)
                  vTmpStr2 = SUBSTRING (oNum, INDEX(oNum, "с]") + 2)
                  vTmpInt  = 0.
               FOR EACH term-obl WHERE term-obl.contract  =  vCountract
                                   AND term-obl.cont-code =  vCounCode 
                  ...
                  vSurr = term-obl.contract         + "," +
                          ...
                          STRING(term-obl.nn).
                  vNumDogOb = ENTRY(1, GetXAttrValueEx ("term-obl", vSurr, "НомДогОб", ""),"@").
              
                  IF vTmpStr  <> "" THEN vTmpstr3 = REPLACE(vNumDogOb,vTmpStr,"").
                  ELSE IF vTmpStr =  "" THEN vTmpstr3 = vNumDogOb. 
                  IF vTmpStr2 <> "" THEN vTmpstr3 = REPLACE(vTmpStr3,vTmpStr2,"").
              
                  vTmpInt2 = INT64(vTmpStr3) NO-ERROR.
              
                  IF NOT ERROR-STATUS:ERROR AND vTmpInt2 >  vTmpInt THEN vTmpInt = vTmpInt2.
               END.   
            END.
            ELSE
               vTmpInt = oCounterValue.   
            IF LENGTH(STRING(vTmpInt + 1)) >= LENGTH(vTmpStrT) THEN
...
PROCEDURE GetStaffByAcct.
   ...
   DO:
      ...
      /*oRate = GET_COMM(vTax,
                       ...
                       "",0,0,
                       iDate).*/
      FIND LAST comm-rate WHERE comm-rate.since       <= iDate
                            AND comm-rate.filial-id = shfilial
                            AND comm-rate.branch-id = ""
                            AND comm-rate.acct        =  '0'
                            AND comm-rate.commission  =  vTax
...
{pfuncdef 
   &DEFPROC="ALL_PARAM"
   &DESCRIPTION="Процедура расчета параметра на дату заданную дату методом,~
 применяемым для браузера brwl-par.p"
   &PARAMETERS="НАЗНАЧЕНИЕ ДОГОВОРА,ИДЕНТИФИКАТОР ДОГОВОРА,~
   Код параметра,Дата расчета"
   &RESULT="Сумма параметра,Валюта параметра,Сумма параметра в рублях"
   }
...
PROCEDURE ALL_PARAM.
   ...
   DEFINE INPUT PARAMETER iDate     AS DATE        NO-UNDO.
   DEFINE OUTPUT PARAMETER oAmt     AS DECIMAL     NO-UNDO. /* Сумма параметра */
   DEFINE OUTPUT PARAMETER oCurr    AS CHARACTER   NO-UNDO. /* Валюта параметра */
   DEFINE OUTPUT PARAMETER oAmtRub  AS DECIMAL     NO-UNDO. /* Сумма параметра в рублях */
   ...
   DO:
      ...
      DO:
         ...
         RUN STNDRT_PARAM IN THIS-PROCEDURE 
                          (iContract,
                           iContCode,
                           iAmtID,
                           iDate,
         ...
         RUN inter_current  IN THIS-PROCEDURE 
            (BUFFER bloan, (iAmtID - vCodOstPar), OUTPUT vAmtDiff) .
         oAmt = oAmt + vAmtDiff.
         IF LOOKUP(STRING(iAmtID), vParamStr) <> 0 THEN
         DO:
            ...
            DO:
               FIND FIRST bterm-obl WHERE
                      ...
                      AND bterm-obl.idnt      =  6
                      AND bterm-obl.end-date  =  iDate
                      AND bterm-obl.nn        =  vCode
               NO-LOCK NO-ERROR.
         ...
         DO:
            vAmtDiff = 0.
            RUN CalcPrmValue IN THIS-PROCEDURE 
                             (iContract,
                              iContCode,
                              iAmtID,
                              oAmt,
   ...
   DO:
      ...
      DO:
         ...
         FOR EACH comm-rate WHERE
                  comm-rate.kau        =  bloan.contract + "," + bloan.cont-code
             AND  comm-rate.commission <> "%Рез"
             AND  comm-rate.commission <> "%ОРез"
         USE-INDEX kau NO-LOCK:
            IF LOOKUP (comm-rate.commission, mProcs) =  0 THEN
               mProcs = mProcs + (IF mProcs <> "" THEN "," ELSE "") + comm-rate.commission.
         END.
         ...
         NO-LOCK:
            ...
            IF GetXAttrValueEx("loan-cond", mSurr, "PayType", "") =  "Пролонгац" THEN
               ASSIGN
                  mProlCount     = mProlCount + 1
                  mFirstProlDate = IF mFirstProlDate =  ?
                                      THEN bloan-cond.since
                                      ELSE mFirstProlDate
               .
         END.
            /* Если пролонгаций еще не было, то берем дату окончания договора */
         IF mFirstProlDate =  ? THEN
         ...
         FIND LAST bloan-cond WHERE
                   bloan-cond.contract  =  bloan.contract
            AND    bloan-cond.cont-code =  bloan.cont-code
            AND    bloan-cond.since     <= bloan.end-date /* Новая дата договора */
         NO-LOCK NO-ERROR.
         ...
         DO:
            ...
            DO mCntr = 1 TO NUM-ENTRIES(mProcs):
               FIND LAST comm-rate WHERE
                  ...
                  AND    comm-rate.since      <= xloan-cond.since /* Дата пролонгации */
                  AND    comm-rate.commission =  ENTRY(mCntr, mProcs)
               ...
               DO:
                  ...
                  IF comm-rate.commission =  "%Деп" THEN
            ...
            FOR EACH bloan-cond WHERE bloan-cond.contract  = iContract
            ...
            IF oResult <> {&RET-ERROR} THEN
               ASSIGN
                  oOk     = TRUE
                  oResult = (IF mTarifErr <> "" THEN mTarifErr + " " ELSE "") +
                            (IF mTarifNew <> "" THEN "Новый тариф: " + mTarifNew + " " ELSE "") +
...
PROCEDURE DepPogProlChance:
   ...
   IF FGetSetting("КонтПроцСтав", "", "ДА") = "НЕТ" THEN
      oOk = TRUE.
   ELSE
   DO:
      ...
      DO:
         ...
         ELSE IF     tmp-code.end-date <> ?
                 AND tmp-code.end-date < iDate THEN
         ...
         DO:
            ...
            FIND LAST bcomm-rate WHERE
                      bcomm-rate.kau        = "ПродТрф," + vTarif
               AND    bcomm-rate.commission = "%Деп"
               AND    bcomm-rate.since      <= iDate
            USE-INDEX kau NO-LOCK NO-ERROR.
               /* Если ставка не менялась, то пролонгируем */
            IF     AVAIL comm-rate
               AND AVAIL bcomm-rate
               AND comm-rate.rate-comm = bcomm-rate.rate-comm THEN
               oOk = TRUE.
               /* Для протокола */
...
PROCEDURE GetTarifRate.
   ...
   DO:
      FIND LAST comm-rate WHERE
                comm-rate.kau        = "ПродТрф," + iTarif
         AND    comm-rate.commission = iCommission
         AND    comm-rate.since      <= iDate
      USE-INDEX kau NO-LOCK NO-ERROR.
      IF AVAIL comm-rate THEN
...
PROCEDURE GetParamTarif.
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      ASSIGN
         ...
         vRate       = IF NUM-ENTRIES(vRate, "R") >= 2
...
PROCEDURE GetLoanTarif.
   ...
   IF NUM-ENTRIES(vStrParam, ";") <> 0 THEN
   DO:
      ASSIGN
         ...
         vPoruchit   = IF vPoruchit = "" THEN "НЕТ" ELSE vPoruchit
         vStrah      = IF vStrah    = "" THEN "НЕТ" ELSE vStrah
         vSummMin    = "0.00"
         vSummMax    = "0.00"
         vSrokMin    = "0"
         vSrokMax    = "0"
   ...
   DO:
      FIND FIRST code WHERE
                 code.class  = 'ПродТарифы'
         AND     code.parent = vProdTmpC
         AND     code.code   = oTarif
      NO-LOCK NO-ERROR.
...
PROCEDURE CalcCommStart.
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      ** есди нет, то ругаемся и выдаем ошибку */
      FIND FIRST commission WHERE commission.commission = iComm
                              AND commission.currency   = iCurrency
                              AND commission.min-value  = 0
                              AND commission.period     = 0
      NO-LOCK NO-ERROR.
      ...
      FIND FIRST comm-rate WHERE comm-rate.commission = iComm
                             AND comm-rate.acct       = "0"
                             AND comm-rate.currency   = iCurrency
                             AND comm-rate.kau        = iContract + "," + iContCode
                             AND comm-rate.min-value  = 0
                             AND comm-rate.period     = 0
      USE-INDEX kau NO-LOCK NO-ERROR.
...
PROCEDURE GetOpCodeFromAcct.
   ...
   FIND FIRST loan-acct WHERE
      ...
      AND     loan-acct.currency  = iCurrency
   ...
   DO:
      ...
      FIND FIRST code WHERE
                 code.class  =     "ТипСчДог"
         AND     code.parent =     loan-acct.acct-type
...
PROCEDURE ReqAcctByRole.
   ...
   DO:
      ...
      FIND LAST loan-acct WHERE
                loan-acct.contract  = iContract
         AND    loan-acct.cont-code = iContCode
         AND    loan-acct.acct-type = iAcctRole
         AND    loan-acct.since     <= iDate
      NO-LOCK NO-ERROR.
      IF AVAIL loan-acct THEN
      DO:
      ...
      DO:
         ...
         DO:
            ASSIGN
               ...
               vBal2      = IF LENGTH(vBal2) >= 5 THEN SUBSTR(vBal2, 1, 5) ELSE ""
               vFlt[1]    = "MustOffLdFlt|acct-cat"  + CHR(1) + "bal-acct"           + CHR(1) +
                            "acct"                   + CHR(1) + "currency"           + CHR(1) + "close-date1"
               vFlt[2]    = "YES|" + (IF iAcctCat      = ? THEN "*" ELSE iAcctCat)      + CHR(1) +
                                     (IF vBal2         = ? THEN "*" ELSE vBal2)         + CHR(1) +
                                     (IF pick-value    = ? THEN "*" ELSE pick-value)    + CHR(1) +
                                                                 "*"                     + CHR(1) + "?"
               pick-value = ?
            .
            IF iMess THEN
...
PROCEDURE CRCONDEX:
   ...
   IF iPayType = ? THEN iPayType = "Остаток".
   IF iLIstOutStav = ? THEN iLIstOutStav = "".
   ...
   DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
      ...
      FIND FIRST b-loan-cond WHERE
                 b-loan-cond.contract  = iContract
         AND     b-loan-cond.cont-code = iContCode
         AND     b-loan-cond.since     = iDate
      NO-LOCK NO-ERROR.
      ...
      IF NOT AVAIL loan-cond THEN
         ...
         /* Создаем список действующих ставок */
      FOR EACH comm-rate WHERE
               comm-rate.kau        = b-loan.contract + "," + b-loan.cont-code
          AND  NOT CAN-DO(iLIstOutStav,comm-rate.commission) NO-LOCK:
         IF LOOKUP (comm-rate.commission, vProcs) = 0 THEN
            vProcs = vProcs + comm-rate.commission + ",".
      END.
         /* Создаем условие */
      CREATE b-loan-cond.

      IF GetXAttrValueEx("loan-cond", loan-cond.contract + "," + loan-cond.cont-code + "," + STRING(loan-cond.since), "СхемаПлат","") = "Дифференцированная" THEN
      ...
      ** на которую создается условие, создать(проверив) плановую сумму (idnt=2)
      ** на дату условия на сумму ноль */
      IF b-loan.end-date <= iDate 
         AND NOT CAN-FIND(term-obl WHERE 
                          term-obl.contract  = b-loan.contract
                      AND term-obl.cont-code = b-loan.cont-code
                      AND term-obl.end-date  = iDate
                      AND term-obl.idnt      = 2 NO-LOCK)
      THEN 
      DO:
         CREATE term-obl.
         ASSIGN
      ...
      FIND LAST  fterm-obl  WHERE
                 fterm-obl.contract  = b-loan.contract
            AND  fterm-obl.cont-code = b-loan.cont-code
            AND  fterm-obl.idnt      = 2
            AND  fterm-obl.end-date  <= iDate
            NO-LOCK NO-ERROR .
...
/* корректируем количество льготных периодов */
      ...
      /* Пересчитать остаток льг.периодов по ОД и %% */
      IF   vlgot   > 0 
        OR vlgotpr > 0 THEN
         RUN calclgotper.p (iContract,
                            iContCode,
                            loan-cond.since,
                            b-loan-cond.since,
                            GetXAttrValueEx("loan-cond",
                                             GetSurrogateBuffer(
                                                       "loan-cond",
                                                       (BUFFER b-loan-cond:HANDLE)),
                                             "cred-offset",""),
                            INPUT-OUTPUT vlgot,
                            INPUT-OUTPUT vlgotpr).
      ...
      DO vCntr = 1 TO NUM-ENTRIES(vProcs):
         IF ENTRY(vCntr, vProcs) <> "" THEN
         DO:
            FIND LAST comm-rate WHERE
                      comm-rate.kau        = iContract + "," + iContCode
               AND    comm-rate.commission = ENTRY(vCntr, vProcs)
               AND    comm-rate.since      < iDate
               AND    comm-rate.acct       = "0"
               NO-LOCK NO-ERROR.
            IF     AVAIL comm-rate
               AND comm-rate.commission <> "%Рез" THEN
            DO:
               IF CAN-FIND (FIRST b-comm-rate WHERE
                                  b-comm-rate.kau        = comm-rate.kau
                              AND b-comm-rate.since      = iDate
                              AND b-comm-rate.commission = comm-rate.commission
                              AND b-comm-rate.acct       = comm-rate.acct) THEN
               DO:
...
PROCEDURE UpdComm:
   ...
   DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
      ...
      IF iStav <> 0 THEN DO:
         ...
         FIND FIRST comm-rate WHERE
                    comm-rate.kau        = iContract + "," + iContCode
                AND comm-rate.commission = iCommision
                AND comm-rate.since      = iDate
            EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
...
PROCEDURE NachProcRate.
   ...
   DO:
      ...
      UpdateSignsEx(loan.Class-Code, vSurr, "БазаНач",  vBaseNach + (IF vN = 0 THEN "" ELSE ",") + iBasePar).
      UpdateSignsEx(loan.Class-Code, vSurr, "БазаСтав", vBaseStav + (IF vN = 0 THEN "" ELSE ",") + iCommission).
...
PROCEDURE CrNewTermObl.
   ...
   DO ON ERROR  UNDO MAIN, LEAVE MAIN
      ...
      FOR EACH bSigns WHERE bSigns.file-name = "loan-cond"
                        AND bSigns.surrogate = bLoanCond2.contract + "," + bLoanCond2.cont-code
         NO-LOCK:
...
PROCEDURE pGetBefDepRate.
   ...
   DO ON ERROR  UNDO MAIN, LEAVE MAIN
      ...
      FOR EACH pro-obl WHERE
               pro-obl.contract  = loan.contract
           AND pro-obl.cont-code = loan.cont-code
           AND pro-obl.idnt      = 3
      ...
      FIND FIRST ttRate WHERE
              ttRate.firstday <= vNumDays
          AND ttRate.lastday  >= vNumDays
      NO-LOCK NO-ERROR.
...
PROCEDURE ValidCloseLoan.
   ...
   FOR EACH loan-par WHERE loan-par.amt-id < 1000 NO-LOCK:
      ...
      RUN inter_current IN THIS-PROCEDURE (BUFFER loan,loan-par.amt-id,OUTPUT summ).
...
PROCEDURE CloseLoan.
   &SCOPED-DEFINE vCounter_fl-o vCounter_fl-o
   ...
   DEFINE VAR {&vCounter_fl-o} AS INT64 NO-UNDO.
   ...
   DEFINE VAR vFlUndo   AS LOG  NO-UNDO INIT NO.    /* yes - Обнаружена ошибка, откат  */
   DEFINE VAR fl1       AS LOG.
   DEFINE VAR vFl3      AS LOG  NO-UNDO INIT YES. /* Есть незакрытые срочные обязательства */
   DEFINE VAR vFl1      AS LOG  NO-UNDO INIT YES. /* Есть незакрытые обязательства по %% */
   ...
   DEFINE BUFFER xtrans-status FOR xtrans-status. 
   ...
   THEN DO:
      ...
      ON ENDKEY UNDO, LEAVE:
         ...
         END.
         IF     iWhat <> 1
            AND DATE(iDate + 1) < loan.since THEN /* договор может быть пересчитан на 1 день вперед */
         DO:
            {f-error.i &class   = КодОш
                       &par     = 2l
                       &RunProc = "IF vDispMess THEN RUN DispMess IN SOURCE-PROCEDURE (mess). "
         ...
         EXCLUSIVE-LOCK:
            ...
            ASSIGN
               vFl3  = NO /* Были незакрытые срочные обязательства */
               fl-er = NO.
            ...
            DO:
               ASSIGN
                  ...
                  mess = SUBSTITUTE("Закрыто срочное обязательство с плановой датой : &1",
                                    term-obl.end-date).
               IF vDispMess
               THEN
                  RUN DispMess IN SOURCE-PROCEDURE (mess).
         ...
         EXCLUSIVE-LOCK:
            ...
            ASSIGN
               vFl1  = NO /* Были незакрытые обязательства по %% */
               fl-er = NO
            .
         ...
         FINALLY:
            IF  NOT vFlUndo
            AND     {&vCounter_fl-o} = 0 THEN
            DO: 
               /* проверка на включенную модель состояния и возможность присвоения статуса ЗАКР */
               IF GetXattrEx(loan.class-code, "StatModelOn", "Initial") = "YES" THEN 
               DO: 
                  FIND FIRST xtrans-status WHERE 
                             xtrans-status.class-code      = loan.class-code
                         AND xtrans-status.status-code     = loan.loan-status 
                         AND xtrans-status.status-code-new = "ЗАКР"
                  NO-LOCK NO-ERROR. 
                  IF NOT AVAIL xtrans-status THEN 
                     ASSIGN  
                        vFlUndo = YES  /* Флаг для отката */
                        mess = "Ошибка: На классе " + loan.class-code + " отсутствует переход " +
                               "из статуса " + loan.loan-status + " в статус ЗАКР."                   
                     . 
                  ELSE 
                     ASSIGN
                        mess      = IF vFl3 THEN "" 
                                    ELSE "~nУспешно закрыты все срочные обязательства"
                        mess      = mess + IF vFl1 THEN "" 
                                    ELSE "~nУспешно закрыты все обязательства по погашению процентов"
                        mess      = mess + "~nДОГОВОР ЗАКРЫТ "
                        mess1     = ""
      ...
      IF vPutStrem
      THEN
         RUN PutStrem IN SOURCE-PROCEDURE (mess + mess1).
   END.
...
FUNCTION fLgPeriod RETURN LOGICAL (
   ...
   DO:
      ...
      FOR EACH bterm-obl WHERE bterm-obl.contract  = bloan-cond.contract
                           AND bterm-obl.cont-code = bloan-cond.cont-code
                           AND bterm-obl.idnt      = 1
                           AND bterm-obl.nn        = 1
      NO-LOCK
      vi = 1 TO vLgCnt:
      END.
      ...
      IF iDate < bterm-obl.end-date THEN oResult = YES. /* переданная дата находится внутри льготного периода по договору */
   END.
   RETURN oResult.
END FUNCTION.
...
PROCEDURE pTranshFill:
   ...
   DO:
      FIND FIRST op-entry WHERE
                 op-entry.op       = iOp
             AND op-entry.op-entry = iOpEntry
      NO-LOCK NO-ERROR.
      ...
      ASSIGN
         ...
         iTranshOnly = IF iTranshOnly = ? THEN YES ELSE iTranshOnly                
         iRole       = IF NOT {assigned iRole} THEN "*" ELSE iRole                
         iContract   = IF iContract = ? THEN "" ELSE iContract
         iContCode   = IF iContCode = ? THEN "" ELSE iContCode
      .
      FOR EACH loan-acct WHERE
         (
           (   iContract           = ""
           AND iContCode           = ""
           AND loan-acct.acct      = vAcct
           AND loan-acct.currency  = vCurr
           )
         OR
           (   iContract           <> ""
           AND iContCode           = ""
         ...
         AND loan-acct.since <= vDate
         AND ( NOT iTranshOnly
            OR NUM-ENTRIES (loan-acct.cont-code, " ") > 1
             )
         AND NOT CAN-FIND(FIRST bloan-acct WHERE
                                bloan-acct.contract  = loan-acct.contract
...
PROCEDURE pTranshCheck:
   ...
   IF iCheckLoan AND oResult THEN
      ...
      FOR FIRST tt-Transh WHERE
                tt-Transh.op       = iOp
            AND tt-Transh.op-entry = iOpEntry,
          FIRST loan WHERE
                loan.contract  = tt-Transh.contract
            AND loan.cont-code = ENTRY (1, tt-Transh.cont-code, " ")
          NO-LOCK,
          FIRST btt-Transh WHERE
                btt-Transh.op       = iOp
            AND btt-Transh.op-entry = iOpEntry
            AND CAN-FIND (FIRST bloan WHERE
                                bloan.contract  = btt-Transh.contract
                            AND bloan.cont-code = ENTRY (1, btt-Transh.cont-code, " ")
   ...
   IF iCheckRole AND oResult THEN
      ...
      FOR FIRST tt-Transh WHERE
            ...
            AND CAN-FIND (FIRST btt-Transh WHERE
                                btt-Transh.op        = iOp
                            AND btt-Transh.op-entry  = iOpEntry
                            AND btt-Transh.acct-type <> tt-Transh.acct-type
                         )
      :
         oResult = NO.
      END.
...
PROCEDURE pTranshPosting:
   ...
   DO:
      FIND FIRST op-entry WHERE
              op-entry.op       = iOp
          AND op-entry.op-entry = iOpEnrty
      NO-LOCK NO-ERROR.
      IF NOT AVAIL op-entry THEN
         LEAVE MAIN.
      ...
      ASSIGN
         vDate    = (IF op.contract-date  = ?  THEN op-entry.op-date ELSE op.contract-date)
         vSumm    = (IF op-entry.currency = "" THEN op-entry.amt-rub ELSE op-entry.amt-cur)  
         vSummTek = vSumm
      .
         /* вычитаем уже разнесенную сумму */
      FOR EACH loan-int WHERE
               loan-int.op       = op-entry.op
           AND loan-int.op-entry = op-entry.op-entry
      NO-LOCK:
      ...
      FOR EACH tt-Transh WHERE
               tt-Transh.op       = op-entry.op
           AND tt-Transh.op-entry = op-entry.op-entry
      BY tt-Transh.open-date:
         /* находим текущее условие */
         FIND LAST loan-cond WHERE
                   loan-cond.contract  = tt-Transh.contract
               AND loan-cond.cont-code = tt-Transh.cont-code
               AND loan-cond.since     <= vDate
         NO-LOCK NO-ERROR.
         IF NOT AVAIL loan-cond THEN
            NEXT TRANSH.
         IF loan-cond.cred-period = "П" THEN
            vDate = tt-Transh.end-date.
         /* находим существующую операцию */
         FIND FIRST loan-int WHERE
                    loan-int.op        = tt-Transh.op
                AND loan-int.op-entry  = tt-Transh.op-entry
                AND loan-int.contract  = tt-Transh.contract
                AND loan-int.cont-code = tt-Transh.cont-code
         NO-LOCK NO-ERROR.
         ...
         CASE tt-Transh.acct-type:
            ...
            DO:
                  ...
                  ** на текущую дату */
               FIND LAST term-obl WHERE
                        term-obl.contract  = tt-Transh.contract
                    AND term-obl.cont-code = tt-Transh.cont-code
                    AND term-obl.idnt      = 3
               ...
               NO-LOCK:
                  ...
                  RUN summ-t.p (OUTPUT vSummOb,
                                ...
                                vDate ).
                  ASSIGN
                     tt-Transh.amt-dos = tt-Transh.amt-dos + (IF vSummTek >= vSummOb THEN vSummOb ELSE
                                                             (IF vSummTek > 0 THEN vSummTek ELSE 0))
                  .
...
PROCEDURE pTranshCreateInt:
   ...
   DO TRANS ON ERROR UNDO, RETRY:
      ...
      DO:
         /* возврат ошибки */
         ASSIGN 
            oResult = "ERROR:" + ERROR-STATUS:GET-MESSAGE(1) + " " + RETURN-VALUE
            .  
      ...
      END.
      FOR EACH tt-Transh WHERE (tt-Transh.amt     > 0
                             OR tt-Transh.amt-dos > 0 )
      ON ERROR UNDO TRANSH, RETRY TRANSH:
         FIND FIRST op-entry WHERE
                    op-entry.op       = tt-Transh.op
                AND op-entry.op-entry = tt-Transh.op-entry
         NO-LOCK NO-ERROR.
         IF NOT AVAIL op-entry THEN
            UNDO TRANSH, RETRY TRANSH.
         vCounter = LOOKUP(tt-Transh.acct-type, iRole).
         IF vCounter = 0 OR vCounter > NUM-ENTRIES(iOper) THEN
            UNDO TRANSH, RETRY TRANSH.
         FIND FIRST chowhe WHERE chowhe.id-op = INT64(ENTRY(vCounter, iOper)) NO-LOCK NO-ERROR.
...
DEFINE VARIABLE vCounter AS CHARACTER NO-UNDO .
   ...
   IF    GetXAttrInit (iClass, "AutoGenNum") = "ДА" THEN
   DO:
...
DEFINE VARIABLE vTYpeLim  AS CHARACTER NO-UNDO .
   ...
   FIND FIRST  tloan WHERE
               tloan.contract  = iContract
         AND   tloan.cont-code = ENTRY(1,iContCode," ")
         NO-LOCK NO-ERROR .
   IF NOT AVAILABLE tloan THEN RETURN ERROR "Не найден договор " + iContract + " " + ENTRY(1,iContCode," ")  .

   IF iDateEnd = ? THEN iDateEnd = tloan.end-date.
   ...
   FOR EACH    loan WHERE
         ...
         &ELSE
              (loan.contract  = iContract
         AND   loan.cont-code = tloan.cont-code)
         OR
              (loan.contract  = iContract
   ...
   FOR EACH limits WHERE
            limits.file-name  = "loan"
        AND limits.surrogate  = iContract + "," + ENTRY(1,iContCode, " ")
        AND limits.Class-Code = vTypeLim
        AND limits.open-date  < iDateEnd
        AND limits.quantity   = 0
...
DEFINE VARIABLE         mFlPer    AS LOG       NO-UNDO .
   ...
   CASE iSign:
      ...
      WHEN "EQ" THEN
         FIND LAST term-obl WHERE
                   term-obl.contract  = iContract
               AND term-obl.cont-code = iContCode
               AND term-obl.idnt      = iTypeGraf
               AND (IF NOT mFlPer THEN term-obl.end-date = iDate ELSE TRUE)
               AND (IF mFlPer THEN term-obl.dsc-beg-date = iDate ELSE TRUE)
         NO-LOCK NO-ERROR.
      WHEN "LE" THEN
         FIND LAST term-obl WHERE
                   term-obl.contract  = iContract
   ...
   IF AVAIL term-obl THEN
   DO:
      IF iTypeDate = "ПЛАН" THEN
         oDate = term-obl.end-date.
      ELSE IF iTypeDate = "ПЕРИОД" THEN
...
IF FGetSetting ("ГрафИст", "СохрГрафИзм",?) <> "Да"  THEN RETURN.
...
DEF BUFFER bsigns FOR signs.
   ...
   IF iFlagGT THEN DO:
      FOR EACH  bterm-obl-hist WHERE
                bterm-obl-hist.contract  = iContract
            AND bterm-obl-hist.cont-code = iContcode
            AND bterm-obl-hist.idnt      = iIdnt
            AND bterm-obl-hist.since     >= iSince
            EXCLUSIVE-LOCK :
         FIND FIRST bsigns WHERE
                bsigns.file-name = "term-obl-hist"
            AND bsigns.surrogate = STRING(term-obl-hist.tobl-id)
            AND bsigns.code      = "Signed" 
            EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
         ...
         FIND FIRST bsigns WHERE
                bsigns.file-name = "term-obl-hist"
            AND bsigns.surrogate = STRING(bterm-obl-hist.tobl-id)
            AND bsigns.code      = "SignDate" 
            EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
...
/* получение последней плановой даты (дата окончания договора 
** или значение ДР ПослПД из условия, если на классе договора ДР ПослПДЗаПо = ДА) */
{pfuncdef 
   &DEFPROC = "GetLastPayDt"
   &DESCRIPTION = "получение последней плановой даты (дата окончания договора  ~
                   или значение ДР ПослПД из условия, если на классе договора  ~
                   ДР ПослПДЗаПо = ДА) "
   &PARAMETERS = "тип договора, номер договора, дата"
   &RESULT = "DATE"
   &SAMPLE = "GetLastPayDt (loan.contract,loan.cont-code,loan.since)"
}
FUNCTION GetLastPayDt RETURNS DATE (
   iContract   AS CHAR,
   iContcode   AS CHAR,
   iDate       AS DATE):

   DEF VAR vLoanEndDate AS DATE INIT ? NO-UNDO.

   DEF BUFFER loan-cond FOR loan-cond.

   {&GET-LOAN}

   IF GetXattrInit (loan.class-code,
                    "ПослПДЗаПо") = "ДА" THEN
   DO:
      FIND LAST loan-cond WHERE
      ...
      IF AVAIL loan-cond THEN 
         vLoanEndDate = DATE(GetXAttrValue("loan-cond",
                                            loan-cond.contract  + "," +
                                            loan-cond.cont-code + "," +
                                            STRING(loan-cond.since),
                                            "ПослПД")) NO-ERROR.
   END.
   IF vLoanEndDate = ? THEN
      vLoanEndDate = loan.end-date.
...
PROCEDURE GetFloatRate:
   ...
   DO:
      ...
      GetRefCrVal("init-float",
                  ...
                  ?,
                  (TEMP-TABLE ttIndicate:HANDLE)).
      FOR EACH ttIndicate WHERE ttIndicate.fChar = iCommission:
          /* далее по границе "ОТ" */
          GetRefCrVal("init-float",
                      "amt-min",
                      iDate,
          ...
          FOR EACH ttIndicate1 WHERE ttIndicate1.fDec <= iBaseAmt:
              /* далее по границе "ДО" */
              GetRefCrVal("init-float",
                          "amt-max",
                          iDate,
              ...
              FOR FIRST ttIndicate2 WHERE ttIndicate2.fDec > iBaseAmt
                                       OR ttIndicate2.fDec = 0: /* или нет верхней границы */
...
PROCEDURE Cr_Cond_DblAnn.
  ...
  DEF VAR vFirstPeriod  AS INT64   NO-UNDO.
  DEF VAR vPartAmount   AS DECIMAL   NO-UNDO.
  DEF VAR vKolLgtPer    AS INT64   NO-UNDO.  
  DEF VAR vCredOffSet   AS CHARACTER NO-UNDO.    
  DEF VAR vAnnuitKorr   AS INT64   NO-UNDO. 
  ...
  FIND LAST bloan-cond WHERE bloan-cond.contract  = iContract
                         AND bloan-cond.cont-code = iContCode
  ...
  ASSIGN
    vKolLgtPer  = INT64(GetXattrValueEx("loan-cond",
                                      bloan-cond.contract + "," 
    ...
    vCredOffSet = GetXattrValueEx("loan-cond",
                                   ...
                                   "cred-offset",
                                   "--")
    vAnnuitKorr = INT64(GetXattrValueEx("loan-cond",
                                      bloan-cond.contract + "," 
                                      + bloan-cond.cont-code + "," + STRING(bloan-cond.since),
                                      "АннуитКорр",
                                      "0"))
  ...
  * за период уменьшения и сколько из этой суммы должны были погасить по первоначальному условию */
  ELSE DO:
     RUN GetCurAmt(iContract,
                   ...
                   vSince,
                   OUTPUT vAmount).
     FIND FIRST bbloan-cond WHERE  bbloan-cond.contract  = iContract
                               AND bbloan-cond.cont-code = iContCode
                               AND bbloan-cond.since     = loan.open-date
      NO-LOCK NO-ERROR.
  ...
  DO TRANSACTION ON ERROR UNDO TR,LEAVE TR
    ON STOP UNDO TR,LEAVE TR:
          ...
          IF iRecalcAnn THEN DO:
              ...
              RUN CalcAnnuitet2 IN THIS-PROCEDURE (loan.contract,
                                loan.cont-code,
                                vSince,
                                loan.end-date,
                                vAmount,
...
PROCEDURE GetFirstPerPartAmt.
    ...
    DEFINE OUTPUT PARAMETER  oFirstPeriod     AS INT64   NO-UNDO.
    ...
    ASSIGN
        oPartAmount  = DEC(GetXattrValueEx("loan-cond",
                                           ...
                                           "PartAmount",
                                           "0"))
        oFirstPeriod = INT64(GetXattrValueEx("loan-cond",
                                           iContract + "," + iContCode + "," + STRING(iSince),
                                           "FirstPeriod",
                                           "0"))
          .
    ...
    ** для расчета второй аннуитетной суммы */
    ...
    DO WHILE  oPartAmount = 0 
         AND oFirstPeriod = 0:
       FIND LAST bbloan-cond WHERE bbloan-cond.contract  = iContract
                               AND bbloan-cond.cont-code = iContCode
                               AND bbloan-cond.since     < vDateCond
      NO-LOCK NO-ERROR.
...
PROCEDURE ChangeLoanDate.
   ...
   DEF VAR vNDays       AS INT64   NO-UNDO. /* Количество дней действия дог-ра */
   DEF VAR vNMonthes    AS INT64   NO-UNDO. /* Количество мусяцев действия дог-ра */
   DEF VAR vNYears      AS INT64   NO-UNDO. /* Количество лет действия дог-ра */
   DEF VAR vNEnd-date   AS DATE  NO-UNDO. /* Дата, сдвинутая в соотв. с раб/нераб днями */
   DEF VAR vMove        AS INT64   NO-UNDO. /* Направление сдвига даты */
   DEF VAR vCounter     AS INT64   NO-UNDO. /* Счетчик для удаления всех графиков */
   ...
   DEF VAR vCredDate    AS INT64   NO-UNDO. /* cred-date с условия */
   DEF VAR vCredPer     AS CHAR  NO-UNDO. /* cred-period с условия */
   DEF VAR vCredMon     AS CHAR  NO-UNDO. /* cred-month с условия */
   DEF VAR vKolLgtPer   AS INT64   NO-UNDO. /* КолЛьгтПер с уловия */
   ...
   DEF VAR vAnnuitCorr  AS INT64   NO-UNDO. /* ДР АннуитКорр */
   DEF VAR vSummDepos   AS DEC   NO-UNDO. /* ДР sum-depos */
   DEF VAR vFirstPeriod AS INT64   NO-UNDO. /* ДР FirstPeriod */
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      .
      FIND FIRST ins-loan WHERE ins-loan.contract  = ENTRY(1,vSurrIns)
                            AND ins-loan.cont-code = ENTRY(2,vSurrIns)
      NO-LOCK NO-ERROR.
      ...
      DO:
         ...
         DO:
            ...
            DO:
               ...
               DO:
                  ...
                  IF vBal2Acct <> STRING(acct.bal-acct) THEN
                  DO:
                     pick-value = ?.
         ...
         ** !!!!! ПОТОМ эту проверку вынести выше, т.к. не все из этого блока тогда имеет смысл !!!! */
         IF iNewDate <> loan-cond.since THEN
         DO:
            CREATE bloan-cond.
         ...
         ** а также дату начала условия */
         ...
         ASSIGN
            vCondClass   = IF iNewDate <> loan-cond.since THEN bloan-cond.class-code
                                                          ELSE loan-cond.class-code
            vCredDate    = IF iNewDate <> loan-cond.since THEN bloan-cond.cred-date
                                                          ELSE loan-cond.cred-date
            vCredPer     = IF iNewDate <> loan-cond.since THEN bloan-cond.cred-period
                                                          ELSE loan-cond.cred-period
            vCredMon     = IF iNewDate <> loan-cond.since THEN STRING(bloan-cond.cred-month)
                                                          ELSE STRING(loan-cond.cred-month)
            vKolLgtPer   = INT64(GetXAttrValue("loan-cond",vLCSurr,"КолЛьгтПер"))
            vAnnuitCorr  = INT64(GetXAttrValueEx("loan-cond",vLCSurr,"АннуитКорр",?))
            vSummDepos   = DEC(GetXAttrValueEx("loan",loan.contract + "," + loan.cont-code,"Sum-depos","0"))
            vFirstPeriod = INT64(GetXattrValueEx("loan-cond",vLCSurr,"FirstPeriod","0"))
            vPartAmount  = DEC(GetXattrValueEx("loan-cond",vLCSurr,"PartAmount","0"))
         .
         
         IF iChEndDate <> "ДА" THEN
         DO:
            ASSIGN
               vNDays     = INT64(GetXAttrValueEx ("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + STRING(loan-cond.since),"NDays","0"))
               vNMonthes  = INT64(GetXAttrValueEx ("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + STRING(loan-cond.since),"NMonthes","0"))
               vNYears    = INT64(GetXAttrValueEx ("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + STRING(loan-cond.since),"NYears","0"))
            ...
            ** если они отличаются - значит получилась дата выходного и надо двигать */
            IF vNEnd-date <> GetFirstDayForLoan((BUFFER loan:HANDLE),
                                                vNEnd-date,
                                               -1) THEN
            DO:
         ...
         ** условий, или если не изменяли дату окончания договора */
         IF   vMove      <> 0 
           OR iNewDate   = vOldDate
           OR iChEndDate = "ДА" THEN
         DO:
         ...
         END.
         IF GetXAttrValueEx("loan",loan.contract + "," + loan.cont-code,"ФиксМес",GetXAttrInit(loan.class-code,"ФиксМес")) = "Да" THEN
         DO:
         ...
         IF iNewDate <> vOldDate THEN
         DO:
            FOR EACH bsigns WHERE bsigns.file-name = "loan-cond"
                              AND bsigns.surrogate = loan.contract + "," + loan.cont-code + "," + STRING(vOldDate)
            NO-LOCK:
               {profind.i &fnoer=TRUE &fnowh=TRUE
         ...
         FIND FIRST bcomm-rate WHERE
                    bcomm-rate.kau   = loan-cond.contract + "," + loan-cond.cont-code
         NO-LOCK NO-ERROR.
         DO WHILE AVAIL bcomm-rate:
            ...
            FIND FIRST bcomm-rate WHERE
                       bcomm-rate.kau        = loan-cond.contract + "," + loan-cond.cont-code
                   AND bcomm-rate.commission > vComm
                   AND bcomm-rate.since      = vOldDate
            NO-LOCK NO-ERROR.
         END.  
                                       
         /* Теперь изменяем даты по созданным обеспечениям, если они были */
         FOR EACH bterm-obl WHERE bterm-obl.contract  = loan.contract
                              AND bterm-obl.cont-code = loan.cont-code
                              AND bterm-obl.idnt      = 5
         NO-LOCK:
         ...
         IF GetXAttrValue("loan-cond",vLCSurr,"СхемаПлат") = "Аннуитетная" THEN
         DO:
            FIND FIRST vcomm-rate WHERE vcomm-rate.kau        = loan.contract + "," + loan.cont-code
                                    AND vcomm-rate.commission = "%Кред"
                                    AND vcomm-rate.since      = iNewDate
            NO-LOCK NO-ERROR.
            
            IF AVAIL vcomm-rate THEN
               vRateCred = vcomm-rate.rate-comm.

            RUN CalcAnnuitet IN THIS-PROCEDURE (loan.contract,
                              loan.cont-code,
                              loan.open-date,
                              loan.end-date,
                              vCredSumm,
      ...
      ** Сначала чистим их все ( с idnt = 1,2,3 ) */
      DO vCounter = 1 TO 3:
         FOR EACH bterm-obl WHERE bterm-obl.contract  = iContract
                              AND bterm-obl.cont-code = iContCode
                              AND bterm-obl.idnt      = vCounter
         NO-LOCK:
            {profind.i &fnoer=TRUE &fnowh=TRUE
                  &fway=FIRST &fbuf=term-obl &flock=EXCLUSIVE-LOCK &ffor=TRUE
                  &fcond="WHERE RECID(term-obl) EQ RECID(bterm-obl) "}
...
/* Находим обеспечение */
   FOR EACH term-obl WHERE 
        ...
        AND ((term-obl.sop-date <> ? 
          ...
          AND term-obl.end-date >= iDate))
        AND term-obl.cont-type  = iTypeObesp
   NO-LOCK:
        
      IF iDate > DATE(GetXattrValueEx("term-obl", 
      ...
      DO:
         ...
         FIND FIRST gar-loan WHERE 
                    gar-loan.contract  = "КредГа" 
                AND gar-loan.cont-code = term-obl.lnk-cont-code
         NO-LOCK NO-ERROR.
         IF NOT AVAIL gar-loan THEN 
             FIND FIRST gar-loan WHERE 
                        gar-loan.contract  = "ДепГа" 
                    AND gar-loan.cont-code = term-obl.lnk-cont-code
             NO-LOCK NO-ERROR.
   ...
   IF NOT vDogObesp THEN
      ...
      NO-LOCK:
         ...
         vTypeShDog = GetXAttrValueEx ("term-obl", vterm-obl-sur, "ВидДогОб", "") + IF  INT64(vnn) =  0 THEN "" ELSE vnn.
         IF vTypeShDog BEGINS iTypeObesp 
         THEN DO:
            FOR FIRST code WHERE code.parent = vTypeShDog
                             AND code.class  = "ТипСчДог"
            NO-LOCK:
                         
               vParamCode = code.code.            
                 /*первые два символа не дб, или кр - не нужны*/
               SUBSTRING(vParamCode,1,2) = "".
               FOR FIRST chowhe WHERE chowhe.id-op =  INT64(vParamCode)
               NO-LOCK:
                   vTmp = IF chowhe.id-k = ? THEN chowhe.id-d ELSE chowhe.id-k.
                   IF LOOKUP(STRING(vTmp),vParam) <> 0
                   THEN DO:
...
procedure GetIndComm:
   ...
   {shemnach.i &mSchemCls = oSchemCls &loan = loan}
    ...
    FOR EACH bcode WHERE bcode.class  = "СхемНачКом" 
                     AND bcode.parent = oSchemCls + ":СхемНачКом" 
    NO-LOCK,
       EACH code WHERE 
             code.class = "НачКом"
         AND code.code  = ENTRY(1,bcode.code,":")
    NO-LOCK:
       ...
       vTmp = IF NUM-ENTRIES(code.val) >= 3 THEN ENTRY(3, code.val) ELSE "-1".
       {additem.i oIdDop vTmp}          /* 3-й параметр */
       vTmp = IF GetCodeMiscEx("СхемНачКом", code.code + ":" + oSchemCls, 3, "Нет") = "Да" 
                 THEN "Да" 
                 ELSE "Нет".
...
DEFINE OUTPUT PARAMETER oAcct AS CHARACTER NO-UNDO .
   ...
   IF Num-entries(vstr) < iOrder THEN DO:
     oAcct = "" .
     RETURN.
   END.
...
PROCEDURE GetMaxNumGraphHist:
   ...
   FOR EACH term-obl-hist WHERE
            term-obl-hist.contract  = iContract
        AND term-obl-hist.cont-code = iCont-code
   NO-LOCK:
      vNGr = INT64(GetXattrValueEx("term-obl-hist",
                                    STRING(term-obl-hist.tobl-id),
                                    "NumGr",
                                    "")).
      IF vNGr > vMaxNGr THEN vMaxNGr = vNGr.
   END.
...
PROCEDURE GetFirstInvest.
   ...
   FOR EACH op-entry WHERE 
            op-entry.acct-cr   = iAcctCred
      AND   op-entry.op-date   <= iDate
      AND   op-entry.op-status >= iOpStatus
      AND   op-entry.currency  = iCurrency  
      AND   op-entry.op        <> ?  
   NO-LOCK
   BY op-entry.op-date:
      IF oDate = ? THEN
         oDate = op-entry.op-date.
      IF oDate = op-entry.op-date THEN
         oAmnt = oAmnt + (IF iCurrency <> "" THEN op-entry.amt-cur ELSE op-entry.amt-rub).
      ELSE
         LEAVE.
   END.
END PROCEDURE.
...
PROCEDURE SaveGraphToHistory:
   ...
   IF  GetSysConf("НеСохранятьГрафикПогашенияВИсторию") = "ДА" 
    OR FGetSetting ("ПрмСорхГр", iChType,"НЕТ") <> "ДА" THEN 
...
{pfuncdef 
   &DEFPROC="LoanDateEndHolyday"
   &DESCRIPTION="Процедура возвращает 1 в случае, если дата ~ 
окончания договора попала на прошедшие выходные, в противном случае ~
возвращается 0"
   &PARAMETERS="НАЗНАЧЕНИЕ ДОГОВОРА,ИДЕНТИФИКАТОР ДОГОВОРА,ДАТА"
   &RESULT="1 или 0"
   &SAMPLE="LoanDateEndHolyday('Кредит','123',01/01/2015,OUTPUT oRez)"}
     
PROCEDURE LoanDateEndHolyday:
   DEF INPUT  PARAM iContract AS CHAR NO-UNDO. 
   DEF INPUT  PARAM iContCode AS CHAR NO-UNDO.
   DEF INPUT  PARAM iDate     AS DATE NO-UNDO.
   
   DEF OUTPUT PARAM oRezult   AS CHAR NO-UNDO.
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      IF NOT AVAILABLE loan THEN LEAVE mb.
      
      /* Узнаем - предыдущий день от даты опердня - выходной или рабочий */
      
      IF isWorkDayLoan(BUFFER loan:HANDLE, iDate - 1) THEN LEAVE mb.
      
      IF loan.end-date < iDate AND
         loan.end-date > GetFstWrkDay(loan.class-code, 
                                       loan.user-id, 
                                       iDate - 1, 
                                       15, 
                                       - 1) THEN oRezult = "1". 
...
{pfuncdef 
   &DEFPROC="LoanFirstWorkDayInM"
   &DESCRIPTION="Процедура возвращает 1 в случае, если если дата ОД ~ 
- первый рабочий день месяца, при этом последний КАЛЕНДАРНЫЙ день предыдущего ~ 
месяца был не рабочим в филиале, и дата окончания договора строго меньше ~ 
даты ОД."
   &PARAMETERS="НАЗНАЧЕНИЕ ДОГОВОРА,ИДЕНТИФИКАТОР ДОГОВОРА,ДАТА"
   &RESULT="1 или 0"
   &SAMPLE="LoanFirstWorkDayInM('Кредит','123',01/01/2015,OUTPUT oRez)"}   
PROCEDURE LoanFirstWorkDayInM:
   DEF INPUT  PARAM iContract AS CHAR NO-UNDO. 
   DEF INPUT  PARAM iContCode AS CHAR NO-UNDO.
   DEF INPUT  PARAM iDate     AS DATE NO-UNDO.
   ...
   DO ON ERROR UNDO, LEAVE:
      ...
      vDate = GetFstWrkDay(loan.class-code, 
                           loan.user-id, 
                           iDate - 1, 
                           15, 
                           - 1).

      IF     isWorkDayLoan(BUFFER loan:HANDLE,iDate)
         AND MONTH(iDate) <> MONTH(vDate)
         AND loan.end-date < iDate THEN oRezult = "1".
   END.
   
END PROCEDURE.

DEF TEMP-TABLE ttComm2332U NO-UNDO
   FIELD fBeg-date AS DATE
   FIELD fEnd-date AS DATE
   FIELD fRate     AS DEC
. 

{pfuncdef 
   &DEFPROC="CalcAvgCommCond"
   &DESCRIPTION="Функция вовзращает среднюю плавающую ставку в случае ~
   присутвия ее на дату окончания расчета~
   иначе возвращается фиксированная ставка"
   &PARAMETERS="НАЗНАЧЕНИЕ ДОГОВОРА,ИДЕНТИФИКАТОР ДОГОВОРА,~
   ДАТА НАЧАЛА,ДАТА КОНЦА"
   &RESULT="СРЕДНЯЯ СТАВКА"
   &SAMPLE="CalcAvgCommCond('Кредит','123',01/01/2015,01/12/2015)"}
FUNCTION CalcAvgCommCond RETURNS DEC
  (iContract  AS CHAR,  /* Назначение */
   iContCode  AS CHAR,  /* Номер договора */
   iBegDate   AS DATE,  /* Дата начала расчета */
   iEndDate   AS DATE)  /* Дата конца расчета */
:
    
   DEF VAR vPrevDate   AS DATE  NO-UNDO.
   DEF VAR vCnt        AS INT64 NO-UNDO.
   DEF VAR vDateChange AS DATE  NO-UNDO.
   DEF VAR vRateChange AS DEC   NO-UNDO.
   DEF VAR vTransh     AS LOG   NO-UNDO.
   
   DEF BUFFER loan       FOR loan.
   DEF BUFFER bloan      FOR loan.
   DEF BUFFER comm-rate  FOR comm-rate.
   DEF BUFFER comm-cond  FOR comm-cond.
   DEF BUFFER bttComm2332U  FOR ttComm2332U.
   ...
   DO:
      RUN Fill-SysMes IN h_tmess ("", "", "",
                                  "Не найден договор " + iContCode).
      RETURN ?.
   END.
      
      /* Ищем ставку на отчетную дату */
   FIND LAST comm-rate WHERE
             comm-rate.kau        = loan.contract + "," + loan.cont-code
   ...
   IF NOT AVAIL comm-rate THEN
   DO:   /* Если это транш и на нем не нашли ставку, то
            найдем на охватывающем */
      IF NUM-ENTRIES(loan.cont-code, " ") = 2 THEN
      DO:
         FIND FIRST bloan WHERE
                    bloan.contract  = loan.contract
         ...
         DO:
            ...
            DO:
               RUN Fill-SysMes IN h_tmess ("", "", "",
                                           "На охватывающем договоре " + bloan.cont-code + 
                                           " не найдена процентная ставка '%Кред'" +
                                           " на отчетную " +
                                           QUOTER(STRING(iEndDate))).
               RETURN ?.       
            END.
   ...
   END.
      ...
      /* Ищем условие плавающей ставки на отчетную дату */
   ...
   NO-LOCK NO-ERROR.
      ...
      /* Если не нашли условие по плавающей ставке
         или нашли, но признак плавающей ставки = Нет,
         то возвращаем результат из comm-rate */
   IF    NOT AVAIL comm-cond
      OR (    AVAIL comm-cond
          AND NOT comm-cond.FloatType) THEN
      RETURN comm-rate.rate-comm.
      
      /* Если это транш и на нем не было найдена ставка,
         то расчет осуществляется с охватывающего договора
         и поэтому необходимо найти охватывающий договор */
   IF vTransh THEN
   DO:
      FIND FIRST loan WHERE
                 loan.contract  = iContract
   ...
   END.
      ...
      /* Если ставка равна или раньше дате начала 
         расчетного периода, то возвращаем результат из comm-rate */
   IF comm-rate.since <= iBegDate THEN
      RETURN comm-rate.rate-comm.
   ELSE
   DO:
      CREATE ttComm2332U.
      ASSIGN
         ttComm2332U.fBeg-date = comm-rate.since
         ttComm2332U.fEnd-date = iEndDate
         vDateChange           = comm-rate.since - 1
         ttComm2332U.fRate     = comm-rate.rate-comm
      .
         
      FIND FIRST ttComm2332U NO-LOCK NO-ERROR.
         
         /* Если дата начала первого периода не равна входящей дате начала,
            то сторим периоды по ставкам */
      IF vDateChange >= iBegDate THEN
      DO:
         COMMRATE:
         /* SORT-ACCESS comm-rate */
         FOR EACH comm-rate WHERE
                  comm-rate.kau        = loan.contract + "," + loan.cont-code
              AND comm-rate.commission = "%Кред"
              AND comm-rate.since      <= vDateChange
         NO-LOCK
         BY comm-rate.since DESC:
            
            CREATE ttComm2332U.
            ASSIGN
               ttComm2332U.fBeg-date = IF comm-rate.since <= iBegDate THEN 
                                          iBegDate
                                       ELSE
                                          comm-rate.since
               ttComm2332U.fEnd-date = vDateChange
               vDateChange           = comm-rate.since - 1
               ttComm2332U.fRate     = comm-rate.rate-comm
            .
      ...
      vCnt = iEndDate - iBegDate + 1.

         /* Расчитываем среднюю ставку */
      FOR EACH ttComm2332U EXCLUSIVE-LOCK:
         vRateChange = vRateChange + 
                       ttComm2332U.fRate * 
                      (ttComm2332U.fEnd-date - ttComm2332U.fBeg-date + 1) / 
                       vCnt.
      END.
...
{pfuncdef 
   &DEFPROC="GetInfoRateFOR2332U"
   &DESCRIPTION="Процедура возвращает информацию о ставке ~
   в переданном периоде"
   &PARAMETERS="НАЗНАЧЕНИЕ ДОГОВОРА,ИДЕНТИФИКАТОР ДОГОВОРА,~
   ДАТА НАЧАЛА,ДАТА КОНЦА"
   &RESULT="ТИП СТАВКИ,СТАВКА НА НАЧАЛО,ИЗМЕНЕННАЯ СТАВКА,~
   ДАТА ИЗМЕНЕНИЯ,ПСК"
   &SAMPLE="RUN GetInfoRateFOR2332U('Кредит','123',01/01/2015,01/12/2015,~
   OUTPUT oTypeRate,OUTPUT oRate,OUTPUT oRateChange,OUTPUT oDateChange,OUTPUT oEPS)"}
PROCEDURE GetInfoRateFOR2332U:
   DEF INPUT PARAM  iContract   AS CHAR  NO-UNDO. /* Назначение */
   DEF INPUT PARAM  iContCode   AS CHAR  NO-UNDO. /* Номер договора */
   DEF INPUT PARAM  iBegDate    AS DATE  NO-UNDO. /* Дата начала расчета */
   DEF INPUT PARAM  iEndDate    AS DATE  NO-UNDO. /* Дата конца расчета */  
   DEF OUTPUT PARAM oTypeRate   AS INT64 NO-UNDO. /* Тип ставки: 
                                                     1 - фиксированная
                                                     2 - плавающая */
   DEF OUTPUT PARAM oRate       AS DEC   NO-UNDO. /* Ставка на начало 
                                                     действия договора */
   DEF OUTPUT PARAM oRateChange AS DEC   NO-UNDO. /* Измененная ставка */
   DEF OUTPUT PARAM oDateChange AS DATE  NO-UNDO. /* Дата изменения */
   DEF OUTPUT PARAM oEPS        AS DEC   NO-UNDO. /* Полная стоимость кредита */
   ...
   END.
      ...
      /* Ищем условие плавающей ставки на конец расчетного периода */
   FIND LAST comm-cond WHERE
             comm-cond.contract   = loan.contract
         AND comm-cond.cont-code  = loan.cont-code
         AND comm-cond.commission = "%Кред"
   ...
   NO-LOCK NO-ERROR.
      ...
      /* Если не найдено условие плавающей ставки 
         на конец расчетного периода,
         или найдено, но признак плавающей ставки = Нет,
         то считаем, что это фиксированная ставка */
   IF    NOT AVAIL comm-cond
      OR (    AVAIL comm-cond
          AND NOT comm-cond.FloatType) THEN
   DO:
      
         /* Ищем ставку на дату заключения договора */
      FIND FIRST comm-rate WHERE
                 comm-rate.kau        = loan.contract + "," + loan.cont-code
             AND comm-rate.commission = "%Кред"
            ...
            DO:
               ...
               DO:
                  RUN Fill-SysMes IN h_tmess ("", "", "",
                                        "На охватывающем договоре " + bloan.cont-code + 
                                        " не найдена процентная ставка '%Кред'" +
                                        " на дату открытия договора " +
                                        QUOTER(bloan.open-date)).
                  RETURN ?.
               END.
            END.
            ELSE
   ...
   DO:
      ...
      DO:
         ...
         DO:
            ...
            IF AVAIL bloan THEN
            DO:
               oRate = CalcAvgCommCond (loan.contract,
                                        loan.cont-code,
                                        bloan.open-date,
                                        loan.open-date).
            END.
   ...
   END.
      ...
      /*  Необходимо проверить, нет ли привязки к карточному договору.
          Получаем счет с ролью "КредРасч" на договоре */
   FIND LAST loan-acct WHERE 
   ...
   DO:
         /* Получаем привязки этого счета к карточному договору с ролью SCS@<Валюта>.
            Сначала для физических лиц, если не нашли, то для юридических лиц */
      FIND LAST bloan-acct WHERE
                bloan-acct.contract  = "card-pers"
            AND bloan-acct.acct      = loan-acct.acct
            AND bloan-acct.currency  = loan-acct.currency    
            AND bloan-acct.acct-type BEGINS "SCS@"
            AND bloan-acct.since     <= iEndDate
      NO-LOCK NO-ERROR.
      
      IF NOT AVAIL bloan-acct THEN
         FIND LAST bloan-acct WHERE
                   bloan-acct.contract  = "card-corp"
               AND bloan-acct.acct      = loan-acct.acct
               AND bloan-acct.currency  = loan-acct.currency    
               AND bloan-acct.acct-type BEGINS "SCS@"
               AND bloan-acct.since     <= iEndDate
   ...
   END.
      ...
      /* Если привязка к карточному договору есть, 
         то бежим по всем связанным договорам и
   ...
   DO:
      ...
      FOR EACH comm-rate WHERE 
           ...
           AND comm-rate.commission = "%Кред"
      NO-LOCK:
         oRateChange = MAX (oRateChange, comm-rate.rate-comm).
      END.
   END.
      /* Иначе проверяем не является ли договор овердрафтом
         или грейс-течением и берем действующую ставку 
         на отчетный период */
   ELSE IF    loan.Class-Code = "l_agr_with_per"
           OR CAN-DO(GetXclassAllChildsEx("l_agr_with_per"), 
   ...
   DO:
      ...
      IF AVAIL comm-rate THEN
         oRateChange = comm-rate.rate-comm.
   END.
/*
      /* Для всех остальных производим расчет средней ставки
         по текущему договору */
   ELSE
   DO:
      oRateChange = CalcAvgCommCond (loan.contract,
                                     loan.cont-code,
                                     iBegDate,
                                     iEndDate).
   END.
*/
      /* Для всех остальных также берем последнюю (см. 0307329 - "Согласно 
         4212-У в графе 11 раздела 3 указываются условия договора, действующие 
   ...
   DO:
      ...
      IF AVAIL comm-rate THEN
         oRateChange = comm-rate.rate-comm.
   END.
   /* временное решение, далее предполагается переделка этого инструмента */
   IF GetSysConf("РАСЧЕТ ЭПС") <> "НЕТ" THEN
      oEPS = GetEpsLoan (loan.contract,
                         loan.cont-code,
                         iEndDate) * 100.
...
{pfuncdef 
   &DEFPROC     = "ProlTranshList"
   &DESCRIPTION = "Создает условие"
   &PARAMETERS  = "СУРРОГАТ УСЛОВИЯ С ОХВАТЫВАЮЩЕГО"
   &RESULT      = "УСЛОВИЯ НА ТРАНШАХ"
   &SAMPLE      = "RUN ProlTranshList(@transh_list, @cond_surr)"
}
PROCEDURE ProlTranshList: 
   DEF INPUT  PARAM iCondSurr AS CHAR NO-UNDO.
   DEF OUTPUT PARAM oResult   AS CHAR NO-UNDO.
   /* локализация */   
   DEFINE BUFFER loan  FOR loan. 
   ...
   /* переменные */
   DEF VAR vResult AS CHARACTER NO-UNDO.  
     
   /* ищем договор от условия */
   FIND FIRST loan WHERE 
              loan.contract  = ENTRY(1, iCondSurr)
          AND loan.cont-code = ENTRY(2, iCondSurr)
   NO-LOCK NO-ERROR. 
   /* выводим список траншей по договору */
   {empty ttRetVal}
   DO TRANSACTION:
   ...
   /* т.к. обработка идет по многим траншам, протоколируем действия */
   RUN Init-SysMes IN h_tmess ("op-kind,КонвНаст",
   "Конвертация по заявке <номер>","").
   /* для каждого выбранного транша создаем условие */   
   FOR EACH ttRetVal,
      FIRST bloan WHERE 
            bloan.contract  = ENTRY(1, ttRetVal.PickValue)
        AND bloan.cont-code = ENTRY(2, ttRetVal.PickValue)
   NO-LOCK:
      RUN ProlTranshOne IN THIS-PROCEDURE(bloan.contract, 
                                          bloan.cont-code, 
                                          0, 
                                          DATE(ENTRY(3, iCondSurr)),
                                          OUTPUT vResult).  
   END. 
   /* вывод протокола */
...
{pfuncdef 
   &DEFPROC     = "ProlTranshOne"
   &DESCRIPTION = "Создает условие для одного транша при пролонгации"
   &PARAMETERS  = "СУРРОГАТ ТРАНША,СУРРОГАТ УСЛОВИЯ С ОХВАТЫВАЮЩЕГО"
   &RESULT      = "УСЛОВИЕ НА ТРАНШЕ"
   &SAMPLE      = "RUN ProlTranshOne(@contract, @cont-code, @shtype, @since)"
}
PROCEDURE ProlTranshOne:
   DEF INPUT  PARAM iContract  AS CHAR NO-UNDO.
   DEF INPUT  PARAM iContCode  AS CHAR NO-UNDO.
   DEF INPUT  PARAM iShiftType AS CHAR NO-UNDO.
   DEF INPUT  PARAM iDate      AS DATE NO-UNDO.
   ...
   DEF VAR vProlCount          AS INT64 NO-UNDO INIT 1.  /* Кол-во пролонгаций договора с учетом текущей */
   DEF VAR vFirstProlDate      AS DATE  NO-UNDO.         /* Конечная дата первоначального срока договора */
   DEF VAR vSurr               AS CHAR  NO-UNDO.         /* Суррогат */
   DEF VAR vSurC               AS CHAR  NO-UNDO.         /* Суррогат копируемого условия */
   DEF VAR vAmount             AS DEC   NO-UNDO.         /* Cумма ОД (сумма параметров 0+2). */
   DEF VAR vAmtParam           AS DEC   NO-UNDO.         /* Сумма на параметре */
   DEF VAR vDbSumDec           AS DEC   NO-UNDO.
   DEF VAR vCrSumDec           AS DEC   NO-UNDO.
   DEF VAR vMove               AS INT64 NO-UNDO.         /* Направение сдвига */
   DEF VAR vOldEndDate         AS DATE  NO-UNDO.         /* Первоначальная конечная дата договора */
   DEF VAR vProcs              AS CHAR  NO-UNDO.         /* Список %% ставок для копирования */
   DEF VAR vCntr               AS INT64 NO-UNDO.         /* Счетчик */
   DEF VAR vDopRecList         AS CHAR  NO-UNDO.         /* Список ДР для копирования */
   DEF VAR vRates              AS CHAR  NO-UNDO.
   DEF VAR vCondCount          AS INT64 NO-UNDO.
   DEF VAR vNN                 AS INT64 NO-UNDO.
   DEF VAR vProlOb             AS CHAR  NO-UNDO.
   
   /* локализация */
   DEF BUFFER uploan     FOR loan.
   DEF BUFFER bloan      FOR loan.
   DEF BUFFER term-obl   FOR term-obl.
   DEF BUFFER bterm-obl  FOR term-obl.
   DEF BUFFER loan-cond  FOR loan-cond.
   ...
   DEF BUFFER pro-obl    FOR pro-obl.
   DEF BUFFER prev-obl   FOR pro-obl.
...
MAIN-BLOCK: 
   DO:
      ...
      FIND FIRST bloan WHERE
             ...
             AND bloan.cont-code = iContCode
      EXCLUSIVE-LOCK NO-WAIT NO-ERROR.
      /* проверяем, что нет условия с датой равной или большей */
      FIND FIRST loan-cond WHERE  
                 loan-cond.contract  = bloan.contract 
             AND loan-cond.cont-code = bloan.cont-code
             AND loan-cond.since     >= iDate
      NO-LOCK NO-ERROR. 
      IF AVAIL loan-cond THEN 
      DO:
         RUN Fill-SysMes IN h_tmess ("","","0", 
             "Для договора № " + bloan.doc-ref + " уже существует условие с датой равной" + 
             " или позднее " + STRING(iDate, "99.99.9999")).
         LEAVE MAIN-BLOCK.  
      END. 
      /* проверяем, что есть действующее условие, которое можно пролонгировать */
      FIND FIRST loan-cond WHERE  
                 loan-cond.contract  = bloan.contract 
             AND loan-cond.cont-code = bloan.cont-code
             AND loan-cond.since     < iDate
             AND iDate               <= bloan.end-date
      NO-LOCK NO-ERROR. 
      IF NOT AVAIL loan-cond THEN 
      DO:
         RUN Fill-SysMes IN h_tmess ("","","0", 
             "Для договора № " + bloan.doc-ref + " отсутствует действующее условие на дату " + 
      ...
      DO:
         ...
         NO-LOCK:
            IF LOOKUP (comm-rate.commission, vProcs) = 0 THEN
               vProcs = vProcs + (IF vProcs <> "" THEN "," ELSE "") + comm-rate.commission.
         END.
         /* Подсчитаем число пролонгаций */
         FOR EACH pro-obl WHERE 
                  pro-obl.contract  = bloan.contract
              AND pro-obl.cont-code = bloan.cont-code
              AND pro-obl.pr-date   <= iDate
         NO-LOCK:
            ASSIGN
               vProlCount     = vProlCount + 1
               vFirstProlDate = IF vFirstProlDate = ? THEN 
                                   pro-obl.pr-date
                                ELSE 
                                   vFirstProlDate
            .
         END. 
         /* Если пролонгаций еще не было, то берем дату окончания договора */
         IF vFirstProlDate = ? THEN
            vFirstProlDate = bloan.end-date.
         /* ищем охватывающий договор, что бы взять дату окончания с него  */
         FIND FIRST uploan WHERE 
                    uploan.contract  = bloan.contract 
                AND uploan.cont-code BEGINS ENTRY(1, bloan.cont-code, " ")
         NO-LOCK NO-ERROR. 
         ASSIGN
            vOldEndDate       = bloan.end-date
            bloan.end-date    = uploan.end-date
            bloan.loan-status = "ПРОЛ"
            vMove             = IF iShiftType = "3" THEN -1 ELSE IF iShiftType = "2" THEN 1 ELSE 0
         .
         /* Сдвинем дату, если необходимо */
         IF vMove <> 0 THEN
            bloan.end-date = GetFstWrkDay (bloan.Class-Code,
                                           ...
                                           vMove).
         /* Найдем параметр 0 */
         RUN STNDRT_PARAM IN THIS-PROCEDURE (iContract,
                                  iContCode,
                                  0,
                                  iDate,
                                  OUTPUT vAmtParam,
         ...
         ELSE 
         DO: 
            /* если выдачи не было, то возьмем запись с графика */    
            FIND LAST term-obl WHERE
            ...
            IF AVAIL term-obl THEN 
               vAmount = term-obl.amt-rub. 
         END.   
         /* Найдем условие с которого скопировать реквизиты */
         FIND LAST bloan-cond WHERE
                   bloan-cond.contract  = bloan.contract
               AND bloan-cond.cont-code = bloan.cont-code
               AND bloan-cond.since     <= bloan.end-date /* Новая дата договора */
         ...
         IF AVAIL bloan-cond THEN
         DO:
            /* Создаем новое условие на договоре с типом "остаток" */
            CREATE xloan-cond.
            BUFFER-COPY bloan-cond EXCEPT since TO xloan-cond
               ASSIGN
                  xloan-cond.since = iDate
            .
            ASSIGN
               /* Суррогат копируемого условия */
               vSurC       = bloan-cond.contract + "," + bloan-cond.cont-code + "," + 
                             STRING(bloan-cond.since)
               /* Суррогат нового условия */
               vSurr       = xloan-cond.contract + "," + xloan-cond.cont-code + "," + 
                             STRING(xloan-cond.since)
               /* Список копируемых ДР */
               vDopRecList = "cred-work-calend,int-work-calend,cred-mode,int-mode,СхемаПлат," +
                             "КолЛьгтПер,DateDelay,DateDelayInt,КолЛьгтПерПрц,PayType," + 
                             "cred-curr-next,NDays,NYears,NMonthes,ПродТрф"
            .
            /* Копируем ДР с найденнного условия */
            DO vCntr = 1 TO NUM-ENTRIES(vDopRecList):
               UpdateSignsEx (xloan-cond.class-code,
                              vSurr,
                              ENTRY(vCntr, vDopRecList),
                              GetXAttrValue("loan-cond", vSurC, ENTRY(vCntr, vDopRecList))).
            END.
            /* Сумму остатка по траншу */
            UpdateSignsEx (xloan-cond.class-code,
                           vSurr,
                           "PaySum",
                           TRIM(STRING(vAmount, ">>>,>>>,>>>,>>>,>>9.99"))).
            /* Тип платежа - Пролонгация */
            UpdateSignsEx (xloan-cond.class-code,
                           vSurr,
                           "PayType",
            ...
            ** плановых остатков то изменяем сумму */
            ...
            DO:
               ...
               DO:
                  ...
                  ASSIGN
                     ...
                     bterm-obl.idnt         = 2
                     bterm-obl.nn           = term-obl.nn + 1
                     bterm-obl.amt          = vAmount
                     bterm-obl.dsc-beg-date = iDate
                  .
                  RELEASE bterm-obl NO-ERROR.
               END.
            END.
            /* ищем запись с нулевым остатком на конец предыдущего периода */
            FIND LAST term-obl WHERE
                      term-obl.contract  = iContract
                  AND term-obl.cont-code = iContCode
                  AND term-obl.idnt      = 2
            ...
            IF AVAIL term-obl THEN
               ASSIGN
                  term-obl.end-date = bloan.end-date
               .
            /* если записи с нулем нет, то создаем ее  */
            ELSE
            DO:
               FIND LAST term-obl WHERE
                         term-obl.contract  = iContract
               ...
               DO:
                  ...
                  ASSIGN
                     ...
                     bterm-obl.nn           = term-obl.nn + 1
                     bterm-obl.amt          = 0
                     bterm-obl.dsc-beg-date = bloan.end-date
                  .
                  RELEASE bterm-obl NO-ERROR.
               END.
            END.

            /* ==============  формируем графики ОД и %  ============== */
            ...
            /* Пересчитываем график платежей по процентам. Сдвиг платежей, 
            ** даты которых приходятся на выходные дни,
            ** проводится в соответствии с настройкой транзакции. (параметр iShiftType) */
            RUN SetSysConf IN h_base ("ОБЯЗАТЕЛЬСТВА ПО ВОЗВРАТУ СДВИГ", iShiftType).
            RUN SetSysConf IN h_base ("ПЛАТЕЖИ ПО ПРОЦЕНТАМ СДВИГ",  iShiftType).
            ...
            FOR EACH tloan-cond WHERE 
                     tloan-cond.contract  = bloan.contract
                 AND tloan-cond.cont-code = bloan.cont-code
            NO-LOCK:
               vCondCount = vCondCount + 1.
            END.
            ...
            IF oResult <> {&RET-ERROR} THEN
               RUN Fill-SysMes IN h_tmess ("","","0", "Пролонгирован договор № " + bloan.doc-ref + 
                            " . Первонач.срок = " + STRING(xloan-cond.since) +
                            " Дата пролонгации = " + STRING(xloan-cond.since) +
                            " Новая дата окончания = " + STRING(bloan.end-date) +
                            " Ставка = " + vRates +
            ...
            ELSE
            DO: 
               RUN Fill-SysMes IN h_tmess ("","","0", "Ошибка пролонгации договора № " +
                                                       bloan.doc-ref). 
            END. 
            
            /* ==============      запись о пролонгации           ============== */
            /* проверяем, есть ли уже пролонгация, что бы не получить ошибку */
            FIND FIRST pro-obl WHERE
                       pro-obl.contract  = xloan-cond.contract
                   AND pro-obl.cont-code = xloan-cond.cont-code
                   AND pro-obl.idnt      = 3
                   AND pro-obl.pr-date   = xloan-cond.since
            NO-LOCK NO-ERROR.
            /* пролонгации нет - создаем */
            IF NOT AVAIL pro-obl THEN 
            DO:
               /* ищем последнюю запись по плановому гашению */
               FIND LAST term-obl WHERE
                         term-obl.contract  = bloan.contract
                     AND term-obl.cont-code = bloan.cont-code
                     AND term-obl.idnt      = 3
               ...
               IF NOT AVAIL term-obl THEN 
                  RUN Fill-SysMes IN h_tmess ("","","0", 
                      "Не найдено плановых платежей, невозможно создать запись " + 
                      "о пролонгации для договора № " + bloan.doc-ref). 
               ELSE 
               DO:      
                  /* ищем предыдущую запись о пролонгации */
                  FIND LAST prev-obl /* USE-INDEX */ USE-INDEX nn WHERE 
                            prev-obl.contract  = xloan-cond.contract
                        AND prev-obl.cont-code = xloan-cond.cont-code
                        AND prev-obl.idnt      = 3
                        AND prev-obl.nn        = term-obl.nn
                  NO-LOCK NO-ERROR.
                  /* номер пролонгации */
                  vNN = IF AVAIL prev-obl THEN 
                           prev-obl.nn + 1
                        ELSE 
                           1.
                  ...
                  ASSIGN
                     ...
                     pro-obl.end-date   = bloan.end-date
                     ...
                     /* Дата пролонгации */
                     pro-obl.pr-date    = xloan-cond.since
                 .
                  UpdateSigns ("pro-obl" ,
...
{pfuncdef 
   &DEFPROC     = "RecalcGraphLoan"
   &DESCRIPTION = "Запуск механизма разворота графиков (алгоритм из i-avterm.p)"
   &PARAMETERS  = "RECID ДОГОВОРА,RECID УСЛОВИЯ,СУММА (term-obl.idnt = 2),РЕЖИМ"
   &RESULT      = "Пересчет графиков договора"
   &SAMPLE      = "RecalcGraphLoan (RECID(loan),RECID(loan-cond),term-obl.amt-rub,{&MODE_ADD})."
   }   
PROCEDURE RecalcGraphLoan:
   DEF INPUT PARAM iRecLoan    AS RECID NO-UNDO. /* recid договора*/
   DEF INPUT PARAM iRecCond    AS RECID NO-UNDO. /* recid условия*/
   DEF INPUT PARAM iSumm       AS DEC   NO-UNDO. /* Сумма (term-obl.idnt = 2)*/
   DEF INPUT PARAM iMode       AS DEC   NO-UNDO. /* Режим */
   
   DEF VAR vCredOffset     AS CHAR  NO-UNDO.
   DEF VAR vSurrLoanCond   AS CHAR  NO-UNDO.
   DEF VAR vOffset         AS CHAR  NO-UNDO INIT ",->,<-".
   DEF VAR vDelayOffset    AS CHAR  NO-UNDO.  /* сдвиг даты окончания плат.периода (осн.долг) */
   DEF VAR vDelayOffsetInt AS CHAR  NO-UNDO.  /* сдвиг даты окончания плат.периода (проценты) */
   DEF VAR vCondCount      AS INT64 NO-UNDO.
   DEF VAR vTypeCond       AS LOG   NO-UNDO.

   DEF BUFFER loan      FOR loan.
   DEF BUFFER loan-cond FOR loan-cond.

   FIND FIRST loan WHERE RECID(loan) = iRecLoan NO-LOCK NO-ERROR.

   IF NOT AVAILABLE loan THEN
   DO: 
      RUN Fill-SysMes IN h_tmess("","","-1","Договор не найден").
      RETURN.
   END.

   FIND FIRST loan-cond WHERE RECID(loan-cond) = iRecCond EXCLUSIVE-LOCK NO-WAIT NO-ERROR.

   IF NOT AVAILABLE loan-cond THEN
   DO: 
      IF LOCKED loan-cond THEN 
           RUN Fill-SysMes IN h_tmess("","","-1",
                                      "Условие договора заблокировано другим пользователем").
      ELSE RUN Fill-SysMes IN h_tmess("","","-1","Условие не найдено").
      RETURN.
   END.

   ASSIGN
      vSurrLoanCond   = GetSurrogateBuffer("loan-cond",(BUFFER loan-cond:HANDLE))
      vCredOffset     = GetXAttrValueEx("loan-cond",vSurrLoanCond,"cred-offset","")
      vDelayOffset    = GetXAttrValueEx("loan-cond",vSurrLoanCond,"delay-offset","")
      vDelayOffsetInt = GetXAttrValueEx("loan-cond",vSurrLoanCond,"delay-offset-int","")
      vTypeCond       = GetXAttrValue("loan-cond",vSurrLoanCond,"СхемаПлат") =  "Аннуитетная" 
   .
   IF vTypeCond THEN
      ASSIGN
...
{pfuncdef 
   &DEFPROC     = "QuaranteeBadClient"
   &DESCRIPTION = "Информация о финансовом положении гаранта/поручителя по кредиту"
   &PARAMETERS  = "ДАТА,СУРРОГАТ ДОГОВОРА,НОМЕР ДОГОВОРА,СЧЕТ ОБЕСПЕЧЕНИЯ,РОЛЬ ОБЕСПЕЧЕНИЯ"
   &RESULT      = "СУРРОГАТ КЛИЕНТА, НАИМЕНОВАНИЕ,ДАТА ИНФОРМАЦИИ,КОЛИЧЕСТВО ДНЕЙ"
   &SAMPLE      = "RUN QuaranteeBadClient(@date,@contract, @cont-code, @acct, @acct-type)"
}  
PROCEDURE QuaranteeBadClient:
   DEF INPUT  PARAM iDate      AS DATE  NO-UNDO. /* Дата расчет */
   DEF INPUT  PARAM iContract  AS CHAR  NO-UNDO. /* Назначение договора */
   DEF INPUT  PARAM iContCode  AS CHAR  NO-UNDO. /* Номер договора */  
   DEF INPUT  PARAM iAcct      AS CHAR  NO-UNDO. /* Номер счета */
   DEF INPUT  PARAM iAcctType  AS CHAR  NO-UNDO. /* Роль счета */
   DEF OUTPUT PARAM oSurrCli   AS CHAR  NO-UNDO. /* Суррогат клиента */
   DEF OUTPUT PARAM oNameCli   AS CHAR  NO-UNDO. /* Наименование клиента */
   DEF OUTPUT PARAM oDateInfo  AS DATE  NO-UNDO. /* Дата информации */
   DEF OUTPUT PARAM oCntDay    AS INT64 NO-UNDO. /* Кол-во дней */
   
   DEF VAR vSurrTObl AS CHAR  NO-UNDO.
   ...
   DEF VAR vName     AS CHAR  NO-UNDO EXTENT 3.
   ...
   FOR EACH term-obl WHERE
        ...
        AND term-obl.idnt      = 5
        AND (   term-obl.sop-date  = ?
             OR term-obl.sop-date  >= iDate)
   NO-LOCK:
      ASSIGN
         vSurrTObl = term-obl.contract + "," +
                     term-obl.cont-code + "," +
                     STRING(term-obl.idnt) + "," +
                     STRING(term-obl.end-date) + "," +
                     STRING(term-obl.nn)
         vVidDog   = GetXattrValueEx ("term-obl", vSurrTObl, "ВидДогОб", "")
         vNumPP    = GetXattrValueEx ("term-obl", vSurrTObl, "НомерПП",  "")
         vVidDog   = vVidDog + (IF vNumPP <> "0" THEN vNumPP ELSE "")
      .
         /* Ищем (вид договора + номер) EQ роли счета */
      IF loan-acct.acct-type = vVidDog THEN
         ASSIGN
            vCustCat = term-obl.symbol
            vCustID  = term-obl.fop
         .
   END.
   ...
   FIND LAST tmpsigns WHERE
             tmpsigns.file-name = vFileName
         AND tmpsigns.surrogate = STRING(vCustID)
         AND tmpsigns.code      = "ФинСост"
         AND tmpsigns.since     <= iDate
   NO-LOCK NO-ERROR.
   
   IF    NOT AVAIL tmpsigns
      OR tmpsigns.code-value <> "Плохое" THEN
      RETURN.
   
   ASSIGN   
      oSurrCli  = vCustCat + "," + STRING(vCustID)
      oDateInfo = tmpsigns.since
      oCntDay   = iDate - tmpsigns.since
   .

   CASE vCustCat:
      WHEN "Ч" OR WHEN "Ю" THEN
      DO:
         RUN GetCustName IN h_base (vCustCat,
                                    vCustID,
                                    ?,
                                    OUTPUT vName[1],
                                    OUTPUT vName[2],
         ...
         oNameCli = vName[1] + " " + vName[2].
      END.
      WHEN "Б" THEN
      DO:
         RUN GetCustNameShort IN h_base (vCustCat,
                                         vCustID,
                                         OUTPUT oNameCli).
      END.
   END CASE.
...
/* Получение коды комиссии по параметру на основе классификатора "НачКом" */
PROCEDURE GetCommByPar PRIVATE:
   DEF INPUT PARAM  iParam AS INT64 NO-UNDO. /* Код параметра КиД */
   DEF OUTPUT PARAM oComm  AS CHAR  NO-UNDO. /* Список комисси по параметру из НачКом */

   DEF BUFFER ycode FOR code.

   FOR EACH ycode WHERE ycode.class        =     "НачКом"
                    AND ycode.val          BEGINS STRING(iParam)
                    AND ENTRY(1,ycode.val) =     STRING(iParam)
   NO-LOCK:
      oComm = oComm + "," + ycode.code.
   END.

   oComm = SUBSTRING(oComm,2,LENGTH(oComm)).

   RETURN oComm.

END PROCEDURE.

/* Возвращает валюту комисии (параметра) по коду параметра договора */
{pfuncdef 
   &DEFPROC     = "GetCommVal"
   &DESCRIPTION = "Возвращает валюту комисии (параметра) по коду параметра договора"
   &PARAMETERS  = "НАЗНАЧЕНИЕ ДОГОВОРА, НОМЕР ДОГОВОРА,~
ДАТА НА КОТОРУЮ ИЩЕМ КОМИССИЮ, КОД ПАРАМЕТРА КИД"
   &RESULT      = "ВАЛЮТА ПАРАМЕТРА"
   &SAMPLE      = "RUN GetCommVal IN h_loan (~
tt-loan.contract,tt-loan.cont-code,tt-loan.since,0,~
OUTPUT vCurr)."
} 
PROCEDURE GetCommVal:
   DEF INPUT  PARAM iContract  AS CHAR  NO-UNDO. /* Назначение договора */
   DEF INPUT  PARAM iContCode  AS CHAR  NO-UNDO. /* Номер договора */
   DEF INPUT  PARAM iSince     AS DATE  NO-UNDO. /* Дата на которую ищем комиссию */
   DEF INPUT  PARAM iParam     AS INT64 NO-UNDO. /* код параметра КиД */
   ...
   /* Получаем коды комиссий по параметру из "НачКом" */
   RUN GetCommByPar IN THIS-PROCEDURE (iParam, OUTPUT vComm).
   
   comms:
   DO vI = 1 TO NUM-ENTRIES(vComm):

         /* Ищем действующую комиссию */
      RUN GET_COMM_LOAN_BUF IN THIS-PROCEDURE (iContract,
                                               iContCode,
                                               ENTRY(vI,vComm),
                                               iSince,
                                               BUFFER comm-rate).
      ...
      IF AVAIL comm-rate THEN
      DO:
         oCurr = GetCommRateCurrency(BUFFER comm-rate,
                                 BUFFER loan,
                                     ENTRY(vI,vComm),
                                     iSince).
         LEAVE comms.
   ...
   RETURN oCurr.
END PROCEDURE.
/* определяем, является ли договор рамочным */
{pfuncdef 
   &DEFPROC = "IsLoanUnComLines"
   &DESCRIPTION = "определяем, является ли договор рамочным "
   &PARAMETERS = "тип договора, номер договора"
   &RESULT = "TRUE/FALSE"
   &SAMPLE = "IsLoanUnComLines (loan.contract,loan.cont-code)"
}
FUNCTION IsLoanUnComLines RETURNS LOGICAL (
   iContract AS CHARACTER,
   iContCode AS CHARACTER):
   
   DEF VAR vIsLoanUnComLines AS LOGICAL NO-UNDO INIT ?.
   DEF BUFFER loan  FOR loan.
   DEF BUFFER signs FOR signs.
   ...
   IF AVAIL loan THEN
      vIsLoanUnComLines = CAN-DO("loan-transh-un-sim", loan.class-code).
   
   RETURN vIsLoanUnComLines.
END FUNCTION.

/* определяем, является ли договор рамочным */
{pfuncdef 
   &DEFPROC = "IsLoanUnComLinesClass"
   &DESCRIPTION = "определяем, является ли договор рамочным "
   &PARAMETERS = "тип договора, номер договора"
   &RESULT = "TRUE/FALSE"
   &SAMPLE = "IsLoanUnComLinesClass (iClassCode)"
}
FUNCTION IsLoanUnComLinesClass RETURNS LOGICAL (
   INPUT iClassCode AS CHARACTER):

   RETURN (IF iClassCode = "loan-transh-un-sim" THEN YES ELSE NO).

END FUNCTION.

/* инструмент для расчета суммы начисленных доходов по договору/соглашению для ф.123 */
{pfuncdef 
   &DEFPROC     = "GetLoanRevenues"
   &DESCRIPTION = "инструмент для расчета суммы начисленных доходов ~
                   по договору/соглашению для ф.123"
   &PARAMETERS  = "НАЗНАЧЕНИЕ ДОГОВОРА,НОМЕР ДОГОВОРА ~
                   НАЧАЛЬНАЯ ДАТА РАСЧЕТА,КОНЕЧНАЯ ДАТА РАСЧЕТА"
   &RESULT      = "СУММА НАЧИСЛЕННЫХ ДОХОДОВ, КК НА ДАТУ РАСЧЕТА, ДАТА ИЗМЕНЕНИЯ КК"
   &SAMPLE      = "RUN GetLoanRevenues IN h_loan (tt-loan.contract, ~
                                                  tt-loan.cont-code, ~
                                                  gbeg-date, ~
                                                  gend-date, ~
                                                  OUTPUT vSumma, ~
...
PROCEDURE GetLoanRevenues:
   DEF INPUT  PARAM iContract  AS CHAR  NO-UNDO. /* Назначение догвора  */
   DEF INPUT  PARAM iContCode  AS CHAR  NO-UNDO. /* Номер договора  */
   DEF INPUT  PARAM iBegDate   AS DATE  NO-UNDO. /* Дата начала расчета */
   DEF INPUT  PARAM iEndDate   AS DATE  NO-UNDO. /* Конечная дата расчета  */
   DEF OUTPUT PARAM oSumma     AS DEC   NO-UNDO. /* Сумма начисленных доходов */
   DEF OUTPUT PARAM oKK        AS INT64 NO-UNDO. /* Категория качества на дату расчета */
   DEF OUTPUT PARAM oSinceKK   AS DATE  NO-UNDO. /* Дата изменения категории качества */
   
   DEF VAR vSpisOper AS CHAR  NO-UNDO.
   DEF VAR vOper     AS INT64 NO-UNDO.
   DEF VAR vI        AS INT64 NO-UNDO.
   ...
   IF NOT AVAIL loan THEN
      ...
      /* Если начальная дата не задана, то
         за начальную дату берем дату открытия договора */
   IF iBegDate = ? THEN
      iBegDate = loan.open-date.
      
   vSpisOper = fGetSetting ("Форма123", "ОпНачДохОтч", "").
   
      /* Бежим по операциям из ДР ОпНачДохОтч */
   IF {assigned vSpisOper} THEN
   DO vI = 1 TO NUM-ENTRIES(vSpisOper):
      
      vOper = INT64(ENTRY(1, vSpisOper)).
      ...
      DO:   
            /* Если договор является охватывающим, то
               суммируем операции по всем траншам */
         IF loan.cont-type = "Течение" THEN
         DO:
            FOR EACH bloan WHERE
                     bloan.contract  =     loan.contract
                 AND bloan.cont-code BEGINS loan.cont-code + " "
                 AND NUM-ENTRIES(bloan.cont-code, " ") = 2
            NO-LOCK:
               FOR EACH loan-int WHERE
                        loan-int.contract  = bloan.contract 
               ...
               NO-LOCK:
                  oSumma = oSumma + loan-int.amt-rub.
               END.
            END.
         END.
         ELSE
   ...
   END.
      ...
      /* Для получения КК смотрим, не находится ли договор в ПОСе */
   vPos = LnInBagOnDate (loan.contract, loan.cont-code, iEndDate).
      /* Если договор в ПОСе, то берем КК с ПОСа */
   IF vPos <> ? THEN 
   DO:
      FIND FIRST bloan WHERE
                 bloan.contract  = "ПОС"
             AND bloan.cont-code = vPos
      NO-LOCK NO-ERROR.
      IF AVAIL bloan THEN 
      DO:
         FIND LAST comm-rate WHERE
                   comm-rate.kau        = "ПОС," + bloan.cont-code
               AND comm-rate.commission = "%Рез"
               AND comm-rate.since      <= iEndDate
               AND comm-rate.acct       = "0"
         NO-LOCK NO-ERROR.
         IF     NOT AVAIL comm-rate 
            AND LENGTH(bloan.parent-contract) > 0 THEN
            FIND LAST comm-rate WHERE
                      comm-rate.kau        = "ПОС," + bloan.parent-cont-code
                  AND comm-rate.commission = "%Рез"
                  AND comm-rate.since      <= iEndDate
                  AND comm-rate.acct       = "0"
            NO-LOCK NO-ERROR.
         ...
         DO:
            ASSIGN
               oKK      = INT64(GetXAttrValueEx("comm-rate",
                                                STRING(comm-rate.comm-rate-id),
                                                "КатегорияКач",
                                                ?))
               oKK      = IF oKK = ? THEN 
                             PsGetGrRiska (comm-rate.rate-comm,
                                           bloan.cust-cat, 
                                           comm-rate.since)
                          ELSE
                             oKK
               oSinceKK = comm-rate.since
            .
         END.
      END.
   END.
      /* В противном случае берем с самого договора, 
         а если это течение, то с охватывающего */
   ELSE 
   DO:
      FIND LAST comm-rate WHERE
                comm-rate.kau        = loan.contract + "," + loan.cont-code
      ...
      IF     NOT AVAILABLE comm-rate              
         ...
         AND NOT CAN-FIND (FIRST bloan WHERE 
                                 bloan.contract  = loan.contract
                             AND bloan.cont-code = ENTRY(1, loan.cont-code, " ")
                             AND bloan.cont-type = "Течение"
                           NO-LOCK)
      THEN
         FIND LAST comm-rate WHERE
                   comm-rate.kau        = loan.contract + "," + ENTRY (1, loan.cont-code, " ")
               AND comm-rate.commission = "%Рез"
               AND comm-rate.since      <= iEndDate
               AND comm-rate.acct       = "0"
         NO-LOCK NO-ERROR.
      ...
      IF AVAIL comm-rate THEN 
         ASSIGN
            oKK      = INT64(GetXAttrValueEx("comm-rate",
                                             ...
                                             ?))
            oKK      = IF oKK = ? THEN 
                          LnGetGrRiska(comm-rate.rate-comm,
                                       comm-rate.since)
                       ELSE
                          oKK
            oSinceKK = comm-rate.since
...
/* $LINTUSER='seei' */
/* $LINTDATE='02/05/2017 14:13:50.665+03:00' */
/*prosignqLrJT827HoYhrPh4mLibeg*/