...
FUNCTION NumWorkday RETURN INT64 (INPUT in-date-from AS DATE,INPUT in-date-to AS DATE).
  ...
  DO vDate = in-date-from TO in-date-to:
    IF NOT HOLIDAY(vDate) AND NOT CAN-DO("1,7",STRING(WEEKDAY(vDate))) THEN i = i + 1.
  END.
  RETURN i.
END FUNCTION.
...
DEFINE VARIABLE mVs-ost     AS DECIMAL  label "" INIT 0  NO-UNDO.
...
DEFINE TEMP-TABLE otch
      ...
      FIELD DateTimeOut  AS CHAR           /* Время выгрузки отчета */  
...
def var DateTimeOut as char no-undo.
def var nameCl as char no-undo.
def var telCl as char no-undo.
def var plan_date as date no-undo.
def var plan_date0 as date no-undo.
def var plan_date2 as date no-undo.
...
def var neDateDG as char no-undo.
def var neIspDG as char no-undo.
...
def var summdosrg as decimal no-undo. 
def var ii as int64.

DEFINE VARIABLE mRs-ost     AS DECIMAL  NO-UNDO.
DEFINE VARIABLE mVs-ost     AS DECIMAL  NO-UNDO.
...
DateTimeOut = STRING(NOW,"99/99/9999 HH:MM:SS").

FOR EACH bsigns
   WHERE bsigns.file-name   EQ "loan"
     AND bsigns.code        EQ "dg_summ_dosr_g"
     /* and (bsigns.surrogate eq 'Кредит,45-00-112503-ДПА@0000') */  
...
mSum-all = calcppdg().
   ...
   IF AVAIL loan-acct THEN DO:
     ...
     ELSE ost408_s = abs(sh-val). */
      ...
      IF loan-acct.currency = '' THEN
         ost408_s = abs(mRs-ost).
      ELSE ost408_s = abs(mVs-ost).
  ...
  /*  mcommrate = GET_COMM("%Кред",
   ...
   neIspDG = IF (ost408 + ost408_s) >= mSum-all THEN "ПДГ" ELSE "".
   IF neIspDG <> "ПДГ" 
	AND ABS(DATE(vDateDG) - (opendate + 1)) > 30 
	AND GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_type_dosr_g","") <> 'ПДГ'
        THEN vDateDG = "01.01.1970".


   plan_date0 = DATE(vDatePodZay).
   DO ii = 1 TO 3:
    plan_date0 = plan_date0 + 1.
    IF HOLIDAY(plan_date0) OR CAN-DO("1,7",STRING(WEEKDAY(plan_date0))) THEN DO:
        ii = ii - 1.
    END.
   END. 
   
   plan_date2 = DATE("01.01.1970").
   /* ПЛАНОВОЕ ПОГАШЕНИЕ ССУДЫ */
   FOR EACH term-obl WHERE /*term-obl.amt-rub <> 0 AND */
         term-obl.contract = loan.contract
      AND term-obl.cont-code = loan.cont-code
      ...
      AND term-obl.end-date >= plan_date0  NO-LOCK
      BY term-obl.end-date:
         plan_date2 = term-obl.end-date.
         LEAVE.
   END.
   /* ПЛАТЕЖЕЙ ПО ПРОЦЕНТАМ */
   FOR EACH term-obl WHERE /*term-obl.amt-rub <> 0 AND */
      ...
      AND term-obl.end-date >= plan_date0 NO-LOCK
      BY term-obl.end-date:
         IF plan_date2 EQ DATE("01.01.1970") THEN
           plan_date2 = term-obl.end-date.
         ELSE
           plan_date2 = MIN(term-obl.end-date,plan_date2).
        LEAVE.
   END.

/*   message string(plan_date0) + '  ' + string(plan_date2) view-as alert-box. */
   ...
   */
   ...
   IF vDateIspDG NE "ПДГ" AND GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_type_dosr_g","") <> 'ПДГ'
      THEN vPerDateDG = IF DATE(vDateIspDG) > DATE(vDateDG) THEN "ПД" ELSE "". 
   
   neDateDG = GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_Date_dosr_g","-").
   IF neIspDG <> "ПДГ" 
	AND ABS(DATE(vDateDG) - (opendate + 1)) > 30 
	AND GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_type_dosr_g","") <> 'ПДГ' THEN DO:
     vDateDG     = GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_Date_dosr_g","01.01.1970"). 
     if DATE(vDateDG) NE DATE("01.01.1970") and DATE(vDateDG) <> plan_date and DATE(vDateDG) <> plan_date2 AND neDateDG <> "-" then do:
        neDateDG = vDateDG.
     end.
     else do:
	neDateDG = "-".
   ...
   IF neDateDG <> '-' AND DATE(neDateDG) > loan.since 
      THEN vDateIspDG = "ПДГ".
   vDateIspDG = STRING(plan_date2,"99.99.9999").

   IF vDateIspDG NE "ПДГ" 
    AND neDateDG <> "-"
      THEN do:
           IF DATE(vDateIspDG) > DATE(neDateDG) THEN vPerDateDG =  "ПД".
      end. 
      
   summdosrg = decimal(GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"dg_summ_dosr_g","0")).
   ...
   ASSIGN
      ...
      otch.IspDG       = IF (ost408 + ost408_s) >= mSum-all AND summdosrg >= mSum-all THEN "ПДГ" ELSE ""
      otch.DateDG      = neDateDG
 ...
 <Worksheet ss:Name="lll">
  <Table >
   ...
   <Row ss:AutoFitHeight="1">
     ...
     <Cell ss:StyleID="s67"><Data ss:Type="String">Время создания отчета</Data></Cell>
   </Row>
   <Row ss:AutoFitHeight="1">
     ...
     <Cell ss:StyleID="s67"><Data ss:Type="String">24</Data></Cell>
   </Row>\n'.
...
FOR EACH otch NO-LOCK BY otch.cont_code BY cont_code:
   PUT STREAM vvs UNFORMATTED '<Row ss:AutoFitHeight="0">'.
     ...
     PUT STREAM vvs UNFORMATTED  '<Cell><Data ss:Type="String">'.
     PUT STREAM vvs UNFORMATTED otch.DateTimeOut + '</Data></Cell>\n'.                          
