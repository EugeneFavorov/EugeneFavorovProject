...
/* sveta        RUN FrmtAmt(mFirstPay,loan.currency,(IF ENTRY(4, per, "|") EQ "Пр2" THEN 2 ELSE 1), OUTPUT rproc).*/
  ...
  IF strpar MATCHES "КлОГРН" THEN  /*IF per EQ "КПП" THEN*/
   ...
   DO:
       ...
       DO:
         ...
         IF AVAILABLE person THEN
             ...
             WHEN "ПСер" THEN 
                 rproc = ENTRY(1,person.document," ") + " " + ENTRY(2,person.document," ").
			 WHEN "ПНом" THEN
				 rproc = ENTRY(3,person.document," ").
			 WHEN "День" THEN
   ...
   IF per MATCHES "КодПродМПроп2" THEN
   DO:
		FIND FIRST loan-cond WHERE loan-cond.cont-code EQ loan.cont-code
                             AND loan-cond.since <= XDate NO-LOCK NO-ERROR.
