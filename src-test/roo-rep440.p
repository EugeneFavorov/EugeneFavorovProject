/*
     Filename: roo-rep440.p
      Comment: äÆ≠‚‡Æ´Ï≠†Ô ¢•§Æ¨Æ·‚Ï ·ÆÆ°È•≠®© 440-è
   Parameters:
         Uses:
      Used by:
      Created:
...
DEFINE VARIABLE mDateTimeROO  AS CHARACTER NO-UNDO.
DEFINE VARIABLE mDateTimeBOS  AS CHARACTER NO-UNDO.
DEFINE VARIABLE mStateROO     AS CHARACTER NO-UNDO.
DEFINE VARIABLE mStateBOS     AS CHARACTER NO-UNDO.
DEFINE VARIABLE mFileNameROO  AS CHARACTER NO-UNDO.
...
DEFINE VARIABLE rowStyleStateROO AS CHARACTER NO-UNDO.
...
DEFINE VARIABLE mTextPB2         AS CHARACTER NO-UNDO.
...
DEFINE VARIABLE mIsCustCorp      AS LOGICAL   NO-UNDO.
DEFINE VARIABLE mIsPerson        AS LOGICAL   NO-UNDO.

DEFINE VARIABLE zAcct            AS CHARACTER NO-UNDO.
DEFINE VARIABLE zAcct2           AS CHARACTER NO-UNDO.

DEFINE VARIABLE mAcctFound       AS LOGICAL   NO-UNDO.
...
vNum = 0.
mInt = 0.
RUN dbgprint.p ("roo-rep440 ","Begin " + STRING(TIME,"hh:mm:ss")).

FOR EACH Packet WHERE TRUE
   AND CAN-DO ("PTax440",TRIM(Packet.Class-Code))
   /*AND Packet.PacketID EQ 17837382*/   /*ZNO*/
   /*AND Packet.PacketID EQ 34207879*/ 
   /*AND Packet.PacketID  EQ 36529991*/   /*ROO*/
   ...
   RUN dbgprint.p ("roo-rep440 ","Packet.PacketID = " + STRING(Packet.PacketID) + "~nvNum = " + STRING(vNum)).
   ...
   THEN mFileNameROO = FileExch.Name.
   ELSE mFileNameROO = "".

   IF SUBSTRING(mFileNameROO,1,3) EQ "ROO" THEN
      RUN dbgprint.p ("roo-rep440 ","mFileNameROO = " + mFileNameROO + " PackDate = " + STRING(Packet.PackDate,"99/99/9999")).
   ...
   DO:
      mTypeInf = GetFileVar440(STRING(PacketText.Contents),"í®Øà≠‰","").
      mVidZapr = GetFileVar440(STRING(PacketText.Contents),"Ç®§á†Ø‡","").
      mTypZapr = GetFileVar440(STRING(PacketText.Contents),"í®Øá†Ø‡","").
   END.
   
   RUN dbgprint.p ("roo-rep440 ",
   ...
   IF mTypeInf EQ 'êÖòçééíåÖç' OR SUBSTRING(mFileNameROO,1,3) EQ "ROO" THEN
   DO:

      mDateTimeROO = STRING(Packet.PackDate,"99.99.9999") + " " + STRING(Packet.PackTime,"hh:mm:ss").
      
      mStateROO    = Packet.State.
      IF mStateROO EQ "éòÅä"
      THEN rowStyleStateROO = "s78red".
      ELSE rowStyleStateROO = "s78".
      ...
      FOR EACH PackObject
         ...
         DO:
            /*RUN dbgprint.p ("roo-rep440 " + " {&line-number}","mFileName = " + mFileName).*/
      ...
      RUN dbgprint.p ("roo-rep440 ","Packet.PacketID = " + STRING(Packet.PacketID) + 
      ...
      DO:
         mDateTimeBOS = STRING(xPacketBVS.PackDate,"99.99.9999") + " " + STRING(xPacketBVS.PackTime,"hh:mm:ss").
         mStateBOS    = xPacketBVS.State.
         IF mStateBOS EQ "éòÅä"
         THEN rowStyleStateBOS = "s78red".
         ELSE rowStyleStateBOS = "s78".
         mFileNameBOS = IF AVAIL(xFileExchBVS) THEN xFileExchBVS.Name ELSE GetXAttrValueEx("Packet",STRING(xPacketBVS.PacketID),"FileName",""). 
      END.
   
	   RUN dbgprint.p ("roo-rep440 ",
	      "~nmDateTimeBOS  = " + mDateTimeBOS +
	      "~nmStateBOS     = " + mStateBOS    +
	      "~nmFileNameBOS  = " + mFileNameBOS
	      ).
      ...
      zAcct = IF AVAIL(PacketText) THEN GetFileAllVar440(STRING(PacketText.Contents),"çÆ¨ëÁ","") ELSE "".
      
      mAcctFound = NO.
      FOR EACH acct WHERE TRUE
         AND acct.filial-id EQ shFilial
         AND acct.number    EQ zAcct
         NO-LOCK:
         mAcctFound = YES.
      END.
      
	   RUN dbgprint.p ("roo-rep440","~n1 zAcct = " + zAcct + " mAcctFound = " + STRING(mAcctFound) + ".").
      
      IF zAcct NE "" AND mAcctFound EQ YES THEN
      DO:
         mINNPP = IF AVAIL(PacketText) THEN GetFileVar440(STRING(PacketText.Contents),"àççûã","X") ELSE "X".
	      RUN dbgprint.p ("roo-rep440","~nmINNPP = " + mINNPP).
         
         mIsCustCorp = NO.
         FOR EACH cust-corp WHERE TRUE
            AND cust-corp.inn EQ mINNPP 
            NO-LOCK,
         ...
         mINNPP = IF AVAIL(PacketText) THEN GetFileVar440(STRING(PacketText.Contents),"àççàè","X") ELSE "X".
	      RUN dbgprint.p ("roo-rep440","~nmINNPP = " + mINNPP).
         
         mIsPerson = NO.   
         FOR EACH person WHERE person.inn EQ mINNPP NO-LOCK,
            ...
            RUN dbgprint.p ("roo-rep440","~nperson.inn = " + person.inn).
         ...
         IF  mIsCustCorp EQ NO
         AND mIsPerson   EQ NO THEN
         DO:
            DO mCnt = 1 TO NUM-ENTRIES(zAcct):
               FOR EACH acct WHERE TRUE
                  AND acct.filial-id EQ shFilial
                  AND acct.number    EQ ENTRY(mCnt,zAcct)
                  NO-LOCK:

                  RUN dbgprint.p ("roo-rep440","~nacct.number = " + acct.number).
         ...
         END.
      END.
      ELSE IF zAcct NE "" AND mAcctFound EQ NO THEN
      DO:
         RUN dbgprint.p ("roo-rep440","~n2 zAcct = " + zAcct + " mAcctFound = " + STRING(mAcctFound) + ".").
         DO mCnt = 1 TO NUM-ENTRIES(zAcct):
            IF mCnt EQ 1 THEN
            DO:
               {roo-rep1.i}
      ...
      ELSE
      DO:
         zAcct2 = IF AVAIL(xPacketPB2Text) THEN GetFileAllVar440(STRING(xPacketPB2Text.Contents),"á≠†Áê•™¢","") ELSE "".
         RUN dbgprint.p ("roo-rep440","~nzAcct2 = " + zAcct2 + ".").
         IF zAcct2 NE "" THEN
         DO mCnt = 1 TO NUM-ENTRIES(zAcct2):
            FOR EACH acct WHERE TRUE
               AND acct.filial-id EQ shFilial
               AND acct.number    EQ ENTRY(mCnt,zAcct2)
               NO-LOCK:
      ...
      DO:
         mInt = mInt + 1.
         PUT SCREEN ROW SCREEN-LINES + MESSAGE-LINES + 1 COL 1 'é‚Á•‚ ‰Æ‡¨®‡„•‚·Ô' + mSpin[mInt MOD 8 + 1] COLOR bright.
      END.
   END.
END.
...
/*{preview2.i &filename="roo-rep440.xml" }*/
