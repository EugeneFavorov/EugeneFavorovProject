...
sname = '/data/home/kam/' + STRING(YEAR(end-date),"9999") + STRING(MONTH(end-date),"99") + STRING(DAY(end-date),"99") + '_clnd_fact.txt'.
...
*/
        ...
        WHEN 298 THEN DO:  /* 298 Перенос %% с треб. на проср.%% за пр.о.д */
            ...
            idk = 233.
        END.    
        WHEN 121 THEN DO:  /* 46 Оплата требований (метод начислений) */
            idd = 5.
            idk = 35.
        END.        
    END CASE.    
    ...
    for each term-obl where term-obl.contract EQ 'Кредит'
        ...
        no-lock by term-obl.end-date:
          ...
          find first ttOp2 where   
              ...
              and (ttOp2.in-id-op = 298 or ttOp2.in-id-op = 119) 
              no-error.
