...
/*
       ...
       IF  mSignsVal eq "" THEN        
              mSignsVal = if GetTempXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "УчредОрг",end-date, "") NE "" THEN GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "УчредОрг", "") else "Нет".
