...
DEFINE VARIABLE mDR                     AS CHARACTER INIT "РискОтмыв" NO-UNDO.
...
IF CAN-DO('ank*,infank_ip*', iParms) THEN
DO:
...
do:
 ...
 FIND FIRST person WHERE RECID(person) EQ tmprecid.id NO-LOCK NO-ERROR.
RUN Insert_TTName("DogNum", STRING(person.person-id)).
...
DO:     
     RUN signat("person",string(person.person-id), '').
...
FIND FIRST acct WHERE  person.person-id EQ acct.cust-id use-INDEX open-date NO-LOCK NO-ERROR. 
IF AVAILABLE acct THEN  
do:  
    RUN Insert_TTName("acct",   STRING(acct.acct, "x(25)")).  
    mStatus = getxattrvalue("acct",acct.acct + "," + acct.currency,"СотрОткрСч"). 
    
    IF  CAN-DO("serv*,sync", mStatus) THEN  
        RUN signat("person",string(person.person-id), mStatus).
    else
    do:
 
        FIND _user WHERE _user._userid =  mStatus NO-LOCK NO-ERROR.
        IF AVAIL _user THEN 
        DO:
            RUN Insert_TTName("_Kur-Name", _user._user-name).   
            RUN Insert_TTName("Кур_Должн", GetXAttrValueEx("_user", _User._Userid, "Должность", "")). 
        END.       
        else    
            RUN signat("person",string(person.person-id), mStatus).
    end.    
...
ELSE
do:
    RUN Insert_TTName("_Kur-Name", ""). 
    RUN Insert_TTName("Кур_Должн", "").   
...
/*do:
 if  GetXAttrValueEx("person",STRING(person.person-id), "РискОтмывИП","") ne ""  Then 
...
If (SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) eq "ankip") then*/

    

 /* mSignsVal = GetTempXAttrValueEx("person",STRING(person.person-id), "ОценкаРиска", end-date ,"").
  mSignsV = ''.
...
(GetXAttrValueEx("person",STRING(person.person-id), "ОценкаРиска",""))).
           ...
           IF  (mSignsVal = "") or CAN-DO(",низкий", GetTempXAttrValueEx("person",STRING(person.person-id),  
                                IF GetXAttrValueEx("person",STRING(person.person-id), "Субъект","") = "ФЛП" THEN "РискОтмывИП" else "РискОтмыв"
                                , today,"")) /* or   GetTempXAttrValueEx("person",STRING(person.person-id),  "РискОтмыв" , today,"") eq "")   */    THEN 


  mSignsV= GetTempXAttrValueEx("person",STRING(person.person-id), "ОценкаРиска", end-date ,"").                 
               
               
mSignsV= GetTempXAttrValueEx("person",STRING(person.person-id), "РискОтмыв", end-date ,"").   

IF (CAN-DO(",низкий", mSignsV)) then mSignsV =  "Отсутствуют признаки легализации".

 if  GetXAttrValueEx(iClass,iSurr, "Субъект","") EQ "ФЛП" THEN mDR  =  "РискОтмывИП".                 
 */              

IF CAN-DO("*ankip*", iParms) then  mDR  =  "РискОтмывИП". 

mSignsV = GetTempXAttrValueEx("person",STRING(person.person-id), mDR, end-date ,""). 
...
do:   
    ...
    if   mSignsVal eq ""  then
        mSignsV =  "Отсутствуют признаки легализации".
    else  
    do:
      mSignsV = ''.         
          DO i = 1 TO NUM-ENTRIES(mSignsVal,";") :
     
            find first code where code.parent = "УрРиска"
                and code.class = "УрРиска"
                and code.code = TRIM(ENTRY(i, mSignsVal,';'))  no-lock no-error.          
          
            if avail code then   
                mSignsV = (If trim(mSignsV) ne '' then mSignsV + '; ' else '')  + Code.name.
            else  mSignsV = mSignsVal.                              
        end. 
    end.  
end.
...
RUN Insert_TTName("ОценкаРиска", mSignsV).
...
mSignsVal = GetXAttrValueEx("person",STRING(person.person-id), "ОКВЭД", "").
 
mSignsVal = ENTRY(1,mSignsVal,',').

mSignsVal =  mSignsVal + ' ' + GetCodeName("ОКВЭД2", mSignsVal).

IF trim(mSignsVal) EQ "" THEN 
...
DO:
    ...
    END.
END.
mSignsVal = IF not {assigned mSignsVal} THEN "нет" ELSE mSignsVal.
RUN Insert_TTName ("VidDey",mSignsVal).
...
/**********************/  
f = GetXAttrValueEx("person",STRING(person.person-id),"soursce_inf","").
...
do: 
    ...
    CASE f:
        WHEN "Со слов клиента"  THEN                         
        RUN Insert_TTName("Ben_fio_new", person.name-last + " " + person.first-names ).                         
        WHEN "Самостоятельно из анализа документов" THEN 
        RUN Insert_TTName("Ben_fio", person.name-last + " " + person.first-names ).       
        WHEN "Из всех источников" OR 
        WHEN ""   THEN 
            do:  
                RUN Insert_TTName("Ben_fio_new", person.name-last + " " + person.first-names ).       
                RUN Insert_TTName("Ben_fio", person.name-last + " " + person.first-names ).                      
...
/* старое
 ...
 If mSignsVal ='' then mSignsVal = GetXAttrValueEx("_user", STRING(USERID("bisquit")), "Отделение", "").       
            ...
            Do: 
            mSignsV = 'Мирхасанова Т.С.'.
            mStatus = "Главный операционист".
            end.
            else
            ...
            Do:
                ...
                If (end-date GE date("11/07/2017"))  and  (end-date LE date("31/07/2017"))  THEN
                   do:
                    mSignsV = "Нестеркина Л.А.".
                    mStatus = "Исполнительный директор/Директор Департамента ".                       
                    end.               
                  else
                    do:
                    mSignsV = "Нестеркина Л.А.".
                    mStatus = "Заместитель директора департамента поддержки бизнеса".                       
                    end. 
            end.                             
            else 
...
ELSE IF (iParms = 'ankfiz|') THEN
    ...
    ELSE         
       do:
    If end-date LE date("15/02/2018") THEN 
...
do:   
    ...
    DO:              
        ...
        If (end-date GE date("29/11/2016")) and  (end-date LT date("15/02/2018"))  THEN 
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "1" + SUBSTRING ( iParms, LENGTH (iParms), 1) .
        
        If (end-date GE date("16/02/2018")) THEN 
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "2" + SUBSTRING ( iParms, LENGTH (iParms), 1) .   
    ...
    IF CAN-DO("*ankip*", iParms) THEN
        ...
        iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "7" + SUBSTRING ( iParms, LENGTH (iParms), 1) . 
...
PROCEDURE benef:   
    ...
    InfSignsV2 = GetCodeName("КодДокум", person.document-id) .              
    RUN Insert_TTName("documtype", InfSignsV2).                        /*тип документа*/                
    ...
    RUN RetAdr.p(pers_id, "Ч", "АдрФакт", ?, OUTPUT mAdrFact).
...
PROCEDURE signat:   
    ...
    DEFINE INPUT PARAMETER vUser       AS CHARACTER NO-UNDO.
    
    DEFINE VARIABLE mSignsV                  AS CHARACTER NO-UNDO INIT "Филиал неопределен".
    DEFINE VARIABLE mStatus                  AS CHARACTER NO-UNDO INIT "Филиал неопределен".
    DEFINE VARIABLE mSignsVal                AS CHARACTER NO-UNDO.
    DEFINE VARIABLE mEnd-date                AS date NO-UNDO.
  
     
    mSignsVal = trim(GetXAttrValueEx(vTable,vTable-id, "branch-id","")).   
    mEnd-date = end-date. 
      
    IF   CAN-DO("04*,0000,0101,0102,0106,0109,0110,0111", mSignsVal) THEN mSignsVal = "0000".
    IF   CAN-DO("05*", mSignsVal) THEN mSignsVal = "0500".
    IF   CAN-DO("serv*,sync", mUser) THEN 
    do:  
        mSignsVal = "0000".        
        mEnd-date = date('02/08/2017'). 
    end.    
   FOR EACH code NO-LOCK
        WHERE code.class eq  "Подписант_ДБС" 
        AND code.val   eq   mSignsVal 
        and date(CODE.misc[1]) LE mEnd-date
        and (date(CODE.misc[2]) GE mEnd-date OR date(CODE.misc[2]) EQ ?)
        :                                  
        mSignsV = code.name.
        mStatus = code.misc[3].
    end.     
