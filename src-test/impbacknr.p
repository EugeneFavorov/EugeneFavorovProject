/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: 
      Comment: Обратный выкуп по реестру "Норвик"
   Parameters:
         Uses:
      Used by:
      Created: kam
...
ffname = '/data/home2/bis/quit41d/imp-exp/norvik/in/1.csv'.
...
REPEAT ON ENDKEY UNDO, LEAVE:
	...
	ost40817 = 0.
		...
		sName = ENTRY(3,fstr,';').
		sSumQ = ConvertDecimal(ENTRY(12,fstr,';')). /* ОД         = L */
		sSumR = ConvertDecimal(ENTRY(13,fstr,';')). /* Проср.ОД   = M */ 
		sSumS = ConvertDecimal(ENTRY(14,fstr,';')). /* Начисл%    = N */
		sSumT = ConvertDecimal(ENTRY(15,fstr,';')). /* Просроч%   = O */
		sSumU = ConvertDecimal(ENTRY(16,fstr,';')). /* Просроч% на просрОД = P */
		sSumAA = ConvertDecimal(ENTRY(17,fstr,';')). /* Сумма передаваемых прав требований = T */
				...
				IF AVAIL loan-acct THEN DO:
					IF sSumQ > 0 THEN DO:
						...
						ASSIGN
							...
							ttOp.in-acctcr = '47422810400000000888'
							ttOp.in-loanop = '4'
