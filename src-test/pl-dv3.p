...
DEF VAR vTR AS CHARACTER NO-UNDO.
DEF VAR mZNP AS CHARACTER NO-UNDO.
...
DO:
  ...
  mZNP = GetXattrValueEx("loan",
                    loan.contract + "," + loan.cont-code,
                        "ДатаНачПереч",
                        ?).
            IF mZNP = ? OR mZNP = "?"
            THEN DO:
            oMess = "Отстутствует заявление на периодическое перечисление.".
            RETURN.
            END.
   /* Ищем счет вклада */
   ...
   DO:
      ...
      IF bop-entry.acct-db NE vTR
      THEN DO:  
         oMess =  " C " + STRING(mLim-date) + " пополнение только со счета, указанного в пункте 4.4 договора в размере не более процентов, начисленных за предыдущий месяц.".
         RETURN.  
      END.
   ...
   ELSE DO:
      oMess =  "1C " + STRING(mLim-date) + " пополнение только со счета, указанного в пункте 4.4 договора в размере не более процентов, начисленных за предыдущий месяц.".
      RETURN.
   END.
   END.
