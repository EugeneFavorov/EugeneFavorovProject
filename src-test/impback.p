...
FUNCTION ConvertDecimal RETURN DECIMAL (iStr AS CHAR):
   iStr = replace(iStr,' ','').  /* ***kam */
   iStr = replace(iStr,',','.').
/*   iStr = replace(iStr,' ','').
   iStr = replace(iStr,',',''). */  /* ***ccc */
...
ffname = '/data/home2/bis/quit41d/imp-exp/soyuz/in/1.csv'.
...
REPEAT ON ENDKEY UNDO, LEAVE:
	...
	iInt = iInt + 1.
	ost40817 = 0.
		sCode = TRIM(ENTRY(4,fstr,';')).
		sName = ENTRY(3,fstr,';').
		sSumQ = ConvertDecimal(ENTRY(14,fstr,';')). /* ОД         = L */
		sSumR = ConvertDecimal(ENTRY(15,fstr,';')). /* Проср.ОД   = M */ 
		sSumS = ConvertDecimal(ENTRY(16,fstr,';')). /* Начисл%    = N */
		sSumT = ConvertDecimal(ENTRY(17,fstr,';')). /* Просроч%   = O */
		sSumU = ConvertDecimal(ENTRY(18,fstr,';')). /* Просроч% на просрОД = P */
		sSumAA = ConvertDecimal(ENTRY(22,fstr,';')). /* Сумма передаваемых прав требований = T */
      strDateProd = TRIM(ENTRY(6,fstr,';')). /* Дата продажи договора */
	
/*      MESSAGE sSumQ ";" sSumR ";"sSumS ";"sSumT ";"sSumU ";"sSumAA ";"
      VIEW-AS ALERT-BOX.
*/      
