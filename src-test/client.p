...
DEFINE VARIABLE tclient        AS CHARACTER NO-UNDO. 
...
ON RETURN OF tclient IN FRAME fMain 
    DO:
        ...
        if tclient:SCREEN-VALUE eq "ИП" then 
        tclient = "Ч".
        else tclient = "Ю".  
        APPLY "TAB" TO SELF.
    END.
...
IF tclient = 'Ю' THEN   
    ...
    FOR EACH b-cust-corp   WHERE b-cust-corp.date-out = ?  NO-LOCK,   
        FIRST bacct WHERE  b-cust-corp.cust-id EQ bacct.cust-id
        AND    bacct.cust-cat EQ 'Ю' 
        AND    bacct.contract EQ 'Расчет'
        AND    bacct.close-date = ?   
        AND    CAN-DO(mBranch,bacct.filial-id)
        NO-LOCK : 
        tel = "".
        if CAN-DO(groupOABS,GetXAttrValueEx("cust-corp",string(b-cust-corp.cust-id),"groupOABS",""))  then
        do:           
            
            CREATE tt-inf.
            ASSIGN               
                tt-inf.mINN = b-cust-corp.inn
                tt-inf.mID  = b-cust-corp.cust-id.                                                                                
            RUN ChangeSettings("cust-corp",string(b-cust-corp.cust-id),b-cust-corp.name-short,"Ю").     
        end.
         FIND FIRST code WHERE 
             code.class          = 'StopList'
        and  code.parent         = 'StopList'
        and  code.name           = 'OSK'
        and  code.misc[2]       EQ string(tt-inf.mID) 
        and  code.misc[1]       EQ tclient 
        NO-LOCK NO-ERROR.
...
ELSE 
  /* tclient = 'Ч'.*/
   FOR EACH b-person WHERE b-person.date-out = ? NO-LOCK,
        ...
        do:
              tel = "".  
            tel = IF b-person.phone[1] NE "," THEN 
