/*
    ...
    Copyright: (C) 1992-2010 ЗАО "Банковские информационные системы"
     Filename: dog_bk.p
      Comment: Формирование договора оказания услуг электронного банкинга в системе <iBank 2> для юр.лиц
   Parameters:  
      Created: ayv
*/
...
NO-LOCK:
	...
	END.
	ELSE DO:
		pblank = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"ДолРук").
		nblank = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"ФИОрук").
		mTempC = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"основа").
		IF mTempC = "Устав" THEN mTempC = mTempC + "а".
		oblank = mTempC.
	END.
...
DEFINE FRAME frame-set
   ...
   eblank			  LABEL			"Количество групп ЭП    "SKIP
   pblank 			  LABEL			"Должность представителя"SKIP
   nblank			  LABEL			"ФИО представителя      "SKIP
...
END.
	...
	IF pblank = "" THEN	mTempC = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"ДолРук").
	ELSE mTempC = pblank.
	RUN Insert_TTName ("CliPost",mTempC).
	RUN Insert_TTName ("CliPostViza",mTempC).
	kb.dol = mTempC.
	
	mTempC = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"ОГРН").
	RUN Insert_TTName ("OGRN",mTempC).
	
	mTempC = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"КПП").
	...
	IF nblank = "" THEN 
		mTempC = GetXAttrValue("cust-corp",string(cust-corp.cust-id),"ФИОрук").
	ELSE
		mTempC = nblank.
	...
	mTempC = "".
	RUN RetAdr.p(cust-corp.cust-id,"Ю","АдрЮр",?,OUTPUT mTempC).
    RUN Insert_TTName("AddReg",mTempC). 
	
	RUN RetAdr.p(cust-corp.cust-id,"Ю","АдрФакт",?,OUTPUT mTempC).
	...
	OUTPUT STREAM ws TO VALUE(fname) 
		...
		PUT STREAM ws UNFORMATTED
			...
			"Должность представителя      "+ delim + kb.dol  + eol +
 			"ФИО представителя            "+ delim + kb.fio  + eol +
