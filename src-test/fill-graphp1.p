   ...
   DO:
      ...
      FOR EACH tt-term-obl WHERE
         ...
         RUN CrtRepTbl(tt-term-obl.idnt,
                       IF tt-term-obl.idnt EQ 1 THEN tt-term-obl.dsc-beg-date ELSE tt-term-obl.end-date,
                       tt-term-obl.amt-rub,
                       mID).
         mID = mID + 1.
      END.
    ...
    IF iDate < DATE( 7, 1, 2014) THEN DO:
         ...
         DO:
			/* fullKasko = ttReportTable.tf_actual-payment / 2.*/
			 ...
			 if rko11_price <> 0 and countPlat = 12 then do:
				ttReportTable.tf_actual-payment = 0. /*  fullKasko / rko11_price * pred_tf_rest-debts. 01/07/14 */
				countPlat = 0.
			end.
			 else 
