...
FOR EACH tt-117 WHERE TRUE
   ...
   FIND FIRST op-entry WHERE
      op-entry.op EQ tt-117.op
   NO-LOCK NO-ERROR.
   IF AVAIL(op-entry)
   AND op-entry.acct-db EQ tt-117.c-acc THEN
   DO:
      ...
      DO:
         ...
         DO:
            ...
            AND acct.acct BEGINS "706" 
            THEN mBankINN = "5503016736".
            IF (acct.cust-cat EQ "Ю" OR acct.cust-cat EQ "Ч")
            AND acct.acct BEGINS "40" THEN
            DO:
               ...
               DO:
                  ...
                  NO-LOCK NO-ERROR.
                  IF AVAIL(cust-corp) THEN mBankINN = cust-corp.inn.
               END.
               ...
               DO:
                  ...
                  NO-LOCK NO-ERROR.
                  IF AVAIL(person) THEN mBankINN = person.inn.
               END.
            END.
            RUN dbgprint.p(PROGRAM-NAME(1) + " line = {&line-number}",
               "~nmBankINN = " + mBankINN).
      ...
      END.
   END.  
   ELSE IF AVAIL(op-entry)
   AND op-entry.acct-cr EQ tt-117.c-acc THEN
   DO:
      RUN dbgprint.p(PROGRAM-NAME(1) + " line = {&line-number}",
         "~nop-entry.acct-cr = " + op-entry.acct-cr).
...
/*   	'    <Cell ss:StyleID="s71"><Data ss:Type="String">' + (IF tt-117.pcode EQ "" THEN "643" ELSE tt-117.pcode) + '</Data></Cell>~n' +*/
   ...
   PUT UNFORMATTED
   	...
   	'    <Cell ss:StyleID="s71"><Data ss:Type="String">' + tt-117.agent + '</Data></Cell>~n' +
   	'    <Cell ss:StyleID="s71"><Data ss:Type="String">' + tt-117.cagent + '</Data></Cell>~n' +
   	'    <Cell ss:StyleID="s71"><Data ss:Type="String">' + tt-117.r-PSd + '</Data></Cell>~n' +
   	'    <Cell ss:StyleID="s71"><Data ss:Type="String">' + tt-117.prizn + '</Data></Cell>~n' +
