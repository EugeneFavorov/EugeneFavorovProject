...
DEFINE TEMP-TABLE otch
	...
	FIELD ost913121    AS DECIMAL              /* остаток 91312 */
        ...
        FIELD ost45515     AS DECIMAL              /* остаток КредРез (45515)     */
        FIELD ost45818     AS DECIMAL              /* остаток КредРез1 (45818)    */
        FIELD ost60324     AS DECIMAL              /* остаток КредРезГПош (60324) */
        FIELD ost47425     AS DECIMAL              /* остаток КредРезП (47425)    */
        FIELD ost45918     AS DECIMAL              /* остаток КредРезПр (45918)   */
        FIELD ostKredGar   AS DECIMAL              /* остаток КредГар */
        FIELD ostKredGar1  AS DECIMAL              /* остаток КредГар1 */        
        FIELD ostKredBudKom  AS DECIMAL            /* остаток КредБудКом */        
        FIELD ostKredBudRko  AS DECIMAL            /* остаток КредБудРко */  
        FIELD ostKredTov   AS DECIMAL      
        FIELD raterez      AS DECIMAL              /* ставка Рез%*/
        FIELD ostKredPrOv  AS DECIMAL            /* остаток КредПр%ОВ */
        FIELD ostKredShtOv  AS DECIMAL            /* остаток КредШт%ОВ */  
        FIELD ostKredN     AS DECIMAL            /* остаток КредН */  
        FIELD pos          AS CHAR                 /* ПОС */  
        FIELD block        AS CHARACTER            
        FIELD sumblock     AS DECIMAL
        FIELD peni         AS DECIMAL
        FIELD datenewzal   AS DATE /* дата нового залога */
        FIELD closedate    AS DATE /* дата закрытия КД */
	FIELD sumcred      AS DECIMAL     /* Сумма кредита */
	FIELD datelast	   AS DATE       /* Дата окончания КД */
	FIELD pr_problem   AS CHAR       /* pr_problem */
...
def var ost45515 as decimal no-undo.
def var ost45818 as decimal no-undo.
def var ost60324 as decimal no-undo.
def var ost47425 as decimal no-undo.
def var ost45918 as decimal no-undo.
def var ostKredTov as decimal no-undo.
def var raterez  as decimal no-undo.
def var pos     as char no-undo.
def var msumblock     as decimal no-undo.
def var mblock     as char no-undo.
def var ostKredGar as decimal no-undo.
def var ostKredGar1 as decimal no-undo.
def var ostKredBudKom  AS DECIMAL no-undo.            /* остаток КредБудКом */        
def var ostKredBudRko  AS DECIMAL no-undo.           /* остаток КредБудРко */  
def var ostKredN AS DECIMAL no-undo.
def var peni as DECIMAL no-undo.     
def var datenewzal as date no-undo. /* дата нового залога */
def var tmpInt as int no-undo.
def var sumcred as decimal no-undo.
def var datelast as date no-undo.
...
FOR EACH tmprecid NO-LOCK,
    ...
    FIND LAST comm-rate WHERE comm-rate.kau = Loan.contract + "," + Loan.cont-code
        ...
        AND comm-rate.since <= end-date NO-LOCK NO-ERROR.
        IF AVAIL comm-rate THEN do:
            peni  = comm-rate.rate-comm.
        END.
	...
	AND term-obl.end-date <> ?
    	NO-LOCK
        BY term-obl.end-date desc:
	 datelast = term-obl.end-date.
         LEAVE.
    END.
	...
	IF AVAIL loan-acct THEN DO:
		...
		ELSE ost40817 = abs(sh-val).
                msumblock = ABSOLUTE(GetBlockPositionAll(loan-acct.acct,loan-acct.currency,end-date)).
                mblock    = BlockAcct(loan-acct.acct + "," + loan-acct.currency ,DATETIME(end-date + 1) - 1).
	END.
	...
	IF AVAIL loan-acct THEN DO:
		...
		IF loan-acct.currency = '' THEN
			ost45515 = abs(sh-bal).
		ELSE ost45515 = abs(sh-val).
	END.
        /* остаток КредРез (45818)     */
	FIND LAST loan-acct OF loan WHERE 
		loan-acct.since <= end-date 
	...
	IF AVAIL loan-acct THEN DO:
		...
		IF loan-acct.currency = '' THEN
			ost45818 = abs(sh-bal).
		ELSE ost45818 = abs(sh-val).
	END.

        /* остаток КредРезГПош (60324) */
	FIND LAST loan-acct OF loan WHERE 
		loan-acct.since <= end-date 
		AND loan-acct.acct-type = 'КредРезГПош'
	NO-LOCK NO-ERROR.
	IF AVAIL loan-acct THEN DO:
		RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
		IF loan-acct.currency = '' THEN
			ost60324  = abs(sh-bal).
		ELSE ost60324  = abs(sh-val).
	END.
        /* остаток КредРезП (47425)    */
	FIND LAST loan-acct OF loan WHERE 
		loan-acct.since <= end-date 
		AND loan-acct.acct-type = 'КредРезП'
	NO-LOCK NO-ERROR.
	IF AVAIL loan-acct THEN DO:
		RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
		IF loan-acct.currency = '' THEN
			ost47425  = abs(sh-bal).
		ELSE ost47425  = abs(sh-val).
	END.
        /* остаток КредРезПр (45918)   */
	FIND LAST loan-acct OF loan WHERE 
		loan-acct.since <= end-date 
		AND loan-acct.acct-type = 'КредРезПр'
	NO-LOCK NO-ERROR.
	IF AVAIL loan-acct THEN DO:
		RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
		IF loan-acct.currency = '' THEN
			ost45918  = abs(sh-bal).
		ELSE ost45918  = abs(sh-val).
	END.

        pos = LnInBagOnDate (loan.contract, loan.cont-code, end-date).

        raterez =  LnRsrvRate(loan.contract, loan.cont-code, end-date).

    ostKredGar = 0.
        /* остаток КредГар   */
    FIND LAST loan-acct OF loan WHERE 
        loan-acct.since <= end-date 
        AND loan-acct.acct-type = 'КредГар'
    NO-LOCK NO-ERROR.
    IF AVAIL loan-acct THEN DO:
        RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
        IF loan-acct.currency = '' THEN
            ostKredGar  = abs(sh-bal).
        ELSE ostKredGar  = abs(sh-val).
    END.
    ...
    FIND LAST loan-acct OF loan WHERE 
        loan-acct.since <= end-date 
        AND loan-acct.acct-type = 'КредГар1'
    NO-LOCK NO-ERROR.
    IF AVAIL loan-acct THEN DO:
        RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
        IF loan-acct.currency = '' THEN
            ostKredGar1  = abs(sh-bal).
        ELSE ostKredGar1  = abs(sh-val).
    END.
	
        ostKredBudKom = 0.
        /* остаток ostKredBudKom   */
    FIND LAST loan-acct OF loan WHERE 
        loan-acct.since <= end-date 
        AND loan-acct.acct-type = 'КредБудКом'
    NO-LOCK NO-ERROR.
    IF AVAIL loan-acct THEN DO:
        RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
        IF loan-acct.currency = '' THEN
            ostKredBudKom  = abs(sh-bal).
        ELSE ostKredBudKom  = abs(sh-val).
    END.
        ostKredBudRko = 0.
        /* остаток ostKredBudRko   */
    FIND LAST loan-acct OF loan WHERE 
        loan-acct.since <= end-date 
        AND loan-acct.acct-type = 'КредБудРко'
    NO-LOCK NO-ERROR.
    IF AVAIL loan-acct THEN DO:
        RUN acct-pos IN h_base (loan-acct.acct, loan-acct.currency, end-date, end-date, ?).
        IF loan-acct.currency = '' THEN
            ostKredBudRko = abs(sh-bal).
        ELSE ostKredBudRko = abs(sh-val).
    END.
...
FOR EACH term-obl WHERE
   ...
   BY term-obl.fop-date:
       ...
       IF AVAIL signs THEN DO:
           tmpInt = tmpInt + 1.
           datenewzal = term-obl.fop-date.
       END.
END.       
   IF tmpInt < 2 THEN datenewzal = ?.
	...
	ASSIGN
		...
		otch.ost60323	= ost60323
                otch.ost45515   = ost45515  /* остаток КредРез (45515)     */
                otch.ost45818   = ost45818  /* остаток КредРез1 (45818)    */
                otch.ost60324   = ost60324  /* остаток КредРезГПош (60324) */
                otch.ost47425   = ost47425  /* остаток КредРезП (47425)    */
                otch.ost45918   = ost45918  /* остаток КредРезПр (45918)   */
                otch.raterez    = IF raterez EQ ? THEN 0 ELSE raterez  /* ставка Рез%*/
                otch.pos        = IF pos EQ ? THEN "" ELSE ENTRY(1,pos,"@")       /* ПОС */  
                otch.block      = mblock
                otch.sumblock   = msumblock
                otch.ostKredGar = ostKredGar
                otch.ostKredGar1 = ostKredGar1
                otch.ostKredBudKom = ostKredBudKom
                otch.ostKredBudRko = ostKredBudRko
                otch.ostKredPrOv = ostKredPrOv
                otch.ostKredShtOv = ostKredShtOv
                otch.ostKredN = ostKredN
                otch.peni = peni
                otch.datenewzal = datenewzal
                otch.closedate = loan.close-date
                otch.sumcred = sumcred
                otch.datelast = datelast
		otch.pr_problem        = GetXattrValueEx("loan",loan.contract + "," + loan.cont-code,"pr_problem","")
		.
 ...
 <Worksheet ss:Name="lll">
  <Table >
   ...
   <Row ss:Index="2" ss:AutoFitHeight="0" ss:Height="56.25">
    ...
    <Cell ss:StyleID="s66"><Data ss:Type="String">Сумма кредита</Data></Cell>
	...
	<Cell ss:StyleID="s66"><Data ss:Type="String">Блокировка</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">Сумма блокировки</Data></Cell>
	...
	<Cell ss:StyleID="s66"><Data ss:Type="String">Остаток на 458</Data></Cell>
    ...
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредРез (45515)</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредРез1 (45818)</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредРезГПош (60324)</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредРезП (47425)</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредРезПр (45918)</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредГар</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредГар1</Data></Cell>    
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредБудКом</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредБудРко</Data></Cell>    
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредПр%ОВ</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредШт%ОВ</Data></Cell> 
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредТОВ</Data></Cell>     
    <Cell ss:StyleID="s66"><Data ss:Type="String">остаток КредН</Data></Cell>     
    <Cell ss:StyleID="s66"><Data ss:Type="String">%Рез</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">ПОС</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">Ставка Пени</Data></Cell> 
    <Cell ss:StyleID="s67"><Data ss:Type="String">Замена залога</Data></Cell>
    <Cell ss:StyleID="s67"><Data ss:Type="String">Дата закрытия КД</Data></Cell>
    <Cell ss:StyleID="s67"><Data ss:Type="String">Дата окончания КД</Data></Cell>
    <Cell ss:StyleID="s66"><Data ss:Type="String">Pr_Problem</Data></Cell>
   </Row>\n'.
...
FOR EACH otch NO-LOCK BY otch.cont_code BY cont_code:
    ...
    PUT STREAM vvs UNFORMATTED  '<Cell><Data ss:Type="Number">'.
    PUT STREAM vvs UNFORMATTED STRING(otch.ostKredN) + '</Data></Cell>\n'.
  ...
  PUT STREAM vvs UNFORMATTED '</Data></Cell>\n'.
    ...
    PUT STREAM vvs UNFORMATTED  '<Cell><Data ss:Type="String">'.
    PUT STREAM vvs UNFORMATTED otch.pr_problem + '</Data></Cell>\n'.  
