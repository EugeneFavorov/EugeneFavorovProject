...
FIRST branch WHERE RECID(branch) = tmprecid.id NO-LOCK,
 EACH bop-entry
	...
	FIRST bop
	  ...
	  AND bop.branch-id eq branch.branch-id 
       ...
       ELSE
          ...
          If bop.doc-type eq '037п' THEN
             do:
                 ...
                 user_inspector = bop.user-id.
             end.
           else  
            do:
                 user_id = bop.user-id.  /*GetUserName()*/
...
*/                        
   ...
   /* Окончательные итоги */
   RUN Insert_TTName IN h_prnvd ("tamt[doc]",/*formatAmt*/REPLACE(STRING(vAmt,">>>,>>>,>>>,>>.99"), ".", "-")).
