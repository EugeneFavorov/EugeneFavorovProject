...
DEFINE VARIABLE l        AS INT64              NO-UNDO. 
...
IF NOT CAN-DO('zay_dkbo,zay_kb|,ZayvZakrBV|,dbs*,vypcard*', iParms) THEN
DO:
...
mSignsV= GetTempXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "РискОтмыв", end-date ,"").               
...
do:      
    mSignsVal = GetTempXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "ОценкаРиска", end-date ,"").   
    ...
    do:
      mSignsV = ''.         
        /*IF  mSignsVal = "" THEN 
              mSignsV = "Отсутствуют признаки легализации". 
          ELSE*/
        /*  if NUM-ENTRIES(mSignsVal,";") eq 1 then  mSignsV = mSignsVal.
        else*/
...
/*
	   RUN Insert_TTName("DP", IF (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "date_inspect",""))="" THEN "нет" ELSE          
        ...
        FOR EACH  code WHERE code.class = "Должности"
                          AND code.code  = mSignsVal        NO-LOCK:
            mSignsVal = code.val.
        end.
...
ELSE mSignsV = 'Бенефиц_Влад'.
 	...
 	FOR EACH cust-role WHERE cust-role.file-name EQ "cust-corp"
       ...
       FIND FIRST person WHERE
         ...
         mSignVL = GetXAttrValueEx("cust-role",STRING(cust-role.cust-role-id),"ОсновРешен","").
         if mSignVL = "" then mSignVL = "Прямое владение - наличие преобладающего участия более 25 процентов в капитале организации".
         else
         do:
             FIND FIRST code WHERE 
                 code.class EQ "БенефВлОсРеш"
                 AND code.code   EQ mSignVL    NO-LOCK NO-ERROR.
             IF AVAIL code THEN
                 mSignVL  = code.name.
         end.
         /*RUN Insert_TTName("BenFIO0" + string(i), person.name-last + " " + person.first-names ).               
         RUN Insert_TTName("BenifOsn0" + string(i),  mSignVL ).    
        */               
            
          f = GetXAttrValueEx("cust-role",STRING(cust-role.cust-role-id),"soursce_inf","").  
         ...
         CASE f:
             ...
             WHEN "Самостоятельно из анализа документов" THEN 
                 do:                    
                     RUN Insert_TTName("BenFIO1" + string(j), person.name-last + " " + person.first-names ).               
                     RUN Insert_TTName("BenifOsn1" + string(j),  mSignVL ).   
                     j = j + 1.
                 end.
             WHEN "Из всех источников" OR WHEN ""   THEN 
                 do:  
                     RUN Insert_TTName("BenFIO0" + string(i), person.name-last + " " + person.first-names ).               
                     RUN Insert_TTName("BenifOsn0" + string(i),  mSignVL ).               
                     RUN Insert_TTName("BenFIO1" + string(j), person.name-last + " " + person.first-names ).               
         ...
         END CASE.
          ...
          mSignsV = GetXAttrValueEx("cust-role",STRING(cust-role.cust-role-id),"ДоляУК",""). 
           if mSignsV eq "" then  mSignsV = mSignVL.
        
           RUN Insert_TTName("BenCont" + string(l), mSignsV ).
            ...
            FIND country WHERE country.country-id EQ person.country-id NO-LOCK NO-ERROR.
                ...
                InfSignsV3 = InfSignsV3 + ", " + mSignsV.   
                       ...
                       RUN Insert_TTName("BenFIO" + string(l), InfSignsV).                  
                       RUN Insert_TTName("BenDoc" + string(l), InfSignsV2).  
                       RUN Insert_TTName("BenAdr" + string(l), InfSignsV3). 
                  /*     RUN Insert_TTName("BenCont" + string(l), "Прямое владение - наличие преобладающего участия более 25 процентов в капитале организации").
*/
     /*  i = i + 1.
       i = j + 1.*/
        l = l + 1.
...
mStrTMP = ENTRY(1,mStrTMP1,',').
RUN Insert_TTName("okved1",  mStrTMP + ' ' + GetCodeName("ОКВЭД2", mStrTMP)).

	 /* Находим номер лицензии GetCodeName("ОКВЭД2", mStrTMP) */
      ...
      /*     IF   CAN-DO("0300", mSignsVal) THEN     
            ...
            Do:
                if end-date LE date("10/07/2017") then
                  ...
                  If (end-date GE date("11/07/2017"))  and  (end-date LE date("30/07/2017"))  THEN
                   do:
...
IF   CAN-DO("0100,05*", mSignsVal) THEN mSignsVal = "0500".
...
FOR EACH code NO-LOCK
    ...
    and date(CODE.misc[1]) LE end-date
    and (date(CODE.misc[2]) GE end-date OR date(CODE.misc[2]) EQ ?)
    :                              
    mSignsV = code.name.
    mStatus = code.misc[3].
end.  
