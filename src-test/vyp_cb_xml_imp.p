...
DEFINE TEMP-TABLE  tt-inf
   ...
   FIELD date-beg  AS CHARACTER.

INPUT FROM VALUE("406-2.csv").
REPEAT:
   ...
   DO:
      CREATE tt-inf.
      ASSIGN
         tt-inf.fio    = TRIM(ENTRY(1,mString,";"))
         tt-inf.number = TRIM(ENTRY(2,mString,";")).
   END.
END.
INPUT CLOSE.

FOR EACH tt-inf NO-LOCK:
   
FOR EACH loan WHERE TRUE
   AND loan.contract  EQ "Кредит"
   AND loan.cont-code BEGINS tt-inf.number
   AND loan.filial-id EQ "0000"  
   NO-LOCK,
   EACH loan-acct WHERE TRUE
   AND  loan-acct.contract  EQ loan.contract
   AND  loan-acct.cont-code EQ loan.cont-code
   NO-LOCK,
   EACH acct WHERE TRUE
   AND  acct.acct     EQ loan-acct.acct
...
/*beg-date = acct.open-date.*/
beg-date = DATE("01/05/2017").
end-date = DATE("31/05/2017").

fname = "./" + loan.filial-id + "_" + tt-inf.number + "_" + delfilfromacct(acct.acct) + "_" + STRING(beg-date, '99999999') + "_" +  STRING(end-date, '99999999') + ".xml".
