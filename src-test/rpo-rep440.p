...
vNum = 0.
mInt = 0.
RUN dbgprint.p ("rpo-rep440 ","Begin " + STRING(TIME,"hh:mm:ss")).

FOR EACH Packet WHERE TRUE
   AND CAN-DO ("PTax440",TRIM(Packet.Class-Code))
   /*AND Packet.PacketID EQ 17837382*/  /*ZNO*/
   /*AND Packet.PacketID EQ 35332330*/
   /*AND Packet.PacketID  EQ 36836397*/  /*RPO*/
...
/*   RUN dbgprint.p ("rpo-rep440 ","Packet.PacketID = " + STRING(Packet.PacketID) + "~nvNum = " + STRING(vNum)).*/
   ...
   IF mTypeInf EQ 'êÖòçéèêàéë' OR SUBSTRING(mFileNameRPO,1,3) EQ "RPO" THEN
   DO:
      ...
      FOR EACH PackObject
         ...
         DO:
            RUN dbgprint.p ("rpo-rep440 " + " {&line-number}","mFileName = " + mFileName).
      ...
      RUN dbgprint.p ("rpo-rep440 ","Packet.PacketID = " + STRING(Packet.PacketID) + 
      ...
      DO:
         ...
         ELSE rowStyleStateBOS = "s78".
	      /*mFileNameBOS = GetXAttrValueEx("Packet",STRING(xPacketBVS.PacketID),"FileName","").         /*!!!*/*/
      ...
      DO:
         ...
         FOR EACH cust-corp WHERE TRUE
            ...
            RUN dbgprint.p ("rpo-rep440","~nacct.acct = " + acct.acct).
         ...
         RUN dbgprint.p ("rpo-rep440","~nmCnt1 = " + STRING(mCnt1) + " mCnt2 = " + STRING(mCnt2)).
         
         IF mCnt1 EQ 0 AND mCnt2 EQ 0 THEN
         DO mCnt3 = 1 TO NUM-ENTRIES(zAcct):
            FIND FIRST acct WHERE TRUE
               AND acct.filial-id EQ shFilial
               AND acct.number = ENTRY(mCnt3,zAcct)
            NO-LOCK NO-ERROR.
/*            IF AVAIL(acct) THEN*/
/*            DO:                */
               IF mCnt3 EQ 1 THEN
      ...
      DO:
         PUT UNFORMATTED
            '   <Row ss:AutoFitHeight="1">~n' +
         	...
         	'    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mStateRPO + '</Data></Cell>~n' +
            ...
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mStateBOS + '</Data></Cell>~n' +       
         	'    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String"></Data></Cell>~n' +
         	'    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mGroup      + '</Data></Cell>~n' + 
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="Number">' + TRIM(STRING( - mOstFormLocal,"->>>,>>>,>>>,>>9.99")) + '</Data></Cell>~n' + 
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="Number">' + TRIM(STRING(DEC(mOstFile),"->>>,>>>,>>>,>>9.99"))    + '</Data></Cell>~n' + 
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mResultPB1    + '</Data></Cell>~n' +
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mResultKWTPB1 + '</Data></Cell>~n' +
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mResultKWTBVS + '</Data></Cell>~n' + 
            '    <Cell ss:StyleID="' + rowStyle77 + '"><Data ss:Type="String">' + mResultPB2 + '</Data></Cell>~n' +
            '   </Row>'
         SKIP.
      END.
...
/*{preview2.i &filename="rpo-rep440.xml" }*/
