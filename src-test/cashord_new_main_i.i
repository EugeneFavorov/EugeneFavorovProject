...
DEFINE VARIABLE mStrInn    AS CHARACTER NO-UNDO.
DEFINE VARIABLE mStrName   AS CHARACTER NO-UNDO.
DEFINE VARIABLE mStrBIK    AS CHARACTER NO-UNDO.
DEFINE VARIABLE mRecBIK    AS CHARACTER NO-UNDO.
DEFINE VARIABLE mPayBIK    AS CHARACTER NO-UNDO.
...
IF mchOrdTypeDb = "приходный" THEN DO:
   ...
   &IF DEFINED(LAW_318p) <> 0 &THEN
      ...
      mRecBIK        = FGetSetting("БанкМФО", ?, "").
      
       MESSAGE "1" mRecBIK   VIEW-AS ALERT-BOX ERROR.
            ...
            IF AVAILABLE branch THEN
               ASSIGN
                  ...
                  mRecKPP        = GetXAttrValueEx("branch",
                                                   ...
                                                   "КПП",
                                                   "")
                  mRecBIK        = GetXAttrValueEx("branch",
                                                   Surrogate(BUFFER branch:HANDLE),
                                                   "БанкМФО",
                                                   "")    
                      .
                      
                       MESSAGE "2"  mRecBIK   VIEW-AS ALERT-BOX ERROR.
            ...
            ELSE DO:
               &IF DEFINED(LAW_318p) <> 0 &THEN
                  ...
                  AND {assigned mIdCustAttr} THEN DO:
                     ...
                     mRecBIK = GetXAttrValueEx(mCustTable1, mIdCustAttr, "БанкМФО", "").
                     ...
                     mRecAcct2 = LocalGetCrAcct2(op.op, AcctDb, AcctCr, AcctKomis).
                      ...
                      MESSAGE "3"  mRecBIK   VIEW-AS ALERT-BOX ERROR.
                  END.
                  ELSE ASSIGN
                     mRecKPP   = ""
                     mRecOKATO = ""
              ...
              /*DO:
               ...
               DO:
                ...
                /*   FIND FIRST banks-code WHERE
                       ...
                       AND 
                       banks-code.bank-code      EQ mchBankBIK
                       NO-LOCK.
                   FIND tbanks OF banks-code
                       NO-LOCK NO-ERROR.    
                                   
                   IF AVAIL tbanks THEN   
                       mStrName =     tbanks.name. */     
                  ...
                  &ENDIF                 
                   ...
                   If Can-do("045209884,044525129,047106641",mchBankBIK) then 
                       mRecInn = mStrInn.                     
                    /* mchReceiver[1] = mStrName.*/
               END.
   ...
   &ENDIF 
     ...
     /*mStrBIK = mchBankBIK. 
     mPayBankBik = '047106641'.     
     */   
        
     MESSAGE AcctCr "1" SUBSTR(AcctCr,27,4) "1"  AcctDb  VIEW-AS ALERT-BOX ERROR.
     ...
     If SUBSTR(AcctCr,27,4) eq '0000' then 
     Do:
         mPayBankBik = '045209884'.
         mchBankBIK  = '044525129'.
     end. 
     If SUBSTR(AcctCr,27,4) eq '0300' then 
     Do:
         mPayBankBik = '044525129'.
         mchBankBIK  = '047106641'.
     end.    
    
     
     /*FIND FIRST branch  WHERE branch.name MATCHES  mPayBank[2]   NO-LOCK NO-ERROR.*/
     
    

      /*mPayBankBik =  */
     mPayBank = mchReceiver[1].
  
     mchReceiver = 'ПАО ' + '"' + 'ПЛЮС БАНК' + '"' .
     mRecInn = '5503016736'.
  
 /* if mchBankBIK eq '047106641' then
  
  
  mPayBankBik = '044525129'.
  
  
   mchReceiver = mPayBank.  */
...
&IF DEFINED(LAW_318p) <> 0 &THEN
   ...
   RUN ProcProcessFreeField(mStrPar,?,?).
      ...
      ASSIGN
         ...
         mPayBank[1]   = "345" +  mCrBranchNam + ", " + mchBankName
      .
      IF FGetSetting("НаимБанкСВКО","Наим3352У","Да") =  "Да" THEN DO:
         {find-act.i &acct=AcctCr &curr=AcctCrCur}
         IF AVAIL(acct) THEN
...
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTUSER='anba' */
/* $LINTDATE='03/05/2017 10:14:05.072+03:00' */
/*prosignkoPJDRwvqKALUZ08m/2uAg*/