/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: 
      Comment: Создаем документы перечисления по реестру "РОскап"
   Parameters:
         Uses:
      Used by:
      Created: kam
...
sname = STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") + '_01_810_PLB_ROSCAP.CQP'.
sname = STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") + '*.CQP'.
...
RUN SetSysConf IN h_base ("MaskFName",sname).

sname = '/data/home2/bis/quit41d/imp-exp/roscap/in/' + sname.
fname = sname.
...
RUN DeleteOldDataProtocol IN h_base ("MaskFName").
...
REPEAT ON ENDKEY UNDO, LEAVE:
	...
	ost40817 = 0.
	CREATE ttOp.
		sId = REPLACE(TRIM(ENTRY(1,fstr,';')),'p','').
		sDate = ConvertDate(ENTRY(2,fstr,';')).
		sSum = ConvertDecimal(ENTRY(3,fstr,';')).
		sName = TRIM(ENTRY(4,fstr,';')).
		sCode = TRIM(ENTRY(5,fstr,';')).
		
		IF sDate <> ? AND sSum > 0 THEN DO:
			sNewSum = sSum.
			FIND FIRST loan WHERE recid(loan) = INT64(sId) 
				NO-LOCK NO-ERROR.
			IF AVAIL loan THEN DO:
				FIND LAST loan-acct OF loan WHERE loan-acct.acct-type = 'КредРасч'
				NO-LOCK NO-ERROR.
...
FOR EACH ttOp WHERE ttOp.in-sNewSum > 0:
						...
						RUN ex-trans.p(INPUT 'perroscap', 
							INPUT TODAY, 
