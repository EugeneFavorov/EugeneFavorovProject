...
CASE iUserInp:
  ...
  /* отрисовываем форму */
    ...
    WHEN '0000' THEN DO:
      ASSIGN
      iUserID = '_SERV,SERV0000,SERV0400,SERVCRED,IRBIS,BIS'.  
    END.
    OTHERWISE
  ...
  WHEN 'all' THEN DO:
    ...
    IF AVAIL(code) THEN DO:
      ...
      FOR EACH links WHERE links.link-id EQ xlink.link-id
                  ...
                  /*   AND   links.beg-date LE end-date*/  /*pda 08.02.16 */
...
*/
        ...
        /* отделяем _SERV,SERV0000,SERV0400,IRBIS,BIS,QBIS для ведомости по ГБ */
        IF CAN-DO('*SERV*,IRBIS,BIS,QBIS',tStr)
          AND NOT CAN-DO('SERV0300',tStr) THEN
          tStr = 'Автоматические транзакции ПАО "Плюс Банк"'.
        ELSE
        /* отделяем SERV0300 для ТФ*/
        IF CAN-DO('SERV0300',tStr) THEN
          tStr = 'Автоматические транзакции ТФ ПАО "Плюс Банк"'.
        ELSE
          tStr = _user._user-name.
