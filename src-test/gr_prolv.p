...
RUN SetSysConf IN h_base ("AUTOTEST:autotest", "YES"). /* kam убираем вывод на экран */
...
DEF VAR NumNewAcct        AS CHARACTER                NO-UNDO.
DEF VAR DateNewAcct        AS DATE                NO-UNDO.
      ...
      DO mIi = 1 TO NUM-ENTRIES(t-type):
         ...
         DO WHILE TRUE
          ...
          DO mCii = 1 TO NUM-ENTRIES (mLoanList):
            ...
            /* kam для вкладов до востр */
            IF loan.close-date <> ? OR loan.open-date > cred THEN NEXT loan_code.
            FIND FIRST loan-acct OF loan WHERE loan-acct.acct-type = 'loan-dps-p' NO-LOCK NO-ERROR.
            IF NOT AVAIL loan-acct THEN NEXT loan_code.
            /* kam ищем допрек и счет для перечисления */

            DateNewAcct = ?.
            FIND FIRST signs WHERE signs.file-name = 'loan' AND signs.code = 'date_trans'
              AND signs.surrogate = 'dps,' + loan.cont-code NO-LOCK NO-ERROR.
            IF AVAIL signs THEN DateNewAcct = signs.date-value.
            IF DateNewAcct = ? OR DateNewAcct <> cred THEN NEXT loan_code.

                                          
              NumNewAcct =  GetXAttrValueEx("loan",loan.contract + "," + loan.cont-code,
                                        "acct_trans",?).

              IF NumNewAcct = ? THEN NEXT loan_code.

              IF NOT CAN-FIND(FIRST acct WHERE acct.acct EQ NumNewAcct) THEN
               DO:
                  CREATE err. 
                  ASSIGN err.err = "Не найден счет для перечисления, вклад: " + loan.cont-code.
                  NEXT loan_code.
               END.
/* message NumNewAcct view-as alert-box. */

/* kam добавляем библиотеку для details: О_НомДог(), О_Откр() */
/*удаление ранее загруженой библиотеки*/
              RUN peb-unl.p("dps-b2p.p").

      IF AVAIL loan THEN
      DO:
         RUN dps-b2p.p PERSISTENT SET h_dpsb2p(RECID(loan),op-kind.op-kind).
      END.
/* 
message 'work' view-as alert-box.
*/
               ...
               FOR EACH op-template OF op-kind
                  ...
                  DO:
                     ...
                     DO:
                        ...
                        .
                        /*
message 'create-op' view-as alert-box.
*/
                        /* Создание документа */
  ...
  /*         if not avail op then message 'nnot' view-as alert-box. */
                        ...
                        /*      
message    tacct-db + ' ' + tacct-cr view-as alert-box.
*/
                        IF    tacct-db EQ ? 
                           OR tacct-cr EQ ? THEN
                        DO:
...
/*
message 'create op-entry' view-as alert-box.
*/
                           CREATE op-entry.
                        ...
                        DO:
                           ...
                           RUN crkau (tacct-db,
                                     (in-contract + "," + in-cont-code + "," 
                                          + (loan-transaction.trans-code + ",ОстВклДв")),
                                      RECID(op)).         
                        END.
...
*/
            ...
            /* kam     RUN ProlParentLoan IN THIS-PROCEDURE (RECID (loan),
                                                      OUTPUT mErr).
                IF mErr = -1 THEN
                   UNDO loan_code, NEXT loan_code.
            */
      ...
      DO:   
        ...
        PUT STREAM err-ved  UNFORMATTED "ОШИБКИ ПРИ ЗАКРЫТИИ: ~n ~n". 
        FOR EACH err:
        ...
        END.
/* kam       ELSE           OUTPUT STREAM err-ved CLOSE. */
      END.
...
/* $LINTENV ='1ut' */
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTDATE='17/10/2014 13:11:03.995+04:00' */
/* $LINTFILE='gr_prol3.p' */
/*prosignaKYrBHJUlOWZ4rS8Ffhesw*/