...
DEFINE var DateClose_  AS CHAR    NO-UNDO.   /* Дата закрытия условия */
DEFINE var Active_     AS CHAR    NO-UNDO.   /* Дата закрытия условия */
...
for each loan-cond where loan-cond.contract  = loan-acct.contract
                     ...
                     no-lock.
   DateClose_  = GetXattrValueEx("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + string(loan-cond.since), "close-date",?).     
   if DateClose_ <> ? then next.
   Active_     = GetXattrValueEx("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + string(loan-cond.since), "Актив",?).     
   if Active_ <> "ДА" then next.
...
PROCEDURE MakeNachkinT01:
   ...
   DEF VAR str_            AS CHAR NO-UNDO.

   output stream err to "spooln.tmp" append.
   LOOP:
   FOR EACH nach_rep where (nach_rep.acct_rem <> ? or nach_rep.match_rem <> ?)  BREAK BY nach_rep.intrvl_beg :
      ...
      PUT STREAM err 
         ...
         nach_rep.day_p_int
         " "
         if nach_rep.acct_curr = "" then  nach_rep.acct_rem else nach_rep.match_rem format ">>,>>>,>>>,>>9.99Cr" 
         " "
         ...
         SKIP
      .
      if nach_rep.acct_curr = "" then do:
        str_ =  STRING(nach_rep.acct_rem).
      end.
      else do:
        str_ = STRING(nach_rep.match_rem). 
      end.
      ...
      str_par = STRING(nach_rep.intrvl_beg)  + "|" +                           
                STRING(nach_rep.intrvl_end)  + "|" +                           
                str_ + "|" +                         
                STRING(nach_rep.day_p_int)  + "|" +                            
                STRING(nach_rep.rate)  + "|" +                              
                STRING(nach_rep.acct_val_per)  + "|"
                .                        
      RUN SetSysConf IN h_base("str_par",str_par).
   END.
...
PROCEDURE CheckErrorRemainderDate.
    ...
    for each fost .
       fost.balance = ipMinRemainderDec.
    end.
    return.
