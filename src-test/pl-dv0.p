...
RUN end_doloan_dps in h_dpspc (iRec,
                              ...
                              OUTPUT mLim-date,
                              OUTPUT oMess).
/*MESSAGE iDn-cont-date ",1" mLim-date
VIEW-AS ALERT-BOX.*/
IF iDn-cont-date > mLim-date THEN
DO:
   ...
   DO:
      ...
      FIND LAST bop-entry WHERE bop-entry.filial-id EQ shfilial
         ...
         AND bop-entry.op-date EQ iDn-cont-date NO-LOCK NO-ERROR.
          ...
          /*FOR EACH bop-entry WHERE bop-entry.filial-id EQ shfilial
                 AND bop-entry.acct-cr EQ loan-acct.acct
                AND bop-entry.op-date EQ iDn-cont-date NO-LOCK:
 MESSAGE bop-entry.acct-db
       VIEW-AS ALERT-BOX.
                       end.*/
                            
  
      FIND FIRST acct WHERE acct.acct EQ bop-entry.acct-db NO-LOCK NO-ERROR.  
     /*MESSAGE loan.cust-id "," acct.cust-id eq ? "," acct.acct BEGINS("2")
      VIEW-AS ALERT-BOX.*/
      IF acct.acct BEGINS("2") 
       /*acct.cust-id eq ?*/
      THEN DO:  
       /*MESSAGE loan.cust-id
         VIEW-AS ALERT-BOX.*/
         oMess =  "C " + STRING(mLim-date) + "  пополнение только безналично со счета клиента." + string(loan.cust-id).
            RETURN.
            END.             
      IF  loan.cust-id NE acct.cust-id
      THEN DO:  
         oMess =  "C " + STRING(mLim-date) + " пополнение только безналично со счета клиента." + string(loan.cust-id) + string(acct.cust-id).
         RETURN.  
      END.
