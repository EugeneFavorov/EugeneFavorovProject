   ...
   vWork = IsWorkDayBranch (op.op-date, iBranch).
   if not vWork then do:
         message "По графику подразделения " iBranch  op.op-date " не рабочий день. Провести документ нельзя."   view-as alert-box.
         return .       
   end.
   bacct-rec = op-entry.acct-cr.
/*
   ...
   if avail acct then do:
     ...
     /*     
      Создать документ списания с 47416 на 421* (но непонятно, почему документ оказался на 47416 ?)
     */
     message "Счет " acct.acct " уже открыт " acct.open-date  view-as alert-box.
     return .
   end.  
*/ 
...
/*
   ...
   4. если все сложилось , то запускаем открытие счетов, пересчет графика 
      1. проверить в классификаторе СчетаРезерв*, наличие зарезервированного счёта из ДР acct-rec.
      2. открыть счёт 421*, из ДР acct-rec. 
      3. привязать счета к договору
...
/*   bSrok = GetRefVal ("СрокПродукта",
                             op.op-date,
                             trim(loan.cont-type) + "," + trim(loan.filial-id)).

   if bSrok = ? then do:
     message "Не найден Срок Продукта в классификаторе СрокПродукта  для типа договора (продукта) " trim(loan.cont-type)  view-as alert-box.
     return .
   end.
*/
...
/*
   ...
   Алгоритм следующий: На пользователе есть ДР Отделение. В нём должно быть подразделение, 
   которое обслуживает клиента, т.е. для филиала 0000, это , предположим, 0001 ОПЕРУ ЮЛ . 
   Для него должен быть заполнен ДР ГрРабПодр . По нему и будем определять.При помощи IsWorkDayBranch, 
   которая для этого и создана.
*/
/*
   ...
   do i= 1 to op.op-date - date(bDataSogl) :
      ...
      ASSIGN
        ...
        vWork = IF  vWork EQ ? THEN NOT holiday(iDate) ELSE vWork
      .
      if vWork then do:
         j = j + 1.
      end.
