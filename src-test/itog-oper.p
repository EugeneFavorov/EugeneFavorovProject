...
DEFINE TEMP-TABLE ttOper
   ...
   INDEX m1 date filial-id  branch-id oper op-kind stat .
...
for each op where op.ins-date >= beg-date and op.ins-date <= end-date and op.op-status <> "А" no-lock.
   ...
   find first ttOper where ttOper.date      = op.ins-date
                       and ttOper.filial-id = op.filial-id
...
RUN printvd.p ("kass-op",INPUT TABLE ttnames).
