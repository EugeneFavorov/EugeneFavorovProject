   ...
   ELSE DO:
      ...
      ELSE    
         ASSIGN
            ...
            mCorrAcct = DelFilFromAcct(bacct.acct).
   END.
   /*pda если не смогли определить банк*/
   IF mBankName EQ "" 
   AND mBankMFO NE "" THEN
   DO:
      FIND FIRST banks-code WHERE
                 banks-code.bank-code-type EQ "МФО-9"
             AND banks-code.bank-code      EQ mBankMFO
      NO-LOCK NO-ERROR.
      ...
      ASSIGN
         mBankName = banks.name + ", " + banks.town-type + " " + banks.town
         mBankAcct = IF AVAIL banks-corr THEN banks-corr.corr-acct ELSE "".
   END.
