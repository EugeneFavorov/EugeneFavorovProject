...
(
(ACCT.CURRENCY EQ ''
AND CAN-DO('СпецПА,СпБрок,ДУ,Текущ,Расчет',ACCT.CONTRACT))
OR
(ACCT.CURRENCY NE ''
AND ACCT.CONTRACT EQ 'Текущ')
)
AND CAN-DO('!409*,407*,40802*,40807*,40821*',ACCT.NUMBER)
AND ACCT.FILIAL-ID = '" + ФИЛИАЛ() + "' NO-LOCK:"
======================================================= */   
   ...
   FOR EACH acct WHERE acct.filial-id   EQ shFilial
                   ...
                   AND (((acct.currency EQ '') AND (can-do('СпецПА,СпБрок,ДУ,Текущ,Расчет', acct.contract))) OR ((acct.currency NE '') AND (acct.contract EQ 'Текущ')))
