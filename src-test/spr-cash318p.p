   ...
   FOR EACH bttSprCash
      ...
      /* ========================== Шапка таблички ============================= */
         ...
         FOR EACH ttSprCash WHERE 
                  ...
                  AND (   ttSprCash.SummDb NE 0
                   OR ttSprCash.SummCr NE 0
                   OR mSprCash0 
               ...
               DO:
                  ...
                  DO:
					 ...
					 /* vByUserId  = op.user-id.*/ 
					  vByUserId = GetUserIDHis(op.user-id,STRING(op.op)) .                         
      ...
      BREAK BY kau-entry.op:
         ...
         DO:
            ...
            FIND FIRST op-entry WHERE op-entry.op EQ op.op 
                                AND (op-entry.acct-cr begins '30301'
                                OR   op-entry.acct-cr begins '30305') 
                          NO-LOCK NO-ERROR.
            IF AVAIL(op-entry) THEN
            
                        /* vByUserId  = op.user-id.  vByUserId   
            MESSAGE  op.user-id VIEW-AS ALERT-BOX.*/
                    /*    vByUserId =op.user-id.   */
                         vByUserId =              GetUserIDHis(op.user-id,STRING(op.op)) .
					 END.
                     ELSE
                     ...
                     DO:
                        ...
                        /* ttSprCash.QtyCr    = IF kau-entry.debit EQ NO AND vFlagOpCr  THEN 1
                           ...
                           ttSprCash.opDate   = DATE(vDate)
                           NO-ERROR.
                           IF     kau-entry.debit EQ NO
                           AND vFlagOpCr 
                           AND (   NOT mPodschDocOtch
                                OR NOT IsOpKasOrd(op.op)) THEN
