...
FIND FIRST tmprecid NO-LOCK NO-ERROR.
    ...
    FIND FIRST cust-corp WHERE RECID(cust-corp) EQ tmprecid.id NO-LOCK NO-ERROR.
       ...
       mName[2] = cust-corp.name-corp.
        ...
        RUN Insert_TTName("OKATO", IF  mSignsV="" THEN "нет" ELSE mSignsV).
            ...
            mSignsV = GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "ОКВЭД", "").
     ...
     /*Лицензии*/
        ...
        RUN Insert_TTName("OrgUpr",mSignsVal).
          ...
          mStrTMP = GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "УставКап", "").
       ...
       IF mSignsVal eq "" THEN            
            mSignsVal = if GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "УчредОрг", "") NE "" THEN GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "УчредОрг", "") else "Нет".
 ...
 /****/
      ...
      RUN Insert_TTName("Bf_Da",  IF (GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id), "nal_bf", ""))="Да" THEN "Х" ELSE "").
        RUN Insert_TTName("Bf_Net", IF(GetXAttrValueEx("cust-corp", STRING(cust-corp.cust-id), "nal_bf", ""))="Нет" THEN "Х" ELSE "").
...
/***Для старых анкет Конец**/               
       ...
       If (end-date GE date("26/12/2015"))  and  (end-date LT date("01/07/2016"))  THEN 
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "2" + SUBSTRING ( iParms, LENGTH (iParms), 1)  .
        If (end-date GE date("01/07/2016"))  and  (end-date LT date("30/08/2016"))  THEN 
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "3" + SUBSTRING ( iParms, LENGTH (iParms), 1)  .     
         If (end-date GE date("30/08/2016"))  and  (end-date LT date("29/11/2016"))  THEN 
