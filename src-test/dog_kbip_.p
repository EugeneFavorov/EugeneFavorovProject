...
END.
	...
	mTempC = "".
	RUN RetAdr.p(person.person-id,"Ч","АдрПроп",dblank,OUTPUT mTempC).
    RUN Insert_TTName("AddReg",mTempC). 
	
	RUN RetAdr.p(person.person-id,"Ч","АдрФакт",dblank,OUTPUT mTempC).
	IF mTempC = "" THEN
		RUN RetAdr.p(person.person-id,"Ч","АдрПроп",dblank,OUTPUT mTempC).
	...
	IF (dblank gE date("15/03/2017")) THEN
	 mark = "_2".  
    else 
     mark = "_1".  
		
	
	IF pr  EQ TRUE THEN RUN printvd.p ("dogkbip"  + mark ,INPUT TABLE ttnames).   
	IF pr1 EQ TRUE THEN RUN printvd.p ("dogkbip1" + mark ,INPUT TABLE ttnames).   
	IF pr2 EQ TRUE THEN RUN printvd.p ("dogkbip2" + mark ,INPUT TABLE ttnames).   
	IF pr3 EQ TRUE THEN RUN printvd.p ("dogkbip3" + mark ,INPUT TABLE ttnames).   
	IF pr4 EQ TRUE THEN RUN printvd.p ("dogkbip4" + mark ,INPUT TABLE ttnames).   
	IF pr5 EQ TRUE THEN RUN printvd.p ("dogkbip5" + mark ,INPUT TABLE ttnames).   
	IF pr6 EQ TRUE THEN RUN printvd.p ("dogkbip6" + mark ,INPUT TABLE ttnames).
	IF pr7 EQ TRUE THEN RUN printvd.p ("dogkbip7" + mark ,INPUT TABLE ttnames).
