...
width 50 CENTERED OVERLAY ROW 10 TITLE "Данные для отчета ".
  ...
  do ON ERROR UNDO, LEAVE
   ...
   UPDATE beg-date end-date commiss
...
tmpdate = beg-date - 5.
  ...
  for each op where 
        ...
        and (op.ben-acct = '40702810800000011476')
       ...
       nameCl = ''.
		...
		if avail loan then do:
                   CardNum = GetXattrValue("loan",loan.contract + ',' + loan.cont-code,"PROCARD_dogno").
                   CardName = 'Правовая помощь онлайн'.
                   CardDate = GetXattrValue("loan",loan.contract + ',' + loan.cont-code,"PROCARD_date").
                   CardSumm = GetXattrValue("loan",loan.contract + ',' + loan.cont-code,"PROCARD_sum").
...
sum1 = 'Итого Сумма1: ' /* '2. Всего по заключенным Заказчиком с привлеченными Исполнителем Клиентами Абонентстким договорам на оказание услуг "Помощь на дорогах" в отчетный период сумма оплаты составила ' */
...
sum1 = 'НДС 18%:          ' + TRIM(STRING(tmpSumNds,"->>>,>>>,>>>,>>9.99")) + ' (' + AmtString + ')'.

RUN Insert_TTName("summ3", sum1).

RUN printvd.p ("otch_pravp", INPUT TABLE ttNames).
