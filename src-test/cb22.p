...
/* Получение минимального остатка за период. */
 ...
 /* RUN GetMinAmt IN nach_h (OUTPUT mMinVal ).*/
  ...
  RUN GetDatMaxAmt IN nach_h (OUTPUT mMaxVal,OUTPUT mMaxDate ).
      ...
      if CAN-DO("ДВ,LiderPlus,Chemp*,Плюс,gold,Partner_l,Vig_d,Partner_v,Uspeh,Premium,Dohod*,Inves,Vig_d,Pens_nak,Nakop*,Vig_val,Prest_uni,Pens_u,Pens_vkl,ДВГОД",loan.cont-type) 
      then
      do:
      mKap = "Да".
      end.
 ...
 <Styles>\n
  ...
  <Table>\n
   ...
   <Row ss:AutoFitHeight="0">\n
    ...
    <Cell><Data ss:Type="String">Срок вклада</Data></Cell>\n
