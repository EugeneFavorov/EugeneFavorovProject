/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: 
      Comment: Создаем документы перечисления по реестру "Норвик"
   Parameters:
         Uses:
      Used by:
      Created: kam
...
FUNCTION ConvertDate RETURN DATE(INPUT iDate AS CHARACTER):
    ...
    mTmpStr = REPLACE(iDate,"/",".").
            ...
            mTmpDay = INT(SUBSTRING(mTmpStr,9)) NO-ERROR.
            IF ERROR-STATUS:ERROR = YES THEN RETURN.
            ERROR-STATUS:ERROR = NO.
            mTmpMonth = INT(SUBSTRING(mTmpStr,6,2)) NO-ERROR.
            IF ERROR-STATUS:ERROR = YES THEN RETURN.
            ERROR-STATUS:ERROR = NO.
...
fname = '/data/home2/bis/quit41d/imp-exp/norvik/in/' + 'plus_810_' + STRING(DAY(sOpDate),"99") + STRING(MONTH(sOpDate),"99") + STRING(YEAR(sOpDate),"9999") + '.txt'.
iInt = 0.
...
REPEAT ON ENDKEY UNDO, LEAVE:
    IMPORT UNFORMATTED fstr.
        ...
        CREATE ttOp.
                ...
                sName = ENTRY(3,fstr,';').
                sDate = ConvertDate(ENTRY(4,fstr,';')).
...
FOR EACH ttOp WHERE ttOp.in-sNewSum > 0:
    ...
    END.
                                                ...
                                                RUN ex-trans.p(INPUT 'pernorved', 
                                                        INPUT TODAY, 
