/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: 
      Comment: Создаем документы перечисления по реестру "Alex"
   Parameters:
         Uses:
      Used by:
      Created: kam
...
FUNCTION ConvertDate RETURN DATE(INPUT iDate AS CHARACTER):
    ...
    mTmpStr = REPLACE(iDate,"/",".").
        IF LENGTH(mTmpStr) = 8 OR LENGTH(mTmpStr) = 10 THEN DO:
            ...
            IF ERROR-STATUS:ERROR = YES THEN RETURN.
            ERROR-STATUS:ERROR = NO.
            mTmpDay = INT(SUBSTRING(mTmpStr,7)) NO-ERROR.
            IF ERROR-STATUS:ERROR = YES THEN RETURN.
            ERROR-STATUS:ERROR = NO.
...
fname = '/data/home2/bis/quit41d/imp-exp/alexbank/in/' + 'rbp_plus_' + STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") +  '.txt'.

/* fname = '/home2/bis/quit41d/imp-exp/alexbank/in/' + 'rbp_plus_' + STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") +  '.txt'.
*/
iInt = 0.

IF SEARCH (fname) = ? THEN DO:
    OUTPUT TO VALUE("/data/home2/bis/quit41d/log/autoreestr/alex" + STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") + ".log").  
/*    OUTPUT TO VALUE("/home2/bis/quit41d/log/autoreestr/alex" + STRING(YEAR(sOpDate),"9999") + STRING(MONTH(sOpDate),"99") + STRING(DAY(sOpDate),"99") + ".log"). */ 
        put unformatted ("Файл не найден " + fname) skip.
    OUTPUT CLOSE.
    RETURN.
END.
...
REPEAT ON ENDKEY UNDO, LEAVE:
    IMPORT UNFORMATTED fstr.
        ...
        ost40817 = 0.
        CREATE ttOp.
                sRecId = TRIM(ENTRY(1,fstr,';')).
                sDate = ConvertDate(ENTRY(2,fstr,';')).                
                sSum = ConvertDecimal(ENTRY(3,fstr,';')).
                sName = ENTRY(4,fstr,';').
                sCode = TRIM(ENTRY(5,fstr,';')).
                        ...
                        IF AVAIL loan THEN DO:
                          ...
                          "").
                                                ...
                                                ASSIGN
                                                        ...
                                                        ttOp.in-acctext =AcctCesRS
                                                        ttOp.in-op-date = sOpDate
                                                .
                                        END. /* IF sSum > 0 THEN DO: */
                                        ELSE DO:
                                                ttOp.mess = TRIM(ttOp.mess + 'Договор ' + sCode) + ', документ не создан, сумма на счете 40817 = ' + STRING(sNewSum).
                                        END.
                                END.
                        END.
                        ELSE DO:
                                 ttOp.mess = TRIM(ttOp.mess + 'Договор ' + sCode) + ' не найден'. 
                        END.
                END.
                ELSE DO:
                        IF sDate = ? THEN 
                                ttOp.mess = TRIM(ttOp.mess + ' Договор ' + sCode) + ' неверный формат даты платежа'.
                        IF sSum = 0 THEN DO:
                                ttOp.mess = TRIM(ttOp.mess + ' Договор ' + sCode) + ' сумма в реестре = 0'.
                        END.
                        IF sSum < 0 THEN DO:
                                ttOp.mess = TRIM(ttOp.mess + ' Договор ' + sCode) + ' неверный формат суммы платежа'.
                        END.
                END.
                RELEASE ttOp.
    fstr = ''.
...
FOR EACH ttOp WHERE ttOp.in-sNewSum > 0:
    ...
    END.
                                                ...
                                                IF NOT mOK THEN
                                                DO:
                                                        ttOp.mess = TRIM(ttOp.mess + ' Договор ' + ttOp.in-cont-code) + ', документ не создан, ' + mMessage.
                                                END.
                                                ELSE DO:
                                                        ttOp.mess = TRIM(ttOp.mess +  ' Договор ' + ttOp.in-cont-code) + ', документ создан, сумма в реестре ' + STRING(ttOp.in-sSum) + ', сумма документа ' + STRING(ttOp.in-sNewSum).
                                                END.
END.
