...
for each op-entry where  op-entry.op-date  >= beg-dt
   ...
   if  CAN-DO(list-bal-3, op.ben-acct) and substr(trim(op.details),2,2) EQ "VO"  and  op-entry.currency = "" and CAN-DO(list-bal, op-entry.acct-cr) then do:
       RUN create_ttDoc(op.op,"Извещение").
   end.
...
FOR each branch WHERE                                                
   ...
   OS-COPY VALUE(cFl) VALUE("/data/home2/bis/quit41d/imp-exp/cur_cont/" + string(year(end-date)) + "-" + string(month(end-date)) + "-" + string(day(end-date))  + "-QMReport-" + branch.branch-id + ".xml" ). 
end.
return.
