/*
    ...
    Copyright: (C) 1992-2007 ЗАО "Банковские информационные системы"
     Filename: 
      Comment: Создаем документы перечисления по реестру обратного выкупа "РосКап"
   Parameters:
         Uses:
      Used by:
      Created: kam
...
DEF VAR sSumSt AS DECIMAL NO-UNDO.
...
ffname = '/data/home2/bis/quit41d/imp-exp/roscap/in/1.csv'.
...
REPEAT ON ENDKEY UNDO, LEAVE:
	...
	iInt = iInt + 1.
	ost40817 = 0.
		sCode = TRIM(ENTRY(1,fstr,';')).
		sId = TRIM(ENTRY(2,fstr,';')).
		sName = ENTRY(3,fstr,';').
		sSumQ = ConvertDecimal(ENTRY(9,fstr,';')). /* ОД         = L */
		sSumR = ConvertDecimal(ENTRY(10,fstr,';')). /* Проср.ОД   = M */ 
		sSumS = ConvertDecimal(ENTRY(11,fstr,';')). /* Начисл%    = N */
		sSumT = ConvertDecimal(ENTRY(13,fstr,';')). /* Просроч%   = O */
		sSumU = ConvertDecimal(ENTRY(12,fstr,';')) + ConvertDecimal(ENTRY(14,fstr,';')). /* Просроч% на просрОД = P */
		sSumSt = ConvertDecimal(ENTRY(16,fstr,';')). /* Штрафы */
		/* sSumAA = ConvertDecimal(ENTRY(20,fstr,';')). /* Сумма передаваемых прав требований = T */ */
        strDateProd = TRIM(ENTRY(7,fstr,';')). /* Дата продажи договора */
	
	ssumaa = ssumQ + ssumR + ssumS + ssumT + ssumU + ssumSt.
/*
...
/*		sDate = TODAY - 1.
		sOpDate = TODAY - 1.  */
		IF sSumAA > 0 THEN DO:
			FIND FIRST loan WHERE STRING(RECID(loan)) = sId
				NO-LOCK NO-ERROR.
				...
				IF AVAIL loan-acct THEN DO:
					IF sSumQ > 0 THEN DO:
						...
						ASSIGN
							...
							ttOp.in-acctcr = '47422810100000000052'
							ttOp.in-loanop = '4'
...
/*						ttOp.mess = ttOp.mess + 'Договор ' + sCode + ', документ не создан, сумма на счете 40817 = ' + STRING(sNewSum). */
				...
				FIND last loan-acct OF loan WHERE loan-acct.acct-type = 'КредШт%ОВ'
				 NO-ERROR.
				IF AVAIL loan-acct THEN DO:
					...
					IF sSumU > 0 THEN DO:
						...
						ASSIGN
							...
							ttOp.in-filial = loan.filial-id
							ttOp.in-sSum = sSumSt
							ttOp.in-sNewSum = sSumSt
							ttOp.in-namecl = sName1
							ttOp.in-acctext ='пени'
							ttOp.in-op-date = sOpDate
							ttOp.in-acctcr = '47422810100000000052'
							ttOp.in-loanop = '12'
							ttOp.in-DateProd = strDateProd
