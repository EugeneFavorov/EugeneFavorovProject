...
DEFINE var vTarif   AS char      NO-UNDO. 
...
DEFINE var ost-sum     as dec     NO-UNDO.   /* Сумма остатков за период  */
DEFINE var ost-s       as dec     NO-UNDO.   /* Средний остаток за период */
DEFINE var SummaPr     as dec     NO-UNDO.   /* Сумма процентов           */
DEFINE var D_year      AS int64   NO-UNDO.   /* Количество дней в году    */               
DEFINE var StrDateNO_  AS char    NO-UNDO.
...
IF num-entries(in_par,"|") <> 3 then do:
   PUT stream err UNFORMATTED "Не верное число параметров" SKIP.
   RETURN pick-value.
end.       
acct     = entry(1,in_par,"|").
...
for each loan-cond where loan-cond.contract  = loan-acct.contract
                     ...
                     no-lock.
   vTarif      = GetXattrValueEx("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + string(loan-cond.since), "ТарифПлан",?).     
   if vTarif <> "СО" then next.
   ...
   if (DB_ >= PB_ and DB_ <= PE_) or (DE_ >= PB_ and DE_ <= PE_) or (DB_ < PB_ and DE_ > PE_) then do:
      ...
      j = RE_ - RB_ .
      do i = 0 to j.
          run acct-pos in h_base (acct.acct,acct.currency,RB_ + i, RB_ + i,?).
          ost-t = - sh-bal.
          if ost-t < vMinlValDec then do:
             ...
             PUT stream err UNFORMATTED " " SKIP.
             pick-value =  string(DateNO_) + "|" + string(SummaNO_) + "|0|0|  /  /  |  /  /  ".
             RETURN pick-value.
          end. 
          ost-sum  = ost-sum + ost-t.
      end.
      ost-s  = round(ost-sum / (j + 1),2).
/*
      D_year = date("01/01/" + string(year(RE_) + 1,"9999")) - date("01/01/" + string(year(RE_),"9999")).
      if ost-s >= vMinlValDec * 1.35 then do:
         vStavka = round(vStavka * vMinlValDec / ost-s,2) + 1.
         SummaPr = round((ost-sum * vStavka ) / (D_year * 100 ),2).
      end.
      else do:
         SummaPr = round((ost-sum * vStavka ) / (D_year * 100),2).
      end.
*/
      if DateNO_ = ? then do:
         ...
         StrDateNO_ = string(DateNO_).
      end.

      pick-value =  StrDateNO_ + "|" + string(SummaNO_) + "|" + string(ost-sum) + "|" + string(ost-s) + "|" + string(RB_) + "|" + string(RE_).
