...
*/
   ...
   define FRAME frame_date_codes 
	 Srok     label "Срок (в днях)" format ">>9"	
         ...
         Summa    label "Сумма"         format ">>>,>>>,>>9.99"
	 tarif    label "Процент"       format ">>9.9999"	
	 with 1  COL 1 down 
         width 50 CENTERED OVERLAY ROW 11 TITLE "Данные для доп.соглашения <Неснижаемый остаток>".
                  ...
                  DO:
                    if int(Srok:SCREEN-VALUE) <> 0 then do:
                       for each loan-cond where loan-cond.contract   = loan-acct.contract
                                            ...
                                            no-lock.
                         if     GetXattrValueEx("loan-cond",loan-cond.contract + "," + loan-cond.cont-code + "," + string(loan-cond.since), "ТарифПлан",?) = "НО" 
   ...
   vOk =  UpdateSigns("loan-cond","Расчет" + "," + loan-cond.cont-code + "," + string(loan-cond.since),"ТарифПлан","НО",no).
   ...
   vOk =  UpdateSigns("loan-cond","Расчет" + "," + loan-cond.cont-code + "," + string(loan-cond.since),"Актив","ДА",yes).
