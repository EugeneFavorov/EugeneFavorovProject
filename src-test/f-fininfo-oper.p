...
&Scoped-Define ENABLED-OBJECTS vVidOper vPeriodOper vAmountOper vSummaOper vCurrency ~
fCustName separator 
...
&Scoped-Define DISPLAYED-OBJECTS vVidOper vPeriodOper vAmountOper vSummaOper vCurrency ~
fCustName separator 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-1 vVidOper vPeriodOper vAmountOper vSummaOper vCurrency ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
&Scoped-define List-2 vVidOper vPeriodOper vAmountOper vSummaOper vCurrency ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
&Scoped-define List-3 vVidOper vPeriodOper vAmountOper vSummaOper vCurrency ~
tt-cust-ident.open-date ~
tt-cust-ident.close-date 
...
DEFINE VARIABLE vVidOper AS CHARACTER FORMAT "X(20)":U 
     LABEL "Код вида операции"
     VIEW-AS FILL-IN
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 20 BY 1
     &ELSE SIZE 57 BY 1 &ENDIF NO-UNDO.

DEFINE VARIABLE vPeriodOper AS CHARACTER FORMAT "X(20)":U 
     LABEL "Частота операций"
     VIEW-AS FILL-IN 
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 20 BY 1
     &ELSE SIZE 61 BY 1 &ENDIF NO-UNDO.

DEFINE VARIABLE vAmountOper AS INT64 FORMAT ">>>>>>9":U 
     LABEL "Количество операций"
     VIEW-AS FILL-IN
     &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 7 BY 1
     &ELSE SIZE 57 BY 1 &ENDIF NO-UNDO.

DEFINE VARIABLE vSummaOper AS DECIMAL FORMAT ">>>>>>>9.99":U 
     LABEL "Сумма операций, в млн."
     VIEW-AS FILL-IN
...
DEFINE FRAME fMain
     vVidOper
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 3 COL 25 COLON-ALIGNED
          &ELSE AT ROW 2.99 COL 8 &ENDIF
          HELP "Код вида операции"
     vPeriodOper
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 4 COL 25 COLON-ALIGNED
          &ELSE AT ROW 4 COL 16 &ENDIF
     vAmountOper
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 5 COL 25 COLON-ALIGNED
          &ELSE AT ROW 5 COL 16 &ENDIF
          HELP "Количество операций"
     vSummaOper
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 6 COL 25 COLON-ALIGNED
          &ELSE AT ROW 5.99 COL 16 &ENDIF
          HELP "Сумма операций"
     vCurrency
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 7 COL 25 COLON-ALIGNED
          &ELSE AT ROW 7 COL 16 &ENDIF
          HELP "Валюта операций"
     tt-cust-ident.open-date
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 8 COL 25 COLON-ALIGNED
          &ELSE AT ROW 11 COL 16 &ENDIF
          HELP "Дата начала действия"
          LABEL "Дата начала действия" FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 10 BY 1
          &ELSE SIZE 10 BY 1 &ENDIF
     tt-cust-ident.close-date
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN AT ROW 9 COL 25 COLON-ALIGNED
          &ELSE AT ROW 11 COL 45 &ENDIF
          HELP "Дата окончания действия"
          LABEL "Дата окончания действия" FORMAT "99/99/9999"
          VIEW-AS FILL-IN 
          &IF '{&WINDOW-SYSTEM}' = 'TTY':U &THEN SIZE 10 BY 1
          &ELSE SIZE 10 BY 1 &ENDIF
     fCustName
...
ON F1 OF vVidOper IN FRAME fMain /* Код вида операции */
DO:
  DO TRANS:
       RUN browseld.p ("code",  /* Класс объекта. */
              "class" + CHR(1) + "parent" + CHR(1) + "title",
              /* Поля для предустановки. */
              "ВидПланОп" + CHR(1) + "ВидПланОп" + CHR(1) + "Виды планируемых операций по лиц. счёту",
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vVidOper TERMINAL-SIMULATION
ON LEAVE OF vVidOper IN FRAME fMain /* Код вида операции */
DO:
  {&BEG_BT_LEAVE}
     FIND FIRST CODE WHERE
         CODE.class  = "ВидПланОп" AND
         CODE.parent = "ВидПланОп" AND
         CODE.code   = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF NOT AVAIL CODE THEN DO:
     RUN Fill-SysMes IN h_tmess ("", "", "-1", "Значения реквизита " + SELF:SCREEN-VALUE + " нет в классификаторе 'ВидПланОп'").
     RETURN NO-APPLY {&RET-ERROR}.
  END.
  {&END_BT_LEAVE}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vPeriodOper TERMINAL-SIMULATION
ON F1 OF vPeriodOper IN FRAME fMain /* Частота операций */
DO:
  DO TRANS:
       RUN browseld.p ("code",  /* Класс объекта. */
              "class" + CHR(1) + "parent" + CHR(1) + "title",
              /* Поля для предустановки. */
              "ЧастОперац" + CHR(1) + "ЧастОперац" + CHR(1) + "Виды планируемых операций по лиц. счёту",
              /* Список значений полей. */
               ?,          /* Поля для блокировки. */
              iLevel).     /* Строка отображения фрейма. */
   END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL vPeriodOper TERMINAL-SIMULATION
ON LEAVE OF vPeriodOper IN FRAME fMain /* Частота операций */
DO:
  {&BEG_BT_LEAVE}
     FIND FIRST CODE WHERE
         CODE.class  = "ЧастОперац" AND
         CODE.parent = "ЧастОперац" AND
         CODE.code   = SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF NOT AVAIL CODE THEN DO:
     RUN Fill-SysMes IN h_tmess ("", "", "-1", "Значения реквизита " + SELF:SCREEN-VALUE + " нет в классификаторе 'ЧастОперац'").
     RETURN NO-APPLY {&RET-ERROR}.
  END.
  {&END_BT_LEAVE}
END.
...
DO:
  {&BEG_BT_LEAVE}
    ...
    DO:
      RUN Fill-SysMes IN h_tmess ("", "", "-1", "Значение '" + SELF:SCREEN-VALUE + "' отсутствует в справочнике валют").
      RETURN NO-APPLY {&RET-ERROR}.
    END.
  {&END_BT_LEAVE}
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  /* MESSAGE mSymbol VIEW-AS ALERT-BOX.*/
...
------------------------------------------------------------------------------*/
  DISPLAY vVidOper vPeriodOper vAmountOper vSummaOper vCurrency fCustName separator 
  ...
  IF AVAILABLE tt-cust-ident THEN 
    DISPLAY tt-cust-ident.open-date 
      ...
      WITH FRAME fMain IN WINDOW TERMINAL-SIMULATION.
  ENABLE vVidOper vPeriodOper vAmountOper vSummaOper vCurrency
...
------------------------------------------------------------------------------*/
   DO WITH FRAME fMain:
      ...
      NO-ERROR.
      ASSIGN
         tt-cust-ident.cust-code   = vVidOper + mSymbol + STRING(tt-cust-ident.open-date)
         tt-cust-ident.issue       = vPeriodOper + mSymbol + 
                                     STRING(vAmountOper) + mSymbol + 
                                     STRING(vSummaOper)  + mSymbol + 
                                     vCurrency
         tt-cust-ident.cust-code-type = "FinInfoOper"
         .
   END.
END PROCEDURE.
...
ELSE DO:
   ASSIGN
      vVidOper    = ENTRY(1,tt-cust-ident.cust-code,mSymbol)
      vPeriodOper = ENTRY(1,tt-cust-ident.issue,mSymbol)
      vAmountOper = INT64(ENTRY(2,tt-cust-ident.issue,mSymbol))
      vSummaOper  = DEC(ENTRY(3,tt-cust-ident.issue,mSymbol))
      vCurrency   = ENTRY(4,tt-cust-ident.issue,mSymbol)
   NO-ERROR.
END.
END PROCEDURE.
