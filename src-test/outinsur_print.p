...
FOR EACH tmprecid NO-LOCK,
   ...
   DO:
      ...
      adrStrah = TRIM(LEFT-TRIM(adr_strah, "," + indStrah)).
   END.
...
BREAK BY otch.adrstrah
   ...
   DO:
      RUN Insert_TTName("strah",otch.strah).
      RUN Insert_TTName("indadr",otch.ind + ', ' + otch.adrstrah).

      RUN Insert_TTName("num[client]",mK).
      RUN Insert_TTName("fio[client]",otch.fio).
      vRekv = DelFilFromLoan(otch.cont_code) + ' от ' + STRING(otch.open_date,"99.99.9999") + ' г.'.
      RUN Insert_TTName("rekv[client]",vRekv).
   END.

   IF FIRST-OF(otch.adrstrah) 
   AND LAST-OF(otch.adrstrah) THEN RUN NextCircle_TTName("client").
   ELSE DO:
      RUN Insert_TTName("num[client]",mK).
      RUN Insert_TTName("fio[client]",otch.fio).
      vRekv = DelFilFromLoan(otch.cont_code) + ' от ' + STRING(otch.open_date,"99.99.9999") + ' г.'.
...
IF loan.cust-cat = 'Ч' THEN DO:
   ...
   mSignsV =  TRIM(STRING(person.adress[1])) + TRIM(STRING(person.adress[2])).
END.
END PROCEDURE.
