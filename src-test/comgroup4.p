...
/* Вставка BAV - ОСРКО.511 */
   /* Проверяем блокировки и очередность документа */
   mResult = BlockAcct(op-entry.acct-db + "," + op-entry.currency, DATETIME(TODAY, TIME)).
   IF CAN-DO(mResult, "Банкрот") OR CAN-DO(mResult, "КонкПр") THEN LEAVE lMain.
   IF CAN-DO("1,2,3", op.order-pay) THEN LEAVE lMain.
/* Конец вставки BAV - ОСРКО.511 */

   IF CAN-DO("ВКЛБ,ФБФМ,",op.op-status) THEN LEAVE lMain.
