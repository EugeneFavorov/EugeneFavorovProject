...
DEFINE VARIABLE mMoveMoney AS LOGICAL   NO-UNDO.
...
&IF DEFINED(pFSS) EQ 1 &THEN
FIND FIRST code WHERE code.class EQ "ФСС"
                  AND code.code  EQ mCodeNalog
   NO-LOCK NO-ERROR.
&ENDIF
...
RUN Insert_TTName("ДатаН_1",ENTRY(1,term2str(Beg-Date, Beg-Date)," ")).
RUN Insert_TTName("ДатаН_2",ENTRY(2,term2str(Beg-Date, Beg-Date)," ")).
RUN Insert_TTName("ДатаН_3",ENTRY(3,term2str(Beg-Date, Beg-Date)," ") + " " + ENTRY(4,term2str(Beg-Date,Beg-Date)," ")).
...
/* ------- FEV: Изменение реквизитов в выписке для закрытого МФ ------- */
IF acct.filial-id EQ '0400'
  THEN
    DO:
      FIND FIRST branch WHERE branch.branch-id EQ '0000'
      NO-LOCK NO-ERROR.
      RUN Insert_TTName("Банк", branch.name).
      mStrTMP = GetXAttrValueEx("branch", '0000', "КПП", "").
      RUN Insert_TTName("Банк-КПП", mStrTMP).
      mStrTMP = GetXAttrValueEx("branch", '0000', "БанкМФО", "").
...
DO:
   ...
   IF AVAIL code THEN
      mName[1]  = code.name.
END.
ELSE IF mCustCat EQ "Ч" 
    AND GetXAttrValueEx("person", STRING(mCustId), "субъект", "") EQ "ФЛП" THEN
       mName[1]  = "Индивидуальный предприниматель " + mName[1].   
...
&ENDIF
   ...
   /*pda 26/09/16 назначение полное имя */
   FIND FIRST contract WHERE
      contract.contract EQ acct.contract NO-ERROR.
   RUN Insert_TTName("namecontr[acct]",contract.name-contract).
   ...
   mBegDate = beg-date.
   mEndDate = end-date.
   &IF DEFINED(onedate) EQ 1 &THEN
      mBegDate = end-date.
   &ENDIF

   /*pda условие пересечения временных интервалов*/
   IF  acct.open-date   <= mEndDate 
   AND (acct.close-date >= mBegDate OR acct.close-date EQ ?) THEN
      RUN Insert_TTName("openacct","1").
   ELSE RUN Insert_TTName("openacct","0").
   ...
   FOR EACH op-entry WHERE     (    op-entry.acct-db EQ acct.acct
      ...
      NO-LOCK,
       FIRST op OF op-entry WHERE op.op-status GE gop-status NO-LOCK BY op.op-date:
      mItem = mItem + 1.
...
&ENDIF
      ...
      RUN Insert_TTName("Имя[op]", IF mPName NE "" THEN  replace(replace(mPName,chr(10)," "),chr(13)," ")
                                                   ELSE FILL("-", 10)).
      RUN GetOpDetails(BUFFER op, OUTPUT mDetails) NO-ERROR.
      RUN Insert_TTName("details[op]", IF mDetails NE "" THEN replace(replace(REPLACE(mDetails,"~n"," "),chr(10),""),chr(13),"")
                                                         ELSE FILL("-", 10)).
       ...
       IF TRIM( ENTRY(vI,mIn-proc,vSepChar)) NE "" THEN DO:
          IF INDEX(mIn-proc,"~n") NE 0 THEN DO:
            ...
            DO vNSym = 1 TO vNumStr:
               RUN Insert_TTName("sign[sig]", "ivv" + replace(replace(TRIM( ENTRY(vNSym,ENTRY(vI,mIn-proc,vSepChar),"~n")),chr(10),""),chr(13),"")).
               RUN NextCircle_TTName("sig").            
            END.
          END.
          ELSE DO:
            RUN Insert_TTName("sign[sig]", "ivv" + replace(replace(TRIM( ENTRY(vI,mIn-proc,vSepChar)) ,chr(10),""),chr(13),"")  ).
            RUN NextCircle_TTName("sig").            
          END.
       END.
    END.  
...
/* Если не было операций по счету за указанный период то печатаем */
/* только тит. лист (для выгрузки в WORD)*/
IF sh-db  EQ 0 AND
   sh-cr  EQ 0 AND
   sh-vdb EQ 0 AND
   sh-vcr EQ 0 THEN mMoveMoney = FALSE.
   ELSE mMoveMoney = TRUE.

IF NOT mMoveMoney AND {&prreg} EQ 'ВПОНС-WRD' 
   THEN RUN printvd.p('ВПОНС-WRD1',
                       INPUT TABLE ttnames
                     ).
ELSE IF NOT mMoveMoney AND {&prreg} EQ 'ВПОНС377Фw' 
   THEN RUN printvd.p('ВПОНС377Ф1',
                      INPUT TABLE ttnames
                     ).
ELSE DO: 
  &IF DEFINED(ticlax) EQ 0 &THEN
...
/* $LINTENV ='1ut' */
/* $LINTVSS ='$/ws2-tst/bq/' */
/* $LINTDATE='17/11/2014 13:07:22.334+04:00' */
/* $LINTFILE='ticlac.i' */
/*prosign5DNgWNXTfMMbpcjwQbwYbA */