...
DEFINE VARIABLE vLoanBeg    AS DATE     NO-UNDO.    /* Дата выдачи кредита */
...
/*
    ...
    FOR EACH signs
        WHERE signs.file-name   = 'term-obl'
          ...
          AND ENTRY(1,signs.Surrogate)  EQ "Кредит"
          AND ENTRY(3,signs.Surrogate)  EQ "5"
        NO-LOCK,
    FIRST bTerm
        WHERE bTerm.contract    =      ENTRY(1,signs.surrogate)
          AND bTerm.cont-code   =      ENTRY(2,signs.surrogate)
          AND bTerm.idnt        = INT (ENTRY(3,signs.surrogate))
          AND bTerm.end-date    = DATE(ENTRY(4,signs.surrogate))
          AND bTerm.nn          = INT (ENTRY(5,signs.surrogate))
          AND bTerm.symbol      = 'Ч'
          AND bTerm.fop         = person.person-id
        NO-LOCK,
    FIRST bLoan
        WHERE bLoan.contract    = bTerm.contract
          AND bLoan.cont-code   = bTerm.cont-code
          AND bLoan.filial-id   = shFilial
          AND bLoan.open-date   >= idate1
          AND bLoan.open-date   <= idate2
        NO-LOCK:

        strXattr    = GetXattrValue("loan", bLoan.contract + "," + bLoan.cont-code, "UnlGuar"). /* Список выгруженных поручительств */
        strTmp      = GetXAttrValue("term-obl", signs.surrogate, "НомДогОб").   /* Номер договора поручительства */
        ...
        THEN DO:
            ...
            vLoanUpd = LN_GetGarantBeg(RECID(bTerm), bLoan.open-date).
            vLoanUpd = IF (vLoanUpd < bLoan.open-date) THEN bLoan.open-date ELSE vLoanUpd.
            RUN CREexportTrade( "поручитель", hSAXWriter, "Ч", person.person-id,
                          vLoanUpd, BUFFER bLoan, BUFFER bTerm, TRUE, TODAY).
            strXattr = strXattr + (IF (strXattr = "") THEN "" ELSE "|") + strTmp.
            UpdateSigns("loan", bLoan.contract + "," + bLoan.cont-code, "UnlGuar", strXattr, NO).
            dog_cnt = dog_cnt + 1.
            ij = ij + 1.
        END.

        IF      (LOOKUP("-" + strTmp, strXattr, "|") = 0)                       /* Это поручительство не выгружалось */
            AND (bLoan.close-date <> ?)                                         /* на дату закрытия */
        THEN DO:
            /* Если выгружается первый договор в секции, то сначала выгрузим клиента */
            IF dog_cnt EQ 0  THEN DO:
                RUN CREexportPerson( hSAXWriter, BUFFER person,BUFFER bLoan).
        ...
        FOR EACH bTerm WHERE bTerm.cont-code EQ bLoan.cont-code
            ...
            IF LOOKUP(strTmp,strXattr,"|") <= 0 THEN DO:
                ...
                IF dog_cnt = 0 THEN
                  ...
                  dog_cnt = dog_cnt + 1.
                RUN CREexportDspt(hSAXWriter,BUFFER bTerm, TODAY).
            END.
        END.
        IF dog_cnt > 0 THEN
...
FOR EACH cust-corp
    WHERE mycustcat         = 'Ю'
      AND cust-corp.cust-id = mycustid
    no-lock
    ON ERROR UNDO, THROW:
...
/*
    ...
    FIRST bTerm
        WHERE bTerm.contract    =      ENTRY(1,signs.surrogate)
          ...
          AND bTerm.nn          = INT (ENTRY(5,signs.surrogate))
          AND bTerm.symbol      = 'Ю'
          AND bTerm.fop         = cust-corp.cust-id
        NO-LOCK,
    FIRST bLoan
        WHERE bLoan.contract    = bTerm.contract
          AND bLoan.cont-code   = bTerm.cont-code
