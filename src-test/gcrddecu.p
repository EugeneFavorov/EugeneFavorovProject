...
/* Вставка Плюс банк */
DEFINE VARIABLE vED275    AS CHARACTER   NO-UNDO INIT "".
...
/* Вставка Плюс банк */
      FOR EACH PackObject
         WHERE (PackObject.file-name   = 'op-entry')
           AND (PackObject.Surrogate   = STRING(kau-entry.op) + "," + STRING(kau-entry.op-entry))
         NO-LOCK,
      EACH Packet
         WHERE (Packet.PacketID        = PackObject.PacketID)
           AND (Packet.mail-format     = "XML-ED275")
         NO-LOCK,
      FIRST PacketText OF Packet
         NO-LOCK:

         vED275 = LEFT-TRIM(SUBSTRING(PacketText.Contents, 10, 9), "0")
                + " от " +  SUBSTRING(PacketText.Contents, 29, 8).
      END.
/* Конец вставки Плюс банк */
   end.
   ELSE DO:
...
/* Вставка Плюс банк */
      work-op.details   = IF {assigned vED275}
                          THEN ("Отзыв на основании ED275_" + vED275
                              + (IF (AVAIL kau-op) THEN (", Инкассовое поручение N " + kau-op.doc-num + " от " + STRING(kau-op.doc-date)) ELSE ""))
                          ELSE work-op.details
/* Конец вставки Плюс банк */
