...
/*   MESSAGE ENTRY(vCountInt,ipCountTypeChar,"+") SKIP out_result
       VIEW-AS ALERT-BOX INFO BUTTONS OK.
*/
            CREATE ttReport.
            ASSIGN
   ...
   /* Получение значения параметра 4 */
   RUN PRM(
        loan.Contract,            /* Назначение договора */
        loan.Cont-Code,            /* Номер договора */
        4,        /* Код параметра  */
...
DO i = 32 TO 35:
   ...
   ASSIGN ttReport.param_value  = ttReport.param_value + out_result.
END.
...
{setdest.i}
 ...
 IF AVAILABLE bLA THEN
   ...
   RUN acct-pos IN h_base (bLA.acct,
    ...
    END.
 ELSE
    mVkl-acct = "   Вклад НЕ ПРИВЯЗАН К ДОГОВОРУ".
...
FOR EACH ttReport SHARE-LOCK by INTEGER(ttReport.param_id): /* 5 */
    ...
    IF ABSOLUTE(ttReport.param_value) GT 0 THEN 
	DISPLAY
