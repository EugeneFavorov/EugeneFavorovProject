...
FOR EACH tmprecid NO-LOCK: 
   ...
   run signat.p ("precrdprvvv02",substr(loan.cont-code,1,5),str_title,OUTPUT mUser).
   ...
   END.
      ...
      IF AVAIL cust-role then DO:
         ...
         IF AVAIL b_person then DO:
            vDeystv = if b_person.gender then "действующий" else "действующая". 
         end.
  ...
  /* Поиск наименоваия Суда */
   ...
   FIND first code WHERE code.class EQ 'НаименСуда' 
                     ...
                     and code.code = substr(loan.cont-code,1,5)
                     NO-LOCK NO-ERROR.     
