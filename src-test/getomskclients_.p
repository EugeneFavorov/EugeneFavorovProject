...
FUNCTION convertDoc2BIS RETURN CHAR(
    ...
    IF iDocType EQ 'Документ' THEN DO:
...
/*    IF NOT AVAIL clients-abs THEN UNDO, THROW NEW Progress.Lang.AppError( "Не найден клиент в banker.clients " + STRING(bank.clients-mfr.cid)).*/
    ...
    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED STRING( NOW, "99/99/99 HH:MM:SS") + " (" + STRING( bank.clients-mfr.id) + ") ==== начало обработки ====" SKIP. &ENDIF
    iTime = MTIME.
    MYNEXT:
    DO ON ERROR UNDO, THROW:
        ...
        */
    	...
    	ELSE NEXT.  
/*    	UNDO, THROW NEW Progress.Lang.AppError( "не определили тип клиента" ).*/
	...
	if shFilial NE strBranchId THEN DO ON ERROR UNDO, THROW:
	    ...
	    RUN SetEnvironment IN h_base (strBranchId). * контекст выбранного филиала *
	    gend-date = today. */
	    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED '    + смена тек филиала на ' + shFilial SKIP. &ENDIF
	END.

	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + ищем клиента" SKIP. &ENDIF
	/*===== ищем клиента =====*/
        RELEASE person.
        RELEASE cust-corp.
    	IF (iTypeCl = 1 OR iTypeCl = 3) THEN DO: /* ФЛ ИП */
    	    /* ищем по ДР CID */
    	    IF iTypeCl = 1 THEN
    	    FIND FIRST signs
    	    ...
    	    clActionStatus = IF AVAIL person THEN 2 /* 2 - update из ОмскАбс */ ELSE 1 /* 1 - новый из омскАБС */ .
        END. ELSE
        IF iTypeCl = 0 THEN DO: /* ЮЛ */
    	    /* ищем по iBisId */
    	    IF iBisId NE ?
        ...
        END. ELSE UNDO, THROW NEW Progress.Lang.AppError( "не понятный тип клиента iTypeCl=" + STRING(iTypeCl) ).
	...
	cntPriznak = 0.
    	IF (iTypeCl = 1 OR iTypeCl = 3) THEN DO: /* ФЛ ИП */
	    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + ищем по документу и удостоверяемся что это именно тот клиент" SKIP. &ENDIF
    	    FOR EACH doc-mfr
			...
			&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + нашли клиента по документу" +
    	    ...
    	    END.
        END. ELSE
        IF /* AVAIL cust-corp AND*/ iTypeCl = 0 THEN DO ON ERROR UNDO, THROW: /* ЮЛ */
    	    cntPriznak = 0.
		...
		put unformatted "'" bank.clients-mfr.kpp "' '" GetXattrValueEx( "cust-corp", STRING( cust-corp.cust-id), "КПП", "") "'" SKIP.
		put unformatted "'" bank.clients-mfr.ogrn "' '" GetXattrValueEx( "cust-corp", STRING( cust-corp.cust-id), "ОГРН", "") "'" SKIP.
	...
	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + проверяем надо ли синхронизировать" SKIP. &ENDIF
    	IF AVAIL person AND (iTypeCl = 1 OR iTypeCl = 3) THEN DO ON ERROR UNDO, THROW: /* ФЛ ИП */
    	    DEF VAR eText AS CHAR NO-UNDO.
    	    tmpStr = GetXAttrValueEx("person",STRING(iBisId),"syncOmskABS","Нет").
    	    IF tmpStr <> "Да" THEN DO ON ERROR UNDO, THROW:
    		...
    		IF (IF person.gender THEN "М" ELSE "Ж") <> Convert2Gender( bank.clients-mfr.fname, bank.clients-mfr.sex)
    		 THEN DO: 
    		    tmpStr = 'Нет'. 
    		    eText = (IF eText <> "" THEN eText + ";" ELSE "") + 
    			"не совпал пол".
        	...
        	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + клиент совпадает " + STRING( person.person-id) SKIP. &ENDIF
    	    END.
        END.
    	IF AVAIL cust-corp AND (iTypeCl = 0) THEN DO ON ERROR UNDO, THROW: /* ЮЛ */
	...
	IF iTypeCl = 1 OR iTypeCl = 3 THEN DO ON ERROR UNDO, THROW:
		...
		if (not avail history OR can-do("04*", strBranchId)) AND person.person-id <> 53852 then
    		     UNDO, THROW NEW Progress.Lang.AppError( "пока синхронизируем только Омских клиентов.").
    	    END.
	...
	/* если клиент старый и его нет в БИС, то проверяем наличие счетов у клиента */
	 ...
	 THEN DO:
		...
		IF (NOT AVAIL guaranty) AND (NOT AVAIL usr_warranty) /* CAN-FIND( guaranty
		 WHERE bank.guaranty.cid EQ iCid AND PRKEY <> 'DF0' NO-LOCK )*/ THEN
		   UNDO, THROW NEW Progress.Lang.AppError( "Клиент _CID=" + STRING(iCid) + " не имеет открытых счетов в банке").
	...
	&IF DEFINED( DEBUG-LOG) &THEN ELSE PUT UNFORMATTED "    + проверяем клиента" SKIP. &ENDIF
        /*===== проверяем клиента =====*/
        ...
        IF NOT AVAIL adr-mfr THEN DO:
            IF clActionStatus = 1 THEN DO:
                ...
                END.
                ELSE IF iTypeCl = 0 THEN DO: /* если юрик, то адрес обязателен! */
	...
	/* выбираем основной адрес для клиента */
        ...
        FOR EACH adr-mfr WHERE adr-mfr.clientid = bank.clients-mfr.id 
            ...
            IF (iTypeCl = 0 AND adr-mfr.adresstype = "АдрЮр")
              OR ((iTypeCl = 1 OR iTypeCl = 3) AND adr-mfr.adresstype = "АдрПроп") THEN DO:
        	strAdress = ConvertAdress2BIS(TRIM(adr-mfr.adress)).        
		strStatusAdr = ENTRY(1,strAdress,"|").
   ...
   */     
        ...
        */
        ...
        IF bank.clients-mfr.inn = ? OR TRIM(bank.clients-mfr.inn) = '' THEN DO ON ERROR UNDO, THROW:
            /* IF clActionStatus = 1 THEN DO ON ERROR UNDO, THROW: */
                IF iTypeCl = 0 THEN DO ON ERROR UNDO, THROW: /* если юрик - ИНН обязателен */
            	    UNDO, THROW NEW Progress.Lang.AppError( {&ERROR_INN} + " INN=? обязателен").
                    /* RUN SET_ERROR_CLIENT(INPUT {&STATUS_INN_ERROR}, INPUT {&ERROR_INN} + " INN=? обязателен").
                    UNDO MYNEXT, LEAVE MYNEXT. */
                END.
                strInn = "".
            /* END.
        ...
        ELSE DO ON ERROR UNDO, THROW:
            ...
            IF IsMyNumber(strInn) <> FALSE OR (LENGTH(strInn) <> 10 AND LENGTH(strInn) <> 12) THEN DO ON ERROR UNDO, THROW: /* формат ИНН */
                IF iTypeCl = 0 THEN DO ON ERROR UNDO, THROW:
            	    UNDO, THROW NEW Progress.Lang.AppError( {&ERROR_INN} + "не 10 и 12 знаков и обязателен").
                    /* RUN SET_ERROR_CLIENT(INPUT {&STATUS_INN_ERROR}, INPUT {&ERROR_INN} + "не 10 и 12 знаков и обязателен").
                    UNDO MYNEXT, LEAVE MYNEXT. */
                END.
        ...
        IF bank.clients-mfr.ogrn = ? OR TRIM(bank.clients-mfr.ogrn) = "" THEN DO ON ERROR UNDO, THROW:
            IF iTypeCl = 0 THEN DO ON ERROR UNDO, THROW: /* если юрик - ОГРН обязателен */
        ...
        ELSE strFormType = "".
        IF iTypeCl = 0 THEN DO ON ERROR UNDO, THROW: /* если юрик - вид предприятия обязателен и по справочнику */ 
        ...
        IF iTypeCl = 0 THEN DO ON ERROR UNDO, THROW: /* если юрик - (субъект) обязателен и по справочнику */ 
            FIND FIRST code
...
/* put unformatted iSmsPhone "#" strPhone2 SKIP. */
	...
	/*    IF NUM-ENTRIES(person.phone[1]) < 2
    	...
    	FOR EACH doc-mfr
    	 WHERE doc-mfr.clientid = bank.clients-mfr.id NO-LOCK ON ERROR UNDO, THROW:
    	    ...
    	    ELSE DO:
        	strDocumentId = TRIM(doc-mfr.doctype).
        	FIND FIRST code WHERE  code.class EQ "КодДокум" AND code.parent EQ "КодДокум" AND code.code = strDocumentId NO-LOCK NO-ERROR.
        	IF NOT AVAIL code THEN DO:
        	    IF doc-mfr.volume EQ 1
        	    THEN UNDO, THROW NEW Progress.Lang.AppError( {&ERROR_DOC_TYPE}).
        	    ELSE NEXT.
	...
	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + создаем ФЛ person-id=" + STRING( person.person-id) SKIP. &ENDIF
        VALIDATE person.
        bCreated = TRUE.
        ...
        UpdateSigns("person",STRING(iBisId),"Document4Date_vid",STRING(dateOpenDoc,"99.99.9999"),?).
            /* IF strKodReg <> ? AND strKodReg <> "" THEN
                UpdateSigns("person",STRING(iBisId),"КодРег",strKodReg,?).*/
            /*
            0 Юридическое лицо
            1 Физическое лицо
            2 Банк
...
put unformatted icid 2222 skip.
    ...
    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + обновление клиента, его документов, адресов" SKIP. &ENDIF
    UpdateClient:
    DO ON ERROR UNDO, THROW:
        IF iTypeCl = 1 OR iTypeCl = 3 THEN DO ON ERROR UNDO, THROW:  /* PERSON */
            ...
            /* IF iBisId = ? OR iBisId <= 0 THEN DO:  * пробуем отыскать по активному документу (volume = 0) *
                IF strDocumentId <> ? AND strDocumentId <> "" AND strDocument <> ? AND strDocument <> "" THEN DO:
                    FIND FIRST cust-ident WHERE cust-ident.cust-code-type = strDocumentId 
                        AND cust-ident.cust-cat = 'Ч' AND cust-ident.class-code = 'p-cust-ident'
                        AND cust-ident.cust-code = strDocument NO-LOCK NO-ERROR.
...
/* put unformatted "##" + person.phone[2] SKIP. */
    	    ...
    	    IF iTypeCl = 1 THEN DO:
    		IF person.country-id EQ "RUS" THEN tmpStr = "ФЛ". ELSE tmpStr = "ФЛН".
    	    END. ELSE DO:
    		tmpStr = "ФЛП".
    	    END.
	    ...
	    /*
            IF bank.clients-mfr.phonestr <> ? AND TRIM(bank.clients-mfr.phonestr) <> ""
             THEN sUpdateSigns("person",STRING(iBisId),"Телефон3",TRIM(bank.clients-mfr.phonestr),?).
	    ...
	    */
            ...
            AND bank.clients-mfr.sersvidreg + "," + bank.clients-mfr.numsvidreg <> ?
             THEN sUpdateSigns("person",STRING(iBisId),"УчДокГр", bank.clients-mfr.sersvidreg + "," + bank.clients-mfr.numsvidreg, ?).
                ...
                /* IF bank.clients-mfr.highorg <> ? AND TRIM(bank.clients-mfr.highorg) <> ""
                 THEN
                    UpdateSigns("cust-corp",STRING(iBisId),"ОргУправ",TRIM(bank.clients-mfr.highorg),?). */
                /*IF bank.clients-mfr.sumreg <> ? THEN
                    UpdateSigns("cust-corp",STRING(iBisId),"УставКап",STRING(bank.clients-mfr.sumreg,,  "->,>>>,>>>,>>9.99"),?). */
	    ...
	    strSerNal = (IF bank.clients-mfr.sernal <> ? AND TRIM(bank.clients-mfr.sernal) <> ""
	     THEN TRIM(bank.clients-mfr.sernal) ELSE '').
            ...
            /* ELSE sUpdateSigns("person",STRING(iBisId),"УчДокДата",?,?). */
            /* уровень риска и оценка риска */
            ...
            /* IF bank.clients-mfr.vidbsns <> ? AND TRIM(bank.clients-mfr.vidbsns) <> ""
             THEN sUpdateSigns("person",STRING(iBisId),"ВидДеят",TRIM(bank.clients-mfr.vidbsns),?). */
	    ...
	    END.
            ...
            sUpdateSigns("person",STRING(iBisId),"ДатаПровТерр", STRING(Today,"99/99/9999"),?). 
                               

        END. /*         IF iTypeCl = 1 OR iTypeCl = 3 THEN DO:   ищем PERSON  */ 

        ELSE IF iTypeCl = 0 THEN DO:  /* CUST-CORP */
            IF iBisId = ? OR iBisId <= 0 THEN DO:  /* пробуем отыскать по INN и имени */
	...
	/* роль клиента в филиале */
        IF bCreated THEN DO ON ERROR UNDO, THROW:
            		...
            		IF AVAIL cust-role AND cust-role.surrogate NE strBranchId THEN DO ON ERROR UNDO, THROW:
                	    ...
                	    .
            		    VALIDATE cust-role.
            		    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + Исправили филиал " + strBranchId SKIP. &ENDIF
            		END.
        END. ELSE DO ON ERROR UNDO, THROW:
                	FIND FIRST cust-role
                	 WHERE cust-role.class-code = "ImaginClient"
        ...
        IF NOT AVAIL cust-role THEN DO ON ERROR UNDO, THROW:
            ...
            &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + Добавили филиал " + strBranchId SKIP. &ENDIF
        END.
	...
	RUN GetTypeMainAdr ( strCustCat, OUTPUT vAddrMainType, OUTPUT vAdrCntXattr).
		...
		IF NOT bCreated THEN DO:
                	...
                	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + адрес " + strAdressType + " уже был у клиента, наверно он старый и в БИС данные новее." SKIP. &ENDIF
            		RELEASE cust-ident.
            		NEXT.
            	    END.
		END.
		...
		/* это был нормальный вариант с датой регистрации адреса
                FIND FIRST cust-ident USE-INDEX cust
                 WHERE cust-ident.class-code     = "p-cust-adr"
                   AND cust-ident.cust-cat       = strCustCat
                   AND cust-ident.cust-id        = iBisId
                   ...
                   EXCLUSIVE-LOCK NO-ERROR. *
                 * если строка с адресом совпадает, но регион не проставлен, то адрес не трогаем чтобы не затереть регион в БИС *
		IF AVAIL cust-ident AND cust-ident.issue EQ strAdress
		 AND (strKodReg EQ "" OR strKodReg EQ ?) THEN DO ON ERROR UNDO, THROW:
		    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + Строка адреса совпадает, но в АБС нет региона, оставим данные БИС." SKIP. &ENDIF
		    NEXT.
		END. */
                ...
                IF NOT AVAIL cust-ident OR
                 ...
                 THEN DO:
                    ...
                    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + нет старого адреса " + strAdressType + " или он не совпадает." SKIP. &ENDIF
                    /* IF AVAIL cust-ident THEN DO ON ERROR UNDO, THROW:
                	PUT UNFORMATTED
                    ...
                    IF AVAIL cust-ident AND cust-ident.open-date NE dateAdressOpen
                     THEN DO:
                	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + адрес " + strAdressType + " заводим новый другой датой." SKIP. &ENDIF
                        RELEASE cust-ident.
		    END.
            	    IF NOT AVAIL cust-ident THEN DO:
                	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + создаем адрес " + strAdressType SKIP. &ENDIF
                	CREATE cust-ident.
                	ASSIGN
            	    ...
            	    IF cust-ident.issue NE strAdress OR cust-ident.issue EQ ?
            	     THEN DO:
                	&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + присваиваем сам адрес " + strAdressType SKIP. &ENDIF
            		ASSIGN
			...
			IF strCustCat EQ "Ч" THEN
            		...
            		IF strCustCat EQ "Ч" THEN DO:
                	    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + присваиваем основной адрес " + strAdressType SKIP. &ENDIF
		    	    ASSIGN person.address[1] = strAdress.
			    VALIDATE person.
            		END.
            		IF strCustCat EQ "Ю" THEN DO:
                	    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + присваиваем основной адрес " + strAdressType SKIP. &ENDIF
		    	    ASSIGN cust-corp.addr-of-low[1] = strAdress.
			    VALIDATE cust-corp.
            		END.
			/* &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED
			    "    + Синхронизируем ДР адреса " + strAdressType + " код " + GetXattrValueEx( "person", string( iBisId), "УникКодАдреса", "-") SKIP.
			    &ENDIF */
            		
			/* RUN SyncAdrSubjToCident IN h_cust (
			            STRING(BUFFER person:HANDLE), "Ч", dateAdressOpen, strAdressType). */
			&IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED
                ...
                FOR EACH b2cust-ident
                 WHERE b2cust-ident.class-code     = "p-cust-adr"
                   ...
                   EXCLUSIVE-LOCK:
            	    ...
            	    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "     + старый адрес " +
            		 strAdressType + " от " + STRING(b2cust-ident.open-date) + " клиента закрыт" SKIP. &ENDIF
		END.
	...
	IF INDEX( txtErr, "заблокирован другим пользователем") < 1 THEN DO:
    ...
    &IF DEFINED( DEBUG-LOG) &THEN PUT UNFORMATTED "    + время обработки клиента " + STRING( MTIME - iTime) + " м.сек." SKIP. &ENDIF
