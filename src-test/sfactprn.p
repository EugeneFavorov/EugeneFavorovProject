...
DEFINE VAR mRukName      AS CHAR NO-UNDO. /*ФИО руководителя*/
DEFINE VAR mGBName       AS CHAR NO-UNDO. /*ФИО главбуха    */
   ...
   NO-LOCK:
      ...
      ASSIGN 
         mNameSrv = SFAssetName(term-obl.contract + "," +
                                ...
                                STRING(term-obl.nn),
                                term-obl.symbol)
         ttServ.NameServ = SplitStr(mNameSrv,45,'~n')
...
/*      MESSAGE mSFBuyerFIO view-as alert-box.*/

         IF loan.cust-cat EQ "Ч" AND GetXattrValue("person", STRING(loan.cust-id),"Предпр") NE "" THEN
         DO:
