...
IF iForm NE 'NO' THEN DO:
  ...
  for each acct where can-do("СпБрок,СпецПА,Кон138,КонЗдт,КонРез", acct.contract)
                ...
                /*and acct.acct <> "40702810600501010991     @0500"*/
                /*and acct.acct  = "40702810005850108272     @0500"*/
      no-lock:
               /*RUN chk-acct-kom.p (acct.acct, end-date, 0).*/
               /*message acct.acct pick-value view-as alert-box.*/
               /*IF string(pick-value) = '0' THEN NEXT.*/
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Sheet">
  <Table ss:ExpandedColumnCount="7" ss:ExpandedRowCount="666" x:FullColumns="1"
   ...
   <Column ss:StyleID="s62" ss:AutoFitWidth="0" ss:Width="400"/>
   <Column ss:StyleID="s63" ss:AutoFitWidth="0" ss:Width="66" ss:Span="1"/>
   <Column ss:Index="5" ss:AutoFitWidth="0" ss:Width="66" ss:Span="1"/>
  ...
  no-lock:
           ...
           no-lock:
                    ...
                    find first bop where bop.op = op-entry.op
                                     ...
                                     and can-do('01КЛ,01БУМ', bop.doc-type)
...
/*====== ИП платит на свои ФЛ счета ==================================================================================================================================================================================*/
   ...
   THEN DO:
            ...
            FOR EACH bop-entry2 WHERE bop-entry2.op         = op-entry.op
                                  AND bop-entry2.filial-id EQ shFilial
                                  AND bop-entry2.acct-db   EQ acct.acct
                                  AND can-do('√,√√,ФБК,ФБН,ФБО,ФБП,ФДД', bop-entry2.op-status)
                                  AND bop-entry2.op-date   >= beg-date 
                                  AND bop-entry2.op-date   <= end-date
...
PROCEDURE GetName:
 ...
 RUN chk-acct-kom.p (acct.acct, end-date, 0).
 IF string(pick-value) = '0' THEN sname = sname + " НЕТ РАСЧЁТНОГО СЧЁТА".
END. 
