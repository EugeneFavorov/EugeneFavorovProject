*/ 
{globals.i}                                 /* £´Æ°†´Ï≠Î• Ø•‡•¨•≠≠Î•         */
...
ASSIGN
    ...
    mMergeSimilarOE = LOGICAL(FGetSetting("è´†‚ÑÆ™", "é°Í•§®≠Ô‚Ïè‡Æ¢", "ç•‚") = "Ñ†")
.
...
IF mDoSplit = ? THEN DO:
   ...
   IF mCanBeSplitted AND vGroupNum = 1 THEN DO:
      &IF DEFINED(SESSION-REMOTE) &THEN
         ...
         IF GetSysConf("elsafe") NE "yes" THEN
         DO:
...
IF AVAIL(user-proc) THEN 
   ...
   mSignatures = GetXAttrValue("user-proc",
                 ...
                 "èÆ§Ø®·®").  
  IF op.user-id BEGINS "SERV" 
    AND op.user-inspector NE ?  
    AND op.user-inspector NE ""  
    THEN
    DO:
      FIND FIRST user-proc WHERE user-proc.PROCEDURE EQ "aa" 
       NO-LOCK NO-ERROR.
     IF AVAIL(user-proc) THEN
        RUN SetSysConf IN h_base ("user-proc-id",RECID(user-proc)). /* §´Ô signatur.i */
    END.
...
PROCEDURE PrintGroup.
   ...
   /* §´Ô ™Æ≠¢•‡·®Æ≠≠ÎÂ §Æ™„¨•≠‚Æ¢ Ø‡® Ø•Á†‚® conv-ord £‡„ØØ®‡„•¨ ØÆ acct-cr */
...
/* end kau*/
   ...
   IF op.op-kind EQ "1416éî" 
   THEN Author[1] = 'ëÆ·‚†¢®‚•´Ï é¨·™®© ‰®´®†´ èÄé "è´Ó· Å†≠™"'.
   ELSE Author[1] = "ëÆ·‚†¢®‚•´Ï " + Author[1].
...
PROCEDURE memorn_pre:
   ...
   ELSE mIsIspravl = NO.
   IF mIsIspravl
   THEN PUT UNFORMATTED " åÖåéêàÄãúçõâ àëèêÄÇàíÖãúçõâ éêÑÖê N "
   vDocNum
...
PROCEDURE memorn_post.
   ...
   PUT UNFORMATTED " ´®·‚†Â." SKIP(1).

   IF FGetSetting("è´†‚ÑÆ™", "ÇÎ¢Æ§çÆ¨éØ•‡", "ç•‚") = "Ñ†"
   THEN PUT UNFORMATTED "ì≠®™†´Ï≠Î© ≠Æ¨•‡ ÆØ•‡†Ê®®: " (IF AVAIL op THEN STRING(op.op) ELSE "") SKIP.
...
PROCEDURE PrintSignatures.
      ...
      PUT UNFORMATTED " èÆ§Ø®·®"  + chr(10) + chr(10) .
