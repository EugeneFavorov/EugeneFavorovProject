...
do: 
    */
    ...
    do:   
        ...
        If (end-date GE date("17/08/2015"))  and  (end-date LE date("23/11/2015"))  THEN /*включительно*/       
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "2" + SUBSTRING ( iParms, LENGTH (iParms), 1)  .
            
        If (end-date GE date("24/11/2015"))  and  (end-date LE date("29/11/2015"))  THEN 
            iParms = SUBSTRING ( iParms, 1, (LENGTH (iParms ) - 1) ) + "3" + SUBSTRING ( iParms, LENGTH (iParms), 1)  .
            
        If (end-date GE date("30/11/2015"))  and  (end-date LE date("26/12/2015"))  THEN 
        ...
        If (end-date GE date("30/08/2016")) THEN 
...
PROCEDURE benef:   
    ...
    RUN RetAdr.p(pers_id, "Ч", "АдрПроп", end-date, OUTPUT mAdrReg).  
    RUN Insert_TTName("АдрЮр",mAdrReg).
    
    RUN RetAdr.p(pers_id, "Ч", "АдрФакт", end-date, OUTPUT mAdrReg).
    RUN Insert_TTName("АдрФакт",mAdrReg).

    RUN RetAdr.p(pers_id, "Ч", "АдрУвед", end-date, OUTPUT mAdrReg).  
