...
for each signs WHERE signs.file-name EQ 'person'                      
                 AND signs.code eq 'ВидСотр'                      
                 AND signs.code-value begins 'vip'
                 no-lock.
   for each loan where loan.contract   = "Кредит"
                   and loan.close-date = ?
/*                   and (loan.cont-code  MATCHES "*КС*" or loan.cont-code  MATCHES "*кс*" ) */
                   and loan.cust-cat   = "Ч"
                   and loan.filial-id <> "0400"
                   AND loan.cust-id    = int(signs.surrogate)
                   no-lock.
      ...
      if avail loan-acct then do:
          if substr(loan-acct.acct,1,3) <> "455" and substr(loan-acct.acct,1,3) <>  "457" then next.
      ...
      find last comm-rate WHERE comm-rate.kau eq loan.contract + ',' + entry(1,loan.cont-code," ")
      ...
      ASSIGN 
        ...
        ttCred.cont-code        = loan.cont-code /* entry(1,loan.cont-code,"@") */
...
PUT UNFORMATTED XLRow(0) XLCellHat("Все ФИЗИЧЕСКИЕ ЛИЦА со статусом VIP имеющие ссудные счета 455* и 457* за " + STRING(end-date, "99.99.9999"), 13) XLRowEnd().
