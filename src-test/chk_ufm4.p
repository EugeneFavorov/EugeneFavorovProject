...
DEFINE VARIABLE mDR3          AS LOGICAL    NO-UNDO.
DEFINE VARIABLE mStrAdd       AS LOGICAL    NO-UNDO.
DEFINE VARIABLE mStrInFile    AS LOGICAL    NO-UNDO.
DEFINE VARIABLE mDRKod        AS CHARACTER  NO-UNDO.
...
FORM 
    ...
    mDR3
    label "Изменение ДР АнализОФМ"
    VIEW-AS TOGGLE-BOX SIZE 30 BY 1 AT ROW 3 COL 17 PFCOLOR 1
    HELP "Заполнение ДР АнализОФМ из поля Код"    
    SKIP  
     
    mDRKod      
    FORMAT "x(1)"        
    VIEW-AS FILL-IN SIZE 2 BY 1 AT ROW 3 COL 50 LABEL "Код" 
    HELP "Код для заполнения ДР АнализОФМ"
    SKIP    
   
    mStrAdd
    label "Дополнение[Х] или замена[ ] строки примечания" 
    VIEW-AS TOGGLE-BOX SIZE 50 BY 1 AT ROW 4 COL 17 PFCOLOR 1
    HELP "Дополнение или Замена строки примечания"    
    SKIP 
    
    mStrInFile
    label "Примечание из третьей колонки файла" 
    VIEW-AS TOGGLE-BOX SIZE 50 BY 1 AT ROW 5 COL 17 PFCOLOR 1
    HELP "Примечание из третьей колонки файла"    
    SKIP 
   
    WITH FRAME frPrim TITLE " Параметры заполнения " 
...
PAUSE 0.
UPDATE
   mFraza   mDR3   mDRKod  mStrAdd mStrInFile
WITH FRAME frPrim.
HIDE FRAME frPrim.
...
REPEAT:
   ...
   DO:
      ...
      ASSIGN
         ...
         tt-inf.txt   = if mStrInFile then  ENTRY(3,mString,";") else mFraza.         
         tt-inf.an3   = if mDR3 then mDRKod  else "".  
...
FOR EACH tt-inf EXCLUSIVE-LOCK:
   ...
   DO:
      ...
      DO:
         ...
         IF mCLNum eq 0 THEN 
            PUT UNFORMATTED "Не является клиентом ДР АнализОФМ установить не удается" + tt-inf.inn "   " tt-inf.lname  SKIP.
         else
         do:
         ...
         END.
            ...
            FOR EACH b-code WHERE TRUE
               ...
               AND b-code.name    EQ mSLName    
               AND b-code.misc[5] EQ tt-inf.inn
                EXCLUSIVE-LOCK:
                 PUT UNFORMATTED "Уже в Стоп-Лист   : " tt-inf.inn " " tt-inf.lname  SKIP. 
              ...
              do:
                  b-code.misc[7] = tt-inf.txt.
                  PUT UNFORMATTED "Заменено примечание   : " tt-inf.inn " " tt-inf.lname  SKIP.
              end.
