...
DEFINE VARIABLE mObrabot        AS INT64    NO-UNDO.
...
#  7 - на всякий случай -юзер
#  8 - опердень
*/

IF NUM-ENTRIES(session:parameter) GE 8 THEN
  ASSIGN
      mTranList = IF NUM-ENTRIES(ENTRY(6,session:parameter)," ") GT 1 THEN 
                     ENTRY(2,ENTRY(6,session:parameter)," ") 
                  ELSE 
                     ENTRY(6,session:parameter)
      mFilList  = ENTRY(5,session:parameter).
ELSE 
   ASSIGN
      mFilList  = "0000"
...
DO mInt2 = 1 TO NUM-ENTRIES(mTranList,";"):
   ...
   END.
   ELSE
      MESSAGE "<#ТранзакцияВыполнена#>=" + ENTRY(mInt2,mTranList,";") + "=" + STRING(NOW,"99/99/9999 HH:MM:SS").
END.
...
PROCEDURE RunTransaction2:
   ...
   message "<#ТранзакцияЗапуск#>=" + iOpkind + '=' + STRING(NOW,"99/99/9999 HH:MM:SS").
