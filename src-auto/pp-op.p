...
PROCEDURE va__op-entry__amt-rub:
   ...
   IF vValue < 0 THEN       
   DO:
   RUN Fill-SysMes IN h_tmess ("", "", "-1", "Счет по кредиту " + vCAcct).
...
PROCEDURE va__op-entry__acct-db:
   ...
   IF {assigned RETURN-VALUE} THEN RETURN RETURN-VALUE.
...
PROCEDURE vg__op-entry__acct-db__acct-cr__symbol:
   ...
   ASSIGN
      ...
      IF AVAIL cAcct AND cAcct.cust-cat EQ "Ч" THEN
      DO:
         IF AVAIL(op)
         AND NOT {assigned op.name-ben} THEN
         DO:
            IF NOT CAN-DO("01*",op.doc-type) THEN
            DO: 
               RUN SetSysConf IN h_base ("CHKPERSDOC_CALL_PROCEDURE","pp-op 1").
               RUN dbgprint.p (PROGRAM-NAME(1) + " line = {&line-number}",
                  "~ncAcct.acct = " + cAcct.acct).  
               RUN chkpersdoc.p (op.op-kind,
                                 cAcct.cust-cat,
                                 cAcct.cust-id,
                                 op.op-date,
                                 "").
               RUN SetSysConf IN h_base("CHKPERSDOC_CALL_PROCEDURE",?).
               pick-value = "NO".
         ...
         END.
      END.
      IF AVAIL(dAcct) AND dAcct.cust-cat EQ "Ч" THEN 
      DO:
         RUN dbgprint.p (PROGRAM-NAME(1) + " line = {&line-number}",
            "~ndAcct.acct = " + dAcct.acct).
         IF AVAIL(op) 
         AND NOT CAN-DO("015,02",op.doc-type) THEN
         DO:
            RUN SetSysConf IN h_base ("CHKPERSDOC_CALL_PROCEDURE","pp-op 2"). 
            RUN chkpersdoc.p (op.op-kind,
                              dAcct.cust-cat,
                              dAcct.cust-id,
                              op.op-date,
...
PROCEDURE vg__op__op:
   ...
   DO vI = 1 TO NUM-ENTRIES(vAcctChkStr):
      ...
      IF {assigned vAcctChk} THEN DO:
         ...
         IF AVAIL bf_chkacct AND bf_chkacct.cust-cat EQ "Ч" THEN 
         DO:
            RUN dbgprint.p (PROGRAM-NAME(1) + " line = {&line-number}",
            "~nbf_chkacct.acct = " + bf_chkacct.acct).
            RUN SetSysConf IN h_base ("CHKPERSDOC_CALL_PROCEDURE","pp-op 3").
   ...
   DO vI = 1 TO NUM-ENTRIES(vAttrChkStr):
      ...
      IF NUM-ENTRIES(vAttrChk) EQ 2 THEN DO:
         ...
         RUN dbgprint.p (PROGRAM-NAME(1) + " line = {&line-number}",
            "~nvAttrChk = " + vAttrChk).
         RUN SetSysConf IN h_base ("CHKPERSDOC_CALL_PROCEDURE","pp-op 4").
         RUN chkpersdoc.p (iObject:buffer-FIELD("op-kind"):BUFFER-VALUE,
                           "Ч",
...
PROCEDURE vo__op:
   ...
   IF vBlack1 THEN DO:
      ...
      THEN DO:
         RUN dbgprint.p (PROGRAM-NAME(1) + " line = {&line-number}",
         	"~nЗапуск chk-black-list.p из pp-op" + 
         	"~nop = " + STRING(iObject:buffer-FIELD("op"):BUFFER-VALUE)).
   ...
   /* kam Проверка допрека на дебетовом счете проводки */
   DEFINE BUFFER kkopentry FOR op-entry.
   DEFINE BUFFER kkacct FOR ACCT.
   DEFINE VARIABLE tmpdate as date no-undo.
   DEFINE VARIABLE tmpstr AS CHAR NO-UNDO.
   DEF VAR vChv AS LOGICAL NO-UNDO.
   DEF VAR vChv2 AS LOGICAL NO-UNDO.
   vChv = TRUE.
   vChv2 = FALSE.

	FIND FIRST kkopentry WHERE kkopentry.op = iObject:BUFFER-FIELD("op"):BUFFER-VALUE NO-LOCK NO-ERROR.
	IF AVAIL kkopentry then do:
            tmpstr = GetXattrValueEx ("acct", kkopentry.acct-cr + "," + kkopentry.currency, "DateMessCollect", "").
            if tmpstr <> "" then do:
		tmpdate = date(tmpstr).
		if tmpdate >= today then do:
			vChv2 = true.
		        MESSAGE COLOR DISPLAY 'Кредит клиента продан в \n Непубличное Акционерное общество "Первое коллекторское бюро"
			\n денежные средства в счет погашения кредита не принимать, 
			\n контакты НАО "ПКБ" тел.: 8-800-234-40-04 (бесплатные звонки по России),

			\n e-mail: Ecc@collector.ru, сайт: www.collector.ru"
			\n\n
			Принимаем средства?' 
			view-as alert-box QUESTION BUTTONS YES-NO title "ВНИМАНИЕ!" set vChv.
			IF vChv THEN
	  		    RETURN 'ERROR'. 
   ...
   /* 	 end of kam Проверка допрека на дебетовом счете проводки */
   ...
   DO:
      vOk = No.
      RUN chk-over-op.p (kop.op,OUTPUT vOk) NO-ERROR.
      IF vOk NE Yes THEN
      DO:
         RUN Fill-SysMes IN h_tmess ("", "", "-1", "Ошибка!").
         RETURN 'ERROR'.
   ...
   /* Проверка соответствия счета и ИНН */
   DEFINE VARIABLE mAcctRec AS CHARACTER NO-UNDO.
   DEFINE VARIABLE mAcctPol AS CHARACTER NO-UNDO.
   FIND FIRST kop WHERE kop.op = iObject:BUFFER-FIELD("op"):BUFFER-VALUE NO-LOCK NO-ERROR.
   IF AVAIL kop THEN
   DO:
      vOk = No.
      
      FIND FIRST kkopentry WHERE
         kkopentry.op = iObject:BUFFER-FIELD("op"):BUFFER-VALUE 
      NO-LOCK NO-ERROR.
      
	   IF AVAIL kkopentry THEN
      DO:
         mAcctRec = GetXattrValueEx("op",STRING(iObject:buffer-FIELD("op"):BUFFER-VALUE),"acct-rec","").
         IF  NOT {assigned kop.ben-acct} 
         AND NOT {assigned mAcctRec} THEN
            mAcctPol = kkopentry.acct-cr.
         IF      {assigned kop.ben-acct} 
         AND NOT {assigned mAcctRec} THEN
            mAcctPol = kop.ben-acct.
         IF      {assigned mAcctRec} 
         AND NOT {assigned kop.ben-acct} THEN
            mAcctPol = mAcctRec.
         IF      {assigned mAcctRec} 
         AND     {assigned kop.ben-acct} THEN
            mAcctPol = mAcctRec.
         ...
         IF CAN-DO("40802*,40817*,40820*,423*,426*",mAcctPol)
   	      AND LENGTH(kop.inn) EQ 10 THEN
            RUN Fill-SysMes IN h_tmess ("", "", "0", "ИНН не соответствует маске счета получателя.~nПроверьте ИНН получателя.").
         IF CAN-DO("40702*",mAcctPol)
   	      AND LENGTH(kop.inn) EQ 12 THEN
            RUN Fill-SysMes IN h_tmess ("", "", "0", "ИНН не соответствует маске счета получателя.~nПроверьте ИНН получателя.").
         
/*	      IF     CAN-DO("40702*",kkopentry.acct-db)                                                                                */
/*   	      AND CAN-DO("40802*,40817*,40820*",mAcctPol)                                                                           */
/*   	      AND LENGTH(kop.inn) EQ 10 THEN                                                                                        */
/*            RUN Fill-SysMes IN h_tmess ("", "", "0", "ИНН не соответствует маске счета получателя.~nПроверьте ИНН получателя.").*/
/*         IF     CAN-DO("40802*,40817*,40820*",kkopentry.acct-db)                                                                */
/*   	      AND CAN-DO("40702*",mAcctPol)                                                                                         */
/*   	      AND LENGTH(kop.inn) EQ 12 THEN                                                                                        */
/*            RUN Fill-SysMes IN h_tmess ("", "", "0", "ИНН не соответствует маске счета получателя.~nПроверьте ИНН получателя.").*/
      END.
   END.
...
PROCEDURE v__e__handler:
   ...
   CASE iException:
      ...
      DO:
         RUN b-ravrf.p (vOp).
         IF RETURN-VALUE NE "" THEN oNoCont = YES.
         oWasTreated = YES.
         RETURN .
      END.
      WHEN "СчетУИН" THEN
...
PROCEDURE v__method__check:
   ...
   IF CAN-DO("no-method,no-proc",RETURN-VALUE)  THEN
...
{pfuncdef
   &DefProc     = "GetPaymentType"
   &Description = "Определение типа платежа"
   &Parameters  = "iOp          - идентификатор документа (op.op);~
oPaymentType - [OUTPUT] тип платежа."
   &Result      = "В случае ошибки (документ не найден) oPaymentType ~
будет содержать неопределённое значение. Для налоговых, таможенных и ~
бюджетных платежей oPaymentType принимает одно из значений ~"налоговый~", ~
~"таможенный~", ~"бюджетный~" соответственно. В остальных случаях ~
oPaymentType будет содержать пустую строку."
   &Sample      = "RUN GetPaymentType IN h_op (vOp, OUTPUT vPaymentType)."}
PROCEDURE GetPaymentType:
   DEFINE INPUT  PARAMETER iOp          LIKE op.op     NO-UNDO.
   DEFINE OUTPUT PARAMETER oPaymentType AS   CHARACTER NO-UNDO INITIAL ?.
   ...
   IF AVAILABLE op THEN DO:
      vS = Surrogate(BUFFER op:HANDLE).
      RUN GetPaymentTypeEx IN THIS-PROCEDURE (GetXAttrValue("op", vS, "КБК"),
                                              GetXAttrValue("op", vS, "ПокОП"),
                                              op.ben-acct,
                                              OUTPUT oPaymentType).
   END.
END PROCEDURE.
...
{pfuncdef
   &DefProc     = "GetPaymentTypeEx"
   &Description = "Определение типа платежа по значениям реквизитов документа"
   &Parameters  = "iKBK         - значение КБК;~
iPokOP       - значение реквизита ПокОП (основание платежа (106));~
iBenAcct     - счёт получателя (ben-acct);~
oPaymentType - [OUTPUT] тип платежа."
   &Result      = "oPaymentType принимает одно из значений ~"налоговый~", ~
~"таможенный~", ~"бюджетный~" для налоговых, таможенных и бюджетных платежей ~
соответственно. В остальных случаях oPaymentType будет содержать пустую строку."
   &Sample      = "RUN GetPaymentTypeEx IN h_op ~
(vKBK, vPokOP, op.ben-acct, OUTPUT vPaymentType)."}
PROCEDURE GetPaymentTypeEx:
   DEFINE INPUT  PARAMETER iKBK         AS   CHARACTER   NO-UNDO.
   DEFINE INPUT  PARAMETER iPokOP       AS   CHARACTER   NO-UNDO.
   DEFINE INPUT  PARAMETER iBenAcct     LIKE op.ben-acct NO-UNDO.
   DEFINE OUTPUT PARAMETER oPaymentType AS   CHARACTER   NO-UNDO.

   DEFINE BUFFER op FOR op.

   DEFINE VARIABLE vAffinity AS CHARACTER NO-UNDO.
   DEFINE VARIABLE vMaybeTax AS LOGICAL   NO-UNDO.
   DEFINE VARIABLE vMaybeCH  AS LOGICAL   NO-UNDO.

   ASSIGN
      vAffinity = GetCodeDesc("Нал:ОП", iPokOP, 1, ?)
      vMaybeTax = CAN-DO(FGetSetting("ГНИ", "КБКНалог", ""), iKBK)
      vMaybeCH  = CAN-DO(FGetSetting("ГНИ", "КБКТамож", ""), iKBK)
   .
   IF vMaybeTax AND
      CAN-DO(vAffinity, "н")
   THEN
      oPaymentType = "налоговый".
   ELSE IF vMaybeCH AND
           CAN-DO(vAffinity, "т")
   THEN
      oPaymentType = "таможенный".
   ELSE IF NOT (vMaybeTax OR vMaybeCH) AND
           CAN-DO(FGetSetting("ГНИ", "bal-budget", ""), iBenAcct)
   THEN
      oPaymentType = "бюджетный".
END PROCEDURE.
/*---------------------------------------------------------------------------*/
{pfuncdef
   &DefProc     = "CheckNalPerByType"
   &Description = "Проверка формата налогового периода "
   &Parameters  = "iType - "таможенный" или "бюджетный" или "налоговый";iNalP - налоговый период"
   &Result      = "YES/NO"
   &Sample      = "RUN CheckByType IN h_op (vType, iVal, OUTPUT vRes)."}
PROCEDURE CheckNalPerByType:

   DEF INPUT  PARAMETER iType AS CHAR     NO-UNDO.
   DEF INPUT  PARAMETER iNalP AS CHAR     NO-UNDO.
   DEF OUTPUT PARAMETER oRes  AS LOGIC    NO-UNDO.

   DEF VAR vDate      AS DATE    NO-UNDO.
   DEF VAR vDateFlag  AS LOGIC   NO-UNDO INIT NO.
   DEF VAR vDD        AS CHAR    NO-UNDO.
   DEF VAR vMM        AS CHAR    NO-UNDO.
   DEF VAR vMMint     AS INT64   NO-UNDO.
   DEF VAR vYYYY      AS CHAR    NO-UNDO.
   DEF VAR vCode      AS CHAR    NO-UNDO.
   DEF VAR vI         AS INT64   NO-UNDO.

   DEFINE BUFFER code FOR code.

&SCOPED-DEFINE ReturnYES   DO: oRes = YES. RETURN. END.
&SCOPED-DEFINE ReturnNO    DO: oRes = NO.  RETURN. END.

   CASE iType:
      WHEN "таможенный" THEN DO:
         DO vI = 1 TO 8:
            IF NOT IsDigit(SUBSTRING(iNalP, vI, 1)) THEN 
               {&ReturnNO}
         END.
      END.

      WHEN "бюджетный" THEN DO:
         IF iNalP NE "0" THEN
            {&ReturnNO}
      END.

      WHEN "налоговый" THEN DO:
         IF iNalP EQ "0" THEN
            {&ReturnYES}

         IF NUM-ENTRIES(iNalP, ".") NE 3 THEN
            {&ReturnNO}
         ...
         IF (LENGTH(vDD) NE 2) OR (LENGTH(vMM) NE 2) OR (LENGTH(vYYYY) NE 4) THEN
            {&ReturnNO}

         IF IsDigit(SUBSTRING(vDD, 1, 1)) AND IsDigit(SUBSTRING(vDD, 2, 1)) THEN 
            ASSIGN
               vDate     = DATE(iNalP)
               vDateFlag = YES
            NO-ERROR.
         ELSE
            vDate = DATE(SUBSTITUTE("01.&1.&2", IF vMM EQ "00" THEN "01" ELSE vMM, vYYYY)) NO-ERROR.

         IF vDate EQ ? THEN
            {&ReturnNO}

         IF vDateFlag EQ YES THEN
            {&ReturnYES}

         ASSIGN
            vCode    = GetCode ("Нал:НП", vDD)
            vMMint   = INT64(vMM)
         NO-ERROR.
         ...
         FOR FIRST code WHERE 
            code.class EQ 'Нал:НП' AND
            code.code  EQ vDD
               NO-LOCK:

            oRes = vMMint GE INT64(code.DESCRIPTION[2]) AND vMMint LE INT64(code.val).
         END.
